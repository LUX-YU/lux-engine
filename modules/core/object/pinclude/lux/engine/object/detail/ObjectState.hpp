...
enum class EListenerLane : std::uint8_t { DIRECT, QUEUED, PENDING };
...
struct ObjectState final {
  ObjectState(LuxObject *value, ObjectDispatcherRef dispatcher_value,
  ...
  [[nodiscard]] lux::cxx::intrusive_ptr<ConnectionControl>
  install(const SignalDescriptor &signal,
          lux::cxx::intrusive_ptr<ObjectState> receiver,
          ObjectInvokeThunk invoke, EDelivery delivery,
          std::shared_ptr<void> context);
  ...
  void addIncoming(lux::cxx::intrusive_ptr<ObjectState> sender,
                   lux::cxx::intrusive_ptr<ConnectionControl> control);

  void removeIncoming(const ObjectState *sender,
                      const ConnectionControl *control) noexcept;

private:
  void append(SignalBucket &bucket, ConnectionControl &control,
              EDelivery delivery);
