...
class CameraExtraction final : public RenderSyncStage
{
  public:
    explicit CameraExtraction(const RenderSyncStageCreateInfo &info)
        : registry_(info.registry), scene_(info.scene), page_size_(info.coordinate_page_size),
          origin_(info.scene_origin_page), views_(*info.views),
          ...
          transform_added_(registry_.on_construct<WorldTransform3D>().connect<&CameraExtraction::poseChanged>(*this)),
          transform_changed_(registry_.on_update<WorldTransform3D>().connect<&CameraExtraction::poseChanged>(*this)),
          transform_removed_(registry_.on_destroy<WorldTransform3D>().connect<&CameraExtraction::poseChanged>(*this))
    {
    }

    bool hasPendingChanges() const noexcept override
    {
        return dirty_ || views_.revision != published_views_revision_;
    }
    ...
    ERenderSyncPrepareResult prepare(render::RenderProgramBuilder<> &builder) noexcept override
    {
        ...
        prepared_views_revision_ = views_.revision;
        for (const auto &association : views_.values)
        {
            const auto entity = association.camera;
            if (!registry_.valid(entity) || !registry_.all_of<Camera, WorldTransform3D>(entity))
            {
                continue;
            }
            const auto &camera = registry_.get<Camera>(entity);
            if (association.view.isNull() || !association.extent.width || !association.extent.height ||
                std::ranges::find(prepared_, association.view) != prepared_.end())
            {
            ...
            const auto projection =
                cameraProjection(camera, double(association.extent.width) / association.extent.height);
            if (!view || !projection)
            {
                return ERenderSyncPrepareResult::FAILED;
            }
            ...
            prepared_.push_back(association.view);
    ...
    void commitPrepared() noexcept override
    {
        ...
        published_views_revision_ = prepared_views_revision_;
  ...
  private:
    ...
    const RenderViewAssociations &views_;
    ...
    std::uint64_t published_views_revision_{}, prepared_views_revision_{};
    bool dirty_{true};
...
lux::cxx::expected<std::unique_ptr<RenderSyncStage>, RenderSyncStageCreateFailure> createCameraExtraction(
    const RenderSyncStageCreateInfo &info) noexcept
{
    if (!info.views || info.scene.isNull() || !info.feature_handle.isValid() ||
        !std::isfinite(info.coordinate_page_size) || info.coordinate_page_size <= 0.0)
    {
...
RenderFeatureSceneBinding cameraRenderFeatureBinding() noexcept
{
    ...
    static constexpr std::array observations{ComponentObservationSpec{lux::cxx::typeToken<Camera>(), events},
