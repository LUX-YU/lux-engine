...
#include <lux/engine/asset/MaterialGraphCodec.hpp>   // detail::encode/decodeMaterialGraph
...
namespace lux::asset
{
    ...
    // ─────────────────────────────────────────────────────────────────────────
    //  .luxasset Material (node-graph) format v3
    //
    //  The asset holds a CONCRETE rdesc::MaterialGraph; binary exists only here, at
    //  the I/O seam. The graph is the SSOT for shading-model / param defaults /
    //  render-state / texture-slot count — none of those are stored separately
    //  (the bridge derives them). Everything lives in the header's "info" section
    //  (data_size == 0), like the other SerDesers. Layout (LE POD copies):
    //
    //    u32 version (= kFormatVersion)
    //    u32 graph_blob_len;     byte × graph_blob_len   (detail::encodeMaterialGraph)
    ...
    //
    //  ABI-don't-care: the old v1/v2 layout (loose shading_model/param/render-state
    //  fields + a graph string) is gone; pre-v3 assets are rejected and must be
    //  re-baked. The strict version check is intentional — v3 fundamentally reshapes
    //  the blob (graph data structure, not loose fields), so a lenient prefix-read is
    //  meaningless here.
    // ─────────────────────────────────────────────────────────────────────────
    namespace
    {
        ...
        std::unique_ptr<MaterialData> decodeMaterialFields(Cursor& c)
        {
            ...
            // authoring graph (the SSOT): binary blob -> rdesc::MaterialGraph.
            {
                std::vector<std::byte> graph_blob;
                if (!c.readByteVector(graph_blob)) return nullptr;
                std::string gerr;
                if (!detail::decodeMaterialGraph(std::span<const std::byte>(graph_blob), payload->graph, &gerr))
                    return nullptr;
            }
            ...
            if (!c.readU32Vector(payload->gbuffer_spirv)) return nullptr;
            {
                ...
                if (!info_bytes.empty() &&
                    !ShaderInfo::deserialize(std::span<const std::byte>(info_bytes), payload->gbuffer_info, &err))
                    return nullptr;
            }
            ...
            if (!c.readU32Vector(payload->forward_spirv)) return nullptr;
            {
                ...
                if (!c.readByteVector(info_bytes)) return nullptr;
                if (!info_bytes.empty() &&
                    !ShaderInfo::deserialize(std::span<const std::byte>(info_bytes), payload->forward_info, &err))
                    return nullptr;
            }
    ...
    MaterialSerDeser::decodeData(const void* bytes, std::size_t len) noexcept
    {
        ...
        try
        {
            // Wrap the caller's image as a byte vector so we can reuse the exact
            // header validation loadHeaderRaw provides (POD copies; no I/O).
        ...
        {
            // bad_alloc from oversize length / any unexpected throw -> error.
            return unexpected(EAssetError::ASSET_DESERIALIZE_FAIL);
        }
    }
    ...
    MaterialSerDeser::exportAsLuxAssetStream(const LuxAsset& asset, std::ofstream& ofile)
    {
        ...
        const std::vector<std::byte> graph_blob = detail::encodeMaterialGraph(d->graph);
        const std::vector<std::byte> gb_info    = ShaderInfo::serialize(d->gbuffer_info);
        const std::vector<std::byte> fwd_info   = ShaderInfo::serialize(d->forward_info);

        std::vector<std::byte> info;
        info.reserve(64 + graph_blob.size() + d->gbuffer_spirv.size() * 4 +
                     d->forward_spirv.size() * 4 + gb_info.size() + fwd_info.size());

        appendPod<std::uint32_t>(info, kFormatVersion);

        // authoring graph (binary blob).
        appendPod<std::uint32_t>(info, static_cast<std::uint32_t>(graph_blob.size()));
        appendBytes(info, graph_blob.data(), graph_blob.size());
