...
namespace lux::animation
{
    namespace
    {
        ...
        float normalizeTime(float time, float duration, bool loop) noexcept
        {
            ...
            if (loop) {
                ...
                if (wrapped < 0.0f) wrapped += duration;  // negative time → forward wrap
                return wrapped;
            }
        ...
        struct SpanResult { std::size_t lo; std::size_t hi; float alpha; };
        ...
        SpanResult findSpan(const std::vector<float>& times, float time) noexcept
        {
            ...
            if (hi == 0)            return { 0, 0, 0.0f };                  // before first
            if (hi == times.size()) return { hi - 1, hi - 1, 0.0f };        // at/after last
            std::size_t lo = hi - 1;
            float t0 = times[lo];
            float t1 = times[hi];
        ...
        Eigen::Vector3f sampleVec3(const std::vector<float>&         times,
                                   ...
                                   const Eigen::Vector3f&             fallback) noexcept
        {
            ...
            if (s.lo == s.hi) return values[s.lo];
            return values[s.lo] + s.alpha * (values[s.hi] - values[s.lo]);
        }

        Eigen::Quaternionf sampleQuat(const std::vector<float>&             times,
                                      const std::vector<Eigen::Quaternionf>& values,
                                      float                                  time,
                                      const Eigen::Quaternionf&              fallback) noexcept
        {
            if (values.empty()) return fallback;
            if (values.size() == 1) return values.front();
            auto s = findSpan(times, time);
        ...
        void decomposeBindLocal(const Eigen::Affine3f& m,
                                Eigen::Vector3f&       out_t,
                                Eigen::Quaternionf&    out_r,
                                Eigen::Vector3f&       out_s) noexcept
        {
            out_t = m.translation();
            ...
            Eigen::Matrix3f R = L;
            for (int i = 0; i < 3; ++i) {
                if (out_s[i] > 0.0f) R.col(i) /= out_s[i];
            }
    ...
    void precomputeBindLocals(const lux::rdesc::Skeleton& skeleton,
                              std::vector<BindLocalTRS>&  out)
    {
        const std::size_t n = skeleton.bones.size();
        out.resize(n);
        for (std::size_t i = 0; i < n; ++i)
            decomposeBindLocal(skeleton.bones[i].bind_local,
                               out[i].translation, out[i].rotation, out[i].scale);
    }
    ...
    void samplePose(const lux::rdesc::AnimationClip& clip,
                    ...
                    std::span<const BindLocalTRS>    bind_trs)
    {
        ...
        for (const auto& track : clip.tracks) {
            if (track.bone_index < 0) continue;
            const auto idx = static_cast<std::size_t>(track.bone_index);
            if (idx >= n) continue;  // stale track, skeleton edited after baking
    ...
    void buildSkinningMatrices(const lux::rdesc::Pose&        pose,
                               const lux::rdesc::Skeleton&    skeleton,
                               std::vector<Eigen::Matrix4f>&  out_matrices)
    {
        ...
        for (std::size_t i = 0; i < n; ++i) {
            ...
            assert(p < static_cast<int32_t>(i) &&
                   "Skeleton must be topologically sorted (parent_index < self_index)");
