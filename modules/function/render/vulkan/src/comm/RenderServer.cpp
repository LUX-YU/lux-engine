...
namespace lux::render
{
    ...
    Expected<void> GeneralRenderServer::init(ServerConfig config)
    {
        ...
        control_server_->setDispatchFailureSink(
            +[](void* user_state, EDispatchFailure, const RenderError& error) noexcept {
                auto& im = *static_cast<GeneralRenderServer::Impl*>(user_state);
                im.error_sink_.emit(error, RenderErrorEvent::kNoScene, im.current_stamp_.serial);
            }
        );
    ...
    ) noexcept
    {
        ...
        auto it =
            std::find_if(active_uploads_.begin(), active_uploads_.end(), [request_id](const ActiveUpload& upload) {
        ...
        if (is_stale_result)
        {
            ...
            error_sink_.emit(
                renderError<err::upload::ResultIdentityMismatch>(request_id, resource_index, resource_gen),
                RenderErrorEvent::kNoScene,
                current_stamp_.serial
            );
            return;
    ...
    bool GeneralRenderServer::Impl::observeTransferResult(const TransferCompletion& completion) noexcept
    {
        ...
        {
            const bool terminal =
                std::find(recent_upload_terminals_.begin(), recent_upload_terminals_.end(), completion.request_id) !=
    ...
    )
    {
        ...
        if (disp == FinalizeDisposition::Succeeded)
        {
            ...
            {
                DeferredReplyEntry
                    reply{c.request_id, c.kind, c.resource_handle, c.resource_gen, c.logical_base_mip, 0u};
    ...
    bool GeneralRenderServer::Impl::processUploadCompletions()
    {
        ...
        auto new_end =
            std::remove_if(pending_completions_.begin(), pending_completions_.end(), [&](TransferCompletion& c) {
                if (c.timeline_value <= gpu_value)
                {
                    finalizeCompletion(
                        c,
                        needs_qfot && c.requires_queue_family_ownership_transfer,
                        src_family,
                        dst_family
                    );
                    made_progress = true;
    ...
    Expected<RenderTargetId>
    GeneralRenderServer::Impl::createSurfaceTargetInternal(RenderSurface&& surface, VkExtent2D extent)
    {
    ...
    {
        auto& im = *impl_;
        const auto flush_lane =
            [&]<class ReplyRing>(ERequestLane lane, ReplyRing& responses, bool primary_publication_pending) {
                ...
                for (const auto& dr : im.pending_deferred_replies_)
                {
                    ...
                    case Kind::MeshBuffer:
                        builder.push<MeshUploadedReply>(
                            ...
                            MeshUploadedReply{
                                dr.status ? RMeshHandle{} : RMeshHandle{dr.resource_index, dr.resource_gen},
                                dr.status},
                            0,
                            dr.request_id
                        );
                    ...
                    case Kind::Texture2D:
                        builder.push<Texture2DCreatedReply>(
                            ...
                            Texture2DCreatedReply{
                                dr.status ? RTextureHandle{} : RTextureHandle{dr.resource_index, dr.resource_gen},
                                dr.status},
                            0,
                            dr.request_id
                        );
                    ...
                    case Kind::Texture2DReplacement:
                        builder.push<TextureMipRangeReplacedReply>(
                            ...
                            TextureMipRangeReplacedReply{
                                RTextureHandle{dr.resource_index, dr.resource_gen},
                                dr.logical_base_mip,
                                dr.status},
                            0,
                            dr.request_id
                ...
                std::erase_if(im.pending_deferred_replies_, [lane](const auto& reply) {
    ...
    bool GeneralRenderServer::stepPendingSurfaceReleases()
    {
        ...
        if (im.frame_driver_)
        {
            const bool watermark_short = std::any_of(
                ...
                [&](const Impl::PendingSurfaceRelease& r) { return !r.torn_down && gpu_completed < r.retire_serial; }
            );
            if (watermark_short)
            {
            ...
            for (const auto& r : im.pending_surface_releases_)
                if (r.torn_down && r.request_id != 0)
                    builder.push<TargetReleasedReply>(
                        ...
                        TargetReleasedReply{r.target, 0u},
                        0,
                        r.request_id
                    );
    ...
    {
        auto& im = *impl_;
        im.frame_orchestrator_.renderTargets(im.targets(), *im.renderer_, im.scene_view_batch_, fs);
    }
