...
namespace lux::render
{
    ...
    Expected<void> SkinningFeature::init(const Config& cfg)
    {
        ...
        // retrieved from computeSetLayout(handle, 0).
        {
            ...
            const auto resolved =
                resolveShaderStage(shaders, cfg.compute_shader, EBuiltinShader::SKIN_COMPUTE_COMP);
            if (!resolved)
            ...
            auto pipeline = ctx.pipelineManager().registerComputePipelineReflected(
                prepared->module(0), prepared->info(0), "SkinningCompute", specs);
            if (!pipeline)
                return lux::cxx::unexpected(pipeline.error());
            compute_pipeline_ = *pipeline;

            const VkDescriptorSetLayout skin_layout =
                ctx.pipelineManager().computeSetLayout(compute_pipeline_, 0);
    ...
    void SkinningFeature::addPasses(RGBuilder& builder)
    {
        ...
        auto* vpr = renderScene().sceneRegistry().find<VertexPoolRegistry>();
        if (!vpr || !vpr->isInitialized()) return;
        ...
        builder.addPass("Skinning", ERGPassType::COMPUTE)
            ...
            .useEngineSet(EDescriptorSetSlot::VertexPool)   // a macro override places this at set1 (canonical 7); the slot is resolved at compile time
    ...
    VkDescriptorSet SkinningFeature::resolveSkinDS(const void* self, std::uint32_t frame_slot,
