...
[[nodiscard]] inline std::span<const std::byte> encodeDragDropPayload(
    PayloadTypeIdView type, std::span<const std::byte> bytes,
    std::array<std::byte, kInlineDragDropBytes> &inline_storage,
    std::vector<std::byte> &heap_storage) {
  const auto encoded_size =
      sizeof(DragDropPayloadHeader) + type.name().size() + bytes.size();
  ...
  const DragDropPayloadHeader header{
      type.hash(), static_cast<std::uint32_t>(type.name().size())};
  std::memcpy(encoded.data(), &header, sizeof(header));
  std::memcpy(encoded.data() + sizeof(header), type.name().data(),
              type.name().size());
  if (!bytes.empty()) {
    std::memcpy(encoded.data() + sizeof(header) + type.name().size(),
