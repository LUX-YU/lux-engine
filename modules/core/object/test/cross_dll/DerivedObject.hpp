...
{
    class LUX_OBJECT_CROSS_DLL_DERIVED_API LUX_OBJECT() DerivedObject final : public Object<DerivedObject, BaseObject>
    {
    public:
        static const signal_type<int> derivedChanged;
        static const signal_type<int> derivedChangedB;

        void publishDerived(int value)
        {
            notify<derivedChanged>(value);
        }
        void publishDerivedB(int value)
        {
            notify<derivedChangedB>(value);
        }
        ...
        LUX_METHOD(connectable = true)
        void receiveB(const int& value) noexcept
        {
            last_value_b = value;
        }
