...
vec3 calculatePointLight(PointLightGPU light, vec3 lightPosition, vec3 worldPos, vec3 normal, vec3 viewDir, vec3 kd, vec3 ks, float shininess)
{
...
vec3 calculateSpotLight(SpotLightGPU light, vec3 lightPosition, vec3 worldPos, vec3 normal, vec3 viewDir, vec3 kd, vec3 ks, float shininess)
{
    vec3 lightDir = lightPosition - worldPos;
    float distance = length(lightDir);
...
vec3 accumulateLighting(vec3 worldPos, vec3 normal, vec3 viewDir, vec3 kd, vec3 ks, float shininess)
{
    ...
    for (int i = 0; i < uPointLights.lights.length(); ++i) {
        ...
        totalLighting += calculatePointLight(uPointLights.lights[i], luxPointLightViewPosition(uint(i)), worldPos, normal, viewDir, kd, ks, shininess);
    }

    for (int i = 0; i < uSpotLights.lights.length(); ++i) {
        if (dot(uSpotLights.lights[i].direction, uSpotLights.lights[i].direction) < 1e-8)
            continue;
        totalLighting += calculateSpotLight(uSpotLights.lights[i], luxSpotLightViewPosition(uint(i)), worldPos, normal, viewDir, kd, ks, shininess);
    }
...
// Available when shadow_pcf.glsl / shadow_evsm.glsl is included before this file.
...
vec3 accumulateLightingShadow(vec3 worldPos, vec3 normal, vec3 viewDir, vec3 kd, vec3 ks, float shininess, float view_depth)
{
    ...
    for (int i = 0; i < uPointLights.lights.length(); ++i) {
        ...
        totalLighting += calculatePointLight(uPointLights.lights[i], luxPointLightViewPosition(uint(i)), worldPos, normal, viewDir, kd, ks, shininess) * shadow;
    }
    ...
    for (int i = 0; i < uSpotLights.lights.length(); ++i) {
        ...
        totalLighting += calculateSpotLight(uSpotLights.lights[i], luxSpotLightViewPosition(uint(i)), worldPos, normal, viewDir, kd, ks, shininess) * shadow;
    }
