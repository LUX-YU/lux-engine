...
#include <lux/engine/render/core/EShadowTechnique.hpp>   // EShadowTechnique (technique id)
...
namespace lux::render
{
    namespace
    {
        constexpr uint32_t kMeshShadowExtFlagHZB = 1u << 0;
...
#define LUX_SHADOW_DEBUG_MULTI_VIEW 0
        ...
        /// Layout (std430): vec4 planes[slice_count * 6], then vec4[slice_count] for group indices.
        [[nodiscard]] VkDeviceSize shadowCullSsboSize(uint32_t slice_count)
        {
            ...
            const VkDeviceSize groups_bytes = static_cast<VkDeviceSize>(slice_count) * sizeof(Frustum::Plane); // one vec4 per slice
            return planes_bytes + groups_bytes;
        }
    ...
    void MeshShadowFeature::initAndAttachTo(RenderScene &scene)
    {
        // ---- Ensure builtin shader defaults ----
        {
            ...
            cfg_.shadow_cull_shader    = ensureBuiltinShader(shaders, cfg_.shadow_cull_shader,    EBuiltinShader::MESH_CULL_UNIFIED_COMP);
            cfg_.shadow_compact_shader = ensureBuiltinShader(shaders, cfg_.shadow_compact_shader, EBuiltinShader::MDC_COMPACT_COMP);
            cfg_.shadow_clear_shader   = ensureBuiltinShader(shaders, cfg_.shadow_clear_shader,   EBuiltinShader::CLEAR_COUNT_BUFFERS_COMP);
            // Legacy PCF caster shaders. The caster pipeline now resolves its
            // vert/frag from the active IShadowTechnique (casterVertVariant /
            // casterFragVariant — see ensureCasterPipeline), so these Config
            // fields are no longer read by any pipeline; kept only for wire-Config
            // compatibility (MeshShadowOperation). Candidate for removal.
            cfg_.shadow_vert_shader    = ensureBuiltinShader(shaders, cfg_.shadow_vert_shader,    EBuiltinShader::SHADOW_DEPTH_VERT);
            cfg_.shadow_frag_shader    = ensureBuiltinShader(shaders, cfg_.shadow_frag_shader,    EBuiltinShader::SHADOW_DEPTH_FRAG);
        }

        auto &ctx = renderContext();
        const uint32_t hzb_mode_spec = ((cfg_.extension_flags & kMeshShadowExtFlagHZB) != 0u) ? 1u : 0u;
        ...
        // Shared instance storage (init is idempotent — safe if ForwardMeshFeature already did it)
        instance_res_ = scene.sceneRegistry().find<InstanceResources>();
        {
            ...
            si.arena = &scene.descriptorArena();
            si.initial_capacity = 8192;
            si.max_capacity = 65536;
            instance_res_->init(si);
        }
        ...
        if (shadow_visible_set_layout_ == VK_NULL_HANDLE)
        {
            ...
            auto id = ctx.descriptorService().registerLayout({.bindings = {&visible_binding, 1},
                                                              .debug_name = "ShadowVisibleSetLayout"});
        ...
        initCompactPipeline(cfg_.shadow_compact_shader, "MeshShadowCompactLayout");
        ...
        if (cfg_.shadow_clear_shader.valid())
        {
            auto *shaders = ctx.globalRegistry().find<ShaderResources>();
            auto *shader_obj = shaders ? shaders->get(cfg_.shadow_clear_shader) : nullptr;
            if (shader_obj)
            {
    ...
    void MeshShadowFeature::onFrameBegin(const FeatureFrameContext &ctx)
    {
        ...
        // Populate generic extension data (Phase 2 decoupling path).
        ...
        uint32_t atlas_resolution)
    {
        ...
        // Build group map payload for the SSBO: one vec4 per slice, group index in .x as float bits.
        {
            ...
            const size_t group_bytes = static_cast<size_t>(fd.shadow_slice_count) * sizeof(GroupVec4);
    ...
    void MeshShadowFeature::addPasses(RGBuilder &builder)
    {
        ...
        {
            RGBufferDescription desc{};
            desc.size = static_cast<VkDeviceSize>(safe_visible) * sizeof(uint32_t);
            desc.stride = sizeof(uint32_t);
        ...
        // 0/1/4/5). Sync for the frustum UBO is still the .after("ShadowViewUpload")
        ...
        RGResourceHandle cull_meta_rg{};
        {
            ...
            cull_meta_rg = builder.importBuffer("MeshShadowCullMeta", desc, imp);
        }
        RGResourceHandle section_rg{};
        {
            RGBufferDescription desc{};
            ...
            section_rg = builder.importBuffer("MeshShadowSectionTable", desc, imp);
        }
        RGResourceHandle property_rg{};
        {
            ...
            property_rg = builder.importBuffer("MeshShadowProperty", desc, imp);
        }
        ...
        {
            auto shadow_clear_tds = builder.createTransientDS("MeshShadowClearDS", shadow_clear_ds_layout_, {
        ...
        if (clear_counters_enabled)
            ...
            cull_pass.after("ShadowViewUpload");
        cull_pass.setKernel("ShadowCull", makeKernelConfig(MeshCullKernelConfig{
                                              .frustum_ubo_rg = {},
                                              ...
                                              .geometry_mask = supportedGeometryMask(EPassDomain::Shadow),
        ...
        auto shadow_draw = builder.addPass("MeshShadowDraw", ERGPassType::GRAPHICS);
        ...
        shadow_draw
            ...
            .bindResourceDS(0, shadow_res, &ShadowResources::resolveDS, EDSBindMode::PER_FIF,
                            builder.trackExternalBuffer("ext.ShadowCascade"), ERGResourceType::BUFFER)
        ...
        {
            shadow_draw.bindImmutableDS(3, vp_ds);  // set 3 = vertex pool
        ...
        {
            // No vertex pool — _vp pipeline cannot read vertices. Fall back to
            // the legacy single-pipeline path that ignores set 3 (will fail
            // to render correctly, but at least won't crash on missing DS).
            shadow_draw.forcePassPipeline();
        }
    ...
    void MeshShadowFeature::ensureCasterPipeline(IShadowTechnique& tech)
    {
        ...
        ShaderHandle vh{};
        vh = ensureBuiltinShader(shaders, vh, tech.casterVertVariant());
        auto* vs_obj = shaders->get(vh);
        if (!vs_obj) return;
        auto& shadow_vs = *vs_obj;

        ShaderHandle fh{};
        fh = ensureBuiltinShader(shaders, fh, tech.casterFragVariant());
        auto* fs_obj = shaders->get(fh);
        if (!fs_obj) return;
        auto& shadow_fs = *fs_obj;

        // 4-set layout shared by static + skinned variants —
        // [0]=shadow slices, [1]=instance, [2]=visible, [3]=bindless vertex pool.
        // The static depth VS ignores set 3; the skinned (_vp) VS reads vertices
        // from it. Falls back to the legacy 3-set layout if no vertex pool.
        auto* vpr = renderScene().sceneRegistry().find<VertexPoolRegistry>();
        const VkDescriptorSetLayout vp_layout = vpr ? vpr->descriptorSetLayout() : VK_NULL_HANDLE;
        const std::array<VkDescriptorSetLayout, 4> sl{
            shadow_set0,
            instance_res_->descriptorSetLayout(),
            shadow_visible_set_layout_,
            vp_layout,
        };
        const uint32_t set_count = (vp_layout != VK_NULL_HANDLE) ? 4u : 3u;

        const auto layout = ctx.pipelineLayoutService().getOrCreate({
            .set_layouts    = std::span<const VkDescriptorSetLayout>(sl.data(), set_count),
            .push_constants = {},
            .debug_name     = "MeshShadowCasterLayout"});
        if (!layout.has_value()) return;

        auto tmpl = makeOpaqueMeshTemplate();
        tmpl.descriptor_set_count = set_count;
        tmpl.vertex_shader        = shadow_vs.module;
        tmpl.fragment_shader      = shadow_fs.module;
        ...
        // `shadow_common.glsl`. Front-face culling — a tempting "free bias" —
        ...
        tmpl.pipeline_layout      = layout.value();
        ...
        if (auto* vlr = ctx.globalRegistry().find<VertexLayoutRegistry>();
            vlr && vlr->hasLayout(kDefaultVertexLayoutId))
        ...
        std::vector<const lux::rdesc::ShaderInfo*> infos = {&shadow_vs.info, &shadow_fs.info};
    ...
    {
        // Reuse the shared 8-binding cull layout used by other mesh features.
