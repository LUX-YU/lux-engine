...
namespace lux::render
{
    namespace
    {
        [[nodiscard]] const ReflectedSet* findReflectedSet(const PipelineReflectedInfo& refl, uint32_t set) noexcept
        {
            ...
            return nullptr; // this slot is a reflection hole / a slot explicitly declared but not referenced by this
                            // pipeline's shader
        }
    ...
    RenderGraphCompiler::computeGraphDescriptorLayouts(RGCompiledGraph& compiled, PipelineManager& pipeline_manager)
    {
        ...
        const auto findOrAdd = [&plan](EPlannedSetKind kind, uint32_t set, uint64_t owner) -> PlannedSet& {
        ...
        const auto checkVariantCoherence = [&](const RGPassDescription& pass, std::uint32_t pass_index) {
            ...
            for (const auto& extra : pass.additional_pipelines)
            {
                ...
                for (std::size_t i = 0; i < common; ++i)
                {
                    ...
                    plan.warnings.push_back(
                        renderError<err::graph::VariantLayoutIncompatible>(pass_index, static_cast<std::uint32_t>(i))
                    );
        ...
        for (std::uint32_t pass_index = 0; pass_index < compiled.original_graph.passes.size(); ++pass_index)
        {
            ...
            {
                absorb(
                    pipeline_manager.templateReflection(pass.pipeline_template),
            ...
            for (const auto& extra : pass.additional_pipelines)
            {
                if (extra.valid())
                {
                    absorb(pipeline_manager.templateReflection(extra), graphicsOwnerKey(extra.index));
                }
            }
        ...
        //     the graph.
        {
            ...
            for (std::size_t i = 0; i < plan.sets.size(); ++i)
            {
                ...
                for (std::size_t j = i + 1; j < plan.sets.size(); ++j)
                {
                    ...
                    const bool overlap = a.domain_binding_offset < b.domain_binding_offset + b.domain_binding_count &&
        ...
        //  then does "green" actually mean "switched over".
        {
            ...
            const auto recordGraphics = [&](GraphicsPipelineHandle handle) {
                ...
                record(
                    pipeline_manager.templateReflection(handle),
                    t.debug_name.empty() ? ("template#" + std::to_string(handle.index)) : t.debug_name
                );
            };
        ...
        const auto layoutForSlot = [&plan](const ReflectedSlot& slot, uint64_t owner) -> VkDescriptorSetLayout {
        ...
        const auto writeBack = [&](GraphicsPipelineHandle handle) {
            ...
            auto changed = pipeline_manager.rebuildTemplateLayout(handle, {layouts.data(), layouts.size()});
            if (changed && *changed)
            ...
            const std::string who =
                gtmpl.debug_name.empty() ? ("template#" + std::to_string(handle.index)) : gtmpl.debug_name;
            ...
            {
                if (!contains(derived, s, idx) && !(merged_pipeline && authoredOkInMergedMode(s, idx)))
                    plan.warnings.push_back(
                        renderError<err::graph::AuthoredDerivedSlotMismatch>(static_cast<std::uint32_t>(s), idx, 0u)
                    );
            }
            for (const auto& [s, idx] : derived)
            {
                if (!contains(authored, s, idx))
                    plan.warnings.push_back(
                        renderError<err::graph::AuthoredDerivedSlotMismatch>(static_cast<std::uint32_t>(s), idx, 1u)
                    );
            }
        };
        ...
        const auto project = [&](const PipelineReflectedInfo* refl, const char* name, uint32_t pass_index) {
            if (!refl || refl->slots.empty())
            {
                return;
            }

            plan.max_sets_current = std::max(plan.max_sets_current, static_cast<uint32_t>(refl->slots.size()));
            ...
            {
                if (!feature_domain_has_engine)
                    --strict; // the FEATURE domain was already empty
            ...
            if (projected > budget)
                plan.over_budget.push_back(
                    std::string{name} + "(" + std::to_string(projected) + " > " + std::to_string(budget) + ")"
                );
            if (strict > budget)
                plan.over_budget_strict.push_back(
                    std::string{name} + "(" + std::to_string(strict) + " > " + std::to_string(budget) + ")"
                );
        };
        ...
        const auto accumulate = [&](const PipelineReflectedInfo* refl, uint64_t owner, const char* pass_name) {
            if (!refl || refl->slots.empty())
            {
                return;
            }
                ...
                const auto flag = [&](uint32_t got, uint32_t min, const char* what) {
                    if (got > min)
                        plan.stage_over_min.push_back(
                            std::string{pass_name} + " / " + stage + " " + what + " " + std::to_string(got) +
                            " > Vulkan 最低保证 " + std::to_string(min)
                        );
                };
        ...
        for (std::size_t pi = 0; pi < compiled.original_graph.passes.size(); ++pi)
        {
            ...
            if (pass.pipeline_template.valid())
            {
                project(pipeline_manager.templateReflection(pass.pipeline_template), pass.name.c_str(), pass_index);
                accumulate(
                    pipeline_manager.templateReflection(pass.pipeline_template),
                    graphicsOwnerKey(pass.pipeline_template.index),
                    pass.name.c_str()
            ...
            for (const auto& extra : pass.additional_pipelines)
                if (extra.valid())
                {
                    ...
                    accumulate(
                        pipeline_manager.templateReflection(extra),
        ...
        const auto mergedFor = [&](const PipelineReflectedInfo* refl, uint64_t owner) {
            ...
            for (std::size_t d = 0; d < 4; ++d)
                if (touches[d])
                {
                    addDomain(v, domain_v[d]);
                    addDomain(f, domain_f[d]);
                    addDomain(c, domain_c[d]);
                }
