...
struct Allocation
{
    ...
    static void* allocate(void* context, void* pointer, std::size_t old_size, std::size_t size)
    {
        ...
        if (self.fail && (!pointer || size > old_size)) { ++self.failed; return nullptr; }
...
int main(int argc, char** argv)
{
    ...
    const bool diagnostics = mode.ends_with("diagnostics");
    const bool protected_mode = mode.starts_with("protected");
    ...
    if (mode == "protected-errors")
    {
        int live{};
        struct Lifetime
        {
            int& live;
            explicit Lifetime(int& count) : live(count) { ++live; }
            ~Lifetime() { --live; }
        };
        {
            ...
            {
                protected_value.fail_after = point;
                assert(!protected_value.push(state) && lua_gettop(state) == 0 && live == 1);
                std::printf("PROTECTED_ERROR point=%d top=0 live=1\n", point);
            }
            protected_value.fail_after = 0;
            allocation.fail = true;
            assert(!protected_value.push(state) && allocation.failed > 0 && lua_gettop(state) == 0 && live == 1);
            allocation.fail = false;
            assert(protected_value.push(state) && lua_gettop(state) == 1 && live == 1);
        }
        assert(live == 0);
        lua_close(state);
        std::puts("PROTECTED_ERROR OOM scratch recovery outer-owner live=0 PASS");
        return 0;
    }
    ...
    if (diagnostics) lua_sethook(state, ProtectedRecord::count, LUA_MASKCALL, 0);
    ...
    std::printf("VALUE_PROTECTION mode=%.*s c_calls=%zu accounting=%d\n",
        static_cast<int>(mode.size()), mode.data(), ProtectedRecord::calls, diagnostics);
}
