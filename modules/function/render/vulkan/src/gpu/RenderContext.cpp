...
{
    Expected<std::shared_ptr<RenderContext>> RenderContext::create(
        ResourceContext& res_ctx,
        CreateInfo info
    )
    {
        if (info.frames_in_flight < 1
            || info.frames_in_flight > kMaxFramesInFlight)
        {
        ...
        if (!info.pipeline_mgr
            || !info.descriptor_layouts
            || !info.global_resources)
        {
    ...
    RenderContext::RenderContext(
        ...
        , capacity_plan_(info.capacity_plan)
        , texture_sampling_catalog_(
              builtinTextureSamplingRepresentationCatalog())
    {
        ...
        pipeline_layout_service_ = std::make_unique<PipelineLayoutService>(
            vk_device_,
            res_ctx.deviceContext().physicalDevice()
        ...
        pipeline_mgr_->setReflectedLayoutEnv(
            *descriptor_layouts_, *descriptor_service_, *pipeline_layout_service_);
    ...
    namespace
    {
        ...
        {
        public:
            explicit ExportableBufferCandidate(VkDevice device) noexcept
                : device_(device)
            {
            }
            ...
            ExportableBufferCandidate& operator=(
                const ExportableBufferCandidate&
            ) = delete;

            [[nodiscard]] ExportableBuffer publish(
                std::uint64_t external_handle,
    ...
    Expected<ExportableBuffer> RenderContext::createExportableBuffer(
        uint64_t size,
    ...
    )
    {
        ...
        const VkResult buffer_created = vkCreateBuffer(
            dev,
            &buffer_info,
            nullptr,
            &candidate.buffer
        ...
        {
            return renderFailure<err::device::VulkanCallFailed>(
                encodeVkResult(allocated)
            );
        }
        if (candidate.memory == VK_NULL_HANDLE)
            return renderFailure<err::device::VulkanObjectCreationFailed>();

        const VkResult bound = vkBindBufferMemory(
            dev,
            candidate.buffer,
            candidate.memory,
            0
        ...
        {
            return renderFailure<err::device::VulkanCallFailed>(
                encodeVkResult(bound)
            );
        }

        lux::gapi::vk::ExternalHandle handle =
            lux::gapi::vk::kInvalidExternalHandle;
        const VkResult exported = exportMemoryHandle(
            dev,
            candidate.memory,
            handle
        );
        ...
        return candidate.publish(
            toU64(handle),
            static_cast<uint64_t>(mem_req.size)
        );
    }

    Expected<ExportableTimelineSemaphore>
    RenderContext::createExportableTimelineSemaphore()
    {
        ...
        const VkResult created = vkCreateSemaphore(
            dev,
            &sem_info,
            nullptr,
            &raw_semaphore
        ...
        auto semaphore = lux::gapi::vk::Semaphore::adopt(
            dev,
            raw_semaphore
        );
