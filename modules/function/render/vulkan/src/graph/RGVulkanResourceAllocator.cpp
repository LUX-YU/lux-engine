...
namespace lux::render
{
    ...
    bool RGVulkanResourceAllocator::tryAcquireFromPool(const TransientResourceKey& key, RGPhysicalResource& out)
    {
    ...
    void RGVulkanResourceAllocator::returnToPool(const TransientResourceKey& key, const RGPhysicalResource& res)
    {
    ...
    )
    {
        ...
        for (size_t i = 0; i < resource_count; ++i)
        {
            ...
            std::optional<RenderError> create_err;
                    ...
                    [&](auto&& arg) {
                        ...
                        if constexpr (std::is_same_v<T, RGTextureDescription>)
                        {
                            ...
                            const uint32_t count =
                                phys.image_getter(images.data(), static_cast<uint32_t>(images.size()));
                            if (count == 0u)
                            {
                                create_err = renderError<err::internal::InvalidArgument>();
                                return;
                        ...
                        else if constexpr (std::is_same_v<T, RGBufferDescription>)
                        {
                            ...
                            const uint32_t count =
                                phys.buffer_getter(buffers.data(), static_cast<uint32_t>(buffers.size()));
                            if (count == 0u)
                            {
                                create_err = renderError<err::internal::InvalidArgument>();
                                return;
    ...
    Expected<void> RGVulkanResourceAllocator::createImage(
        const RGResourceDescription& resource,
        RGPhysicalResource& phy,
        VkExtent2D extent,
        uint32_t frames_in_flight
    )
    {
        ...
        const uint32_t copies = (phy.lifetime == ERGResourceLifetime::PING_PONG)
        ...
        // 纯 attachment 用途的 transient(usage 无 SAMPLED/STORAGE/TRANSFER,
        // 即内容从不离开渲染作用域——如 local_read 后的 G-buffer)标记
        // TRANSIENT_ATTACHMENT 并首选 LAZILY_ALLOCATED:tiler 上零物理显存,
        // 桌面无该内存类型时 VMA 按 preferred 自动回退普通显存。这类分配
        // 不进 transient pool(pool 钉死了普通 GPU_ONLY 内存类型)。
        constexpr VkImageUsageFlags kAttachmentOnlyUsage = VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT |
        ...
        //
        // 惰性附件用的是**另一个**池(钉在 LAZILY_ALLOCATED 内存类型上),因为
        // VmaPool 的内存类型是创建时钉死的,两种内存不能共用一个池。设备没有惰性
        // 类型时,惰性分支退回**普通 transient 池** —— TRANSIENT_ATTACHMENT 这个
        // usage 位照加(它本身无害,且在有惰性内存的设备上是启用条件),但内存不再
        // 逐张独立分配。
        //
        // 此前这里写的是「preferredFlags 加 LAZILY,让 VMA 自行回退」并且整支跳过
        // 池:在没有惰性内存的桌面上,每张纯附件 transient 都是一次独立
        // vkAllocateMemory,FIF × 三张 G-buffer —— 恰是建池要避免的那件事,而且
        // 从外面完全看不出来发生了回退。
        if (phy.lifetime == ERGResourceLifetime::TRANSIENT)
        {
            if (lazy_attachment && lazy_memory_ == ELazyMemory::Unprobed)
            {
                ...
                // 这是设备属性,不随图变化,所以只做一次。
                uint32_t lazy_type = 0;
                VmaAllocationCreateInfo probe{};
                probe.usage = VMA_MEMORY_USAGE_GPU_ONLY;
                probe.requiredFlags = VK_MEMORY_PROPERTY_LAZILY_ALLOCATED_BIT;
                ...
                if (found == VK_SUCCESS)
                {
                    ...
                    pool_ci.memoryTypeIndex = lazy_type;
    ...
    Expected<void> RGVulkanResourceAllocator::createBuffer(
        const RGResourceDescription& resource,
        RGPhysicalResource& phy,
        uint32_t frames_in_flight
    )
