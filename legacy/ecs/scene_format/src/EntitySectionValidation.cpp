...
#include "EntitySectionCodecCommon.hpp"
...
namespace lux::ecs::scene_format
{
    namespace
    {
        ...
        [[nodiscard]] bool validStableIdWithinLimits(
            const lux::ecs::ComponentSchemaId& id,
            const EntitySectionCodecLimits& limits) noexcept
        {
            return lux::ecs::isValidComponentSchemaId(id) &&
        ...
        [[nodiscard]] EntitySectionCodecResult<void>
        validateSectionImpl(
            ...
            const EntitySectionCodecLimits& limits)
        {
                 ...
                 ++index)
            {
                ...
                {
                    return lux::cxx::unexpected(failure(
                        EEntitySectionCodecError::INVALID_NAME,
            ...
            {
                return lux::cxx::unexpected(failure(
                    EEntitySectionCodecError::LIMIT_EXCEEDED,
                    "EntitySection collection exceeds codec limit"));
            }
            for (const auto& schema : image.schemas)
            {
                if (!validStableIdWithinLimits(schema.id, limits) ||
            ...
            {
                return lux::cxx::unexpected(failure(
                    EEntitySectionCodecError::DUPLICATE_ID,
            ...
            for (const auto& archetype : image.archetypes)
            {
                ...
                {
                    return lux::cxx::unexpected(failure(
                        EEntitySectionCodecError::INVALID_REFERENCE,
            ...
            for (const auto& attachment : image.attachments)
            {
                ...
                {
                    return lux::cxx::unexpected(failure(
                        EEntitySectionCodecError::DIGEST_MISMATCH,
