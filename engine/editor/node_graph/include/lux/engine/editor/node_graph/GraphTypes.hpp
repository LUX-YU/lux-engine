...
namespace lux::editor::node_graph
{
    ...
    /// domain Pin* — domain graph rebuilds (deletes) its pin objects on
    ...
    /// unlimited; domain graph exec-in = unlimited (fan-in join), exec-out = 1,
    ...
    /// Palette entry. String identity mirrors domain graph's NodeCreatInfo
