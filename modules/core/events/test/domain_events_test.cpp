...
static void
check(bool cond, const std::string& desc)
{
    if (cond)
    {
        std::cout << "  PASS  " << desc << "\n";
...
main()
{
    ...
    banner("basic publish / subscribe / delivery");
    {
        ...
        int calls = 0;
        int seen_v = 0;
        auto sub = bus.subscribe<ev::Basic>(pump, [&](const ev::Basic& e) {
            ++calls;
            seen_v = e.v;
        }
        );
    ...
    banner("multi-pump delivery / same-pump shared batch");
    {
        ...
        auto s1 = bus.subscribe<ev::Multi>(pump, [&](const ev::Multi& e) {
            ++a1;
            addr1 = &e;
        }
        );
        auto s2 = bus.subscribe<ev::Multi>(pump, [&](const ev::Multi& e) {
            ++a2;
            addr2 = &e;
        }
        );
        auto s3 = bus.subscribe<ev::Multi>(pump_b, [&](const ev::Multi&) { ++b1; });

        bus.publish(ev::Multi{7});
        pump.drain();
        check(a1 == 1 && a2 == 1, "both subscribers on pump A delivered");
        check(addr1 == addr2, "same-pump subscribers share one batch (same const E& address)");
    ...
    banner("per-channel FIFO");
    {
        ...
        auto sub = bus.subscribe<ev::Order>(pump, [&](const ev::Order& e) { order.push_back(e.v); });
    ...
    {
        int chain_calls = 0;
        auto sub = bus.subscribe<ev::Chain>(pump, [&](const ev::Chain& e) {
            ++chain_calls;
            if (e.remaining > 0)
        ...
        pump.drain();
        check(chain_calls == 1, "handler-published event NOT delivered in the same drain");

        pump.drainUntilEmpty();
        check(chain_calls == 10, "drainUntilEmpty converges the 9-deep republish chain");

        int inf_calls = 0;
        auto inf = bus.subscribe<ev::Infinite>(pump, [&](const ev::Infinite&) {
            ++inf_calls;
        ...
        check(inf_calls == 8, "self-feeding chain stopped by the max_rounds cap");
    ...
    banner("backpressure: UNBOUNDED / DROP_NEWEST / DROP_OLDEST");
    {
        ...
        auto su = bus.subscribe<ev::Unbounded>(pump, [&](const ev::Unbounded&) { ++unbounded_calls; });
        ...
        auto sn = bus.subscribe<ev::DropNew>(pump, [&](const ev::DropNew& e) { got_new.push_back(e.v); });
        for (int i = 0; i < 10; ++i)
            bus.publish(ev::DropNew{i});
        pump.drain();
        check(got_new.size() == 4, "DROP_NEWEST: capacity 4 of 10 delivered");
        check(got_new == std::vector<int>({0, 1, 2, 3}), "DROP_NEWEST keeps the OLDEST four (first is most valuable)");
        ...
        auto so = bus.subscribe<ev::DropOld>(pump, [&](const ev::DropOld& e) { got_old.push_back(e.v); });
        for (int i = 0; i < 10; ++i)
            bus.publish(ev::DropOld{i});
        pump.drain();
        check(got_old.size() == 4, "DROP_OLDEST: capacity 4 of 10 delivered");
        check(got_old == std::vector<int>({6, 7, 8, 9}), "DROP_OLDEST keeps the NEWEST four (latest is the truth)");

        // configure 在订阅之后也生效(已有 PerPump 同步更新)。
        int late_calls = 0;
        auto sc = bus.subscribe<ev::CfgLate>(pump, [&](const ev::CfgLate&) { ++late_calls; });
        ...
        check(
            late_calls == 2,
            "configure after subscribe applies to the "
    ...
    banner("subscription RAII / move / self-unsubscribe in handler");
    {
        int calls = 0;
        {
            auto sub = bus.subscribe<ev::Raii>(pump, [&](const ev::Raii&) { ++calls; });
        ...
        auto sub_a = bus.subscribe<ev::Raii>(pump, [&](const ev::Raii&) { ++calls; });
        Subscription sub_b = std::move(sub_a);
        check(!sub_a.active() && sub_b.active(), "move: source inactive, target active");
        ...
        Subscription self;
        self = bus.subscribe<ev::SelfUnsub>(pump, [&](const ev::SelfUnsub&) {
            ++self_calls;
            self.reset();
        }
        );
        auto witness = bus.subscribe<ev::SelfUnsub>(pump, [&](const ev::SelfUnsub&) { ++witness_calls; });
        for (int i = 0; i < 3; ++i)
            bus.publish(ev::SelfUnsub{i});
        pump.drain();
        check(self_calls == 1, "self-unsubscribe: handler stops mid-batch after reset()");
    ...
    banner("subscribe during drain: delivery starts next batch");
    {
        ...
        Subscription late;
        auto first = bus.subscribe<ev::MidSub>(pump, [&](const ev::MidSub&) {
            ++first_calls;
            if (!late.active())
                late = bus.subscribe<ev::MidSub>(pump, [&](const ev::MidSub&) { ++late_calls; });
        }
        );
        ...
        pump.drain();
        check(first_calls == 2 && late_calls == 0, "mid-drain subscriber does NOT receive the current batch");
    ...
    banner("main-thread ownership / no replay");
    {
        ...
        int calls = 0;
        auto sub = bus.subscribe<ev::Emergency>(pump, [&](const ev::Emergency&) { ++calls; });
        pump.drain();
    ...
    banner("InlineRecord pack / unpack / truncation");
    {
        ...
        check(rec.pack(42) && rec.pack(3.5) && rec.packString("hello"), "pack int + double + string into 64 bytes");
        ...
        check(!tiny.packString("abcdefghij"), "oversize string reports truncation");
        check(tiny.truncated(), "truncated flag set");
        lux::events::InlineRecordReader rd2(tiny);
        std::string_view partial;
        check(
            rd2.unpackString(partial) && partial == "abcdef",
