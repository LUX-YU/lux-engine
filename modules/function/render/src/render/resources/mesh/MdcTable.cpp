...
{
    uint32_t MdcTable::registerInstance(uint8_t geometry_kind, uint32_t bucket_id, uint32_t section_id)
    {
        MdcKey key{geometry_kind, bucket_id, section_id};
    ...
    void MdcTable::unregisterInstance(uint32_t mdc_index)
    {
        ...
        if (e.instance_count > 0)
        {
            ...
            if (e.instance_count == 0)
            {
                ...
                key_to_index_.erase(MdcKey{e.geometry_kind, e.bucket_id, e.section_id});
