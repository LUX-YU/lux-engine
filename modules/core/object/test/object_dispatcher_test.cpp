...
namespace
{
    ...
    {
        CloseReentryProbe(
            lux::object::ObjectDispatcherRef value,
            std::shared_ptr<std::atomic_bool> armed_value,
            std::atomic<lux::object::detail::EPostStatus>* status_value
        ) noexcept
        ...
        ~CloseReentryProbe()
        {
            ...
            status->store(lux::object::detail::post(dispatcher, std::move(nested)), std::memory_order_release);
        }
    ...
    {
    public:
        explicit EventReceiver(lux::object::ObjectDispatcherRef dispatcher) : Object(std::move(dispatcher))
        {
        }
    ...
    void runQueuedScenario(bool disconnect, bool destroy_receiver)
    {
        ...
        auto connection =
            sender.observe<IntSender::changed, &Receiver::receive, lux::object::EDelivery::AUTO>(*receiver_pointer);
        assert(connection);
        sender.publish(7);
        if (disconnect)
        {
            std::thread disconnect_thread([connection_copy = *connection]() mutable { connection_copy.disconnect(); });
...
main()
{
    ...
    assert(
        lux::object::detail::post(inline_queue.dispatcherRef(), std::move(small_message)) ==
    ...
    assert(
        lux::object::detail::post(inline_queue.dispatcherRef(), std::move(large_message)) ==
        lux::object::detail::EPostStatus::POSTED);
    assert(inline_queue.dispatchPending() == 1);
...

    {
        ...
        auto connection =
            sender_without_dispatcher.observe<IntSender::changed, &Receiver::receive, lux::object::EDelivery::QUEUED>(
                receiver
            );
        assert(!connection);
...

    {
        ...
        auto observed =
            sender.observe<IntSender::changed, &Receiver::receive, lux::object::EDelivery::DIRECT>(receiver);
        assert(observed);
...

    {
        ...
        auto scoped = sender.observeScoped<IntSender::changed>(RetainedCallable{std::move(lifetime)});
        assert(scoped);
...

    {
        ...
        auto message =
            lux::object::detail::makeMessage(CloseReentryProbe{queue.dispatcherRef(), std::move(armed), &status});
        assert(
            lux::object::detail::post(queue.dispatcherRef(), std::move(message)) ==
            lux::object::detail::EPostStatus::POSTED);
        queue.close();
...

    {
        ...
        lux::object::ObjectWeakRef expired_target;
        {
            ...
            assert(lux::object::postEvent(expired_target, Ping{23}) == lux::object::EEventPostStatus::POSTED);
        }
    ...
    lux::object::ObjectDispatcherRef closed_dispatcher;
    {
        ...
        EventReceiver receiver{queue.dispatcherRef()};
        assert(lux::object::postEvent(receiver.weakRef(), Ping{31}) == lux::object::EEventPostStatus::POSTED);
        assert(queue.dispatchPending() == 1);
    ...
    assert(
        lux::object::detail::post(closed_dispatcher, std::move(message)) == lux::object::detail::EPostStatus::CLOSED);
    assert(!invoked);
}
