...
void main()
{
    ...
    for (int i = 0; i < uDirectionalLights.lights.length(); ++i) {
        ...
        float diff_t;
        if (use_ramp)
            diff_t = texture(uTex[m.tex[1].bindless], vec2(diff_raw, 0.5)).r;
        else
