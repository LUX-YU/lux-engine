...
class Generator:
    ...
    def body(self, t, attrs):
        ...
        if kind in ("ConstantArrayType", "ArrayType") or template in ("std::array", "std::vector", "std::deque", "std::list"):
            ...
            if not fixed:
                lines += ['    ImGui::BeginDisabled(index == 0);',
                          '    const bool move_up = ImGui::SmallButton("Up");',
                          '    ImGui::EndDisabled();', '    ImGui::SameLine();',
                          '    ImGui::BeginDisabled(index + 1 == value.size());',
                          '    const bool move_down = ImGui::SmallButton("Down");',
                          '    ImGui::EndDisabled();',
                          '    if (move_up || move_down)', '    {',
                          '        const auto changed = prepareContainer(value, state, [&](auto& next) {',
                          '            const auto first = std::next(next.begin(), static_cast<std::ptrdiff_t>(index));',
                          '            const auto second = std::next(next.begin(), static_cast<std::ptrdiff_t>(move_up ? index - 1 : index + 1));',
                          '            std::iter_swap(first, second);',
                          '            return true; });',
                          '        merge(result, immediate(changed));', '        return result;', '    }']
                lines += [ '    const bool remove = ImGui::SmallButton("Remove");', '    if (remove)', '    {',
                          "        const auto changed = prepareContainer(value, state, [&](auto& next) {",
        ...
        if template == "std::variant":
            ...
            for i, arg in enumerate(alternatives):
                lines += ['    {', f'    IdScope alternative_id{{{i}}};',
                          f'    const bool selected_{i} = ImGui::Selectable({literal(arg)}, value.index() == {i});',
                          f'    if (selected_{i})', '    {',
                          '        merge(result, immediate(prepareContainer(value, state, [](auto& next) {',
    ...
    def associative(self, t):
        ...
        if mapping:
            lines += ['    IdScope mapped_scope{1};',
                      f'    merge(result, {value_fn}(item.second, state));']
        lines += ['    const bool remove = ImGui::SmallButton("Remove");', '    if (remove)', '    {',
                  '        merge(result, immediate(prepareContainer(value, state, [&](auto& next) {',
    ...
    def component(self, name):
        ...
        def field_binding(tid, properties, identity, label, access_path, immutable):
            ...
            if kind in ("ConstantArrayType", "ArrayType"):
                ...
                if not element or count is None:
                    raise ValueError(f"fixed array extent unavailable: {tid}")
                lines = []
                for index in range(int(count)):
                    lines += field_binding(element, properties, f"{identity}[{index}]", f"{label}[{index}]",
                                           f"{access_path}[{index}]", immutable)
                return lines
            if kind == "RecordType" and tid not in self.config.get("custom_types", []) and properties.get("widget", "default") == "default":
                ...
                if record and "luxref::class" in annotations(record):
                    ...
                    for child_id in record.get("field_decls", []):
                        ...
                        attributes = annotations(child)
                        if child.get("visibility") != 1 or "luxref::property::skip" in attributes:
                            continue
                        read_only = immutable or attributes.get("readonly") == "true" or attributes.get("widget") == "readonly"
                        lines += field_binding(child["type_id"], attributes, f'{identity}.{child["name"]}',
                                               attributes.get("display_name", child["name"]),
                                               f'{access_path}.{child["name"]}', read_only)
                    return lines + ['            ImGui::TreePop();', '        }']
            fn = self.draw_function(tid, properties)
            return [f'        state.field<{name}, std::remove_cvref_t<decltype(std::declval<{name}>().{access_path})>>(',
                    f'            document, target, frame, {literal(identity)}, {literal(label)},',
                    f'            [](auto& component) noexcept {{ return &component.{access_path}; }},',
                    f'            {fn}, {str(immutable).lower()});']
        ...
        for field_id in decl.get("field_decls", []):
            ...
            if field.get("visibility") != 1 or "luxref::property::skip" in properties:
                continue
            label = properties.get("display_name", field["name"])
            immutable = properties.get("readonly") == "true" or properties.get("widget") == "readonly"
            immutable = immutable or name.endswith("::Parent")
            draws += field_binding(field["type_id"], properties, field["fq_name"], label, field["name"], immutable)
        includes = '\n'.join(f'#include <{h}>' for h in self.config.get('custom_headers', []))
        text = f'''// Generated typed Inspector for {name}; linked only into the Editor GUI target.
...
ComponentBinding binding_{suffix}()
{{
    return {{lux::cxx::typeToken<{name}>(), {literal(attrs.get('display_name', decl['name']))}, draw_{suffix}}};
}}
}}
'''
        return suffix, text
...
def generate(config, data):
    ...
    for component in config["components"]:
        ...
        declarations.append(f"ComponentBinding binding_{suffix}();")
