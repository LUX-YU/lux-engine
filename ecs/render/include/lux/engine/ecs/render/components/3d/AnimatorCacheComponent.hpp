...
#include <lux/engine/resource/asset/AssetRef.hpp>            // 驻留票(流送闸门可见性)
...
namespace lux::ecs
{
    struct AnimatorCacheComponent
    {
        ...
        // 驻留票:动画正在用这两个资产,流送驱逐的 isReferenced 闸门必须看得见
        //(此前动画消费者对账本全瞎,驱逐清单只能靠「不驱逐 skeleton」硬撑)。
        // 票在**换装分支**换发,与每帧指针重查无关 —— 指针缓存与驻留兴趣是两件事:
        // 指针每帧刷新防悬垂,票跨帧持有防驱逐。
        lux::asset::AssetRef skeleton_ref{};
