...
namespace lux::editor::gui
{
    ...
    WindowResult<void> EditorWindow::installLayout(const lux::ui::SplitLayout &layout) noexcept
    {
        if (auto check = impl_->check(); !check)
        {
            return check;
        ...
        if (!impl_->ui->validateSplitLayout(layout))
        {
            return fail(EWindowError::LAYOUT_FAILURE);
        }
        {
            impl_->ui->setSplitLayout(layout);
        }
        return {};
    }
