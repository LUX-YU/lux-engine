// =============================================================================
//  MaterialToGraph.cpp  —  ImportedMaterialDesc -> MaterialGraph 转换实现
//  （从退役的 material_graph 中性核迁入 ShaderGen；namespace lux::shadergen::material）
// =============================================================================
...
{
    using namespace ::lux::rdesc;  // 数据模型（MaterialGraph/Node/各节点/槽声明）在 description
    ...
    namespace
    {
        ...
        // 把所有 SampleTexture / 编织节点都建在一张图里的小工具集。
        struct Builder
        {
            MaterialGraph& g;
            node_id        uv;   // 共享的 Input(UV0)
            node_id        out;  // 唯一 OutputSurface
            std::unordered_map<uint32_t, node_id> sample_cache;  // texture_index -> SampleTexture 节点

            explicit Builder(MaterialGraph& graph) : g(graph)
            {
                uv  = g.addNode(std::make_unique<InputNode>());                // 默认 UV0
                out = g.addNode(std::make_unique<OutputSurfaceNode>());
            }

            // 确保图里有覆盖到 texture_index 的纹理槽（图槽 index == texture_index）。
            ...
            // 采样某个 texture_index（同一纹理共享一个 SampleTexture 节点），返回 vec4 节点。
            node_id sample(uint32_t tex_index)
            {
                ...
                g.connect(uv, 0, id, 0);   // uv -> 采样
                sample_cache.emplace(tex_index, id);
                return id;
            }

            // 从 vec4 源 swizzle 出 `out_type`，components 给定每个输出通道取源的哪一分量。
