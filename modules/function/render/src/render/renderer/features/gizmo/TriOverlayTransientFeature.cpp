...
TriOverlayTransientFeature::TriOverlayTransientFeature(Config cfg)
    : RenderFeature(RenderFeature::Config{.name = "TriOverlayTransient"})
    , cfg_(std::move(cfg))
{
}
...
void TriOverlayTransientFeature::initAndAttachTo(RenderScene& /*scene*/)
{
    ...
    // ---- Shaders ----
    cfg_.vertex_shader   = cv.loadBuiltinShader(EBuiltinShader::TRI_OVERLAY_VERT, cfg_.vertex_shader);
    cfg_.fragment_shader = cv.loadBuiltinShader(EBuiltinShader::TRI_OVERLAY_FRAG, cfg_.fragment_shader);
    ...
    tmpl.pipeline_layout = cv.buildStandardGraphicsLayout(
        tmpl.descriptor_set_count, infos, "TriOverlayTransientLayout");
...
void TriOverlayTransientFeature::createSlotBuffers()
{
    ...
    for (auto& slot : slots_) {
        ...
        assert(r == VK_SUCCESS && "Failed to create transient tri overlay buffer");
        (void)r;
