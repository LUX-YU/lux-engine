...
struct CloseReentryProbe final {
  CloseReentryProbe(
      ...
      : dispatcher(std::move(value)), armed(std::move(armed_value)),
        status(status_value) {}
...
void runQueuedScenario(bool disconnect, bool destroy_receiver) {
  ...
  auto connection =
      sender.observe<IntSender::changed, &Receiver::receive,
...
int main() {
  ...
  auto small_message =
      lux::object::detail::makeMessage(SmallMessageProbe{&small_invoked});
  assert(SmallMessageProbe::allocations.load(std::memory_order_relaxed) == 0);
  lux::object::ObjectMessageQueue inline_queue;
  assert(lux::object::detail::post(inline_queue.dispatcherRef(),
                                   std::move(small_message)) ==
...

  {
    ...
    auto connection = sender_without_dispatcher.observe<
        IntSender::changed, &Receiver::receive, lux::object::EDelivery::QUEUED>(
        receiver);
    assert(!connection);
    assert(connection.error() ==
           lux::object::EObserveError::SENDER_HAS_NO_DISPATCHER);
  }

  {
    ...
    assert(
        ...
        lux::object::detail::EPostStatus::POSTED);
    queue.close();
    assert(status.load(std::memory_order_acquire) ==
...

  {
    ...
    assert(lux::object::postEvent(target, Ping{47}) ==
           lux::object::EEventPostStatus::POSTED);
    {
      std::scoped_lock lock{mutex};
      drain = true;
