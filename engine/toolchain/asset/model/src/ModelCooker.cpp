...
namespace lux::toolchain
{
    namespace
    {
        ...
        [[nodiscard]] ModelCookFailure failure(EModelCookError code, std::uint32_t ordinal, std::string detail) noexcept
        {
        ...
        [[nodiscard]] Eigen::Affine3f toEigen(const aiMatrix4x4 &value) noexcept
        {
            ...
            result.matrix() << value.a1, value.a2, value.a3, value.a4, value.b1, value.b2, value.b3, value.b4, value.c1,
                value.c2, value.c3, value.c4, value.d1, value.d2, value.d3, value.d4;
            return result;
        }
        ...
        std::string sourceKey(std::string_view name)
        {
            std::string spelling(name);
            std::replace(spelling.begin(), spelling.end(), '\\', '/');
            const auto path = std::filesystem::path(spelling).lexically_normal();
        ...
        struct CapturedFiles final
        {
            ...
            explicit CapturedFiles(const ModelSource &value) : source(value)
            {
                ...
                for (const auto &file : value.files)
                {
                    ...
                    {
                        rejected_path = file.path.empty() ? "<empty>" : file.path;
                    }
                }
            }
            ...
            const ModelSourceFile *find(std::string_view path)
            {
                ...
                if (key.empty())
                {
                    rejected_path = path.empty() ? "<empty>" : path;
                    return nullptr;
                }
                ...
                if (!std::ranges::any_of(requested.paths, [&](const auto &value) { return sourceKey(value) == key; }))
                {
                    ...
                    std::replace(spelling.begin(), spelling.end(), '\\', '/');
                    requested.paths.push_back(std::filesystem::path(spelling).lexically_normal().generic_string());
                }
                return nullptr;
            }
        };
        ...
        class CapturedStream final : public Assimp::IOStream
        {
          public:
            ...
            std::size_t Read(void *output, std::size_t size, std::size_t count) override
            {
                ...
                const auto units = (std::min)(count, (bytes_.size() - cursor_) / size);
                std::memcpy(output, bytes_.data() + cursor_, units * size);
                cursor_ += units * size;
                return units;
            }
        ...
        [[nodiscard]] lux::cxx::expected<void, ModelCookFailure> cookMaterials(CookContext &context)
        {
            ...
            for (std::uint32_t ordinal = 0U; ordinal < context.scene.mNumMaterials; ++ordinal)
            {
                ...
                auto normal = cookTextureReference(context, source, aiTextureType_NORMALS, ETextureProfile::NORMAL);
                if (!normal)
                {
                    return lux::cxx::unexpected(normal.error());
                }
                imported.normal_texture = *normal;

                auto metallic_roughness =
                    cookTextureReference(context, source, aiTextureType_METALNESS, ETextureProfile::METALLIC_ROUGHNESS);
                ...
                auto occlusion =
                    cookTextureReference(context, source, aiTextureType_AMBIENT_OCCLUSION, ETextureProfile::OCCLUSION);
                if (!occlusion)
                {
                    return lux::cxx::unexpected(occlusion.error());
                }
                imported.occlusion_texture = *occlusion;

                auto emissive_texture =
                    cookTextureReference(context, source, aiTextureType_EMISSIVE, ETextureProfile::EMISSIVE);
                if (!emissive_texture)
                {
                    return lux::cxx::unexpected(emissive_texture.error());
                }
                                ...
                                std::vector<lux::rdesc::Vertex> &vertices)
        {
            ...
            for (std::uint32_t bone = 0U; bone < mesh.mNumBones; ++bone)
            {
                ...
                if (found == context.bone_by_name.end())
                {
                    continue;
                }
        ...
        [[nodiscard]] lux::cxx::expected<void, ModelCookFailure> cookMeshes(CookContext &context)
        {
            ...
            for (std::uint32_t ordinal = 0U; ordinal < context.scene.mNumMeshes; ++ordinal)
            {
                ...
                for (std::uint32_t vertex = 0U; vertex < source.mNumVertices; ++vertex)
                {
                    ...
                    {
                        destination.uv = {source.mTextureCoords[0][vertex].x, source.mTextureCoords[0][vertex].y};
                    }
                    if (source.mTangents)
                    {
                ...
                if (source.HasBones())
                {
                    fillBoneInfluences(context, source, mesh->vertices);
                }
                ...
                if (!source.HasBones() && mesh->indices.size() >= minimum_lod_indices)
                {
                    ...
                    {
                        const auto target =
                            (std::max)(std::size_t{3U}, static_cast<std::size_t>(mesh->indices.size() * ratio));
        ...
        [[nodiscard]] lux::cxx::expected<void, ModelCookFailure> cookAnimations(CookContext &context)
        {
            ...
            for (std::uint32_t ordinal = 0U; ordinal < context.scene.mNumAnimations; ++ordinal)
            {
                ...
                clip->name = source.mName.length == 0U ? "animation-" + std::to_string(ordinal) : source.mName.C_Str();
                ...
                for (std::uint32_t channel = 0U; channel < source.mNumChannels; ++channel)
                {
                    ...
                    for (std::uint32_t key = 0U; key < input.mNumPositionKeys; ++key)
                    {
                        ...
                        if (!track.times_t.empty() && time <= track.times_t.back())
                        {
                            continue;
                        }
                    ...
                    for (std::uint32_t key = 0U; key < input.mNumRotationKeys; ++key)
                    {
                        ...
                        if (!track.times_r.empty() && time <= track.times_r.back())
                        {
                            continue;
                        }
                    ...
                    for (std::uint32_t key = 0U; key < input.mNumScalingKeys; ++key)
                    {
                        ...
                        if (!track.times_s.empty() && time <= track.times_s.back())
                        {
                            continue;
                        }
                ...
                if (clip->duration <= 0.0F)
                {
                    for (const auto &track : clip->tracks)
                    {
                        ...
                        {
                            clip->duration = (std::max)(clip->duration, track.times_t.back());
                        }
                        if (!track.times_r.empty())
                        {
                            clip->duration = (std::max)(clip->duration, track.times_r.back());
                        }
                        if (!track.times_s.empty())
                        {
                            clip->duration = (std::max)(clip->duration, track.times_s.back());
                        }
                    }
                }
                if (clip->duration <= 0.0F)
                ...
                const std::string key =
                    context.captured ? context.animation_keys[ordinal] : std::to_string(ordinal) + "|" + clip->name;
        ...
        const ModelCookConfiguration &configuration, CapturedFiles *captured)
    {
        ...
        }
        {
            ...
            CookContext context{*scene, source, configuration, model_info, captured};
            if (captured)
            {
                auto materials = identityKeys(scene->mNumMaterials, "material", [&](std::uint32_t index) {
                    aiString name;
                    aiGetMaterialString(scene->mMaterials[index], AI_MATKEY_NAME, &name);
                    return std::string(name.C_Str());
                });
                auto meshes = identityKeys(scene->mNumMeshes, "mesh", [&](std::uint32_t index) {
                ...
                if (!meshes)
                {
                    return lux::cxx::unexpected(std::move(meshes.error()));
                }
                if (!animations)
                {
                    return lux::cxx::unexpected(std::move(animations.error()));
                }
                context.material_keys = std::move(*materials);
                context.mesh_keys = std::move(*meshes);
                context.animation_keys = std::move(*animations);
            }
            ...
            context.has_import_transform = !context.import_transform.linear().isApprox(Eigen::Matrix3f::Identity());
                               ...
                               const ModelCookConfiguration &config)
    {
        ...
        {
            return failure(EModelCookError::INVALID_SOURCE, 0, "Invalid captured model source or duplicate file");
        }
        ...
        {
            return std::move(captured.requested);
        }
        if (!result)
        {
        ...
        const std::filesystem::path &root, std::span<const std::string> paths, std::size_t max_bytes)
    {
        ...
        for (const auto &path : paths)
        {
            ...
            const auto relative = full.lexically_relative(source_root);
            if (sourceKey(relative.generic_string()).empty())
            {
            ...
            {
                return rejected("Cannot read model dependency within its byte limit: " + path);
            }
