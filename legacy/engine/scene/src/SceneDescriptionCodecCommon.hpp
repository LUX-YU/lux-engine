...
namespace lux::scene::detail
{
    ...
    [[nodiscard]] inline SceneCodecFailure failure(
    ...
    inline bool readUuid(
        ByteReader& reader,
        lux::asset::asset_id_t& id) noexcept
    {
        std::array<std::uint8_t, 16u> bytes{};
        if (!reader.bytes(bytes.data(), bytes.size()))
            return false;
