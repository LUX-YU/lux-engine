...
{
    return renderError<err::device::FrameLifecycleCallFailed>(static_cast<std::uint32_t>(call), encodeVkResult(result));
}

Expected<void> waitFrameFence(gapi::vk::Fence &fence, VkDevice device, const FrameDriverRuntimeOps &ops) noexcept
{
    ...
    {
        return lux::cxx::unexpected<RenderError>(frameLifecycleFailure(EFrameLifecycleCall::SLOT_FENCE_WAIT, result));
    }
    return {};
}

Expected<void> beginFrameRecording(gapi::vk::Fence &fence, gapi::vk::CommandBuffer &command_buffer, VkDevice device,
                                   const FrameDriverRuntimeOps &ops) noexcept
{
    if (ops.reset_fence == nullptr || ops.reset_command_buffer == nullptr || ops.begin_command_buffer == nullptr)
    {
        return renderFailure<err::internal::InvalidArgument>();
    }
    ...
    if (result != VK_SUCCESS)
    {
        return lux::cxx::unexpected<RenderError>(frameLifecycleFailure(EFrameLifecycleCall::FENCE_RESET, result));
    }
...
FrameDriverCreateCandidate::FrameDriverCreateCandidate(VkDevice device, VkCommandPool command_pool,
                                                       FrameDriverCreateOps ops) noexcept
...
FrameDriverCreateCandidate::FrameDriverCreateCandidate(FrameDriverCreateCandidate &&other) noexcept
    : device_(std::exchange(other.device_, VkDevice{})),
      command_pool_(std::exchange(other.command_pool_, VkCommandPool{})), ops_(other.ops_), fences_(other.fences_),
      command_buffers_(other.command_buffers_), fence_count_(std::exchange(other.fence_count_, 0)),
...
Expected<FrameDriverCreateCandidate> FrameDriverCreateCandidate::create(VkDevice device, VkCommandPool command_pool,
                                                                        ...
                                                                        FrameDriverCreateOps ops) noexcept
{
    ...
    const bool is_invalid_ops =
        is_missing_create_fence || is_missing_destroy_fence || is_missing_allocate_buffers || is_missing_free_buffers;
    if (is_invalid_ops)
    {
        return renderFailure<err::internal::InvalidArgument>();
    }
...
const detail::FrameDriverRuntimeOps kFrameDriverRuntimeOps{
    ...
    .reset_command_buffer = [](gapi::vk::CommandBuffer &command_buffer) noexcept { return command_buffer.reset(); },
    .begin_command_buffer = [](gapi::vk::CommandBuffer &command_buffer,
                               VkCommandBufferUsageFlags flags) noexcept { return command_buffer.begin(flags); },
...
Expected<std::unique_ptr<FrameDriver>> FrameDriver::create(ResourceContext &res_ctx, std::uint32_t frames_in_flight)
{
    auto candidate = detail::FrameDriverCreateCandidate::create(
        res_ctx.deviceContext().logicalDevice(), res_ctx.commandPool(), frames_in_flight, kFrameDriverCreateOps);
    if (!candidate)
    {
...
FrameDriver::FrameDriver(ResourceContext &res_ctx, std::uint32_t frames_in_flight,
                         const detail::FrameDriverCreateCandidate &candidate,
                         std::unique_ptr<SubmissionMerger> submission_merger) noexcept
    ...
    if (!rt.multi_queue_submits.empty())
    {
        ...
        if (!merged_subs.empty())
        {
            ...
            const auto retirement = analyzeFrameRetirement(
                std::span<const RGQueueSubmission *const>{executable_subs.data(), executable_subs.size()}, presenting,
                retire_waits);
            ...
            for (size_t si = 0; si < executable_subs.size(); ++si)
            {
                ...
                if (is_graphics && (presenting || inject_ext_waits))
                {
                    ...
                    waits_with_acquire.insert(waits_with_acquire.end(), sub.wait_semaphores.begin(),
                    ...
                    {
                        waits_with_acquire.insert(waits_with_acquire.end(), rt.external_graphics_waits.begin(),
...
Expected<void> FrameDriver::pollCompletions()
{
    const auto &device = res_ctx_.deviceContext().logicalDevice();
    for (std::uint32_t index = 0; index < frames_in_flight_; ++index)
    {
        if (slot_last_submitted_[index] <= gpu_completed_serial_)
        {
            continue;
        }
        const auto result = vkGetFenceStatus(device, fences_[index]);
        if (result == VK_SUCCESS)
        {
            gpu_completed_serial_ = std::max(gpu_completed_serial_, slot_last_submitted_[index]);
        }
