...
namespace lux::scene
{
    struct SceneMetaManager::Impl final
    {
        ...
        std::unordered_map<std::string, std::vector<system::SystemTypeId>, TransparentStringHash, std::equal_to<>>
            capability_index;
    };

    namespace
    {
        [[nodiscard]] SceneMetaFailure failure(ESceneMetaError code, std::uint64_t subject_hash = 0U,
                                               lux::serialization::SerializationFailure configuration = {}) noexcept
        {
        ...
        [[nodiscard]] bool sameType(const system::SystemTypeId &type,
                                    const system::SystemTypeDescription &description) noexcept
        {
    ...
    lux::cxx::expected<SceneMetaManager, SceneMetaFailure> SceneMetaManager::build(BuildInfo info) noexcept
    {
        ...
        try
        {
            ...
            for (const auto &registration : impl->simulation_systems.all())
            {
                ...
                {
                    return lux::cxx::unexpected(
                        failure(ESceneMetaError::CONFIGURATION_REFLECTION_NOT_FOUND, registration.type.hash));
                }
                ...
                if (has_configuration)
                {
                    ...
                    if (!encoded)
                    {
                        return lux::cxx::unexpected(failure(ESceneMetaError::CONFIGURATION_DEFAULT_ENCODING_FAILURE,
                                                            registration.type.hash, encoded.error()));
                    }
                }
                impl->all_systems.push_back({ESystemDomain::SIMULATION, registration.type, registration.cpp_type,
                                             &registration.description->type, registration.configuration, reflection,
                                             defaults});
            }
            ...
            for (std::size_t index{}; index < impl->scene_systems.size(); ++index)
            {
                ...
                for (std::size_t requirement{}; requirement < registration.requirements.size(); ++requirement)
                {
                    ...
                    const bool duplicate =
                        std::any_of(registration.requirements.begin(), registration.requirements.begin() + requirement,
                ...
                for (const auto &connection : registration.connections)
                {
                    ...
                    {
                        return std::ranges::any_of(registration.requirements,
                                                   [name](const auto &value) noexcept { return value.name == name; });
                    };
                    const bool invalid_requirement = (connection.signal.owner == ESceneConnectionOwner::REQUIREMENT &&
                    ...
                    if (connection.signal.owner == ESceneConnectionOwner::SELF &&
                        !object::reflection::findSignal(meta::ReflectionRegistry::instance(), *object_reflection,
                ...
                impl->all_systems.push_back({ESystemDomain::SCENE, registration.type, registration.cpp_type,
                                             registration.description, registration.configuration, reflection,
                                             defaults});
            }
            ...
            for (const auto &registration : impl->simulation_systems.all())
            {
                for (const auto &access : registration.access.components)
                {
                    ...
                    {
                        return lux::cxx::unexpected(
                            failure(ESceneMetaError::UNKNOWN_COMPONENT_SCHEMA, access.type.hash()));
                    }
                    const auto ordinal = componentOrdinal(access.type);
                    usages[ordinal].push_back({registration.type, ESystemDomain::SIMULATION, access.mode, 0U});
                }
            }
            for (const auto &registration : impl->scene_systems)
            {
                for (const auto &observation : registration.observations)
                {
                    ...
                    {
                        return lux::cxx::unexpected(
                            failure(ESceneMetaError::UNKNOWN_COMPONENT_SCHEMA, observation.component.hash()));
                    }
                    const auto ordinal = componentOrdinal(observation.component);
                    usages[ordinal].push_back({registration.type, ESystemDomain::SCENE, {}, observation.events});
                }
            }
            ...
            for (std::size_t ordinal{}; ordinal < usages.size(); ++ordinal)
            {
                ...
                impl->component_usage.insert(impl->component_usage.end(), usages[ordinal].begin(),
                                             usages[ordinal].end());
            }
        ...
        std::string_view capability) const noexcept
    {
        ...
        return found != impl_->capability_index.end() ? std::span<const system::SystemTypeId>(found->second)
