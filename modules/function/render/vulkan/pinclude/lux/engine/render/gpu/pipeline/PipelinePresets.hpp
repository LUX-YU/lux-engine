...
/**
 * 全屏三角后处理管线预置 —— Tonemap 与 Highlight blur/composite 的同构形状。
 *
 * 覆盖:MESH 几何、零顶点输入(gl_VertexIndex 合成三角)、无深度、无剔除、
 *       set0 = Scene 槽语义(resource_slot_map)、可选标准 alpha 混合、
 *       [0, push_constant_size) 的 VERTEX|FRAGMENT **权威**推送常量段。
 * 不覆盖(调用点的真实方差):着色器模块、注册、set1 反射布局取回。
 *
 * PC 范围为何权威声明而非交给反射:全屏 VS 没有 PC 块,纯反射会把 VERTEX
 * stage 从范围里丢掉(两处调用点原注释一致记载的坑)。
 */
inline GraphicsPipelineTemplate makeFullscreenTemplate(const char* debug_name,
                                                       uint32_t    push_constant_size,
                                                       bool        alpha_blend = false)
{
    GraphicsPipelineTemplate tmpl{};
    tmpl.geometry_type        = EGeometryType::MESH;
    tmpl.debug_name           = debug_name;
    ...
    if (alpha_blend)
    {
        // Standard alpha — SRC_ALPHA / ONE_MINUS_SRC_ALPHA(与 makeTransparentMeshTemplate 同因子)。
        tmpl.blend_enable           = VK_TRUE;
        tmpl.src_color_blend_factor = VK_BLEND_FACTOR_SRC_ALPHA;
        tmpl.dst_color_blend_factor = VK_BLEND_FACTOR_ONE_MINUS_SRC_ALPHA;
        tmpl.color_blend_op         = VK_BLEND_OP_ADD;
