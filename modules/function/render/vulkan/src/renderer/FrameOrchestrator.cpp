...
Expected<FrameOrchestrator::EFrameStart> FrameOrchestrator::beginRenderFrame(RenderTargetRegistry &targets,
                                                                             FrameDriver *driver, FrameTickState &st)
{
...
Expected<void> FrameOrchestrator::prepareTargets(RenderTargetRegistry &targets, Renderer &renderer)
{
    ...
    auto visit = [&](auto &&self, std::size_t index) -> Expected<void> {
        ...
        if (target_marks_[index] == 1)
        {
            return renderFailure<err::graph::DependencyCycle>();
        }
        target_marks_[index] = 1;
        const auto *target = targets.tryGet(keys[index]);
        for (const auto &layer : target->layers)
        {
            if (layer.kind != RenderTargetEntry::CompositeLayer::EKind::SceneView)
            {
                continue;
            }
            auto *scene = renderer.getScene(layer.scene_id);
            if (!scene)
            {
                continue;
            }
            ...
            {
                for (const auto &read : feature->sampledTargets())
                {
                    const auto found = std::find(keys.begin(), keys.end(), read.target);
                    const auto *source = targets.tryGet(read.target);
                    const bool valid_range = read.range.baseMipLevel == 0 && read.range.levelCount == 1 &&
                                             read.range.baseArrayLayer == 0 && read.range.layerCount == 1 &&
                                             read.range.aspectMask == VK_IMAGE_ASPECT_COLOR_BIT;
                    if (found == keys.end() || !source || !source->pool || !valid_range ||
                        read.slot >= TargetSlot::COUNT || !source->layout.hasSlot(read.slot) ||
                        source->layout.slot(read.slot).final_state != ERenderResourceState::SHADER_READ ||
                        !hasUsage(source->layout.slot(read.slot).usage, ERenderImageUsage::SAMPLED) ||
                        read.stages != VK_PIPELINE_STAGE_2_FRAGMENT_SHADER_BIT ||
                        (read.backing_revision && read.backing_revision != source->pool->backingRevision()))
                    {
                        return renderFailure<err::frame::SampledTargetUnavailable>(read.target.index,
                                                                                   read.backing_revision);
                    }
                    auto result = self(self, static_cast<std::size_t>(found - keys.begin()));
                    if (!result)
                    {
                        return result;
                    }
        ...
        target_marks_[index] = 2;
        target_order_.push_back(keys[index]);
        return {};
    };
    for (std::size_t index = 0; index < keys.size(); ++index)
    {
        auto result = visit(visit, index);
        if (!result)
        {
            target_order_.clear();
            return result;
...
Expected<void> FrameOrchestrator::renderTargets(RenderTargetRegistry &targets, Renderer &renderer,
                                                SceneViewBatch &batch, FrameTickState &st)
{
    ...
    auto bindInputs = [&](RenderScene &scene) -> Expected<void> {
        for (auto *feature : scene.enabledFeatures())
        {
            ...
            {
                const auto *source = targets.tryGet(read.target);
                if (!source || !source->pool || !source->pool->recorded(stamp.slotIndex()) ||
                    (read.backing_revision && read.backing_revision != source->pool->backingRevision()))
                {
                    return renderFailure<err::frame::SampledTargetUnavailable>(read.target.index,
                                                                               read.backing_revision);
                }
                const auto &images = source->pool->binding().slot(read.slot);
                if (stamp.slotIndex() >= images.images.size())
                {
                    return renderFailure<err::frame::SampledTargetUnavailable>(read.target.index,
                                                                               read.backing_revision);
                }
                sampled_images_.push_back({read.target, source->pool->backingRevision(),
                                           images.images[stamp.slotIndex()], images.views[stamp.slotIndex()]});
            }
            // Producer graphs declare SHADER_READ as their final state and
            // emit the write -> fragment-read barrier. The dependency order
            // above supplies the execution order on this same submission.
            auto bound = feature->bindSampledTargets(sampled_images_, stamp.slotIndex(), stamp.serial);
            if (!bound)
            {
                return bound;
            }
    ...
    auto amendTargetSlots = [&](RenderTargetEntry &t) -> bool {
        ...
        for (size_t i = 0; i < kTargetSlotCount; ++i)
        {
            ...
            renderer.renderContext().reportError(
                renderError<err::frame::TargetLayoutAmended>(static_cast<std::uint32_t>(i)), RenderErrorEvent::kNoScene,
    ...
    batch.clear();
    for (const auto id : target_order_)
    {
        ...
        auto recorded = runChain(*t, binding);
        if (!recorded)
        {
            return lux::cxx::unexpected(recorded.error());
        }
        t->pool->markRecorded(stamp.slotIndex(), *recorded);
    }

    // 主 Surface。
    if (st.surface_present && st.surface_target && st.rt.present_target)
    {
        auto recorded = runChain(*st.surface_target, *st.rt.present_target);
        if (!recorded)
        {
            return lux::cxx::unexpected(recorded.error());
        }
    ...
    for (auto &s : st.extra_surfaces)
    {
        if (s.target)
        {
            auto recorded = runChain(*s.target, s.binding);
            if (!recorded)
            {
                return lux::cxx::unexpected(recorded.error());
            }
...
Expected<void> FrameOrchestrator::endRenderFrame(RenderTargetRegistry &targets, FrameDriver *driver, FrameTickState &st)
{
    if (driver)
    {
