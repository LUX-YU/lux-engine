...
namespace lux::simulation::ecs
{
    ...
    template<class Binary>
        requires (std::same_as<Binary, serialization::BinaryReader> ||
                  std::same_as<Binary, serialization::BinaryWriter>)
    ...
    public:
          ...
          : binary_(binary), identities_(&captured_identities)
      {
        ...
        [[nodiscard]] Entity invalidReference() const noexcept { return invalid_; }
        [[nodiscard]] auto remaining() const noexcept requires std::same_as<Binary, serialization::BinaryReader>
            ...
            requires std::same_as<Binary, serialization::BinaryWriter>
        {
            ...
            if (value != NullEntity && (!identity.valid() || (registry_ && !registry_->valid(value))))
            {
                ...
                return lux::cxx::unexpected(serialization::SerializationFailure{
                    serialization::ESerializationError::INVALID_VALUE, offset()});
            }
...
namespace lux::serialization
{
    template<> struct Serializer<simulation::ecs::Entity> final
    {
        ...
        static SerializationResult write(simulation::ecs::WorldComponentArchive<BinaryWriter>& writer,
