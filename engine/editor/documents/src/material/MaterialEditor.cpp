...
namespace lux::editor::material
{
        ...
        editing::StateId base, std::span<const lux::material::TextureSlotDecl> slots)
    {
        ...
        for (std::size_t index{}; index < slots.size(); ++index)
        {
            ...
            if (!asset.isNull() && (index >= previous.size() || asset != previous[index].texture))
            {
                ...
                {
                    return lux::cxx::unexpected(
                        editing::makeEditFailure(editing::EEditError::INVALID_ARGUMENT, 0,
    ...
    editing::EditResult<editing::ApplyResult> MaterialEditor::replaceNode(
        editing::StateId base, std::unique_ptr<lux::material::Node> &replacement)
    {
        const auto admitted = data_->canEdit();
        if (!admitted)
        {
    ...
    editing::EditResult<editing::ApplyResult> MaterialEditor::connect(lux::material::PinId from,
    ...
    editing::EditResult<editing::ApplyResult> MaterialEditor::disconnect(lux::material::PinId from,
                                                                         lux::material::PinId to)
    {
        Data::GraphDelta before, after;
    ...
    EditorResult<SaveRequestId> MaterialEditor::requestSave(std::string origin)
    {
        ...
        lux::material::MaterialSourceDocument capture{data_->source.id, data_->source.name,
                                                      data_->source.graph.clone()};
    ...
    void MaterialEditor::beginClose() noexcept
    {
    ...
    namespace
    {
        struct MaterialCodec final
        {
            ...
            {
                auto source =
                    lux::material::decodeMaterialSource({reinterpret_cast<const char *>(bytes.data()), bytes.size()});
                if (!source)
                {
