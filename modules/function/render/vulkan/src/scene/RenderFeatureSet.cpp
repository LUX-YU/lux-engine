#include <lux/engine/render/scene/RenderFeatureSet.hpp>
...
namespace lux::render
{
    ...
    FeatureHandle RenderFeatureSet::insertSlot(std::unique_ptr<RenderFeature> feature)
    {
        ...
        // RenderScene 回填。
        markCacheDirty();
        return features_.insert(Slot{std::move(feature)});
    }

    bool RenderFeatureSet::erase(FeatureHandle handle)
    {
    ...
    RenderFeature* RenderFeatureSet::get(FeatureHandle handle) const
    {
    ...
    bool RenderFeatureSet::contains(FeatureHandle handle) const noexcept
    {
        return features_.contains(handle);
    }

    std::size_t RenderFeatureSet::size() const noexcept
    {
    ...
    bool RenderFeatureSet::hasType(FeatureTypeId type) const noexcept
    {
    ...
    RenderFeature* RenderFeatureSet::findByName(std::string_view name) const noexcept
    {
        for (const auto& slot : features_.values())
    ...
    std::vector<RenderFeatureSet::FeatureParamDesc> RenderFeatureSet::queryParamDescs() const
    {
    ...
    std::span<RenderFeature* const> RenderFeatureSet::all() const
    {
        rebuildEnabledCacheIfNeeded();
        return {all_dense_.data(), all_dense_.size()};
    }

    std::span<RenderFeature* const> RenderFeatureSet::enabled() const
    {
        rebuildEnabledCacheIfNeeded();
        return {enabled_dense_.data(), enabled_dense_.size()};
    }

    void RenderFeatureSet::contributePasses(RGBuilder& builder) const
    {
        ...
        for (const auto& slot : features_.values())
        {
            if (!slot.feature || !slot.feature->isEnabled())
                continue;
            slot.feature->addPasses(builder);
        }
    }
    ...
    bool RenderFeatureSet::ownsViewState(uint32_t feature_index, uint32_t view_index) const noexcept
    {
        auto it = view_states_.find(feature_index);
        return it != view_states_.end() && it->second.contains(view_index);
    }

    void RenderFeatureSet::recordViewState(uint32_t feature_index, uint32_t view_index)
    {
        view_states_[feature_index].insert(view_index);
    }

    void RenderFeatureSet::forgetViewState(uint32_t feature_index, uint32_t view_index) noexcept
    {
        auto it = view_states_.find(feature_index);
    ...
    std::vector<uint32_t> RenderFeatureSet::viewsOwnedBy(uint32_t feature_index) const
    {
    ...
    void RenderFeatureSet::clearLedger() noexcept
    {
        view_states_.clear();
    }
