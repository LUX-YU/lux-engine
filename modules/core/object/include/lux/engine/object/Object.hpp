...
namespace lux::object
{
    namespace detail
    {
        ...
        template <class Owner, class Result, class... Args> struct MemberMethodTraits<Result (Owner::*)(Args...)>
        {
        ...
        struct MemberMethodTraits<Result (Owner::*)(Args...) const> : MemberMethodTraits<Result (Owner::*)(Args...)>
        {
        };

        template <class Owner, class Result, class... Args>
        struct MemberMethodTraits<Result (Owner::*)(Args...) noexcept> : MemberMethodTraits<Result (Owner::*)(Args...)>
        {
        };
    ...
    public:
        ...
        template <auto& StaticSignal, class Payload> void notify(Payload&& payload) noexcept
        {
            using SignalType = std::remove_cvref_t<decltype(StaticSignal)>;
            using DeclaredPayload = typename SignalType::payload_type;
            static_assert(
                std::derived_from<Derived, typename SignalType::owner_type>,
        ...
        [[nodiscard]] lux::cxx::expected<Connection, EObserveError> observe(Receiver& receiver)
            requires std::derived_from<Receiver, LuxObject>
        {
            ...
            else
            {
                ...
                static_assert(
                    std::is_nothrow_invocable_r_v<void, decltype(Method), Receiver&, const Payload&>,
                    "Signal callbacks must be noexcept");
                if constexpr (Delivery != EDelivery::DIRECT)
                {
                    static_assert(std::copy_constructible<Payload>, "Queued-capable Signal payload must be copyable");
                }
            }
        ...
        template <auto& StaticSignal, auto Function> [[nodiscard]] Connection observe()
        {
            using SignalType = std::remove_cvref_t<decltype(StaticSignal)>;
            using Payload = typename SignalType::payload_type;
            static_assert(
            ...
            if constexpr (std::is_void_v<Payload>)
                static_assert(
                    std::is_nothrow_invocable_r_v<void, decltype(Function)>,
                    "Signal callbacks must be noexcept");
            else
                static_assert(
                    std::is_nothrow_invocable_r_v<void, decltype(Function), const Payload&>,
                    "Signal callbacks must be noexcept");
        ...
        template <auto& StaticSignal, class Callable> [[nodiscard]] ScopedConnection observeScoped(Callable&& callable)
        {
            using SignalType = std::remove_cvref_t<decltype(StaticSignal)>;
            using Payload = typename SignalType::payload_type;
            ...
            static_assert(
                ...
                "Signal must belong to this LuxObject inheritance lineage");
            if constexpr (std::is_void_v<Payload>)
                static_assert(std::is_nothrow_invocable_r_v<void, Stored&>, "Signal callbacks must be noexcept");
            else
                static_assert(
