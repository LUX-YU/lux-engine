...
namespace lux::authoring::script
{
    ...
    lux::cxx::expected<lux::script::ScriptSymbolId, EScriptSymbolLedgerError>
    ScriptSymbolLedger::assign(std::string_view source_identity) noexcept
    {
        ...
        if (const auto existing = find(source_identity);
            existing != lux::script::InvalidScriptSymbolId)
        {
            return existing;
        }
        if (next_symbol_ ==
            std::numeric_limits<lux::script::ScriptSymbolId>::max())
        {
            return lux::cxx::unexpected(
