...
namespace lux::input
{
    ...
    ActionMapper::RawExtract ActionMapper::extractRaw(const InputSnapshot& snapshot, const ActionBinding& binding)
    {
        return std::visit(
            [&](const auto& input) -> RawExtract {
                ...
                else if constexpr (std::is_same_v<T, MouseAxisInput>)
                {
                    ...
                    switch (input.axis)
                    {
                    case MouseAxisInput::EAxis::DELTA_X:
                        delta = static_cast<float>(snapshot.cursor_dx);
                        break;
                    case MouseAxisInput::EAxis::DELTA_Y:
                        delta = static_cast<float>(snapshot.cursor_dy);
                        break;
                    case MouseAxisInput::EAxis::SCROLL_X:
                        delta = static_cast<float>(snapshot.scroll_dx);
                        break;
                    case MouseAxisInput::EAxis::SCROLL_Y:
                        delta = static_cast<float>(snapshot.scroll_dy);
                        break;
                    }
    ...
    InputValue ActionMapper::projectContribution(float raw_scalar, const InputValue& contribution)
    {
    ...
    void ActionMapper::evaluateBindings(
        const InputSnapshot& snapshot,
        InputContextStack& stack,
        float dt,
        bool want_kb,
    ...
    )
    {
        ...
        for (auto it = active.rbegin(); it != active.rend(); ++it)
        {
            ...
            for (auto& binding : ctx->actionMap().bindings())
            {
                ...
                if (is_kb && skip_kb)
                    blocked = true;
                if (is_mouse_btn && skip_mouse)
                    blocked = true;
                if (is_axis && skip_mouse)
                    blocked = true;
                if (is_touch && skip_mouse)
                    blocked = true;
                ...
                if (!binding.binding_triggers.empty())
                {
                    ...
                    bs.trigger_state =
                        evaluateTriggerGroup(binding.binding_triggers, rts, projected, dt, isActionActive);
                }
                else
                {
            ...
            if (ctx->consumesKeyboard())
                kb_consumed = true;
            if (ctx->consumesMouse())
                mouse_consumed = true;
        }
    }
    ...
    void ActionMapper::accumulateActions()
    {
        for (auto& bid : binding_states_.keys())
        {
            ...
            if (bs.blocked_by_ui || bs.blocked_by_consume)
                continue;

            // Only accumulate bindings whose trigger passed.
            if (bs.trigger_state == ETriggerState::NONE)
                continue;
            ...
            if (s.dominant_binding == InvalidBindingId || triggerPriority(bs.trigger_state) > 0)
            {
    ...
    void ActionMapper::evaluateActionLayer(float dt)
    {
        ...
        for (auto& id : states_.keys())
        {
            ...
            if (!desc->action_triggers.empty())
            {
                ...
                s.trigger_state = evaluateTriggerGroup(desc->action_triggers, rts, s.value, dt, isActionActive);
