...
class LUX_ENGINE_SCENE_DESCRIPTION_PUBLIC SceneDescriptionBuilder final
{
  public:
    ...
    [[nodiscard]] lux::cxx::expected<void, SceneDescriptionFailure> eraseSystem(system::SystemInstanceId id) noexcept;
    [[nodiscard]] lux::cxx::expected<void, SceneDescriptionFailure> bindRequirement(system::SystemInstanceId system,
    ...
    // Program assembly supplies resolved World/Simulation objects directly.
    // Disk assets must use build(), which still requires valid asset references.
    [[nodiscard]] lux::cxx::expected<SceneDescription, SceneDescriptionFailure> buildResolved() && noexcept;

  private:
    [[nodiscard]] lux::cxx::expected<SceneDescription, SceneDescriptionFailure> finish(bool resolved) noexcept;
    struct Impl;
