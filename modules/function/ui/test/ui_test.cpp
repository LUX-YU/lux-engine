...
namespace
{
    ...
    public:
        ...
        void contextualDelete() noexcept
        {
            ++contextual_count;
        }
        void globalDelete() noexcept
        {
            ++global_count;
        }
        [[nodiscard]] bool contextualEnabled() const noexcept
        {
            return false;
        }
...
main()
{
    ...
    }
    {
        ...
        TestPane duplicate{session.dispatcherRef(), lux::ui::PaneId{"first"}, "Duplicate"};
        ...
        TestPane foreign{foreign_session.dispatcherRef(), lux::ui::PaneId{"foreign"}, "Foreign"};
        ...
        assert(session.focusedContexts()[0] == lux::ui::UiContextIdView{"test.local.second"});
        assert(session.focusedContexts()[2] == lux::ui::UiContextIdView{"test.selection"});
        assert(router.state(*delete_command).found);
        assert(!router.state(*delete_command).enabled);
        assert(router.invoke(*delete_command) == lux::ui::ECommandDispatchResult::DISABLED);
        assert(first.contextual_count == 0);
        assert(first.global_count == 0);
        contextual->reset();
        assert(router.invoke(*delete_command) == lux::ui::ECommandDispatchResult::EXECUTED);
        ...
        const auto temporary = router.findCommand(lux::ui::UiCommandIdView{"temporary"});
        assert(temporary);
        ...
        assert(decoded);
        auto copied_layout = *decoded;
        assert(copied_layout.bytes().size() == decoded->bytes().size());
        ...
        assert(toolbar[1].kind() == lux::ui::EToolbarItemKind::SEPARATOR);
    ...
    lux::ui::PaneFactoryRegistration expired_factory_registration;
    {
    ...
    assert(expired_factory_registration);
    expired_factory_registration.reset();
    assert(!expired_factory_registration);
