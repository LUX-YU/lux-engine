...
void ObjectState::append(SignalBucket &bucket, ConnectionControl &control,
                         EDelivery delivery) {
...
ObjectState::install(const SignalDescriptor &signal,
                     lux::cxx::intrusive_ptr<ObjectState> receiver_value,
                     ObjectInvokeThunk invoke_value, EDelivery delivery,
                     std::shared_ptr<void> context_value) {
  ensureSignalCapacity(
      (std::max)(signal.lineage_size_, signal.dense_index_ + 1u));
...
#endif
  if (control->receiver) {
    control->receiver->addIncoming(lux::cxx::intrusive_ptr<ObjectState>{this},
                                   control);
  }
...
void ObjectState::notify(const SignalDescriptor &signal, const void *payload) {
  ...
  for (std::size_t index = 0; index < queued_count; ++index) {
    ...
    if (post(control->receiver->dispatcher, std::move(message)) ==
        EPostStatus::CLOSED) {
      requestDisconnect(control);
    }
  }
...
void ObjectState::requestDisconnect(ConnectionControl *control) noexcept {
  ...
  static_cast<void>(post(dispatcher, std::move(message)));
}
...
void invokeQueuedConnection(ConnectionControl *control,
...
observeDynamicErased(LuxObject &sender, const SignalDescriptor &signal,
  ...
  return sender.observeIndexed(signal, receiver, invoke, delivery,
                               std::move(context));
}
} // namespace lux::object::detail
...
lux::cxx::intrusive_ptr<detail::ObjectState> LuxObject::ensureState() const {
  ...
  if (!state) {
    auto *candidate = new detail::ObjectState{const_cast<LuxObject *>(this),
                                              dispatcher_, affinity_};
    detail::intrusive_ptr_add_ref(candidate); // object ownership
    if (!state_.compare_exchange_strong(state, candidate,
...
lux::cxx::expected<Connection, EObserveError>
LuxObject::observeIndexed(const detail::SignalDescriptor &signal,
                          LuxObject &receiver, detail::ObjectInvokeThunk invoke,
                          EDelivery delivery, std::shared_ptr<void> context) {
  ...
  auto control = sender_state->install(signal, receiver.ensureState(), invoke,
                                       resolved, std::move(context));
  return Connection{std::move(sender_state), std::move(control)};
}

Connection
LuxObject::observeStaticIndexed(const detail::SignalDescriptor &signal,
                                detail::ObjectInvokeThunk invoke) {
  auto sender_state = ensureState();
  auto control =
      sender_state->install(signal, {}, invoke, EDelivery::DIRECT, {});
  return Connection{std::move(sender_state), std::move(control)};
}

Connection
LuxObject::observeCallableIndexed(const detail::SignalDescriptor &signal,
                                  detail::ObjectInvokeThunk invoke,
