...
namespace
{
    ...
    [[nodiscard]] WorldObjectId objectId(std::uint64_t value)
    {
        ...
        {
            bytes[15U - index] = static_cast<std::uint8_t>(value >> (index * 8U));
        }
    ...
    {
    public:
        SingleWorkspace() : descriptor_{worldPartitionerId("test.single"), 1U}
        {
        }
        ...
        lux::cxx::expected<void, WorldPartitionFailure> doRebuild(const WorldDescription&) noexcept override
        {
            return {};
        }

        lux::cxx::expected<void, WorldPartitionFailure> doObjectAdded(WorldObjectSnapshotView) noexcept override
        {
            return {};
        }

        lux::cxx::expected<void, WorldPartitionFailure> doObjectChanged(WorldObjectSnapshotView) noexcept override
        {
            return {};
        }

        lux::cxx::expected<void, WorldPartitionFailure> doObjectRemoved(WorldObjectId) noexcept override
        {
            return {};
        }
        ...
        doFreeze(const WorldDescription& world) const noexcept override
        {
            ...
            return WorldPartitionBuildProduct::build(descriptor_, std::move(*layout), {});
        }
    ...
    public:
        ...
        lux::cxx::expected<std::unique_ptr<WorldPartitionWorkspace>, WorldPartitionFailure>
        createWorkspaceImplementation() const noexcept override
        {
            try
            {
                return std::unique_ptr<WorldPartitionWorkspace>(std::make_unique<SingleWorkspace>());
            }
            catch (...)
            {
                WorldPartitionFailure failure;
    ...
    [[nodiscard]] std::uint8_t quadrant(WorldObjectSnapshotView object)
    {
        ...
        return static_cast<std::uint8_t>((x >= 128U ? 1U : 0U) | (y >= 128U ? 2U : 0U));
    }

    class QuadtreeWorkspace final : public WorldPartitionWorkspace
    {
    public:
        QuadtreeWorkspace() : descriptor_{worldPartitionerId("test.quadtree2d"), 1U}
        {
        }
        ...
        lux::cxx::expected<void, WorldPartitionFailure> doRebuild(const WorldDescription& world) noexcept override
        {
            try
            {
                objects_.clear();
                for (std::size_t index{}; index < world.objectCount(); ++index)
                {
                    ...
                    const WorldDataSnapshotView snapshot_data{&data.schema(), data.version(), data.payload()};
                    const WorldObjectSnapshotView snapshot{object.id(), std::span(&snapshot_data, 1U)};
        ...
        lux::cxx::expected<void, WorldPartitionFailure> doObjectAdded(WorldObjectSnapshotView object) noexcept override
        {
        ...
        lux::cxx::expected<void, WorldPartitionFailure> doObjectRemoved(WorldObjectId object) noexcept override
        {
            ...
            try
            {
                ...
                for (std::size_t index{}; index < groups.size(); ++index)
                {
                    if (groups[index].empty())
                        continue;
                    auto added =
                        builder.addPartition(partitionId(static_cast<std::uint8_t>(index + 1U)), groups[index]);
                    if (!added)
                        return lux::cxx::unexpected(added.error());
                }
                auto layout = std::move(builder).build();
    ...
    private:
        lux::cxx::expected<void, WorldPartitionFailure> assign(WorldObjectSnapshotView object) noexcept
        {
...
main()
{
    ...
    assert(WorldObjectIdHash{}(hashed_object) == std::hash<uuids::uuid>{}(hashed_object.value));
    const auto hashed_partition = partitionId(17U);
    assert(WorldPartitionIdHash{}(hashed_partition) == std::hash<uuids::uuid>{}(hashed_partition.value));
    ...
    assert(!invalid.addObject(objectId(1U)));
    assert(!invalid.addData(objectId(1U), {}, 1U, {}));
    assert(!invalid.addData(objectId(1U), worldDataSchemaId("test.data"), 0U, {}));
    ...
    assert(failed_object.error().code == EWorldDescriptionError::ALLOCATION_FAILURE);
    assert(failure_builder.addObject(objectId(8U)));

    const std::array failure_payload{std::byte{8U}};
    detail::failNextWorldOperationForTest(detail::EWorldFailurePoint::DESCRIPTION_MUTATION_ALLOCATION);
    const auto failed_data =
        failure_builder.addData(objectId(8U), worldDataSchemaId("test.failure"), 1U, failure_payload);
    assert(!failed_data);
    assert(failed_data.error().code == EWorldDescriptionError::ALLOCATION_FAILURE);
    assert(failure_builder.addData(objectId(8U), worldDataSchemaId("test.failure"), 1U, failure_payload));
    ...
    assert(failed_description_build.error().code == EWorldDescriptionError::ALLOCATION_FAILURE);
    detail::failNextWorldOperationForTest(detail::EWorldFailurePoint::DESCRIPTION_BUILD_SIZE_OVERFLOW);
    const auto overflow_description_build = std::move(failure_builder).build();
    assert(!overflow_description_build);
    assert(overflow_description_build.error().code == EWorldDescriptionError::SIZE_OVERFLOW);
    ...
    assert(shared_schema_world->retainedBytes() < shared_schema_object_count * 80U);
    ...
    const auto failed_partition = failure_layout_builder.addPartition(partitionId(8U), failure_objects);
    assert(!failed_partition);
    assert(failed_partition.error().code == EWorldPartitionError::ALLOCATION_FAILURE);
    assert(failure_layout_builder.addPartition(partitionId(8U), failure_objects));
    ...
    assert(failed_layout_build.error().code == EWorldPartitionError::ALLOCATION_FAILURE);
    detail::failNextWorldOperationForTest(detail::EWorldFailurePoint::PARTITION_BUILD_SIZE_OVERFLOW);
    const auto overflow_layout_build = std::move(failure_layout_builder).build();
    assert(!overflow_layout_build);
    assert(overflow_layout_build.error().code == EWorldPartitionError::SIZE_OVERFLOW);
    ...
    assert(duplicate_result.error().code == EWorldPartitionError::DUPLICATE_OBJECT_ASSIGNMENT);

    SinglePartitioner single;
    auto single_workspace = single.createWorkspace(world);
    assert(single_workspace);
    assert((*single_workspace)->state() == EWorldPartitionWorkspaceState::SYNCHRONIZED);
    ...
    assert(quadtree_product->findIndex(worldPartitionIndexTypeId("test.quadtree.ordinal")));

    // Two interpreters derive different products from the same immutable World.
    assert(quadtree_product->layout().partitionCount() != single_product->layout().partitionCount());
    ...
    assert(after_incremental->layout().partitionCount() == quadtree_product->layout().partitionCount());

    incremental.failNextIncrementalForTest();
    const auto poisoned = incremental.objectChanged(snapshot);
    assert(!poisoned);
    assert(incremental.state() == EWorldPartitionWorkspaceState::STALE);
    const auto stale_freeze = incremental.freeze(world);
    assert(!stale_freeze);
    assert(stale_freeze.error().code == EWorldPartitionError::WORKSPACE_STALE);
    const auto stale_incremental = incremental.objectAdded(snapshot);
    assert(!stale_incremental);
    assert(stale_incremental.error().code == EWorldPartitionError::WORKSPACE_STALE);
    assert(incremental.rebuild(world));
    assert(incremental.state() == EWorldPartitionWorkspaceState::SYNCHRONIZED);
