...
main()
{
    ...
    static_assert(!std::is_same_v<decltype(BaseObject::baseChanged), decltype(DerivedObject::derivedChanged)>);
    ...
    assert(ObjectDiagnosticsAccess::denseIndex(DerivedObject::derivedChanged) == 2u);
    assert(ObjectDiagnosticsAccess::denseIndex(DerivedObject::derivedChangedB) == 3u);
    assert(ObjectDiagnosticsAccess::lineageSize(DerivedObject::derivedChangedB) == 4u);

    DerivedObject sender;
    DerivedObject receiver;
    auto typed =
        sender.observe<BaseObject::baseChanged, &DerivedObject::receive, lux::object::EDelivery::DIRECT>(receiver);
    assert(typed);
    ...
    const auto* derived_class = registry.findClass("lux::object::cross_dll::DerivedObject");
    assert(base_class);
    assert(derived_class);
    assert(registry.findClass("lux::object::cross_dll::InternalObject"));
    const auto dynamic_signal = lux::object::reflection::findSignal(registry, *derived_class, "derivedChanged");
    assert(dynamic_signal);
    const auto inherited_signal = lux::object::reflection::findSignal(registry, *derived_class, "baseChanged");
    assert(inherited_signal);
    const auto inherited_signal_b = lux::object::reflection::findSignal(registry, *derived_class, "baseChangedB");
    ...
    assert(receive_b);
    auto dynamic =
        lux::object::reflection::observe(sender, dynamic_signal, receiver, *receive, lux::object::EDelivery::DIRECT);
    assert(dynamic);
