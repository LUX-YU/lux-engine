...
[[nodiscard]] inline bool projectionDepthRange(const Eigen::Matrix4f &projection, float &near_z, float &far_z) noexcept
{
...
inline void fillViewGpuData(const CameraView &cv, const EntityTransform &ct, const Viewport &vp,
                            const RenderLargePosition3D &render_origin, float coordinate_page_size, ViewGpuData &out)
{
