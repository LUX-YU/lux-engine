 ...
 *
 * Also provides:
 *   - Replay functions for KernelCommand dispatch (Phase 2 decoupling)
...
{
    static constexpr uint32_t kIndirectCommandSize = 20;
                                     ...
                                     const RGCompiledGraph& /*compiled*/)
    {
        ...
        for (uint32_t g = 0; g < bgc; ++g)
        {
            ...
            for (uint32_t m = 0; m < vmc; ++m)
            {
                ...
                // the legacy static-only path (forcePassPipeline) is byte-for-byte
                // unchanged — zero risk to existing shadows.
                if (family_count > 0u)
                {
                    const uint32_t vidx =
                                         ...
                                         uint16_t data_size, KernelReplayContext& ctx)
    {
        ...
        case kCullPushConstants:
        {
            struct {
                uint32_t slot_count;
                uint32_t max_draws_per_bucket;
                uint32_t required_pass_mask;
                uint32_t required_geometry_kind;
                uint32_t slice_count;
                uint32_t bias_group_count;
                uint32_t enabled_geometry_mask;
                uint32_t extension_flags;
                uint32_t reserved0;
                uint32_t reserved1;
                uint64_t active_mask_addr;   // world-partition mask BDA; 0 = none
            } pc;
            ...
            if (ext)
            {
                pc.slice_count      = ext->slice_count;
                pc.bias_group_count = ext->group_count;
            }
