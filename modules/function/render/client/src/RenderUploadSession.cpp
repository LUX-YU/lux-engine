...
namespace lux::render
{
    ...
    UploadSubmitResult<Texture2DCreatedReply> RenderUploadSession::tryCreateTexture2D(
    ...
    UploadSubmitResult<TextureMipRangeReplacedReply> RenderUploadSession::tryReplaceTexture2DMipRange(
    ...
    )
    {
        ...
        return trySubmit<CubeTextureCreatedReply>(
            [faces = std::move(faces), face_size, channels, format](Builder& builder, auto callback) mutable {
                ...
                payload.channels = channels;
                payload.format = format;
                builder.pushResource(type_ids::CreateCubeTexture, payload, std::move(callback));
            }
        );
    }
    ...
    RenderUploadSession::tryCreatePersistentTexture2D(const PersistentTexture2DDesc& desc)
    {
        ...
        return trySubmit<Texture2DCreatedReply>([&desc](Builder& builder, auto callback) {
    ...
    std::uint64_t RenderUploadSession::currentThreadToken() noexcept
    {
        ...
        thread_local const std::uint64_t token = next.fetch_add(1u, std::memory_order_relaxed);
        return token;
    }
    ...
    bool RenderUploadSession::claimCoordinatorThread() noexcept
    {
        ...
        if (owner_thread_token_
                .compare_exchange_strong(unclaimed, current, std::memory_order_acq_rel, std::memory_order_acquire))
        {
            return true;
        }
    ...
    {
        if (owner_thread_token_.load(std::memory_order_acquire) != currentThreadToken())
        {
            renderFatal("RenderUploadSession accessed outside its coordinator owner thread");
        }
    }
} // namespace lux::render
