...
namespace lux::render
{
	...
	Expected<void> DeviceContext::init(EPhysicalDeviceSelectionPolicy policy)
	{
		...
		// VK_ERROR_FEATURE_NOT_PRESENT with no indication of which feature is absent.
		{
			...
			require(feat12.descriptorBindingStorageBufferUpdateAfterBind,"descriptorBindingStorageBufferUpdateAfterBind");
			require(feat12.descriptorBindingUniformBufferUpdateAfterBind,"descriptorBindingUniformBufferUpdateAfterBind");
			...
			require(feat2.features.fragmentStoresAndAtomics,
		...
		if (wl_dynamic_rendering_local_read &&
		    !has_device_ext(VK_KHR_DYNAMIC_RENDERING_LOCAL_READ_EXTENSION_NAME))
		{
		...
		has_external_interop =
			has_device_ext(VK_KHR_EXTERNAL_MEMORY_WIN32_EXTENSION_NAME) &&
			has_device_ext(VK_KHR_EXTERNAL_SEMAPHORE_WIN32_EXTENSION_NAME);
#else
		has_external_interop =
			has_device_ext(VK_KHR_EXTERNAL_MEMORY_FD_EXTENSION_NAME) &&
		...
		VmaAllocatorCreateInfo vma_info{
			...
			.flags          		= wl_buffer_device_address
			    ? VmaAllocatorCreateFlags{VMA_ALLOCATOR_CREATE_BUFFER_DEVICE_ADDRESS_BIT}
