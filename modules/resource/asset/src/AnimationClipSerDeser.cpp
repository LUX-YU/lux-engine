...
namespace lux::asset
{
    ...
    lux::cxx::expected<std::unique_ptr<AnimationClip>, EAssetError>
    AnimationClipSerDeser::decodeData(const void* bytes, std::size_t len) noexcept
    {
        ...
        // loadHeaderRaw works on a std::vector<std::byte>; copy the image in.
        // (A .luxasset clip image is tens-of-kilobytes at most.) The whole
        // function only reads from this owned buffer — no manager, no stream.
    ...
    AnimationClipSerDeser::exportAsLuxAssetStream(const LuxAsset& asset, std::ofstream& ofs)
    {
        ...
        const AnimationClip* clip = static_cast<const AnimationClip*>(casset->rawData());
        if (!clip)
