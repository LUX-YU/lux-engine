...
namespace lux::serialize
{
    ...
    /// `uuids::uuid::as_bytes()`).
    class LUX_CORE_SERIALIZATION_PUBLIC ArchiveWriter
    {
    public:
    ...
    /// Little-endian POD reader over a span. Invalid input latches `ok()` false.
    class LUX_CORE_SERIALIZATION_PUBLIC ArchiveReader
    {
    public:
        ...
        /// Latch a semantic decoding failure discovered by a structured
        /// reader after the raw bytes were available (for example a non-finite
        /// spatial coordinate). Once invalid, a reader never becomes valid
        /// again.
