 ...
 * root, plus the standard sub-folders (`Content/`, `Worlds/`, `Source/`,
 ...
 *
 * Lives in Authoring so the editor and offline toolchain can share it while
 * Player remains unaware of source-workspace semantics.
 */
...
{
    class LUX_ENGINE_AUTHORING_PROJECT_PUBLIC Project
    {
    public:
        ...
        /// `<root>/<name>.luxproject` plus empty `Content/`, `Worlds/`,
        ...
        [[nodiscard]] std::filesystem::path worldsRoot()   const { return root_ / "Worlds"; }
        ...
        /// Resolve `manifest.default_world` (relative POSIX) to an
        /// absolute path. Empty if the manifest doesn't specify one
        /// or the file doesn't exist.
        [[nodiscard]] std::filesystem::path defaultWorldPath() const;
        ...
        /// Recursive scan of `Worlds/` for `.luxworld` files. Absolute paths.
        [[nodiscard]] std::vector<std::filesystem::path> scanWorldFiles() const;
