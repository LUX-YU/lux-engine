...
namespace lux::simulation
{
    ...
    protected:
        ...
        [[nodiscard]] const ScriptInstanceHostContext& hostContext() const noexcept
        {
            if (!attached())
        ...
        [[nodiscard]] bool hasComponent(std::uint64_t component_type) const noexcept
        {
            return hostContext().read(component_type) != nullptr;
        }

        [[nodiscard]] const void* readComponent(std::uint64_t component_type) const noexcept
        {
            return hostContext().read(component_type);
        }

        [[nodiscard]] bool patchComponent(std::uint64_t component_type, const void* value) const noexcept
        {
        ...
        {
            return hostContext().command(command_value, component_type, value);
        }
    ...
    private:
        ...
        struct Access final
        {
            static void attach(EntityBehavior& behavior, ScriptInstanceHostContext& host) noexcept
            {
                behavior.host_ = &host;
            }
        };

        template <class Type> friend void attachEntityBehavior(void*, ScriptInstanceHostContext&) noexcept;
    };

    template <class Type> void attachEntityBehavior(void* object, ScriptInstanceHostContext& host) noexcept
    {
