#pragma once
// ============================================================================
//  Simulation2DSystem.hpp — the unified fixed-step coordinator (lux::gameplay::d2).
//
...
#include <lux/engine/gameplay/2d/D2ScenePlan.hpp>      // FixedStepConfig
...
{
    class Simulation2DSystem final : public lux::gameplay::ISystem
    {
    public:
        ...
        LUX_FUNCTION_PUBLIC void update(lux::meta::EntityRegistry& registry, float dt) override;

        // ── Stats (D-03: substep/lag observability) ──
        [[nodiscard]] int           substepsLastFrame() const noexcept { return substeps_last_frame_; }
        [[nodiscard]] std::uint64_t totalSubsteps()     const noexcept { return total_substeps_; }
        [[nodiscard]] float         accumulated()       const noexcept { return accumulator_; }
        [[nodiscard]] float         laggedTime()        const noexcept { return lagged_time_; }  ///< total discarded backlog
    ...
    private:
        ...
        int           substeps_last_frame_  = 0;
        std::uint64_t total_substeps_       = 0;
        float         lagged_time_          = 0.0f;
    };
