...
namespace lux::simulation
{
    struct LuaScriptBindingBackend::State final
    {
        ...
        State(std::size_t capacity, std::span<const LuaComponentBinding> source_components)
            : state(engine.state()), affinity(std::this_thread::get_id()), instance_capacity(capacity)
        {
            prototypes.reserve(capacity);
            components.assign(source_components.begin(), source_components.end());
        ...
        [[nodiscard]] int
        prototypeFor(const ScriptInstanceCreateContext& context, const lux::asset::ScriptAssetContent& asset) noexcept
        {
            for (const auto& prototype : prototypes)
            {
            ...
            if (asset.payload.empty())
                return LUA_NOREF;
            const auto* body = std::get_if<lux::rdesc::LuaSourceScript>(std::addressof(asset.description.body));
            if (!body)
                return LUA_NOREF;
            lua_rawgeti(state, LUA_REGISTRYINDEX, traceback_ref);
            const auto error_index = lua_gettop(state);
            const auto* source = reinterpret_cast<const char*>(asset.payload.data());
            if (luaL_loadbufferx(state, source, asset.payload.size(), asset.description.module_name.c_str(), "t") !=
                    LUA_OK ||
        ...
        {
            const auto found =
                std::find_if(components.begin(), components.end(), [name](const LuaComponentBinding& value) noexcept {
        ...
        [[nodiscard]] static bool readStrictNumber(lua_State* state, int index, Type& result) noexcept
        {
            ...
            {
                const bool is_invalid_integral = !std::isfinite(value) || std::trunc(value) != value ||
                    value < static_cast<lua_Number>(std::numeric_limits<Type>::lowest()) ||
                    value > static_cast<lua_Number>(std::numeric_limits<Type>::max());
                if (is_invalid_integral)
                {
                    return false;
                }
            }
            ...
            {
                const bool is_invalid_floating = !std::isfinite(value) ||
                    value < -static_cast<lua_Number>(std::numeric_limits<Type>::max()) ||
                    value > static_cast<lua_Number>(std::numeric_limits<Type>::max());
                if (is_invalid_floating)
                    return false;
            }
        ...
        static int patchComponent(lua_State* state) noexcept
        {
            ...
            case LUX_SCRIPT_VK_UINT32:
                ...
                break;
            case LUX_SCRIPT_VK_INT64:
                valid = readStrictNumber(state, 3, *reinterpret_cast<std::int64_t*>(storage));
                break;
            case LUX_SCRIPT_VK_FLOAT:
        ...
        ) noexcept
        {
            ...
            for (const auto& binding : self.components)
            {
                ...
                const bool has_valid_contract = context.host &&
                    context.host->componentContract(binding.component_type, contract);
                const bool is_invalid_identity = !has_valid_contract ||
                ...
                const bool is_invalid_layout = !has_valid_contract ||
                    contract.abi_kind != binding.abi_kind || contract.size != binding.size ||
                    contract.alignment != binding.alignment;
                const bool is_invalid_contract = is_invalid_identity || is_invalid_layout;
            ...
            auto* instance =
                new (std::nothrow) Instance{std::addressof(self), LUA_NOREF, asset.description.model, nullptr};
            if (!instance)
            ...
            lua_pop(self.state, 1);
            if (asset.description.model == lux::rdesc::EScriptModel::ENTITY_BEHAVIOR)
            {
                auto* handle = static_cast<HostHandle*>(lua_newuserdata(self.state, sizeof(HostHandle)));
                *handle = HostHandle{std::addressof(self), context.host, true};
        ...
        ) noexcept
        {
            ...
            {
                if (!self.supportedType(return_type) || return_type.pass != lux::script::EScriptPassMode::VALUE)
                {
                    return EScriptBackendResult::UNSUPPORTED_MARSHAL_TYPE;
                }
            }
            ...
            auto* call = new (std::nothrow) Call{std::addressof(self), instance, function_ref};
            if (!call)
            {
        ...
        static bool pushArgument(lua_State* state, const lux_script_value_slot& value) noexcept
        {
            if (!value.data)
                return false;
            switch (value.kind)
            ...
            case LUX_SCRIPT_VK_BOOL:
                ...
                return true;
            case LUX_SCRIPT_VK_INT32:
                lua_pushinteger(state, *static_cast<const std::int32_t*>(value.data));
                return true;
            case LUX_SCRIPT_VK_UINT32:
                lua_pushinteger(state, *static_cast<const std::uint32_t*>(value.data));
                return true;
            case LUX_SCRIPT_VK_INT64:
                lua_pushinteger(state, static_cast<lua_Integer>(*static_cast<const std::int64_t*>(value.data)));
                return true;
            case LUX_SCRIPT_VK_FLOAT:
        ...
        static int invoke(lux_script_call_frame* frame) noexcept
        {
            ...
            if (call.instance->model == lux::rdesc::EScriptModel::ENTITY_BEHAVIOR)
            {
                lua_rawgeti(self.state, LUA_REGISTRYINDEX, call.instance->table_ref);
                ++argument_count;
            }
            ...
            for (std::uint32_t index{}; index < frame->return_count; ++index)
            {
                ...
                if (!readReturn(self.state, stack_index, frame->returns[index]))
                {
                    lua_settop(self.state, error_index - 1);
                    return -5;
                }
        ...
        static void releaseMethod(void* opaque, ScriptBackendInstance, lux::script::BoundScriptCall call_value) noexcept
        {
            auto& self = *static_cast<State*>(opaque);
            ...
            {
                luaL_unref(self.state, LUA_REGISTRYINDEX, call->function_ref);
    ...
    LuaScriptBindingBackend::LuaScriptResult
    LuaScriptBindingBackend::create(
        std::size_t instance_capacity,
    ...
    ) noexcept
    {
        for (std::size_t index{}; index < components.size(); ++index)
        {
            ...
            const auto* layout = lux::script::scriptBuiltinLayout(component.semantic_type);
            const bool supported_kind =
                component.abi_kind == LUX_SCRIPT_VK_BOOL || component.abi_kind == LUX_SCRIPT_VK_INT32 ||
                component.abi_kind == LUX_SCRIPT_VK_UINT32 || component.abi_kind == LUX_SCRIPT_VK_INT64 ||
                component.abi_kind == LUX_SCRIPT_VK_FLOAT || component.abi_kind == LUX_SCRIPT_VK_DOUBLE;
            const bool is_invalid_identity = component.name.empty() || component.component_type == 0U ||
                component.canonical_name.empty();
            const bool is_invalid_semantic = component.canonical_name.empty() ||
                component.semantic_type != lux::script::scriptSemanticTypeId(component.canonical_name);
            const bool is_invalid_layout = !layout || layout->canonical_name != component.canonical_name ||
                layout->abi_kind != component.abi_kind || layout->size != component.size ||
                layout->alignment != component.alignment;
            const bool is_invalid_contract = is_invalid_identity || is_invalid_semantic ||
                is_invalid_layout || !supported_kind;
            if (is_invalid_contract)
            {
            ...
            for (std::size_t previous{}; previous < index; ++previous)
            {
                ...
                if (components[previous].component_type == component.component_type)
                {
                    return lux::cxx::unexpected(ELuaScriptBindingBackendError::INVALID_COMPONENT_CONTRACT);
                }
            }
        ...
        try
        {
            return LuaScriptBindingBackend{std::make_unique<State>(instance_capacity, components)};
        }
        catch (const std::bad_alloc&)
        {
            return lux::cxx::unexpected(ELuaScriptBindingBackendError::ALLOCATION_FAILURE);
        }
    }

    LuaScriptBindingBackend::LuaScriptBindingBackend(std::unique_ptr<State> state) noexcept : state_(std::move(state))
    {
    }

    LuaScriptBindingBackend::~LuaScriptBindingBackend() = default;
    LuaScriptBindingBackend::LuaScriptBindingBackend(LuaScriptBindingBackend&&) noexcept = default;
    LuaScriptBindingBackend& LuaScriptBindingBackend::operator=(LuaScriptBindingBackend&&) noexcept = default;
