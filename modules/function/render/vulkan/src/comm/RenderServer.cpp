...
GeneralRenderServer::GeneralRenderServer(std::shared_ptr<Channel> frame_channel,
    ...
    : GeneralRenderServer(std::move(frame_channel), std::move(control_channel), std::move(upload_channel),
                          std::move(sync), std::make_unique<Impl>())
{
}
...
GeneralRenderServer::GeneralRenderServer(std::shared_ptr<Channel> frame_channel,
                                         std::shared_ptr<RenderControlChannel<>> control_channel,
                                         std::shared_ptr<RenderUploadChannel<>> upload_channel,
                                         std::shared_ptr<RenderChannelSync> sync, std::unique_ptr<Impl> impl)
    : RenderServer<>(frame_channel, sync, impl->dispatcher), impl_(std::move(impl)),
      control_server_(std::make_unique<RenderControlServer>(std::move(control_channel), sync, impl_->dispatcher)),
      upload_server_(
          std::make_unique<RenderUploadServer>(std::move(upload_channel), std::move(sync), impl_->dispatcher))
{
    impl_->server_ = this;
}
...
Expected<void> GeneralRenderServer::init(ServerConfig config)
{
    ...
    control_server_->setDispatchFailureSink(+[](void *user_state, EDispatchFailure, const RenderError &error) noexcept {
        auto &im = *static_cast<GeneralRenderServer::Impl *>(user_state);
        im.error_sink_.emit(error, RenderErrorEvent::kNoScene, im.current_stamp_.serial);
    });
    upload_server_->setDispatchFailureSink(+[](void *user_state, EDispatchFailure, const RenderError &error) noexcept {
        auto &im = *static_cast<GeneralRenderServer::Impl *>(user_state);
        im.error_sink_.emit(error, RenderErrorEvent::kNoScene, im.current_stamp_.serial);
    });
    return {};
...
void GeneralRenderServer::Impl::transitionUpload(std::uint32_t request_id, TransferCompletion::Kind kind,
                                                 std::uint32_t resource_index, std::uint32_t resource_gen,
                                                 EUploadLifecycleState state) noexcept
{
    ...
    auto it = std::find_if(active_uploads_.begin(), active_uploads_.end(),
                           [request_id](const ActiveUpload &upload) { return upload.request_id == request_id; });
    if (state == EUploadLifecycleState::Accepted)
    {
        if (it != active_uploads_.end())
        {
            ++upload_lifecycle_.duplicate_terminal;
            error_sink_.emit(renderError<err::upload::StateTransitionInvalid>(
                                 request_id, static_cast<std::uint32_t>(it->state), static_cast<std::uint32_t>(state)),
    ...
    const bool is_missing_upload = it == active_uploads_.end();
    const bool is_wrong_kind = !is_missing_upload && it->kind != kind;
    const bool is_wrong_resource = !is_missing_upload && it->resource_index != resource_index;
    const bool is_wrong_generation = !is_missing_upload && it->resource_gen != resource_gen;
    const bool is_stale_result = is_missing_upload || is_wrong_kind || is_wrong_resource || is_wrong_generation;
    if (is_stale_result)
    {
        ++upload_lifecycle_.stale_result;
        error_sink_.emit(renderError<err::upload::ResultIdentityMismatch>(request_id, resource_index, resource_gen),
                         RenderErrorEvent::kNoScene, current_stamp_.serial);
        return;
    }
    ...
    recent_upload_terminal_cursor_ = (recent_upload_terminal_cursor_ + 1u) % kRecentUploadTerminals;
...
bool GeneralRenderServer::Impl::observeTransferResult(const TransferCompletion &completion) noexcept
{
    ...
    const auto it = std::find_if(active_uploads_.begin(), active_uploads_.end(), [&](const ActiveUpload &upload) {
        return upload.request_id == completion.request_id;
    });
    if (it == active_uploads_.end())
    {
        const bool terminal = std::find(recent_upload_terminals_.begin(), recent_upload_terminals_.end(),
                                        completion.request_id) != recent_upload_terminals_.end();
        if (terminal)
        {
            ++upload_lifecycle_.duplicate_terminal;
        }
        ...
        error_sink_.emit(renderError<err::upload::ResultIdentityMismatch>(
                             completion.request_id, completion.resource_handle, completion.resource_gen),
                         RenderErrorEvent::kNoScene, current_stamp_.serial);
        return false;
    }
    const bool is_wrong_kind = it->kind != completion.kind;
    const bool is_wrong_resource = it->resource_index != completion.resource_handle;
    const bool is_wrong_generation = it->resource_gen != completion.resource_gen;
    const bool is_wrong_state = it->state != EUploadLifecycleState::TransferQueued;
    const bool is_stale_result = is_wrong_kind || is_wrong_resource || is_wrong_generation || is_wrong_state;
    if (is_stale_result)
    {
        ++upload_lifecycle_.stale_result;
        error_sink_.emit(renderError<err::upload::ResultIdentityMismatch>(
    ...
    transitionUpload(completion.request_id, completion.kind, completion.resource_handle, completion.resource_gen,
...
{
    transitionUpload(reply.request_id, reply.kind, reply.resource_index, reply.resource_gen,
                     reply.status == 0 ? EUploadLifecycleState::Ready : EUploadLifecycleState::Failed);
}
...
UploadLifecycleSnapshot GeneralRenderServer::uploadLifecycle() const noexcept
{
...
void GeneralRenderServer::Impl::failActiveUploadsForShutdown() noexcept
{
    ...
    {
        const bool already_has_reply =
            std::any_of(pending_deferred_replies_.begin(), pending_deferred_replies_.end(),
                        [&](const DeferredReplyEntry &reply) { return reply.request_id == upload.request_id; });
...
UploadLifecycleSnapshot GeneralRenderServer::closeAcceptedUploads() noexcept
{
    auto &im = *impl_;
    ...
    for (;;)
    {
        ...
        const bool ingress_empty =
            control_server_->endpoint().requests.empty() && upload_server_->endpoint().requests.empty();
        if (ingress_empty)
        {
            break;
        }
        ...
        if (control_server_->drainAndDispatch(impl_.get()) || upload_server_->drainAndDispatch(impl_.get()))
        {
            continue;
        }
    ...
    if (im.transfer_pipeline_)
    {
        ...
        {
            const auto finalize_idle = im.dev_ctx_->logicalDevice().waitIdle();
...
GeneralRenderServer::Impl::FinalizeDisposition GeneralRenderServer::Impl::finalizeMeshCompletion(TransferCompletion &c,
                                                                                                 bool needs_qfot,
                                                                                                 uint32_t src_family,
                                                                                                 uint32_t dst_family)
{
    ...
    else
    {
        if (needs_qfot)
        {
            mr->pushAcquireBarrier(c.mesh.vbo_buf, c.mesh.vbo_offset, c.mesh.vbo_size, src_family, dst_family);
            if (c.mesh.ibo_size > 0)
            {
                mr->pushAcquireBarrier(c.mesh.ibo_buf, c.mesh.ibo_offset, c.mesh.ibo_size, src_family, dst_family);
            }
        }
...
GeneralRenderServer::Impl::FinalizeDisposition GeneralRenderServer::Impl::finalizeTexture2DCompletion(
    TransferCompletion &c, bool needs_qfot, uint32_t src_family, uint32_t dst_family)
{
    auto &tr = render_ctx_->globalRegistry().must<TextureResources>();

    auto &bcs = tr.bindlessSet2D();
    const bool replacement = c.kind == TransferCompletion::Kind::Texture2DReplacement;
    const TextureHandle handle{c.texture.slot_index, c.resource_gen};
    ...
    const auto install = [&] {
        if (replacement)
        {
            bcs.replaceTransferredTexture(c.texture.slot_index, c.texture.image, c.texture.image_alloc, c.texture.view,
                                          c.texture.sampler, c.texture.format, c.texture.mip_levels,
                                          c.texture.array_layers, c.texture.width, c.texture.height);
        }
        else
        {
            bcs.finalizeTransferredTexture(c.texture.slot_index, c.texture.image, c.texture.image_alloc, c.texture.view,
                                           c.texture.sampler, c.texture.format, c.texture.mip_levels,
                                           c.texture.array_layers, c.texture.width, c.texture.height);
        }
    };
    if (!c.gpu_copy_recorded)
    {
        ...
        st.is_cube = false;
        st.face_stride = 0;
        st.mip_copy_count = std::clamp<uint32_t>(c.texture.uploaded_mip_count, 1u, rdesc::kTextureMaxMipCount);
    ...
    else
    {
        if (needs_qfot)
        {
            bcs.pushImageAcquireBarrier(c.texture.image, c.texture.mip_levels, c.texture.array_layers,
    ...
    tr.noteTextureResident(c.texture.slot_index, replacement ? c.logical_base_mip : 0u);
    return FinalizeDisposition::Succeeded;
}
...
void GeneralRenderServer::Impl::finalizeCompletion(TransferCompletion &c, bool needs_qfot, uint32_t src_family,
                                                   uint32_t dst_family)
{
    ...
    if (c.failed)
    {
        ...
        {
            auto &tr = render_ctx_->globalRegistry().must<TextureResources>();
            tr.endMipReplacement(TextureHandle{c.resource_handle, c.resource_gen});
        }
        if (c.request_id != UINT32_MAX)
        {
            pending_deferred_replies_.push_back(
                {c.request_id, c.kind, c.resource_handle, c.resource_gen, c.logical_base_mip, /*status=*/1u});
        }
    ...
    const bool needs_graphics_finalize =
        ...
        ((c.kind == Kind::Texture2D || c.kind == Kind::Texture2DReplacement) && c.texture.needs_mip_gen);
    ...
    if (disp == FinalizeDisposition::Succeeded)
    {
        if (c.request_id != UINT32_MAX)
        {
            DeferredReplyEntry reply{c.request_id, c.kind, c.resource_handle, c.resource_gen, c.logical_base_mip, 0u};
        ...
        if (c.retained_batch_slot != UINT32_MAX)
        {
            if (needs_graphics_finalize)
            {
                graphics_finalize_slot_batch_.push_back(c.retained_batch_slot);
            }
            else
            {
                transfer_pipeline_->releaseAfterGraphicsAcquire(c.retained_batch_slot);
            }
...
void GeneralRenderServer::Impl::reclaimReservedSlot(const TransferCompletion &c)
{
    ...
    switch (c.kind)
    {
    case TransferCompletion::Kind::Texture2D:
        tr.bindlessSet2D().removeTexture(h);
        break;
    case TransferCompletion::Kind::TextureCube:
        tr.bindlessSetCube().removeTexture(h);
        break;
    case TransferCompletion::Kind::Texture2DReplacement:
        tr.endMipReplacement(TextureHandle{h.index, h.gen});
        break;
    case TransferCompletion::Kind::MeshBuffer:
        break; // handled above
    }
}
...
bool GeneralRenderServer::Impl::pollGraphicsFinalizes(std::uint64_t gpu_value)
{
    ...
    std::erase_if(pending_graphics_finalizes_, [&](PendingGraphicsFinalize &pending) {
        ...
        pending_deferred_replies_.insert(pending_deferred_replies_.end(),
...
bool GeneralRenderServer::Impl::submitGraphicsFinalizeBatch()
{
    ...
    const auto fail_batch = [&]() {
        ...
        error_sink_.emit(renderError<err::device::VulkanObjectCreationFailed>(), RenderErrorEvent::kNoScene,
    ...
    for (BindlessCombinedSet *set : {&textures.bindlessSet2D(), &textures.bindlessSetCube()})
    {
    ...
    const auto timeline = transfer_pipeline_->submitGraphicsFinalize(command_buffer);
    if (!timeline)
    {
        fail_batch();
        return true;
...
bool GeneralRenderServer::Impl::processUploadCompletions()
{
    ...
    const VkResult sem_res = vkGetSemaphoreCounterValue(dev_ctx_->logicalDevice().handle(),
                                                        transfer_pipeline_->timelineSemaphore(), &gpu_value);
    if (sem_res != VK_SUCCESS)
    {
        ...
        if (sem_res == VK_ERROR_DEVICE_LOST)
        {
            ...
            const auto abort_completion = [&](TransferCompletion &c) {
                ...
                pending_deferred_replies_.push_back(
                    {c.request_id, c.kind, c.resource_handle, c.resource_gen, c.logical_base_mip, 1u});
            };
            ...
            do
            {
                dn = transfer_pipeline_->drainResults(completion_buf_, kMaxDrainBatch);
    ...
    const bool needs_qfot = transfer_pipeline_->needsQueueFamilyOwnershipTransfer();
    ...
    auto new_end = std::remove_if(pending_completions_.begin(), pending_completions_.end(), [&](TransferCompletion &c) {
        if (c.timeline_value <= gpu_value)
        {
            finalizeCompletion(c, needs_qfot && c.requires_queue_family_ownership_transfer, src_family, dst_family);
            made_progress = true;
    ...
    const uint32_t n = transfer_pipeline_->drainResults(completion_buf_, kMaxDrainBatch);
...
Expected<RenderTargetId> GeneralRenderServer::Impl::createSurfaceTargetInternal(RenderSurface &&surface,
                                                                                VkExtent2D extent)
{
...
Expected<void> GeneralRenderServer::attachToWindow(lux::window::LuxWindow &window)
{
    ...
    auto r = impl_->createSurfaceTargetInternal(std::move(surface), VkExtent2D{framebuffer_width, framebuffer_height});
    if (!r)
    {
...
{
    auto &im = *impl_;
    const auto flush_lane = [&]<class ReplyRing>(ERequestLane lane, ReplyRing &responses,
                                                 bool primary_publication_pending) {
        const bool has_lane = std::any_of(im.pending_deferred_replies_.begin(), im.pending_deferred_replies_.end(),
                                          [lane](const auto &reply) { return requestLane(reply.request_id) == lane; });
        if (!has_lane)
        {
            return;
        }
        ...
        for (const auto &dr : im.pending_deferred_replies_)
        {
            ...
            case Kind::MeshBuffer:
                builder.push<MeshUploadedReply>(
                    ...
                    MeshUploadedReply{dr.status ? RMeshHandle{} : RMeshHandle{dr.resource_index, dr.resource_gen},
                                      dr.status},
                    0, dr.request_id);
                break;
            case Kind::Texture2D:
                builder.push<Texture2DCreatedReply>(
                    ...
                    Texture2DCreatedReply{
                        dr.status ? RTextureHandle{} : RTextureHandle{dr.resource_index, dr.resource_gen}, dr.status},
                    0, dr.request_id);
                break;
            case Kind::Texture2DReplacement:
                builder.push<TextureMipRangeReplacedReply>(
                    type_ids::ReplyTextureMipRangeReplaced,
                    TextureMipRangeReplacedReply{RTextureHandle{dr.resource_index, dr.resource_gen},
                                                 dr.logical_base_mip, dr.status},
                    0, dr.request_id);
                break;
...
void GeneralRenderServer::flushErrorEvents()
{
    ...
    ring.drain([&sink, serial = impl_->current_stamp_.serial](const ValidationEvent &e) {
        sink.emit(renderError<err::validation::LayerReport>(e.severity, e.fingerprint), RenderErrorEvent::kNoScene,
                  serial);
    });
    if (const std::uint32_t lost = ring.dropped(); lost != 0)
    {
        ...
        sink.emit(renderError<err::validation::EventsDropped>(lost), RenderErrorEvent::kNoScene,
    ...
    builder.push<ErrorEventBatchReply>(type_ids::ReplyErrorEventBatch,
...
bool GeneralRenderServer::stepPendingResourceReleases()
{
    ...
    {
        im.gpu_completed_serial_->store(im.frame_driver_ ? im.frame_driver_->gpuCompletedSerial() : 0,
    ...
    if (im.frame_driver_)
    {
        const bool watermark_short =
            std::any_of(im.pending_resource_releases_.begin(), im.pending_resource_releases_.end(),
    ...
    if (any_reply)
    {
        ...
        for (const auto &r : im.pending_resource_releases_)
        {
            if (r.torn_down && r.request_id != 0)
            {
                if (r.scene.isValid())
                {
                    builder.push<GenericOkReply>(type_ids::ReplyGenericOk, {}, 0, r.request_id);
                }
                else
                {
                    builder.push<TargetReleasedReply>(type_ids::ReplyTargetReleased, TargetReleasedReply{r.target, 0u},
                                                      0, r.request_id);
                }
            }
        }
    ...
    im.pending_resource_releases_.erase(
        std::remove_if(im.pending_resource_releases_.begin(), im.pending_resource_releases_.end(),
...
bool GeneralRenderServer::drainTick()
{
    ...
    for (;;)
    {
        ...
        if (!stepPendingResourceReleases())
        {
            return false;
        }
...
GeneralRenderServer::ETickStage GeneralRenderServer::beginRenderTick(FrameTickState &fs)
{
    ...
    if (!prepared)
    {
        // Reject the frame before acquiring/resetting a GPU slot. A later
        // valid Program can recover without terminating the shared backend.
        reportError(prepared.error());
        flushErrorEvents();
        return ETickStage::Skipped;
    }
    auto start_result = orch.beginRenderFrame(im.targets(), im.frame_driver_.get(), fs);
    if (!start_result)
    {
...
bool GeneralRenderServer::stopAfterFrameError(const RenderError &error, std::uint32_t phase)
{
    const auto terminal = isError<err::device::FrameLifecycleCallFailed>(error) ? error
                          : isError<err::device::VulkanCallFailed>(error)
                              ? renderError<err::device::FrameLifecycleCallFailed>(phase, error.args[0])
                              : error;
