...
namespace lux::serialization
{
    ...
    namespace detail
    {
        ...
        [[nodiscard]] consteval WireShape combine(WireShape left, WireShape right) noexcept
        {
            if (left.extent == EWireExtent::VARIABLE || right.extent == EWireExtent::VARIABLE)
            {
                return {EWireExtent::VARIABLE, 0U};
            }
            if (left.fixed_size > std::numeric_limits<std::size_t>::max() - right.fixed_size)
            {
                return {EWireExtent::VARIABLE, 0U};
            }
            const std::size_t size = left.fixed_size + right.fixed_size;
            return {size == 0U ? EWireExtent::TAG : EWireExtent::FIXED, size};
        }

        [[nodiscard]] consteval WireShape repeat(WireShape value, std::size_t count) noexcept
        {
            if (value.extent == EWireExtent::VARIABLE)
                return value;
            if (count != 0U && value.fixed_size > std::numeric_limits<std::size_t>::max() / count)
            {
                return {EWireExtent::VARIABLE, 0U};
            }
        ...
        template <class T, std::size_t Index = 0U> consteval WireShape typeStaticWireShape()
        {
            ...
            else
            {
                ...
                using Member = std::remove_cvref_t<decltype(std::declval<T>().*Descriptor::pointer)>;
                return combine(wireShape<Member>(), typeStaticWireShape<T, Index + 1U>());
            }
        }

        template <class Tuple, std::size_t Index = 0U> consteval WireShape tupleWireShape()
        {
            ...
            {
                return combine(wireShape<std::tuple_element_t<Index, Tuple>>(), tupleWireShape<Tuple, Index + 1U>());
            }
        }
