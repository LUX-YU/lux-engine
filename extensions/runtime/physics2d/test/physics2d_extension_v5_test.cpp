...
main(int argc, char** argv)
{
    ...
    auto prepared = ExtensionModuleManager::prepare(ExtensionModuleRequirement::fromPath(
    ...
    auto entrypoints = modules.entrypoints(extensionId("org.lux.physics2d"));
    check(
        entrypoints.world_systems != nullptr && entrypoints.render_features == nullptr &&
...

    {
        ...
        check(static_cast<bool>(installed), "v5 entrypoint stages systems without a contribution catalog");
        check(
            builder.pendingCount() == 1u && builder.services().contains<physics2d::PhysicsWorldApi>(),
            "Physics2D stages one system and its typed World service"
        );
        const auto published = builder.commit();
        check(
            published.has_value() && schedule.compile().valid() && schedule.systemCount() == 1u &&
    ...
    entrypoints = {};
    committed->clear();
    check(modules.close().has_value(), "Physics2D unloads after World bindings and external leases release");
