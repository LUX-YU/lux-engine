...
)
{
    ...
    auto v = [&](const Eigen::Vector3f& p) -> GizmoVertex { return {p.x(), p.y(), p.z(), color}; };

    std::vector<GizmoVertex> lines;
    lines.reserve(32);

    // Near rectangle (4 edges)
    lines.push_back(v(nbl));
    lines.push_back(v(nbr));
    lines.push_back(v(nbr));
    lines.push_back(v(ntr));
    lines.push_back(v(ntr));
    lines.push_back(v(ntl));
    lines.push_back(v(ntl));
    lines.push_back(v(nbl));

    // Far rectangle (4 edges)
    lines.push_back(v(fbl));
    lines.push_back(v(fbr));
    lines.push_back(v(fbr));
    lines.push_back(v(ftr));
    lines.push_back(v(ftr));
    lines.push_back(v(ftl));
    lines.push_back(v(ftl));
    lines.push_back(v(fbl));
...
static std::vector<GizmoVertex>
buildAxesLines(const Eigen::Vector3f& origin, float length)
{
    std::vector<GizmoVertex> lines;
    lines.reserve(6);
...
static std::vector<GizmoVertex>
buildAABBLines(const Eigen::Vector3f& bmin, const Eigen::Vector3f& bmax, float r, float g, float b)
{
    uint32_t color = GizmoVertex::pack(r, g, b, 1.0f);

    auto v = [&](float x, float y, float z) -> GizmoVertex { return {x, y, z, color}; };
    ...
    // Bottom face (y=y0)
    lines.push_back(v(x0, y0, z0));
    lines.push_back(v(x1, y0, z0));
    lines.push_back(v(x1, y0, z0));
    lines.push_back(v(x1, y0, z1));
    lines.push_back(v(x1, y0, z1));
    lines.push_back(v(x0, y0, z1));
    lines.push_back(v(x0, y0, z1));
    lines.push_back(v(x0, y0, z0));

    // Top face (y=y1)
    lines.push_back(v(x0, y1, z0));
    lines.push_back(v(x1, y1, z0));
    lines.push_back(v(x1, y1, z0));
    lines.push_back(v(x1, y1, z1));
    lines.push_back(v(x1, y1, z1));
    lines.push_back(v(x0, y1, z1));
    lines.push_back(v(x0, y1, z1));
    lines.push_back(v(x0, y1, z0));
...
static std::vector<GizmoVertex>
buildOverlayQuad(const Eigen::Vector3f& center, float half_size, float r, float g, float b, float a)
{
...
static void
appendTo(std::vector<GizmoVertex>& dst, const std::vector<GizmoVertex>& src)
{
...
gizmoTestTask(RenderProgramSession& session, RenderControlSession& control, lux::window::LuxWindow& window)
{
    ...
    auto line_ops = LineListOperationIds::fromOps(line_type_reply.ops, line_type_reply.op_count);
    ...
    auto line_feat_reply = co_await control.addFeature(scene_id, line_type_reply.feature_type_id, line_cc);
    ...
    auto tri_ops = TriOverlayOperationIds::fromOps(tri_type_reply.ops, tri_type_reply.op_count);
    ...
    auto upload_lines = [&](lux::render::RenderSceneId sid, std::span<const GizmoVertex> vs) {
        ...
        p.scene_id = sid;
        p.chunk_id = 0;
        p.vertex_count = static_cast<uint32_t>(vs.size());
    ...
    auto tri_feat_reply = co_await control.addFeature(scene_id, tri_type_reply.feature_type_id, tri_cc);
    ...
    {
        auto frustum =
            buildFrustumLines({0, 2, 0}, 70.f * kPi / 180.f, 16.f / 9.f, 0.5f, 4.0f, 1.0f, 1.0f, 0.0f); // yellow
    ...
    // ─────────────────────────────────────────────────────────────────
    {
        auto sector = buildFovSector({0, 0.02f, 0}, 4.0f, 90.f * kPi / 180.f, 16, 1.0f, 0.3f, 0.1f, 0.3f);
    ...
    // ─────────────────────────────────────────────────────────────────
    {
        ...
        appendTo(all_lines, buildFrustumLines({0, 2, 0}, 70.f * kPi / 180.f, 16.f / 9.f, 0.5f, 4.0f, 1.0f, 1.0f, 0.0f));
        ...
        appendTo(all_tris, buildOverlayQuad({0, 0.01f, 0}, 2.0f, 0.2f, 0.6f, 1.0f, 0.4f));
        appendTo(all_tris, buildFovSector({0, 0.02f, 0}, 4.0f, 90.f * kPi / 180.f, 16, 1.0f, 0.3f, 0.1f, 0.3f));
    ...
    while (!window.shouldClose())
    {
        ...
        if (line_enabled)
        {
            ...
            Eigen::Vector3f frust_origin(3.0f * std::cos(frust_angle), 2.0f, 3.0f * std::sin(frust_angle));

            if (mode == GizmoMode::ALL || mode == GizmoMode::FRUSTUM_ONLY)
                appendTo(
                    lines,
                    buildFrustumLines(frust_origin, 70.f * kPi / 180.f, 16.f / 9.f, 0.3f, 2.5f, 1.0f, 1.0f, 0.0f)
                );
            ...
            if (mode == GizmoMode::ALL || mode == GizmoMode::AXES_AABB_ONLY)
            {
                ...
                appendTo(lines, buildAABBLines({-pulse, 0, -pulse}, {pulse, pulse * 1.5f, pulse}, 0.0f, 1.0f, 1.0f));
            }
        ...
        if (tri_enabled)
        {
            ...
            Eigen::Vector3f quad_center(2.0f * std::cos(quad_angle), 0.01f, 2.0f * std::sin(quad_angle));
            appendTo(tris, buildOverlayQuad(quad_center, 1.0f, 0.2f, 0.8f, 0.2f, 0.35f));
