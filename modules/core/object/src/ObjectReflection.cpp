...
namespace lux::object::reflection
{
                                  ...
                                  std::string_view name) noexcept
    {
        for (const auto &field : object_class.static_fields)
        {
            ...
            return SignalViewAccess::make(std::addressof(field));
        }
        return {};
    }
                                                                 ...
                                                                 lux::object::EDelivery delivery)
    {
        ...
        const auto expected_parameter_count = signal.hasPayload() ? std::size_t{1} : std::size_t{0};
        ...
        if (signal.hasPayload())
        {
            ...
            {
                return lux::cxx::unexpected(EDynamicObserveError::PARAMETER_TYPE_MISMATCH);
            }
        }
        ...
        if (!connected)
        {
            ...
            {
            case EObserveError::PAYLOAD_NOT_QUEUEABLE:
                return lux::cxx::unexpected(EDynamicObserveError::PAYLOAD_NOT_QUEUEABLE);
            case EObserveError::SENDER_HAS_NO_DISPATCHER:
                return lux::cxx::unexpected(EDynamicObserveError::SENDER_HAS_NO_DISPATCHER);
            case EObserveError::RECEIVER_HAS_NO_DISPATCHER:
                return lux::cxx::unexpected(EDynamicObserveError::RECEIVER_HAS_NO_DISPATCHER);
            case EObserveError::DIRECT_CROSS_AFFINITY:
                return lux::cxx::unexpected(EDynamicObserveError::DIRECT_CROSS_AFFINITY);
            case EObserveError::OBJECT_CLOSED:
                return lux::cxx::unexpected(EDynamicObserveError::OBJECT_CLOSED);
            }
            std::terminate();
        }
        return *connected;
