...
namespace lux::asset
{
        ...
        {
            {
                ...
                if (found != page_cache.end())
                {
                    if (found->second.digest != expected)
                        return lux::cxx::unexpected(std::string{"conflicting Pak page digest"});
                    page_lru.splice(page_lru.begin(), page_lru, found->second.lru);
            ...
            std::string error;
            if (!stream || !detail::readPakPage(stream, file_size, offset, loaded, &error))
            {
                return lux::cxx::unexpected(error.empty() ? std::string{"cannot open Pak index"} : error);
            }
            if (!detail::verifyPakPageDigest(loaded, expected))
                return lux::cxx::unexpected(std::string{"Pak index page digest mismatch"});
            ...
            page_cache.emplace(offset, CachedPage{loaded, expected, page_lru.begin()});
            ...
            stats.metadata_resident_bytes = page_cache.size() * detail::kPakPageSize;
            stats.metadata_resident_high_water =
                std::max(stats.metadata_resident_high_water, stats.metadata_resident_bytes);
            return loaded;
        }
        ...
        findEntry(const AssetId& id) const
        {
            ...
            for (std::uint32_t depth = 0u; depth < 64u; ++depth)
            {
                ...
                if (page_header.kind == detail::EPakPageKind::ENTRY_LEAF)
                {
                    ...
                    if (!std::is_sorted(rows.begin(), rows.end(), [](const auto& a, const auto& b) {
                            return a.id < b.id;
                        }))
                    {
                        return lux::cxx::unexpected(std::string{"unordered Pak entry leaf"});
                    }
                    const auto found =
                ...
                if (page_header.kind != detail::EPakPageKind::ENTRY_INTERNAL)
                    return lux::cxx::unexpected(std::string{"wrong page kind in Pak entry tree"});
                ...
                {
                    return lux::cxx::unexpected(std::string{"unordered Pak entry internal page"});
                }
                const auto child =
            ...
            return lux::cxx::unexpected(std::string{"Pak entry tree depth limit exceeded"});
        }

        [[nodiscard]] lux::cxx::expected<std::optional<AssetId>, std::string> findPath(std::string_view path) const
        {
            ...
            for (std::uint32_t depth = 0u; depth < 64u; ++depth)
            {
                ...
                if (page_header.kind == detail::EPakPageKind::PATH_LEAF)
                {
                    ...
                    {
                        return lux::cxx::unexpected(std::string{"unordered Pak path leaf"});
                    }
                    const auto found =
                ...
                if (page_header.kind != detail::EPakPageKind::PATH_INTERNAL)
                    return lux::cxx::unexpected(std::string{"wrong page kind in Pak path tree"});
                ...
                {
                    return lux::cxx::unexpected(std::string{"unordered Pak path internal page"});
                }
                const auto child =
            ...
            return lux::cxx::unexpected(std::string{"Pak path tree depth limit exceeded"});
        }
    };
    ...
    PakAssetProvider::loadFromFile(const std::filesystem::path& pak_path)
    {
        ...
        if (!detail::readPakHeader(stream, file_size, header, &error))
            return lux::cxx::unexpected(lux::format("'{}': {}", pak_path.string(), error));
        ...
        data.header = header;
        data.mount_hint.assign(header.mount_hint, header.mount_hint + header.mount_hint_size);
        ...
        auto entry_root = data.loadPage(header.entry_root_offset, header.entry_root_digest);
        if (!entry_root)
            return lux::cxx::unexpected(entry_root.error());
        const auto entry_kind = detail::pakPageHeader(entry_root.value()).kind;
        if (entry_kind != detail::EPakPageKind::ENTRY_LEAF && entry_kind != detail::EPakPageKind::ENTRY_INTERNAL)
        {
            return lux::cxx::unexpected(std::string{"invalid Pak entry root kind"});
        }
        auto path_root = data.loadPage(header.path_root_offset, header.path_root_digest);
        if (!path_root)
            return lux::cxx::unexpected(path_root.error());
        const auto path_kind = detail::pakPageHeader(path_root.value()).kind;
        if (path_kind != detail::EPakPageKind::PATH_LEAF && path_kind != detail::EPakPageKind::PATH_INTERNAL)
        {
            return lux::cxx::unexpected(std::string{"invalid Pak path root kind"});
        }
        return provider;
    }
    ...
    std::optional<AssetId> PakAssetProvider::resolve(std::string_view rel_vpath) const
    {
    ...
    lux::cxx::expected<AssetBlob, EAssetStorageError> PakAssetProvider::open(const AssetId& id) const
    {
        ...
        if (!stream.read(reinterpret_cast<char*>(bytes.get()), static_cast<std::streamsize>(entry.size)))
        {
            return lux::cxx::unexpected(EAssetStorageError::IO_FAILURE);
        }
        ...
        return AssetBlob::fromSharedArray(std::move(bytes), static_cast<std::size_t>(entry.size));
    }

    void PakAssetProvider::enumerate(const std::function<void(const ProviderEntry&)>& fn) const
    {
        std::ifstream stream(d_->pak_path, std::ios::binary);
        std::vector<detail::PakEntry> entries;
        std::string error;
        if (!stream || !detail::readAllPakEntries(stream, d_->file_size, d_->header, entries, &error))
        {
            return;
        }
        for (const auto& entry : entries)
        {
            if (entry.tombstone())
            {
                fn(ProviderEntry{entry.id, entry.asset_magic, std::string{}, true});
            }
            else if (!entry.vpath.empty())
            {
                fn(ProviderEntry{entry.id, entry.asset_magic, entry.vpath, false});
            }
        }
    }

    std::optional<std::string> PakAssetProvider::pathOf(const AssetId& id) const
    {
        const auto found = d_->findEntry(id);
        const bool is_missing_result = !found || !found.value();
        const bool is_tombstone = !is_missing_result && found.value()->tombstone();
        const bool is_empty_path = !is_missing_result && found.value()->vpath.empty();
        const bool is_invalid_entry = is_missing_result || is_tombstone || is_empty_path;
        if (is_invalid_entry)
        {
            return std::nullopt;
        }
