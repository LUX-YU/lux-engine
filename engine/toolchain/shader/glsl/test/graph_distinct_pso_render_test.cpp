...
int main()
{
    ...
    s.beginFrame({});
    const auto view = await(control, w, control.addView(scene.scene_id, {W, H}, "GraphView"));
    s.beginFrame({});
    ...
    auto compileFrag = [&](const std::vector<uint32_t>& spv, const std::vector<std::byte>& info)
    {
        ...
        s.beginFrame({});
        return await(control, w, control.compileShader(bytes, std::span<const std::byte>{info.data(), info.size()}));
    };
    ...
    s.beginFrame({});
    const auto mat_r = await(upload, w, uploadGraphMaterial(MaterialUploadClient(upload_client.client(), material_ops), data_r, compiled_r.shader, ShaderHandle{}));
    s.beginFrame({});
    const auto mat_g = await(upload, w, uploadGraphMaterial(MaterialUploadClient(upload_client.client(), material_ops), data_g, compiled_g.shader, ShaderHandle{}));
    ...
    s.beginFrame({});
    const auto inst_l = await(s, w, addTransientMeshInstance(MeshStackProxy(s, mesh_stack_ops), scene.scene_id, mesh.handle, mat_r.handle, xform_l));
    s.beginFrame({});
    const auto inst_r = await(s, w, addTransientMeshInstance(MeshStackProxy(s, mesh_stack_ops), scene.scene_id, mesh.handle, mat_g.handle, xform_r));
    s.beginFrame({});
