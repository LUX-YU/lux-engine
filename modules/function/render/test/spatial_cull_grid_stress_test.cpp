...
main()
{
    ...
    for (uint32_t gx = 0; gx < G; ++gx)
        for (uint32_t gy = 0; gy < G; ++gy)
            alive.push_back(
                SpatialCullGrid::InstanceXY{gx * G + gy, gx * kCell + 0.5f * kCell, gy * kCell + 0.5f * kCell}
            );
    ...
    check(
        actives.front() > 0 && actives.front() < kN,
    ...
    check(
        actives[2] < actives[0] || actives[3] < actives[0] || actives[4] < actives[0],
