...
namespace lux::engine::core
{
    ...
    Eigen::Affine3f viewTransform(const EigenVector3<_ScalarV>& camera_position, const EigenMatrix3<_ScalarV>& rotation)
    {
        ...
        T_view.matrix().block<3, 1>(0, 3).array() = -camera_position;
        Eigen::Affine3f R_view(Eigen::Affine3f::Identity());
        R_view.matrix().block<3, 3>(0, 0).array() = rotation.transpose();
    ...
    EigenMatrix4<_RetScalar> lookAt(const _T0 &eye, const _T1 &target, const _T2 &up)
    {
        ...
        ret.block<1, 3>(1, 0) = u;
        ret.block<1, 3>(2, 0) = -f;
        ret.block<1, 4>(3, 0) = EigenVector4<_RetScalar>{0, 0, 0, 1};
        ret.block<3, 1>(0, 3) = EigenVector3<_RetScalar>{-s.dot(eye), -u.dot(eye), f.dot(eye)};
        return ret;
    }
