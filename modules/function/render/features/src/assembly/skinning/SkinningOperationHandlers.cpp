...
namespace lux::render
{
    ...
    namespace
    {
        ...
        struct SkinDeps { SkinningResources* skin; InstanceResources* inst;
                          MeshResources* mesh_res; StaticVertexPoolSet* pools; RenderScene* sc; };
        ...
        void applyBoneSkinningOne(const SkinDeps& d,
                                  ...
                                  uint32_t bone_count)
        {
            ...
            const uint32_t in_pool_id = pools
                ? pools->ensureRegistered(gpu->vbo_segment, gpu->layout_id)
                : ~0u;
            if (in_pool_id == ~0u)
            {
                ctx.reportError(renderError<err::frame::SkinningInputPoolUnavailable>(),
                                scene_index, frame_serial);
                return;
            }
            ...
            if (palette_base == ~0u)
            {
                ctx.reportError(renderError<err::frame::BonePaletteFull>(bone_count),
                                scene_index, frame_serial);
                return;
            }
            ...
            if (!out.valid())
            {
                ctx.reportError(renderError<err::frame::SkinnedVertexPoolExhausted>(vcount),
                                scene_index, frame_serial);
                return;
            }
