...
namespace lux::authoring
{
    namespace
    {
        ...
        findFunction(const lux::rdesc::Script& script, lux::script::ScriptSymbolId symbol) noexcept
        {
            const auto found =
                std::find_if(script.exports.begin(), script.exports.end(), [symbol](const auto& function) noexcept {
        ...
        [[nodiscard]] EScriptBindingAuthoringError
        mapCompatibility(lux::simulation::EScriptBindingCompatibility value) noexcept
        {
        ...
        void copyHookSignature(lux::simulation::SimulationHookPointView hook, ScriptBindingTargetCatalogEntry& entry)
        {
            ...
            for (std::size_t index{}; index < hook.parameterCount(); ++index)
            {
                ...
                entry.parameters.push_back({std::string{type.canonical_name}, type.type_id, type.pass});
            }
            for (std::size_t index{}; index < hook.returnCount(); ++index)
            {
                const auto type = hook.returnAt(index);
                entry.returns.push_back({std::string{type.canonical_name}, type.type_id, type.pass});
            }
        }
    ...
    std::vector<ScriptBindingTargetCatalogEntry>
    makeScriptBindingTargetCatalog(const lux::simulation::SimulationDescription& simulation)
    {
        ...
        for (std::size_t system_index{}; system_index < simulation.systemCount(); ++system_index)
        {
            ...
            for (std::size_t event_index{}; event_index < system.eventCount(); ++event_index)
            {
                ...
                entry.model = event.target() == ESystemEventTarget::GLOBAL ? lux::rdesc::EScriptModel::GLOBAL_MODULE
        ...
        for (const auto point :
             {EBehaviorLifecyclePoint::CONSTRUCT, EBehaviorLifecyclePoint::START, EBehaviorLifecyclePoint::STOP})
        {
            ScriptBindingTargetCatalogEntry entry;
            ...
            if (catalog[index].model == script.model &&
                lux::simulation::evaluateScriptBindingSignatureCompatibility(
                    ...
                    *function,
                    ...
                    catalog[index].returns) == lux::simulation::EScriptBindingCompatibility::COMPATIBLE)
            {
    ...
    ) noexcept
    {
        ...
        if (std::find(mount.bindings.begin(), mount.bindings.end(), binding) != mount.bindings.end())
        {
            return EScriptBindingAuthoringError::DUPLICATE_BINDING;
        }
        const auto compatible = mapCompatibility(
            lux::simulation::evaluateScriptBindingCompatibility(simulation, script.model, *function, binding.target)
        );
        if (compatible != EScriptBindingAuthoringError::SUCCESS)
            return compatible;
        if (const auto* hook = std::get_if<lux::simulation::SystemHookBindingTarget>(std::addressof(binding.target)))
        {
            std::string_view target_instance;
            const auto target_hook = resolveHook(simulation, *hook, target_instance);
            if (target_hook && target_hook.cardinality() == lux::simulation::ESystemHookCardinality::SINGLE)
            {
                for (const auto& existing : mount.bindings)
                {
                    const auto* existing_hook =
                        std::get_if<lux::simulation::SystemHookBindingTarget>(std::addressof(existing.target));
                    if (!existing_hook || existing_hook->hook != hook->hook)
                        continue;
                    std::string_view existing_instance;
                    const auto resolved = resolveHook(simulation, *existing_hook, existing_instance);
                    if (resolved && existing_instance == target_instance)
                    {
                        return EScriptBindingAuthoringError::SINGLE_HOOK_MULTIPLE_HANDLERS;
                    }
                }
            }
        }
    ...
    )
    {
        ...
        for (std::size_t index{}; index < mount.bindings.size(); ++index)
        {
            ...
            const auto* function = findFunction(script, binding.function);
            const auto error = function ? mapCompatibility(lux::simulation::evaluateScriptBindingCompatibility(
                                              simulation,
                                              script.model,
                                              *function,
                                              binding.target)
    ...
    )
    {
        ...
        for (std::size_t mount_index{}; mount_index < mounts.size(); ++mount_index)
        {
            const auto& entry = mounts[mount_index];
            if (!entry.script || !entry.mount)
                continue;
            auto local = diagnoseScriptBindings(simulation, *entry.script, *entry.mount);
            for (auto& diagnostic : local)
                diagnostic.mount_index = mount_index;
            result.insert(result.end(), std::make_move_iterator(local.begin()), std::make_move_iterator(local.end()));
            for (std::size_t binding_index{}; binding_index < entry.mount->bindings.size(); ++binding_index)
            {
                const auto& binding = entry.mount->bindings[binding_index];
                const auto* target =
                    std::get_if<lux::simulation::SystemHookBindingTarget>(std::addressof(binding.target));
                if (!target)
                    continue;
                std::string_view instance;
                const auto hook = resolveHook(simulation, *target, instance);
                if (!hook || hook.cardinality() != lux::simulation::ESystemHookCardinality::SINGLE)
                {
                    continue;
                }
                const auto duplicate =
                    std::find_if(singles.begin(), singles.end(), [&](const SingleTarget& value) noexcept {
                        return value.system_instance == instance && value.hook == target->hook;
                    }
                    );
