 ...
 *
 * Fields marked `cooked_relocation=*` remain reflected for their owning
 * cooked format, but are deliberately absent from this generic archive.
 * Unknown, unannotated leaves are diagnosed and skipped.
 */

#include <lux/engine/core/serialization/Archive.hpp>
#include <lux/engine/core/serialization/NameTable.hpp>
#include <lux/engine/core/serialization/visibility.h>
...
namespace lux::serialize
{
    ...
    class LUX_CORE_SERIALIZATION_PUBLIC TaggedPropertyWriter
    {
    public:
    ...
    /// 该字段是否按 AssetRef(16 字节 uuid)归档。资产引用扫描(编辑器删除
    /// 对话框列引用者)与序列化共用同一张分类表 —— 「什么算资产引用」只许
    /// 有一处定义,别在消费侧再按 type_hash 各记一份。
    [[nodiscard]] LUX_CORE_SERIALIZATION_PUBLIC bool
    isAssetRefField(const lux::meta::RefField& field);
    ...
    class LUX_CORE_SERIALIZATION_PUBLIC TaggedPropertyReader
    {
    public:
        ...
        /// Reads the canonical representation emitted for an exact schema.
        /// Every serializable public field must occur exactly once, in
        /// reflection order, with the matching wire type and exact payload
        /// size. Unknown, missing, duplicate, reordered or type-drifted
        /// fields invalidate the archive instead of being skipped.
        ///
        /// Cooked EntitySection data uses this mode because its component
        /// schema version is already an exact contract. Asset formats which
        /// intentionally support forward/backward field drift keep using
        /// readObject().
        [[nodiscard]] bool readObjectExact(
