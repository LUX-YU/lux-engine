...
namespace lux::asset
{
    ...
    namespace
    {
        ...
        EAssetError validateScriptHeader(const std::vector<std::byte>& file,
    ...
    {
        // Pure, manager-less, exception-free decode of the Script DESCRIPTION
        // (the asset_data_t) out of a complete .luxasset image. The only
        // potentially-throwing operations are the allocations (file copy /
        // make_unique); decodeScriptDescription is itself noexcept. Wrap them
        // so the whole function honours its noexcept contract via expected.
        try
        {
    ...
    ScriptSerDeser::exportAsLuxAssetStream(const LuxAsset& asset, std::ofstream& ofs)
    {
        ...
        const Script* script = static_cast<const Script*>(sasset->rawData());
        if (!script)
