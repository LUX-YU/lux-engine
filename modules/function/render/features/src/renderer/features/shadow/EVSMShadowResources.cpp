...
namespace lux::render
{
    void EVSMShadowResources::init(const InitInfo& info)
    {
        ...
        frames_in_flight_ = std::min(std::max(info.frames_in_flight, 1u), static_cast<uint32_t>(config_ubos_.size()));
        ...
        for (uint32_t fi = 0; fi < frames_in_flight_; ++fi)
        {
            ...
            const VkResult br = vmaCreateBuffer(
                allocator_,
                &buf_ci,
                &alloc_ci,
                &config_ubos_[fi],
    ...
    void EVSMShadowResources::shutdown()
    {
        ...
        // No vkDestroySampler: sampler_ is borrowed from the DescriptorService
        // cache, which keeps it alive until device teardown.
        vkDestroyImageView(device_, moment_view_, nullptr);
        vkDestroyImageView(device_, scratch_view_, nullptr);
        if (moment_image_ != VK_NULL_HANDLE)
            vmaDestroyImage(allocator_, moment_image_, moment_alloc_);
        if (scratch_image_ != VK_NULL_HANDLE)
            vmaDestroyImage(allocator_, scratch_image_, scratch_alloc_);
    ...
    void EVSMShadowResources::writeConfig(const ConfigGPU& cfg)
    {
        for (uint32_t fi = 0; fi < frames_in_flight_; ++fi)
        {
            if (config_ubo_mapped_[fi] == nullptr)
                continue;
    ...
    void EVSMShadowResources::createImage(
        VkImage& image,
        VmaAllocation& alloc,
        VkImageView& view,
    ...
    void EVSMShadowResources::createSampler()
    {
        ...
        //
        // This is exactly SamplerDesc::linearClamp(). The hand-built create-info
        // it replaces also set a white border colour, which was never reachable:
