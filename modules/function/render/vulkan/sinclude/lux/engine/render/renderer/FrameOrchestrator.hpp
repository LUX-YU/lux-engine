...
// ─────────────────────────────────────────────────────────────────────────
// 这组数据的词汇是场景与视图,和线协议无关,因此由帧编排直接拥有。
...
class LUX_FUNCTION_PUBLIC FrameOrchestrator
{
  public:
    ...
    [[nodiscard]] Expected<void> prepareTargets(RenderTargetRegistry &, Renderer &);
    ...
    [[nodiscard]] Expected<EFrameStart> beginRenderFrame(RenderTargetRegistry &targets, FrameDriver *driver,
                                                         FrameTickState &st);

    /// 阶段 2。上传 → 逐离屏 target 的层链渲染 → 主 Surface 的层链渲染 →
    /// 各被触及场景 endViewFrame。
    [[nodiscard]] Expected<void> renderTargets(RenderTargetRegistry &targets, Renderer &renderer, SceneViewBatch &batch,
                                               FrameTickState &st);

    /// 阶段 3。结帧(提交 + 呈现)→ 目标池按栅栏证实的完成水位老化回收。
    /// command-buffer end / queue submit 的永久失败沿 Expected 返回，且错误
    /// 发生后不会继续呈现额外 Surface 或推进退休水位。
    [[nodiscard]] Expected<void> endRenderFrame(RenderTargetRegistry &targets, FrameDriver *driver, FrameTickState &st);

  private:
    FrameClock frame_clock_;
    FrameStamp current_stamp_{};
    std::vector<RenderTargetId> target_order_;
    std::vector<std::uint8_t> target_marks_;
    std::vector<SampledTargetImage> sampled_images_;
};
