...
static void test_set_get_sequence()
{
    ...
    FlowGraph graph;
    const uint64_t x = graph.addVariable("x", i32, lux::meta::RuntimeObject(int32_t{0}));
    ...
    check(
        g_sunk.size() == 2 && g_sunk[0] == 5 && g_sunk[1] == 7,
...
static void test_sum_of_evens_demo()
{
    ...
    FlowGraph graph;
    const uint64_t sum = graph.addVariable("sum", i32, lux::meta::RuntimeObject(int32_t{0}));
    ...
    check(!g_sunk.empty() && g_sunk[0] == 20, "demo: sum of evens in [0,10) == 20");
}
