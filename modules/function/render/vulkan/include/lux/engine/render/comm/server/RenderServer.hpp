...
template <std::size_t PayloadAlignment>
ExternalDataView<PayloadAlignment> resolveExternalDataView(const CommandStorage<PayloadAlignment> &pack,
                                                           ExternalDataRef ref,
...
template <std::size_t PayloadAlignment>
std::span<const std::byte> resolveExternalData(const CommandStorage<PayloadAlignment> &pack, ExternalDataRef ref,
                                               TypeId expected_attachment_type = kInvalidTypeId)
{
...
using ErasedHandlerFn = void (*)(ExecuteContext<RequestAlignment, ReplyAlignment> &, const CmdRecord &,
...
bool replyToCurrent(ExecuteContext<RequestAlignment, ReplyAlignment> &ctx,
                    const typename CommandTraits<CommandPayload>::Reply &payload, std::uint16_t flags = 0)
{
    static_assert(command_has_reply_v<CommandPayload>, "CommandPayload does not declare a reply type.");
...
class FrameDispatcher
{
  ...
  private:
    [[nodiscard]] bool executeCommands(std::span<const CmdRecord> commands,
                                       const CommandStorage<RequestAlignment> &program, Ctx &ctx) const
    {
        ...
        auto emitFailure = [&ctx](const CmdRecord &c, EDispatchFailure code, const RenderError &error) {
            ...
            {
                ctx.replies.template push<CommandFailedReply>(type_ids::ReplyCommandFailed,
                                                              CommandFailedReply{static_cast<uint32_t>(code), error}, 0,
                                                              c.request_id);
            }
        ...
        for (const CmdRecord &cmd : commands)
        {
            ...
            {
                emitFailure(cmd, EDispatchFailure::InvalidOpcode, renderError<err::comm::InvalidOpcode>(cmd.opcode));
                all_ok = false;
                continue;
            }
            ...
            if (entry.fn == nullptr)
            {
                emitFailure(cmd, EDispatchFailure::HandlerRejected, renderError<err::comm::HandlerNotRegistered>(idx));
                all_ok = false;
                continue;
            }
    ...
    static void unaryThunk(Ctx &ctx, const CmdRecord &cmd, std::span<const std::byte> bytes)
    {
        if (bytes.size() != sizeof(T))
        {
            ctx.markDispatchError(renderError<err::comm::PayloadSizeMismatch>(
                static_cast<std::uint32_t>(sizeof(T)), static_cast<std::uint32_t>(bytes.size())));
            return;
        }
        T value{};
...
class RenderServer
{
  public:
    ...
    explicit RenderServer(std::shared_ptr<Channel> channel, std::shared_ptr<RenderChannelSync> sync,
...
template <class Channel> class RenderOperationServer final
{
  public:
    ...
    RenderOperationServer(std::shared_ptr<Channel> channel, std::shared_ptr<RenderChannelSync> sync,
                          Dispatcher &dispatcher)
        : channel_(std::move(channel)), sync_(std::move(sync)), dispatcher_(dispatcher)
    {
    }
...
struct ServerConfig
{
    ...
    // Optional Main-readable fence proof, owned by the thread host and
    // alive through backend destruction. Never inferred from submission.
    std::atomic<std::uint64_t> *gpu_completed_serial{nullptr};
...
class LUX_FUNCTION_PUBLIC GeneralRenderServer : public RenderServer<>
{
  public:
    ...
    GeneralRenderServer(std::shared_ptr<Channel> frame_channel, std::shared_ptr<RenderControlChannel<>> control_channel,
                        std::shared_ptr<RenderUploadChannel<>> upload_channel, std::shared_ptr<RenderChannelSync> sync);
    ...
    [[nodiscard]] CreateSceneResult createScene(
        std::string_view name, std::span<const FeatureInitParam> features = {},
    ...
    [[nodiscard]] Expected<void> bindSwapchain(RenderSceneId scene_id, ViewHandle view,
    ...
    [[nodiscard]] Expected<RTextureHandle> createTexture2D(const lux::rdesc::Texture &texture,
    ...
    [[nodiscard]] ShaderHandle compileShader(std::span<const std::byte> spirv,
  ...
  protected:
    ...
    [[nodiscard]] bool stepPendingResourceReleases();
  ...
  protected:
    ...
    [[nodiscard]] bool renderRenderTick(FrameTickState &fs);
    ...
    void deferSurfaceRelease(RenderTargetId target, std::unique_ptr<PresentContext> ctx,
