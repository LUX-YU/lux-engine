...
namespace
{
    ...
    inline constexpr std::array kBridgeHooks{
...
int main()
{
    ...
    const auto* reflected = registry_meta.findClass(
    ...
    const std::array parameter_ids{
        lux::cxx::type_hash<std::int32_t>()};
    const auto* reflected_function = registry_meta.findFunction(
    ...
    const std::array descriptor_set{
