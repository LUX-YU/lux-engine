...
namespace lux::gapi::vk
{
	...
	public:
		...
		DescriptorSetLayout(VkDevice device, const VkDescriptorSetLayoutCreateInfo& info, VkAllocationCallbacks* allocator = nullptr)
		{
			VK_FUNC_INVOKE(vkCreateDescriptorSetLayout, "Failed to create DescriptorSetLayout object", device, &info, allocator, &descriptor_set_layout);
		}
		...
		inline operator VkDescriptorSetLayout() const noexcept { return descriptor_set_layout; }
		inline const VkDescriptorSetLayout* operator&() const noexcept { return &descriptor_set_layout; }

		inline VkDescriptorSetLayout handle() const noexcept { return descriptor_set_layout; }
		inline const VkDescriptorSetLayout* handlePtr() const noexcept { return &descriptor_set_layout; }
	...
	public:
		...
		{
			VK_FUNC_INVOKE(vkAllocateDescriptorSets, "Failed to allocate DescriptorSet object", device, &info, &descriptor_set);
		}
		...
		void update(VkDevice device, const VkDescriptorBufferInfo& buffer_info, uint32_t binding, uint32_t array_element = 0)
		{
		...
		inline operator VkDescriptorSet() const noexcept { return descriptor_set; }
		inline const VkDescriptorSet* operator&() const noexcept { return &descriptor_set; }

		inline VkDescriptorSet handle() const noexcept { return descriptor_set; }
		inline const VkDescriptorSet* handlePtr() const noexcept { return &descriptor_set; }
	...
	public:
		...
		{
			VK_FUNC_INVOKE(vkCreateDescriptorPool, "Failed to create DescriptorPool object", device, &create_info, allocator, &descriptor_pool);
		}
		...
		inline operator VkDescriptorPool() const noexcept { return descriptor_pool; }
		inline const VkDescriptorPool* operator&() const noexcept { return &descriptor_pool; }

		inline VkDescriptorPool handle() const noexcept { return descriptor_pool; }
		inline const VkDescriptorPool* handlePtr() const noexcept { return &descriptor_pool; }
