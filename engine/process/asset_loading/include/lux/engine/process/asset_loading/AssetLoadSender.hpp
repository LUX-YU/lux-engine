...
namespace detail
{
template <class Owner> struct AssetReadReceiver final
{
...
template <class ConcreteAsset> class AssetLoadSender final
{
  ...
  public:
    ...
    using completion_signatures =
        stdexec::completion_signatures<stdexec::set_value_t(std::shared_ptr<const ConcreteAsset>),
                                       stdexec::set_error_t(AssetLoadFailure), stdexec::set_stopped_t()>;

    AssetLoadSender(AssetReadPort read, lux::asset::AssetId id, lux::asset::AssetDecodeLimits limits,
                    std::stop_token stop) noexcept
        : read_(std::move(read)), id_(id), limits_(limits), stop_(stop)
    {
    }

    template <class Receiver> class State final
    {
      public:
        using operation_state_concept = stdexec::operation_state_t;
        using ReadReceiver = AssetReadReceiver<State>;
        using ReadSender =
            decltype(lux::process::portSender(std::declval<AssetReadPort>(), std::declval<ReadAssetImage>()));
        using ReadState = decltype(stdexec::connect(std::declval<ReadSender>(), std::declval<ReadReceiver>()));

        State(AssetReadPort read, lux::asset::AssetId id, lux::asset::AssetDecodeLimits limits, std::stop_token stop,
              Receiver receiver)
            : receiver_(std::move(receiver)), id_(id), limits_(limits), stop_(stop),
              read_state_(
                  stdexec::connect(lux::process::portSender(std::move(read), ReadAssetImage{id}), ReadReceiver{this}))
        {
        }
        ...
        void readValue(lux::asset::AssetBlob value) noexcept
        {
            ...
            auto decoded = lux::asset::TAssetSerDeser<ConcreteAsset>::decode(id_, std::move(value.bytes), limits_);
            if (stop_.stop_requested() || token.stop_requested())
            {
                stdexec::set_stopped(std::move(receiver_));
                return;
    ...
    template <class Receiver> [[nodiscard]] State<std::decay_t<Receiver>> connect(Receiver &&receiver) &&
    {
        return State<std::decay_t<Receiver>>{std::move(read_), id_, limits_, stop_, std::forward<Receiver>(receiver)};
    }
...
[[nodiscard]] auto loadAsset(AssetReadPort read, lux::asset::AssetId id, lux::asset::AssetDecodeLimits limits,
                             std::stop_token stop = {}) noexcept
{
    return detail::AssetLoadSender<ConcreteAsset>(std::move(read), id, limits, stop);
}
} // namespace lux::process::asset_loading
