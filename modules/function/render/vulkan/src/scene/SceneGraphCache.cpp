...
namespace lux::render
{
    SceneGraphCache::SceneGraphCache(RenderContext& ctx, std::string debug_name)
        : ctx_(ctx), debug_name_(std::move(debug_name))
    {
    ...
    void SceneGraphCache::invalidate(EGraphInvalidationReason reason) noexcept
    {
        state_.valid = false;
    ...
    )
    {
        ...
        if (layout.hasSlot(TargetSlot::SCENE_COLOR))
        {
            ...
            const std::uint32_t transient_slots = required_target_slots & ~present_slots;
            for (std::size_t si = 0; si < kTargetSlotCount; ++si)
            {
                ...
                if (slot_desc.format == lux::rdesc::ETextureFormat::UNDEFINED)
                {
                    continue;
                }
                auto transient_desc = RGTextureDescription::Relative(1.0f, 1.0f, slot_desc.format);
                transient_desc.usage = static_cast<ERGTextureUsageFlags>(ERGTextureUsageBits::COLOR_ATTACHMENT) |
                                       static_cast<ERGTextureUsageFlags>(ERGTextureUsageBits::SAMPLED);
                (void)builder.createTexture(targetSlotName(slot), transient_desc);
            }
            ...
            compile_options.max_color_attachments = ctx_.deviceContext().caps().max_color_attachments;
            ...
            auto compiled =
                RenderGraphCompiler::compile(std::move(description), ctx_.pipelineManager(), compile_options);
            compile_nanoseconds = static_cast<std::uint64_t>(
                std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::steady_clock::now() - compile_started)
                    .count()
            );
        ...
        if (state_.graph)
        {
            ...
            telemetry_.retired_graph_high_water =
                std::max(telemetry_.retired_graph_high_water, static_cast<std::uint64_t>(retired_graphs_.size()));
        }
    ...
    void SceneGraphCache::retireAllViewResources(
