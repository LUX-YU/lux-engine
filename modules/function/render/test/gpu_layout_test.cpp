...
main()
{
    ...
    perspective(2, 2) = -perspective_far / (perspective_far - perspective_near);
    perspective(2, 3) = -(perspective_far * perspective_near) / (perspective_far - perspective_near);
    perspective(3, 2) = -1.0f;
    float recovered_near = 0.0f;
    float recovered_far = 0.0f;
    assert(lux::render::projectionDepthRange(perspective, recovered_near, recovered_far));
    ...
    orthographic(2, 2) = -1.0f / (orthographic_far - orthographic_near);
    orthographic(2, 3) = -orthographic_near / (orthographic_far - orthographic_near);
    assert(lux::render::projectionDepthRange(orthographic, recovered_near, recovered_far));
    ...
    assert(lux::render::canRebaseRenderPageDelta2D(page_delta_2d, origin_delta));
    lux::render::rebaseRenderPageDelta2D(page_delta_2d, origin_delta);
    assert(page_delta_2d[0] == 2 && page_delta_2d[1] == -2);
    const std::int64_t rejected_delta[3]{std::numeric_limits<std::int64_t>::max(), 0, 0};
    assert(!lux::render::canRebaseRenderPageDelta2D(page_delta_2d, rejected_delta));
