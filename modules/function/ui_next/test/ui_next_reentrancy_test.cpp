...
namespace
{
    ...
    public:
        ReentrantPane(
            ...
            : Object(
                  std::move(dispatcher),
                  std::move(id),
                  lux::ui::PaneTypeId{"reentrant.pane"},
                  "Reentrant Pane"
              )
        {
        }
    ...
    public:
        ...
        [[nodiscard]] bool enabledAndReset() noexcept
        {
            registration_to_reset->reset();
            return true;
        }

        [[nodiscard]] bool checked() noexcept
        {
            ++checked_count;
            return true;
        }

        [[nodiscard]] bool enabledAndReallocate() noexcept
        {
            for (int index = 0; index < 32; ++index)
            {
                auto command = router->defineCommand(
                    {lux::ui::UiCommandId{
                         "generated." + std::to_string(next_generated++)
                     },
                     "Generated"}
                );
                assert(command);
        ...
        std::vector<lux::ui::CommandRegistration> generated_registrations;
        int next_generated{0};
        int invoke_count{0};
        int checked_count{0};
    };
    ...
    public:
        ...
        [[nodiscard]] bool destroyDuringEnabled() noexcept
        {
        ...
        std::unique_ptr<SelfDestroyingReceiver>* owner{nullptr};
        bool invoked{false};
    };
} // namespace
...
int main()
{
...

    {
        ...
        auto* target_registration = target == &first ? std::addressof(*first_registration)
        ...
        assert(connection);
        assert(session.focusPane(target->id().view()));
...

    {
        ...
        assert(session.createPane(
            lux::ui::PaneTypeIdView{"replaceable.factory"},
...

    {
        ...
        auto state_registration = router.bindGlobal<
        ...
        auto invoke_registration = router.bindGlobal<
            &CommandReceiver::invoke,
            &CommandReceiver::enabledAndReallocate>(*invoke_command, receiver);
        assert(invoke_registration);
        receiver.registration_to_reset = std::addressof(*invoke_registration);
        assert(
            router.invoke(*invoke_command) ==
        ...
        std::array<lux::ui::UiContextIdView, 2> temporary_views;
        {
            ...
            temporary_views = {
                lux::ui::UiContextIdView{temporary_name},
...

    {
        ...
        auto registration = router.bindGlobal<
            &SelfDestroyingReceiver::invoke,
            &SelfDestroyingReceiver::destroyDuringEnabled>(*command, *receiver);
        assert(registration);
        assert(
            router.invoke(*command) ==
