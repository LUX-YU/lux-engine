...
namespace lux::editor
{
    ...
    void SceneFeatureSettingPanel::drawSceneContributions()
    {
        ImGui::TextUnformatted("Scene Contributions");
        if (!editor_scene_ || !editor_scene_->isLive() ||
            !editor_scene_->worldSource())
        {
            ImGui::TextDisabled("No Authoring World source is open.");
            return;
        }
        const auto& contributions =
            editor_scene_->worldSource()->contributions;
        if (contributions.empty())
        {
            ImGui::TextDisabled("No scene contributions are configured.");
            return;
        }
        ImGui::TextDisabled(
            "Configuration bytes are owned and edited by each contribution.");
        for (const auto& contribution : contributions)
        {
            ImGui::BulletText(
                "%.*s  (schema %u, %zu bytes)",
                static_cast<int>(contribution.id.name().size()),
                contribution.id.name().data(),
                contribution.config_schema_version,
                contribution.config.size());
        }
    }

    void SceneFeatureSettingPanel::drawVisualStateEditor()
    {
    ...
    void SceneFeatureSettingPanel::drawTerrainTab()
    {
        if (!editor_scene_ || !editor_scene_->isLive())
        {
