...
namespace lux::render::kernels
{
    ...
    static void emitShadowCullKernel(
        ProgramEmitter& e,
        uint32_t /*pi*/,
    ...
    static void emitShadowDrawKernel(
        ProgramEmitter& e,
        uint32_t /*pi*/,
        const RGCompiledPass& cpass,
        const RGCompiledGraph& /*compiled*/
    )
    {
        ...
        const uint32_t bgc =
            (cfg.bias_group_count < kMaxShadowBiasGroups) ? cfg.bias_group_count : kMaxShadowBiasGroups;
    ...
    replayShadowCullCommand(uint32_t sub_cmd, const void* data, uint16_t data_size, KernelReplayContext& ctx)
    {
        ...
        case kUploadFrustums: {
            ...
            const VkDeviceSize kGroupMapOffset = static_cast<VkDeviceSize>(ext->slice_count) * 6u * 16u;
        ...
        case kCullPushConstants: {
            ...
            // world-partition active-mask address; both rotate each frame).
            {
                ...
                {
                    const auto* ext = static_cast<const MeshInstanceExtData*>(ctx.frame_ctx.ext_data[isl]);
    ...
    replayShadowDrawCommand(uint32_t sub_cmd, const void* data, uint16_t data_size, KernelReplayContext& ctx)
    {
        ...
        if (ext && g < ShadowFrameExtData::kMaxBiasGroups && ext->draw_lanes[g].active)
        {
    ...
    static uint32_t resolveShadowPatch(uint16_t source_param, const RGFrameContext& frame_ctx)
    {
        switch (source_param)
        {
        case kSliceCount: {
        ...
        case kInstanceSlotCount: {
            ...
            {
                alive = static_cast<const MeshInstanceExtData*>(frame_ctx.ext_data[isl])->slot_count;
            }
