...
buildGroundMesh(float half)
{
    ...
    std::vector<std::uint32_t> indices = {
        ...
        0,
        3,
        2, // and the reverse → double-sided
    };
    return lux::rdesc::Mesh{
        std::move(verts),
        std::move(indices),
...
static void
makeTranslate(float out[16], float x, float y, float z)
{
...
template <class T>
static T
awaitControl(RenderControlSession& control, lux::window::LuxWindow& window, RenderRequest<T> request)
{
...
template <class T>
static T
awaitUpload(RenderUploadSession& upload, lux::window::LuxWindow& window, UploadSubmitResult<T> submitted)
{
    if (!submitted)
        return {};
...
)
{
    ...
    const auto view = awaitControl(control, w, control.addView(scene.scene_id, {W, H}, name));
    const auto target = awaitControl(control, w, control.createOffscreenRenderTarget({W, H}));
    ...
    const auto view_cam_reg =
        awaitControl(control, w, control.registerFeatureType(lux::render::kViewCameraFeatureFactory));
    ...
    awaitControl(control, w, control.addFeature(scene.scene_id, view_cam_reg.feature_type_id, view_cam_cfg));
    ...
    const auto light_reg = awaitControl(control, w, control.registerFeatureType(kLightFeatureFactory));
    LightOperationIds light_ops = LightOperationIds::fromOps(light_reg.ops, light_reg.op_count);
    lux::render::LightCommTag light_cfg{};
    awaitControl(control, w, control.addFeature(scene.scene_id, light_reg.feature_type_id, light_cfg));
    ...
    const auto material_reg =
        awaitControl(control, w, control.registerFeatureType(lux::render::kMaterialFeatureFactory));
    ...
    awaitControl(control, w, control.addFeature(scene.scene_id, material_reg.feature_type_id, material_cfg));
    ...
    const auto mesh_stack_reg =
        awaitControl(control, w, control.registerFeatureType(lux::render::kMeshStackFeatureFactory));
    ...
    awaitControl(control, w, control.addFeature(scene.scene_id, mesh_stack_reg.feature_type_id, mesh_stack_cfg));
    ...
    const auto mat_frag = awaitControl(
        control,
        w,
        control.compileShader(mat_bytes, std::span<const std::byte>{mat_info.data(), mat_info.size()})
    );

    lux::render::GraphMaterialData mat_data{};
    s.beginFrame({});
    const auto mat = awaitUpload(
        upload,
        w,
        uploadGraphMaterial(
    ...
    s.beginFrame({});
    const auto ground_mesh = awaitUpload(
        upload,
        w,
        lux::render::uploadMesh(lux::render::MeshStackUploadClient(upload_client.client(), mesh_stack_ops), ground)
    );
    const lux::rdesc::Mesh sphere = buildSphereMesh(0.6f, 48, 24);
    s.beginFrame({});
    const auto sphere_mesh = awaitUpload(
        upload,
        w,
        lux::render::uploadMesh(lux::render::MeshStackUploadClient(upload_client.client(), mesh_stack_ops), sphere)
    );

    float ground_xform[16];
    makeTranslate(ground_xform, 0.f, 0.f, 0.f);
    float sphere_xform[16];
    makeTranslate(sphere_xform, 0.f, 0.9f, 0.f);
    s.beginFrame({});
    const auto ground_inst = await(
        s,
        w,
    ...
    s.beginFrame({});
    MeshStackProxy(s, mesh_stack_ops)
        .makeInstanceVisibleForView({.scene_id = scene.scene_id, .view = view.view, .object = ground_inst.object});
    MeshStackProxy(s, mesh_stack_ops)
        .makeInstanceVisibleForView({.scene_id = scene.scene_id, .view = view.view, .object = sphere_inst.object});
...
main()
{
    ...
    lux::window::LuxWindow window(static_cast<int>(W), static_cast<int>(H), "two_scene_shadow_isolation_test");
    ...
    const Eigen::Matrix4f P = buildProjMatrix(45.f * 3.14159265f / 180.f, static_cast<float>(W) / H, 0.05f, 50.f);
        ...
        for (int i = 0; i < frames; ++i)
        {
            ...
            for (const auto& sc : scenes)
                viewCameraUpdateTransient(
                    ViewCameraProxy(s, sc.view_cam_ops),
                    sc.scene_id,
                    sc.view,
                    V.data(),
                    P.data(),
    ...
    auto readback = [&](const SceneBundle& sc, std::vector<std::uint8_t>& out) {
        ...
        s.beginFrame({});
        const auto rb = awaitControl(control, w, control.readbackTarget(sc.target, out.data(), out.size()));
        return rb;
    };

    // ── Scene A alone (light pointing one way) ──────────────────────────
    const SceneBundle A = buildScene(s, control, upload, w, "SceneA", W, H, Eigen::Vector3f(-1.0f, -1.2f, -0.35f));
    std::cout << "[3] scene A built (id=" << A.scene_id.index << ")\n";
    warmUp({A}, 12);
    std::vector<std::uint8_t> bufA0;
    const auto rb0 = readback(A, bufA0);
    std::cout << "[4] readback A-alone: status=" << rb0.status << " bytes=" << rb0.bytes_written << "\n";

    // ── Scene B with the OPPOSITE light direction ───────────────────────
    const SceneBundle B = buildScene(s, control, upload, w, "SceneB", W, H, Eigen::Vector3f(+1.0f, -1.2f, +0.35f));
    ...
    std::cout << "[6] readback A-with-B: status=" << rb1.status << " bytes=" << rb1.bytes_written << "\n";
    ...
    std::size_t aa_diff = 0;
    int aa_max = 0;
    pixelDiff(bufA0, bufA1, aa_diff, aa_max);
    // A vs B → the "scenes genuinely differ" signal (want large).
    std::size_t ab_diff = 0;
    int ab_max = 0;
    pixelDiff(bufA0, bufB, ab_diff, ab_max);

    // Content of A-alone (non-blank + lit), so equality isn't a both-blank pass.
    bool a0_uniform = true;
    std::size_t a0_bright = 0;
    for (std::size_t i = 0; i + 4 <= bufA0.size(); i += 4)
    {
        const bool is_after_first_pixel = i >= 4;
        const bool differs_red = bufA0[i] != bufA0[0];
        const bool differs_green = bufA0[i + 1] != bufA0[1];
        const bool differs_blue = bufA0[i + 2] != bufA0[2];
        const bool differs_alpha = bufA0[i + 3] != bufA0[3];
        const bool is_non_uniform = differs_red || differs_green || differs_blue || differs_alpha;
        if (is_after_first_pixel && is_non_uniform)
            a0_uniform = false;
        if (std::max({bufA0[i], bufA0[i + 1], bufA0[i + 2]}) > 40)
            ++a0_bright;
    }
    ...
    std::cout << "  A-alone vs A-with-B: diff_pixels=" << aa_diff << "/" << total << " max_abs_diff=" << aa_max << "\n";
    std::cout << "  A vs B (opposite light): diff_pixels=" << ab_diff << "/" << total << " max_abs_diff=" << ab_max
              << "\n";
    std::cout << "  A-alone: uniform=" << (a0_uniform ? "yes" : "no") << " bright=" << a0_bright << "/" << total
              << "\n";
    ...
    check(isolated, "scene A is UNCHANGED by coexisting scene B (light/shadow isolated)");

    sync->requestStop();
    server_thread.join();

    std::cout << "=== two_scene_shadow_isolation_test " << (fails == 0 ? "PASSED" : "FAILED") << " (fails=" << fails
              << ") ===\n";
    return fails == 0 ? 0 : 1;
}
