    ...
    void drawCanvas()
    {
        ...
        for (const auto &record : document_.nodes())
        {
            ...
            for (const auto &pin : pin_groups_[record.id])
            {
                ...
                canvas::BeginPin(canvas::PinId{pin.id.value}, input ? canvas::PinKind::Input : canvas::PinKind::Output);
    ...
    return {std::string(flowforge::kFlowForgeDocumentType),
            ...
            [](DocumentEditor &base, EditorWindow &window, rendering::EditorRenderer &,
               process::ExecutionRuntime &) -> EditorResult<void> {
                ...
                auto pane = std::make_unique<FlowForgePane>(*document,
                                                            "flowforge-" + std::to_string(document->historyId().value));
