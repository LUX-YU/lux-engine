...
namespace lux::render
{
    namespace
    {
        constexpr std::uint32_t kHeightCount = kTerrainWireSampleEdge * kTerrainWireSampleEdge;
        constexpr std::uint32_t kWeightBytes = kHeightCount * 4u;
        constexpr std::uint32_t kHoleBytes = (kHeightCount + 7u) / 8u;
        constexpr std::uint32_t kFallbackEdge = (kTerrainWireQuadEdge / 2u) + 1u;
        constexpr std::uint32_t kFallbackCount = kFallbackEdge * kFallbackEdge;
        constexpr std::uint32_t kInvalidSlot = 0xffffffffu;

        [[nodiscard]] constexpr std::size_t minMaxOffset() noexcept
        {
            return sizeof(TerrainWirePageDataHeader) + static_cast<std::size_t>(kHeightCount) * sizeof(std::uint16_t) +
    ...
    bool TerrainResources::canRebaseSceneOrigin(const std::int64_t origin_delta[3]) const noexcept
    {
        ...
        {
            if (!canRebaseRenderPageDelta(page.header.origin.page_delta, origin_delta))
            {
                return false;
            }
        }
    ...
    void TerrainResources::rebaseSceneOrigin(const std::int64_t origin_delta[3]) noexcept
    {
        for (auto& [_, page] : pages_)
        {
            rebaseRenderPageDelta(page.header.origin.page_delta, origin_delta);
            uploadMetadata(page);
        }
    }
    ...
    {
        return static_cast<std::size_t>(kFallbackCount) * sizeof(std::uint16_t);
    }
    ...
    bool TerrainResources::ensureFallback(Page& page) noexcept
    {
        ...
        if (free_fallback_slots_.empty())
        {
            ...
            for (auto& [_, candidate] : pages_)
            {
                if (&candidate == &page || candidate.fallback_slot == kInvalidSlot)
                {
                    continue;
                }
    ...
    void TerrainResources::uploadMetadata(Page& page) noexcept
    {
        ...
        {
            meta.origin_full_slot[axis] = page.header.origin.page_delta[axis];
            meta.local_spacing[axis] = page.header.origin.local[axis];
        }
        meta.origin_full_slot[3] = page.cache_slot == kInvalidSlot ? -1 : static_cast<std::int32_t>(page.cache_slot);
        ...
        meta.height_extent[2] = page.header.sample_spacing * kTerrainWireQuadEdge;
        meta.height_extent[3] = 1.0f / (page.header.height_max - page.header.height_min);
        meta.fallback_flags[0] = page.fallback_slot;
        meta.fallback_flags[1] = 1u | (page.cache_slot == kInvalidSlot ? 0u : 2u) |
                                 ((page.transition_active || page.transition_end_coverage > 0.0f) ? 4u : 0u);
    ...
    bool TerrainResources::upsert(const UploadTerrainPagePayload& header, std::span<const std::byte> page_data)
    {
        ...
        const auto root_offset =
            minMaxOffset() + static_cast<std::size_t>(kTerrainWireMinMaxNodeCount - 1u) * sizeof(std::uint16_t) * 2u;
        std::uint16_t actual_quantized_min{0u};
        std::uint16_t actual_quantized_max{0u};
        std::memcpy(&actual_quantized_min, page.data.data() + root_offset, sizeof(actual_quantized_min));
        std::memcpy(
        ...
        page.actual_height_min =
            header.height_min + quantization_extent * (static_cast<float>(actual_quantized_min) / 65535.0f);
        page.actual_height_max =
            header.height_min + quantization_extent * (static_cast<float>(actual_quantized_max) / 65535.0f);
        ...
        if (previous)
        {
            ...
            page.transition_start_coverage = previous->transition_start_coverage;
            page.transition_end_coverage = previous->transition_end_coverage;
        ...
        const auto rollback = [&]() {
            ...
            {
                if (!previous || current->second.fallback_slot != previous->fallback_slot)
                {
                    releaseFallback(current->second);
                }
                if (!previous || current->second.cache_slot != previous->cache_slot)
                {
    ...
    const TerrainResources::Page* TerrainResources::find(TerrainWireId id) const noexcept
    {
    ...
    void TerrainResources::reconcileWanted(
        ...
        float wanted_radius,
        float scene_time,
        std::uint64_t demotion_delay_frames
    ) noexcept
    {
        ...
        for (auto& [_, page] : pages_)
        {
            if (page.transition_active && scene_time >= page.transition_start_time + page.transition_duration)
            {
                page.transition_active = false;
                page.transition_start_coverage = page.transition_end_coverage;
        ...
        const auto projectedError = [&views](const Page& page) noexcept {
            ...
            for (const auto& view : views)
            {
                ...
                const auto center_x = (static_cast<double>(page.header.origin.page_delta[0]) -
                                       static_cast<double>(view.position.page_delta[0])) *
                                          view.coordinate_page_size +
                                      page.header.origin.local[0] - view.position.local[0] + extent * 0.5;
                const auto center_z = (static_cast<double>(page.header.origin.page_delta[2]) -
                                       static_cast<double>(view.position.page_delta[2])) *
                                          view.coordinate_page_size +
                                      page.header.origin.local[2] - view.position.local[2] + extent * 0.5;
                const auto origin_y = (static_cast<double>(page.header.origin.page_delta[1]) -
                                       static_cast<double>(view.position.page_delta[1])) *
                                          view.coordinate_page_size +
                                      page.header.origin.local[1] - view.position.local[1];
                ...
                const auto x = std::max(std::abs(center_x) - horizontal_half, 0.0);
                const auto z = std::max(std::abs(center_z) - horizontal_half, 0.0);
                const auto minimum_y = origin_y + page.actual_height_min;
                const auto maximum_y = origin_y + page.actual_height_max;
                const auto y = minimum_y > 0.0 ? minimum_y : (maximum_y < 0.0 ? -maximum_y : 0.0);
                const auto distance = std::max(std::sqrt(x * x + y * y + z * z), 1.0);
                maximum = std::max(
                    maximum,
                    static_cast<float>(page.header.geometric_error / distance * view.projection_scale)
                );
            }
            return maximum;
        };
        const auto select = [this, &desired, &projectedError](auto&& self, Page& page) -> void {
            ...
            for (std::uint8_t index = 0u; index < page.header.child_count; ++index)
            {
                const auto found = pages_.find(key(page.header.children[index]));
                if (found == pages_.end() || found->second.fallback_slot == kInvalidSlot)
                {
            ...
            const auto threshold =
                page.drawable_target ? page.header.hlod_enter_error_pixels : page.header.hlod_exit_error_pixels;
        ...
        for (auto& [_, page] : pages_)
        {
            if (!page.header.parent.valid() || !pages_.contains(key(page.header.parent)))
            {
                select(select, page);
            }
        }
        ...
        if (!views.empty())
        {
            ...
            for (const auto& [page_key, page] : pages_)
            {
                ...
                const double x = (static_cast<double>(view.position.page_delta[0]) -
                                  static_cast<double>(page.header.origin.page_delta[0])) *
                                     view.coordinate_page_size +
                                 view.position.local[0] - page.header.origin.local[0];
                const double z = (static_cast<double>(view.position.page_delta[2]) -
                                  static_cast<double>(page.header.origin.page_delta[2])) *
                                     view.coordinate_page_size +
                                 view.position.local[2] - page.header.origin.local[2];
                const double extent = static_cast<double>(page.header.sample_spacing) * kTerrainWireQuadEdge;
                const bool is_outside_page = x < 0.0 || z < 0.0 || x > extent || z > extent;
                const bool is_not_finest_page = page.header.sample_spacing >= finest_spacing;
                const bool is_invalid_page = is_outside_page || is_not_finest_page;
                if (is_invalid_page)
                {
                    continue;
                }
                ...
                const double sample_x =
                    std::clamp(x / page.header.sample_spacing, 0.0, static_cast<double>(kTerrainWireQuadEdge));
                const double sample_z =
                    std::clamp(z / page.header.sample_spacing, 0.0, static_cast<double>(kTerrainWireQuadEdge));
                ...
                const auto loadHeight = [&page](std::uint32_t sx, std::uint32_t sz) noexcept {
                    ...
                    std::memcpy(
                        &quantized,
                        page.data.data() + sizeof(TerrainWirePageDataHeader) + index * sizeof(quantized),
                        sizeof(quantized)
                    );
                    return page.header.height_min + (page.header.height_max - page.header.height_min) *
                ...
                const float h0 = std::lerp(loadHeight(x0, z0), loadHeight(x1, z0), tx);
                const float h1 = std::lerp(loadHeight(x0, z1), loadHeight(x1, z1), tx);
                const float local_height = std::lerp(h0, h1, tz);
                const double page_y =
                    static_cast<double>(page.header.origin.page_delta[1]) * view.coordinate_page_size +
                    page.header.origin.local[1];
                const double view_y = static_cast<double>(view.position.page_delta[1]) * view.coordinate_page_size +
                                      view.position.local[1];
                debug_view_surface_valid_ = true;
                debug_view_surface_level_ = page.header.hierarchy_level;
                debug_view_surface_height_ = static_cast<float>(page_y + local_height);
                debug_view_surface_clearance_ = static_cast<float>(view_y - page_y - local_height);
        ...
        const auto finite_radius =
            std::isfinite(wanted_radius) && wanted_radius > 0.0f ? static_cast<double>(wanted_radius) : 0.0;
        for (auto& [page_key, page] : pages_)
        {
            double closest = std::numeric_limits<double>::infinity();
            const auto page_extent = static_cast<double>(page.header.sample_spacing) * kTerrainWireQuadEdge;
            for (const auto& view : views)
            {
                if (!std::isfinite(view.coordinate_page_size) || !(view.coordinate_page_size > 0.0f))
                {
                    continue;
                }
                const auto x = (static_cast<double>(page.header.origin.page_delta[0]) -
                                static_cast<double>(view.position.page_delta[0])) *
                                   view.coordinate_page_size +
                               static_cast<double>(page.header.origin.local[0]) - view.position.local[0] +
                               page_extent * 0.5;
                const auto z = (static_cast<double>(page.header.origin.page_delta[2]) -
                                static_cast<double>(view.position.page_delta[2])) *
                                   view.coordinate_page_size +
                               static_cast<double>(page.header.origin.local[2]) - view.position.local[2] +
                               page_extent * 0.5;
        ...
        for (auto& [_, page] : pages_)
        {
            if (page.cache_slot != kInvalidSlot && frame_serial_ > page.last_wanted_serial &&
                frame_serial_ - page.last_wanted_serial > demotion_delay_frames)
            {
                demote(page);
            }
        }
    ...
    void TerrainResources::setDrawable(Page& page, bool drawable, float scene_time) noexcept
    {
        ...
        page.transition_duration = std::max(static_cast<float>(page.header.transition_milliseconds) / 1000.0f, 0.001f);
    ...
    TerrainPageCacheStatsReply TerrainResources::stats() const noexcept
    {
        ...
        result.selected_patch_count_valid = selected_patch_count_valid_ ? 1u : 0u;
        result.debug_view_surface_valid = debug_view_surface_valid_ ? 1u : 0u;
        result.debug_view_surface_level = debug_view_surface_level_;
        for (std::size_t axis = 0u; axis < 3u; ++axis)
        {
            result.debug_view_page_delta[axis] = debug_view_position_.page_delta[axis];
            result.debug_view_local[axis] = debug_view_position_.local[axis];
        }
        result.debug_view_surface_height = debug_view_surface_height_;
        result.debug_view_surface_clearance = debug_view_surface_clearance_;
        ...
        for (const auto& [_, page] : pages_)
        {
            ...
            if (page.transition_active || page.transition_end_coverage > 0.0f)
            {
                ...
                if (page.header.hierarchy_level < std::size(result.drawable_pages_by_level))
                {
                    ++result.drawable_pages_by_level[page.header.hierarchy_level];
                }
            }
