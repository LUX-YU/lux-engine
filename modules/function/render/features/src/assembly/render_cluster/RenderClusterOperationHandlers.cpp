...
namespace lux::render
{
    ...
    namespace
    {
        ...
        [[nodiscard]] CullValidationCounts
        validateViewCullInputs(RenderScene& scene, const RenderClusterResources& clusters) noexcept
        {
            ...
            for (const std::uint32_t slot_index : candidate_slots)
            {
                ...
                {
                    center[axis] =
                        static_cast<float>(cull.bsphere_page[axis] - camera->render_origin.page_delta[axis]) *
                            page_size +
        ...
        )
        {
            ...
            for (const auto change :
                 resources.reconcileHierarchy(family, true, scene.sceneTime(), resources.transitionDurationSeconds()))
            {
                    ...
                    if (instances->isAlive(slot))
                    {
                        ...
                        property.transition_flags =
                            change.transition == RenderClusterResources::ETransitionAction::NONE
                                ? 0u
                                : 1u | (change.transition == RenderClusterResources::ETransitionAction::FADE_OUT ? 2u
                                                                                                                 : 0u);
    ...
    handleRenderClusterUpload(GeneralRenderServer::Dispatcher::Ctx& context, const UploadRenderClusterPayload& payload)
    {
        ...
        const bool has_valid_alignment = blob.empty() ||
            reinterpret_cast<std::uintptr_t>(blob.data()) % alignof(RenderClusterWireInstance) == 0u;
        ...
        if (is_valid_blob)
        {
            const auto instances = std::span<const RenderClusterWireInstance>{
                reinterpret_cast<const RenderClusterWireInstance*>(blob.data()),
    ...
    )
    {
        ...
        const auto instances =
            resources ? std::min<std::size_t>(resources->instanceCount(), std::numeric_limits<std::uint32_t>::max())
                      : 0u;
        const auto visible_clusters =
            resources
                ? std::min<std::size_t>(resources->visibleClusterCount(), std::numeric_limits<std::uint32_t>::max())
                : 0u;
        const auto visible_instances =
            resources
                ? std::min<std::size_t>(resources->visibleInstanceCount(), std::numeric_limits<std::uint32_t>::max())
                : 0u;
        ...
        const auto cpu_memory =
            resources ? resources->cpuMemorySnapshot() : RenderClusterResources::CpuMemorySnapshot{};
