...
namespace detail
{
    ...
    {
        using WaitFenceFn = VkResult (*)(
            gapi::vk::Fence&,
            VkDevice
        ) noexcept;
        using ResetFenceFn = VkResult (*)(
            gapi::vk::Fence&,
            VkDevice
        ) noexcept;
        using ResetCommandBufferFn = VkResult (*)(
            gapi::vk::CommandBuffer&
        ) noexcept;
        using BeginCommandBufferFn = VkResult (*)(
            gapi::vk::CommandBuffer&,
            VkCommandBufferUsageFlags
        ) noexcept;
        using EndCommandBufferFn = VkResult (*)(
            gapi::vk::CommandBuffer&
        ) noexcept;
    ...
    [[nodiscard]] LUX_FUNCTION_PUBLIC Expected<void> waitFrameFence(
        gapi::vk::Fence& fence,
        VkDevice device,
    ...
    [[nodiscard]] Expected<void> submitFrameThenRecordAndPresent(
        SubmitFn&& submit,
        RecordFn&& record_submission,
        PresentFn&& present
    )
    {
    ...
    public:
        ...
        [[nodiscard]] std::uint32_t fenceCount() const noexcept { return fence_count_; }
...
public:
    ...
    [[nodiscard]] static Expected<std::unique_ptr<FrameDriver>> create(
    ...
    [[nodiscard]] Expected<FrameRuntime> beginFrame(
    ...
    [[nodiscard]] Expected<void> endFrame(
    ...
    [[nodiscard]] uint64_t gpuCompletedSerial() const noexcept { return gpu_completed_serial_; }
...
private:
    ...
    Expected<void> submitRecordedFrame(
        const FrameRuntime& rt,
        bool presenting,
