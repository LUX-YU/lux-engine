...
namespace lux::render
{
    ...
    void handleTrajectoryCreate(GeneralRenderServer::Dispatcher::Ctx& ctx, const CreateTrajectoryPayload& p)
    {
        ...
        const TrajectoryHandle trajectory = traj_res->createTrajectory(decodePoints(ctx, p.point_data, p.point_count));
        ...
        replyToCurrent<CreateTrajectoryPayload>(ctx, TrajectoryCreatedReply{trajectory, kTrajectoryStatusOk});
    }
    ...
    void handleTrajectoryAppend(GeneralRenderServer::Dispatcher::Ctx& ctx, const AppendTrajectoryPointsPayload& p)
    {
        ...
        const bool queued = traj_res->queueAppend(p.trajectory, decodePoints(ctx, p.point_data, p.point_count));
        replyToCurrent<AppendTrajectoryPointsPayload>(
            ctx,
    ...
    void handleTrajectoryClear(GeneralRenderServer::Dispatcher::Ctx& ctx, const ClearTrajectoryPayload& p)
    {
        ...
        replyToCurrent<ClearTrajectoryPayload>(
            ctx,
            GenericOkReply{queued ? kTrajectoryStatusOk : kTrajectoryStatusInvalidHandle}
        );
    }
    ...
    void handleTrajectoryRemove(GeneralRenderServer::Dispatcher::Ctx& ctx, const RemoveTrajectoryPayload& p)
    {
        ...
        replyToCurrent<RemoveTrajectoryPayload>(
            ctx,
            GenericOkReply{queued ? kTrajectoryStatusOk : kTrajectoryStatusInvalidHandle}
        );
    }
    ...
    void handleTrajectoryReplace(GeneralRenderServer::Dispatcher::Ctx& ctx, const ReplaceTrajectoryPointsPayload& p)
    {
        ...
        const bool queued = traj_res->queueReplace(p.trajectory, decodePoints(ctx, p.point_data, p.point_count));
        replyToCurrent<ReplaceTrajectoryPointsPayload>(
            ctx,
            GenericOkReply{queued ? kTrajectoryStatusOk : kTrajectoryStatusInvalidHandle}
        );
    }
