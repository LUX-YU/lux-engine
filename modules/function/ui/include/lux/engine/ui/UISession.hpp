...
namespace lux::ui
{
	...
	public:
		...
		[[nodiscard]] std::unique_ptr<Pane>
		createPane(PaneTypeIdView type, PaneId id);

		[[nodiscard]] CommandRouter&
		commandRouter() noexcept;

		[[nodiscard]] const CommandRouter&
		commandRouter() const noexcept;

		[[nodiscard]] lux::object::ObjectDispatcherRef
		dispatcherRef() const noexcept;
		...
		restoreLayout(const LayoutSnapshot &snapshot);
