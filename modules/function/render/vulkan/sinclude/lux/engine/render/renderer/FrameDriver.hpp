...
[[nodiscard]] LUX_FUNCTION_PUBLIC Expected<void> waitFrameFence(gapi::vk::Fence &fence, VkDevice device,
...
[[nodiscard]] LUX_FUNCTION_PUBLIC Expected<void> beginFrameRecording(gapi::vk::Fence &fence,
...
[[nodiscard]] Expected<void> endFrameRecordingThen(gapi::vk::CommandBuffer &command_buffer,
                                                   const FrameDriverRuntimeOps &ops, SubmitFn &&submit)
{
...
[[nodiscard]] Expected<void> submitFrameThenRecordAndPresent(SubmitFn &&submit, RecordFn &&record_submission,
                                                             PresentFn &&present)
{
...
class LUX_FUNCTION_PUBLIC FrameDriverCreateCandidate final
{
  public:
    ...
    [[nodiscard]] static Expected<FrameDriverCreateCandidate> create(VkDevice device, VkCommandPool command_pool,
...
class LUX_FUNCTION_PUBLIC FrameDriver
{
  public:
    ...
    [[nodiscard]] static Expected<std::unique_ptr<FrameDriver>> create(ResourceContext &res_ctx,
                                                                       std::uint32_t frames_in_flight);
    ...
    [[nodiscard]] Expected<FrameRuntime> beginFrame(uint32_t frame_index, uint64_t frame_id, PresentContext *present);
    ...
    // Non-blocking maintenance; only signalled fences advance the watermark.
    [[nodiscard]] Expected<void> pollCompletions();
  ...
  private:
    FrameDriver(ResourceContext &res_ctx, std::uint32_t frames_in_flight,
