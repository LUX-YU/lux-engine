...
int main()
{
    lux::simulation::ecs::EcsState world{
        lux::simulation::ecs::EcsStateConfig{{256U * 1024U, 32U * 1024U * 1024U}}
    };
    ...
    struct AddPosition final
    {
        ...
        void apply(lux::simulation::ecs::EcsMutation& mutation) noexcept
        {
    ...
    const lux::simulation::ecs::Entity first = edit.create();
    ...
    const lux::simulation::ecs::Entity second = next_edit.create();
    assert(world.valid(second));
    assert(second != first);

    next_edit.emplace<Position>(second, 3);
    lux::simulation::ecs::ChangeCursor<Position> position_cursor;
    auto& journal = lux::simulation::ecs::detail::EcsChangeAccess::log(world);
    assert(
    ...
    assert(modified.status() == lux::simulation::ecs::EChangeReadStatus::CURRENT);
    assert(modified.size() == 1);
    assert((*modified.begin()).entity == second);
    assert(
        (*modified.begin()).kind ==
        lux::simulation::ecs::EComponentChangeKind::MODIFIED
    );
    modified = {};

    std::size_t count{};
    for (auto [entity, position] : world.query<lux::simulation::ecs::Read<Position>>())
    {
    ...
    for (auto [entity, position] :
         next_edit.query<lux::simulation::ecs::Write<Position>>())
    {
        assert(entity == second);
        position.value = 12;
    }
    ...
    auto repeated_query = next_edit.query<lux::simulation::ecs::Write<Position>>();
    ...
    assert(repeated_modified.status() == lux::simulation::ecs::EChangeReadStatus::CURRENT);
    ...
    assert(
        (*removed.begin()).kind ==
        lux::simulation::ecs::EComponentChangeKind::REMOVED
    );
    removed = {};

    lux::simulation::ecs::EntityChangeCursor entity_cursor;
    assert(
    ...
    assert(
        ...
        lux::simulation::ecs::EEntityChangeKind::DESTROYED
    );

    lux::simulation::ecs::EcsState small_world{
        lux::simulation::ecs::EcsStateConfig{
    ...
    lux::simulation::ecs::ChangeCursor<Position> overflow_cursor;
    auto& small_journal =
        lux::simulation::ecs::detail::EcsChangeAccess::log(small_world);
    assert(
    ...
    assert(pinned.status() == lux::simulation::ecs::EChangeReadStatus::CURRENT);
    ...
    assert(recovered.status() == lux::simulation::ecs::EChangeReadStatus::CURRENT);
    assert(recovered.size() == 1U);

    lux::simulation::ecs::EcsState failure_world{
        lux::simulation::ecs::EcsStateConfig{{4096U, 16U * 4096U}}
    };
    auto& failure_journal =
        lux::simulation::ecs::detail::EcsChangeAccess::log(failure_world);
    lux::simulation::ecs::ChangeCursor<FirstStream> first_stream_cursor;
    assert(
    ...
    lux::simulation::ecs::EntityChangeCursor failed_entity_cursor;
    assert(
...

    {
        ...
        failure_journal.failNextStreamDescriptorForTest();
        lux::simulation::ecs::detail::EcsChangePublisher publisher(failure_world);
    ...
    }
    {
        ...
        assert(!publisher.appendEntity(
            failure_entity,
            lux::simulation::ecs::EEntityChangeKind::ADDED
        ));
        assert(failure_journal.epoch() == epoch + 1U);
    }

    lux::simulation::ecs::EcsState task_world{
        lux::simulation::ecs::EcsStateConfig{{4096U, 16U * 4096U}}
    };
    ...
    lux::simulation::ecs::EcsCommandBatch command_batch;
    ...
    auto execution = std::move(*execution_result);
    {
        ...
        assert(
            ...
            lux::simulation::ecs::ECommandResult::ACCEPTED
        );
    }
    lux::simulation::ecs::applyEcsCommands(task_world, command_batch);
    assert(task_world.get<Position>(task_entity).value == 41);
    execution = {};

    lux::simulation::ecs::EcsChangeBatch change_batch;
    ...
    assert(batch_stream);
    assert(batch_stream(task_entity, lux::simulation::ecs::EComponentChangeKind::MODIFIED));
    ...
    lux::simulation::ecs::ChangeCursor<Position> task_cursor;
    auto& task_log = lux::simulation::ecs::detail::EcsChangeAccess::log(task_world);
    assert(
    ...
    lux::simulation::ecs::EcsChangeBatch four_lane_batch;
    ...
    for (const std::uint64_t storage : four_storages)
    {
        ...
        assert(stream);
        assert(stream(task_entity, lux::simulation::ecs::EComponentChangeKind::MODIFIED));
        assert(stream(task_entity, lux::simulation::ecs::EComponentChangeKind::MODIFIED));
    }
    ...
    for (const std::size_t lane_count : {16U, 32U})
    {
        ...
        lux::simulation::ecs::EcsChangeBatch batch;
