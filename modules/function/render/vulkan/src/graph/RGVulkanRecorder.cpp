...
namespace lux::render
{
    ...
    void RGVulkanRecorder::applyLocalReadBoundaryState(
        VkCommandBuffer cmd,
    ...
    )
    {
        ...
        if (emit_barrier)
        {
            ...
            mb.srcAccessMask = VK_ACCESS_2_COLOR_ATTACHMENT_WRITE_BIT | VK_ACCESS_2_DEPTH_STENCIL_ATTACHMENT_WRITE_BIT;
    ...
    namespace
    {
        ...
        bool bindPipelineVariantBridge(void* raw_user, const RGCompiledPass& cpass, uint32_t variant_index)
        {
        ...
        )
        {
            ...
            for (size_t i = 0; i < count; ++i)
            {
                ...
                VkImage image = resolveImageHandle(record_context, physical_resources, ri, frame_index);
        ...
        selectPreBarrierGroups(const RGCompiledGraph& compiled_graph, const RGFrameContext& frame_ctx)
        {
            ...
            {
                sel.groups = need_cross_view_fixup ? &barrier_program->subsequent_view_barriers
                                                   : &barrier_program->first_view_barriers;
                sel.group_index_by_pass = need_cross_view_fixup ? &barrier_program->subsequent_view_group_by_pass
        ...
        )
        {
            ...
            const BarrierProgram::BarrierGroup* pre_group =
                resolveBarrierGroupForPass(pre_barrier_groups, pre_barrier_group_index_by_pass, pass_index);

            const auto& src_img = pre_group ? pre_group->image_barriers : sync.prebuilt_image_barriers;
            const auto& src_img_patch = pre_group ? pre_group->image_patch_resource_idx : sync.image_patch_resource_idx;
            patchImageBarriers(
                img_scratch,
                src_img,
                src_img_patch,
            ...
            const auto& src_buf = pre_group ? pre_group->buffer_barriers : sync.prebuilt_buffer_barriers;
            const auto& src_buf_patch =
                pre_group ? pre_group->buffer_patch_resource_idx : sync.buffer_patch_resource_idx;
            patchBufferBarriers(buf_scratch, src_buf, src_buf_patch, physical_resources, frame_index);
        ...
        {
            if (!timestampRangeValid(context, graph, frame_index) || pass_index >= graph.compiled_passes.size())
            {
                return;
            }

            const uint32_t base = frame_index * context.timestamp_pass_capacity * 2u;
        ...
        {
            const auto& descriptions = graph.original_graph.transient_descriptor_sets;
            for (std::size_t set_index = 0;
                 set_index < descriptions.size() && set_index < context.transient_descriptor_sets.size();
                 ++set_index)
            {
                auto& per_frame = context.transient_descriptor_sets[set_index];
                ...
                for (const auto& source : descriptions[set_index].writes)
                {
                    ...
                    const auto& resource = graph.original_graph.resources[resource_index];
                    ...
                    write.descriptorType = convertDescriptorType(source.descriptor_type);
                    ...
                    case VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT: {
                        ...
                        {
                            view = context.per_frame_views[resource_index][frame_index];
                        }
                    ...
                    case VK_DESCRIPTOR_TYPE_STORAGE_BUFFER_DYNAMIC: {
                        ...
                        {
                            buffer = context.per_frame_buffers[resource_index][frame_index];
                        }
                        if (buffer == VK_NULL_HANDLE)
                        {
                            continue;
    ...
    Expected<RGRecordContext> RGVulkanRecorder::allocateRecordContext(
    ...
    )
    {
        ...
        // has retired, so there is no GPU wait in this path.
        {
            ...
            vkGetPhysicalDeviceQueueFamilyProperties(static_cast<VkPhysicalDevice>(physical), &family_count, nullptr);
            ...
            const auto pass_count = static_cast<uint32_t>(compiled_graph.compiled_passes.size());
            const bool supported = family < families.size() && families[family].timestampValidBits > 0 &&
                                   pass_count > 0 && frames_in_flight > 0 &&
                                   pass_count <= UINT32_MAX / (2u * frames_in_flight);

            record_context.latest_gpu_timing = std::make_shared<const RGGpuTimingSnapshot>();
            if (supported)
            {
                        ...
                        &record_context.timestamp_pool) == VK_SUCCESS)
                {
                    ...
                    record_context.timestamp_frame_ids.assign(frames_in_flight, 0u);
                    record_context.timestamp_result_scratch.resize(static_cast<std::size_t>(pass_count) * 4u);
                }
            }
        }
                    ...
                    for (const auto& w : tds_descs[di].writes)
                    {
                        ...
                        case VK_DESCRIPTOR_TYPE_STORAGE_BUFFER_DYNAMIC: {
                            ...
                            bi.buffer = reinterpret_cast<VkBuffer>(
                                physical_resources.at(buf_tgt.first).getHandle(buf_tgt.second)
                            );
    ...
    )
    {
        ...
        out_depth.storeOp = (pass_info != nullptr) ? pass_info->pass_depth_store_op : VK_ATTACHMENT_STORE_OP_STORE;
    ...
    RGVulkanRecorder::refreshDynamicImportedResources(RGResourceState& state, const RGCompiledGraph& compiled_graph)
    {
        ...
        for (uint32_t ri : compiled_graph.dynamic_external_resources)
        {
            ...
            const uint32_t count = phys.buffer_getter(new_buffers.data(), static_cast<uint32_t>(new_buffers.size()));
            if (count == 0)
            {
                continue;
            }
...
#include "RGVulkanRecorder.Record.inl"
    ...
    )
    {
        ...
        auto tryCallKernelFn = [&]() -> bool {
            ...
            {
                const uint32_t gidx = compiled_graph.render_pass_layout.pass_to_group[replay_state.current_pass];
                if (gidx < record_context.group_extents.size())
                {
                    ext = record_context.group_extents[gidx];
                    kfn_vp = {0.f, 0.f, static_cast<float>(ext.width), static_cast<float>(ext.height), 0.f, 1.f};
                    kfn_sc = {{0, 0}, ext};
                }
            }
            ...
            PipelineVariantBindUser pvbu{.cmd = cmd, .bind_point = replay_state.current_bp, .binding_state = &bs};
        ...
        for (uint32_t ci = begin; ci < end; ++ci)
        {
            ...
            case ECmd::SetPassContext: {
                ...
                const auto& cpass = compiled_graph.compiled_passes[replay_state.current_pass];
                replay_state.current_bp =
                    (cpass.pass->type == ERGPassType::COMPUTE || cpass.pass->type == ERGPassType::ASYNC_COMPUTE)
            ...
            case ECmd::BeginRendering: {
                ...
                for (uint32_t ci = 0; ci < hdr.color_count; ++ci)
                {
                    ...
                    ca.loadOp = (hdr.lr_flags & 0x1)
                                    ? (((hdr.lr_color_clear_mask >> ci) & 0x1) ? VK_ATTACHMENT_LOAD_OP_CLEAR
                                                                               : VK_ATTACHMENT_LOAD_OP_LOAD)
                                    : hdr.color_op;
                    ca.storeOp = ((hdr.store_dontcare_mask >> ci) & 0x1) ? VK_ATTACHMENT_STORE_OP_DONT_CARE
                ...
                depth_attach.imageLayout = (hdr.lr_flags & 0x2) ? VK_IMAGE_LAYOUT_RENDERING_LOCAL_READ
                                                                : VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL;
                depth_attach.loadOp = hdr.depth_op;
                depth_attach.storeOp =
                    (hdr.lr_flags & 0x4) ? VK_ATTACHMENT_STORE_OP_DONT_CARE : VK_ATTACHMENT_STORE_OP_STORE;
                ...
                ri.pDepthAttachment =
                    (hdr.depth_format != VK_FORMAT_UNDEFINED && depth_attach.imageView != VK_NULL_HANDLE)
                        ? &depth_attach
                        : nullptr;
            ...
            case ECmd::DispatchIndirect: {
                ...
                {
                    buffer = reinterpret_cast<VkBuffer>(physical->getHandle(frame_ctx.frame_index));
                }
                if (buffer != VK_NULL_HANDLE)
                {
            ...
            case ECmd::ClearCounters: {
                ...
                const uint32_t clamped_count = std::min(d.buffer_count, ClearCountersKernelConfig::kMaxBuffers);
                ...
                vkCmdPushConstants(cmd, replay_state.current_layout, VK_SHADER_STAGE_COMPUTE_BIT, 0, sizeof(pc), &pc);
    ...
    )
    {
        ...
        if (pass_pipeline != VK_NULL_HANDLE && cpass.render.bind_pipeline)
        {
            ...
            if (fn_object_name_ && debug_named_pipelines_.find(pass_pipeline) == debug_named_pipelines_.end())
            {
        ...
        if (bind_point == VK_PIPELINE_BIND_POINT_GRAPHICS && cpass.render.pipeline_template_handle.valid())
        {
            const auto& tmpl = pipeline_manager_.getTemplate(cpass.render.pipeline_template_handle);
            for (const auto& range : tmpl.push_constant_ranges)
            {
        ...
        if (!cpass.render.ds_bind_recipe.empty() && pass_layout != VK_NULL_HANDLE)
        {
            ...
            for (const DSBindRecipe& recipe : cpass.render.ds_bind_recipe)
            {
                ...
                if (recipe.resolve != nullptr)
                {
                    ...
                    const uint32_t view_id = frame_ctx.view ? frame_ctx.view->handle.index : 0u;
                    ds = recipe.resolve(recipe, frame_ctx.frame_index, frame_ctx.scene_ds, transient_sets, view_id);
                }
        ...
        // ------------------------------------------------------------------
        const VkViewport
            auto_viewport{0.0f, 0.0f, static_cast<float>(extent.width), static_cast<float>(extent.height), 0.0f, 1.0f};
        const VkRect2D auto_scissor{{0, 0}, extent};

        if (bind_point == VK_PIPELINE_BIND_POINT_GRAPHICS && cpass.pass && !cpass.pass->manual_viewport)
        {
