...
//  and arena are untouched; authored SpriteComponent/TilemapComponent fields
...
namespace lux::pack
{
    ...
    /// screen sprites draw on top (the classic top-down depth illusion):
    ...
    struct LUX_COMPONENT() YSort2DComponent
    {
        ...
        [[nodiscard]] float effectivePriority(float world_y) const noexcept
        {
            float y = world_y;
            if (quantize_step > 0.f)
                y = std::floor(y / quantize_step) * quantize_step;
            return band - y * scale;
        }
    };
