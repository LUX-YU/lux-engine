...
class LUX_FUNCTION_PUBLIC TreeRowScope final
{
  public:
    ...
    [[nodiscard]] bool contextRequested() const noexcept
    {
...
class LUX_FUNCTION_PUBLIC Frame final
{
  public:
    ...
    Frame(Context &context, const Theme &theme, FrameInfo info);
    [[nodiscard]] bool uses(const Context &context) const noexcept;
    ...
    [[nodiscard]] EditResult editScalar(std::string_view label, std::int32_t &value,
                                        const ScalarEditSpec<std::int32_t> &spec = {});
    [[nodiscard]] EditResult editScalar(std::string_view label, std::uint32_t &value,
                                        const ScalarEditSpec<std::uint32_t> &spec = {});
    [[nodiscard]] EditResult editScalar(std::string_view label, std::int64_t &value,
                                        const ScalarEditSpec<std::int64_t> &spec = {});
    [[nodiscard]] EditResult editScalar(std::string_view label, std::uint64_t &value,
                                        const ScalarEditSpec<std::uint64_t> &spec = {});
    [[nodiscard]] EditResult editScalar(std::string_view label, float &value, const ScalarEditSpec<float> &spec = {});
    [[nodiscard]] EditResult editScalar(std::string_view label, double &value, const ScalarEditSpec<double> &spec = {});
    [[nodiscard]] EditResult editChoice(std::string_view label, std::int64_t &value,
