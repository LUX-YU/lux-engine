...
namespace lux::render
{
    ...
    class Canvas2DInstanceArena
    {
        ...
        public:
            ...
            bool writeKey(std::uint32_t slot, std::uint32_t gen, std::uint32_t key_hi, bool visible, std::uint8_t group)
            {
                Slot* s = resolve(slot, gen);
                const bool is_missing_slot = s == nullptr;
                const bool is_same_key = !is_missing_slot && s->key_hi == key_hi;
                const bool is_same_visibility = !is_missing_slot && s->visible == visible;
                const bool is_same_group = !is_missing_slot && s->group == group;
                const bool is_unchanged = is_same_key && is_same_visibility && is_same_group;
                if (is_missing_slot || is_unchanged)
                    return false;
                s->key_hi = key_hi;
                s->visible = visible;
                s->group = group;
            ...
            void collectLiveKeys(std::vector<std::uint64_t>& out, std::uint64_t kind_field, std::uint8_t group)
                const override
            {
            ...
            void submitStreams(TransferScheduler& scheduler) override
            {
                ...
                if (!stg)
                    return; // pages stay dirty → retried next tick
    ...
    public:
        ...
        void writeTransform(Image2DHandle h, const float m[6], const std::int32_t page_delta[2])
        {
            ...
            if (auto* r = images_.resolveRecord(h.index, h.gen))
            {
                ...
                std::memcpy(r->page_delta, page_delta, sizeof(r->page_delta));
        ...
        [[nodiscard]] bool canRebaseSceneOrigin(const std::int64_t origin_delta[3]) const noexcept
        {
            return images_.canRebaseSceneOrigin(origin_delta) && fields_.canRebaseSceneOrigin(origin_delta) &&
        ...
        void rebuildOrder()
        {
            ...
            for (std::uint8_t g = 0; g < group_count_; ++g)
            {
                ...
                for (std::size_t k = 0; k < sts.size(); ++k)
                    sts[k]->collectLiveKeys(key_scratch_, static_cast<std::uint64_t>(k) << kCanvas2DSlotBits, g);
