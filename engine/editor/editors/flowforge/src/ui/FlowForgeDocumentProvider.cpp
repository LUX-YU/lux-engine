...
namespace lux::editor::gui
{
        ...
        class FlowForgePane final : public DocumentPane<FlowForgePane, flowforge::FlowForgeEditor>
        {
            ...
            void drawFunctions()
            {
                ...
                if (function_index_state_ != current)
                {
                    ...
                    std::erase_if(
                        function_drafts_, [&](const auto &entry)
                ...
                for (const auto id : function_entries_)
                {
                                        ...
                                        document_.nodeName(id).data()))
                    {
                        ...
                        if (found == function_drafts_.end() || (!found->second.dirty && found->second.base != current))
                        {
                            ...
                            {
                                found =
                                    function_drafts_.insert_or_assign(id, FunctionDraft{std::move(*captured), current})
                                        .first;
                            }
                        }
                        if (found != function_drafts_.end())
                        {
                            ...
                            {
                                if (accept(
                                        document_.setFunctionSignature(draft.base, id, draft.source.name, signature)))
                                {
                                    draft.dirty = false;
                                }
                            }
            ...
            void drawExports()
            {
                ...
                for (std::size_t index{}; index < draft.values.size(); ++index)
                {
                    ...
                    draft.dirty |= ImGui::InputScalar("Symbol", ImGuiDataType_U64, &value.symbol, nullptr, nullptr,
          ...
          public:
            FlowForgePane(flowforge::FlowForgeEditor &document, std::string id)
                : DocumentPane(document, std::move(id), "FlowForge"), canvas_(createCanvas()),
                  name_(document.summary().title),
          ...
          private:
            ...
            template <class Result> bool accept(const Result &result)
            {
                ...
                {
                    error_ = result.error().domain + ":" + std::to_string(result.error().reason) + " " +
                ...
                if (compile_.serial)
                {
                    ...
                    if (status)
                    {
                        ...
                        else
                        {
                            ...
                            if (std::get<flowforge::FlowCompileFailed>(*status).retryable &&
                                ImGui::Button("Retry link"))
                            {
            ...
            void drawCanvas()
            {
                ...
                for (const auto &record : document_.nodes())
                {
                    ...
                    for (const auto &pin : pin_groups_[record.id])
                    {
                        ...
                        ImGui::Text("%s %.*s %.*s", input ? "<" : ">", int(label.size()), label.data(),
    ...
    GuiDocumentProvider flowForgeDocumentProvider(lux::flowforge::FlowSourceEnvironment environment)
    {
        return {std::string(flowforge::kFlowForgeDocumentType),
                ...
                [environment = std::move(environment)](Editor &editor, process::ExecutionRuntime &runtime,
                                                       rendering::EditorRenderer &) -> EditorResult<void>
                {
