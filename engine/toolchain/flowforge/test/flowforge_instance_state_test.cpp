...
static void test_layout_math()
{
    ...
    FlowGraph g;
    const uint64_t v_counter = g.addVariable("counter", i32, lux::meta::RuntimeObject(int32_t{5}));
    const uint64_t v_ratio = g.addVariable("ratio", f64, lux::meta::RuntimeObject(double{1.5}));
    const uint64_t v_flag = g.addVariable("flag", b1, lux::meta::RuntimeObject(bool{true}));
    ...
    check(computeStateLayout(g2).hash != layout.hash, "different variable sets hash differently");
}
...
static FlowGraph makeBumpGraph(const lux::meta::RefFunction& sink_fn, uint64_t& counter_out)
{
...
static void test_instance_state_semantics()
{
    ...
    const std::vector<JitNativeSymbol> syms{{"lux_test_sink_int", reinterpret_cast<void*>(&lux_test_sink_int)}};
    ...
    if (!s1)
        return;
    check(s1->instanceState().size() == layout.size, "instance state block matches the layout size");
    ...
    check(g_sunk == std::vector<int>({6, 7, 8}), "three bumps sink 6, 7, 8 (state persists across invokes)");
    ...
    check(g_sunk == std::vector<int>({6}), "instance 2 starts from the defaults again (6)");
