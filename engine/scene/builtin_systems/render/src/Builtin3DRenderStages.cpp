...
[[nodiscard]] bool encodePosition(const Eigen::Vector3d &absolute_position, double page_size,
                                  const std::array<std::int64_t, 3> &origin,
                                  render::RenderLargePosition3D &output) noexcept
{
...
[[nodiscard]] bool encodeTransform(const simulation::ecs::WorldTransform3D &world, double page_size,
                                   const std::array<std::int64_t, 3> &origin,
                                   render::RenderSpatialTransform3D &output) noexcept
{
...
[[nodiscard]] bool sameTransform(const render::RenderSpatialTransform3D &left,
                                 const render::RenderSpatialTransform3D &right) noexcept
{
...
template <class Departure> void coalesceDepartures(std::vector<Departure> &departures) noexcept
{
...
using MeshChanges = simulation::ecs::ExtractionChangeSet<
    simulation::ecs::Mesh3D, ComponentList<simulation::ecs::WorldTransform3D, ResolvedMeshResources>, ComponentList<>>;
using MeshMembershipLeaves = simulation::ecs::ComponentSetLeaveObserver<
    simulation::ecs::Mesh3D, ComponentList<simulation::ecs::WorldTransform3D, ResolvedMeshResources>, ComponentList<>>;
using MeshStateLeaves = simulation::ecs::ComponentSetLeaveObserver<MeshRenderState, ComponentList<>, ComponentList<>>;
...
class Mesh3DRenderStage final : public RenderSyncStage
{
  ...
  private:
    ...
    void recordMembershipDeparture(Entity entity) noexcept
    {
        ...
        if (state != nullptr && state->owner == this)
        {
            recordDeparture(MeshDeparture{state->entity, entity, state->published, state->lifetime});
        }
    }
    ...
    {
        MeshInputs inputs{};
        return config_.registry.valid(departure.source) && toRenderEntity(departure.source) == departure.entity &&
    ...
    [[nodiscard]] bool prepareEntity(render::RenderProgramBuilder<> &builder, Entity entity)
    {
        ...
        next.lifetime = inputs.resolved->lifetime;
        ...
        next.published = true;
        if (!encodeTransform(*inputs.world, config_.coordinate_page_size, config_.scene_origin_page, next.transform))
        {
            return false;
        }
        ...
        if (state_slot == nullptr)
        {
            ...
            state_slot = &config_.registry.emplace<MeshRenderState>(
                entity, MeshRenderState{.owner = this, .entity = next.entity});
        }
        ...
        const bool requires_upsert = force_full_sync_ || published == nullptr || published->entity != next.entity ||
                                     published->mesh_source != next.mesh_source ||
                                     published->material_source != next.material_source ||
                                     published->mesh != next.mesh || published->material != next.material ||
                                     published->lifetime != next.lifetime;
        if (requires_upsert)
        {
            if (next.lifetime)
            {
                static_cast<void>(builder.emplaceAttachment<std::shared_ptr<const void>>(
                    render::attachment_types::OwnedObject, next.lifetime));
            }
            if (published && published->lifetime)
            {
                static_cast<void>(builder.emplaceAttachment<std::shared_ptr<const void>>(
                    render::attachment_types::OwnedObject, published->lifetime));
            }
            ...
            builder.push(render::opcode_of_v<render::UpsertMeshInstanceOp>,
                         config_.operations.id<render::UpsertMeshInstanceOp>(), payload);
        }
        else
        {
            if (published->flags != next.flags)
            {
                builder.push(render::opcode_of_v<render::UpdateInstanceFlagsOp>,
        ...
        const bool state_changed =
            published == nullptr || published->entity != next.entity || published->mesh_source != next.mesh_source ||
            published->material_source != next.material_source || published->mesh != next.mesh ||
            published->material != next.material || published->lifetime != next.lifetime ||
    ...
    [[nodiscard]] ERenderSyncPrepareResult prepareImpl(render::RenderProgramBuilder<> &builder)
    {
        ...
        for (const auto &departure : departures_)
        {
            ...
            {
                if (departure.lifetime)
                {
                    static_cast<void>(builder.emplaceAttachment<std::shared_ptr<const void>>(
                        render::attachment_types::OwnedObject, departure.lifetime));
                }
                builder.push(render::opcode_of_v<render::RemoveMeshInstanceOp>,
        ...
        if (force_full_sync_)
        {
            auto current = componentView<simulation::ecs::Mesh3D>(
                config_.registry, ComponentList<simulation::ecs::WorldTransform3D, ResolvedMeshResources>{},
                ComponentList<>{});
...
using LightChanges =
    simulation::ecs::ExtractionChangeSet<simulation::ecs::Light3D, ComponentList<simulation::ecs::WorldTransform3D>,
                                         ComponentList<>>;
using LightMembershipLeaves =
    simulation::ecs::ComponentSetLeaveObserver<simulation::ecs::Light3D,
                                               ComponentList<simulation::ecs::WorldTransform3D>, ComponentList<>>;
using LightStateLeaves = simulation::ecs::ComponentSetLeaveObserver<LightRenderState, ComponentList<>, ComponentList<>>;

[[nodiscard]] bool sameLight(const render::UpsertLightPayload &left, const render::UpsertLightPayload &right) noexcept
{
...
class Light3DRenderStage final : public RenderSyncStage
{
  ...
  private:
    ...
    [[nodiscard]] bool membershipRestored(const LightDeparture &departure) const noexcept
    {
        ...
        return config_.registry.all_of<simulation::ecs::Light3D, simulation::ecs::WorldTransform3D>(departure.source);
    }
    ...
    [[nodiscard]] bool makePayload(Entity entity, render::UpsertLightPayload &payload) const noexcept
    {
        ...
        if (!encodePosition(world.value.translation(), config_.coordinate_page_size, config_.scene_origin_page,
    ...
    [[nodiscard]] bool prepareEntity(Entity entity)
    {
        ...
        if (state_slot == nullptr)
        {
            ...
            state_slot = &config_.registry.emplace<LightRenderState>(
                entity, LightRenderState{.owner = this, .entity = payload.entity});
        }
        if (state_slot->owner != this)
        {
            return false;
        ...
        {
            state_updates_.push_back(LightStateUpdate{entity, LightRenderState{this, payload.entity, payload, true}});
        }
        return true;
    }
    ...
    [[nodiscard]] ERenderSyncPrepareResult prepareImpl(render::RenderProgramBuilder<> &builder)
    {
        ...
        for (const auto &departure : departures_)
        {
            ...
            if (departure.was_published)
            {
                builder.push(render::opcode_of_v<render::RemoveLightOp>, config_.operations.id<render::RemoveLightOp>(),
        ...
        if (force_full_sync_)
        {
            auto current = componentView<simulation::ecs::Light3D>(
                config_.registry, ComponentList<simulation::ecs::WorldTransform3D>{}, ComponentList<>{});
            for (const auto entity : current)
            {
        ...
        if (!payloads_.empty())
        {
            auto output = builder.appendBulk<render::UpsertLightPayload>(config_.operations.id<render::LightBatchOp>(),
                                                                         payloads_.size());
...
{
    return !config.scene.isNull() && config.operations.valid() && std::isfinite(config.coordinate_page_size) &&
           config.coordinate_page_size > 0.0;
}
...
lux::cxx::expected<std::unique_ptr<RenderSyncStage>, RenderSyncStageCreateFailure> createMesh3DRenderStage(
    const RenderSyncStageCreateInfo &info) noexcept
{
    const auto live_name = info.catalog.nameOfType(info.feature);
    MeshStageConfig config{info.registry, info.scene, info.catalog.ops<render::MeshStackOperationIds>(live_name),
                           info.coordinate_page_size, info.scene_origin_page};
    if (!info.feature_handle.isValid() || !valid(config))
    {
        return lux::cxx::unexpected(RenderSyncStageCreateFailure{ERenderSyncStageCreateError::INVALID_CONFIGURATION});
    }
    try
    {
    ...
    {
        return lux::cxx::unexpected(RenderSyncStageCreateFailure{ERenderSyncStageCreateError::ALLOCATION_FAILURE});
    }
}

lux::cxx::expected<std::unique_ptr<RenderSyncStage>, RenderSyncStageCreateFailure> createLight3DRenderStage(
    const RenderSyncStageCreateInfo &info) noexcept
{
    const auto live_name = info.catalog.nameOfType(info.feature);
    LightStageConfig config{info.registry, info.scene, info.catalog.ops<render::LightOperationIds>(live_name),
                            info.coordinate_page_size, info.scene_origin_page};
    if (!info.feature_handle.isValid() || !valid(config))
    {
        return lux::cxx::unexpected(RenderSyncStageCreateFailure{ERenderSyncStageCreateError::INVALID_CONFIGURATION});
...
constexpr std::uint8_t AllObservationEvents = static_cast<std::uint8_t>(EComponentObservation::CONSTRUCT) |
...
const std::array Bindings{cameraRenderFeatureBinding(),
                          RenderFeatureSceneBinding{system::systemTypeId(RenderSystem::Description.canonical_name),
                                                    render::featureId("lux.render.mesh_stack.v1"), MeshObservations,
                                                    &createMesh3DRenderStage},
                          RenderFeatureSceneBinding{system::systemTypeId(RenderSystem::Description.canonical_name),
                                                    render::featureId("lux.render.light.v1"), LightObservations,
