...
class LUX_FUNCTION_PUBLIC RenderControlSession final
{
  public:
    ...
    // Admit ownership before issuing any asynchronous work. Empty capacity
    // delays progress; it cannot make a caller lose its release obligation.
    [[nodiscard]] RenderSceneLease prepareScene(const CreateSceneConfig &,
                                                std::vector<SceneFeatureAttachment> features);
    [[nodiscard]] std::size_t maintainScenes(std::size_t command_budget);
    [[nodiscard]] std::size_t maintainResources(std::size_t command_budget);
    [[nodiscard]] std::size_t activeResourceUses() const noexcept;
    [[nodiscard]] Expected<RenderResourceUse> retainResource(std::shared_ptr<void> payload, RenderResourceReleaseStep,
                                                             std::shared_ptr<const void> code = {});
    [[nodiscard]] std::size_t pendingResourceReleases() const noexcept;
    [[nodiscard]] bool canSubmit(std::size_t commands = 1) const noexcept;
    [[nodiscard]] std::size_t sceneRecordCount() const noexcept;
    [[nodiscard]] bool owns(const RenderSceneLease &) const noexcept;
    // Only after server destruction AND retirement of accepted CPU Programs.
    void retireScenesAfterBackendStopped() noexcept;
                                                              ...
                                                              const Config &config)
    {
        ...
        return recordReply<FeatureAddedReply>([&](Builder &builder, auto callback) {
            const auto attachment = builder.template emplaceAttachment<Config>(attachment_types::OwnedObject, config);
    ...
    [[nodiscard]] Expected<bool> flushDeferredReleases(
  ...
  private:
    ...
    [[nodiscard]] RenderRequest<Reply> recordReply(Record &&record, bool blocking = true)
    {
        if (sync_->isStopping())
        {
    ...
    [[nodiscard]] ERenderLeaseCloseStatus closeScene(detail::SceneReleaseRecord &record) noexcept;
    [[nodiscard]] ERenderLeaseCloseStatus closeView(detail::ViewReleaseRecord &record) noexcept;
    [[nodiscard]] RenderTargetCloseResult closeTarget(detail::TargetReleaseRecord &record) noexcept;
    ...
    // Nodes are allocated at adoption, before handing out ownership. Lease
    // destruction only changes an existing node; it never enters a queue.
    std::list<std::shared_ptr<detail::SceneReleaseRecord>> scenes_;
    std::list<detail::ViewReleaseRecord> views_;
    std::list<detail::TargetReleaseRecord> targets_;
    std::list<std::shared_ptr<detail::ResourceReleaseRecord>> resources_;
    bool backend_retired_{};
};
} // namespace lux::render
