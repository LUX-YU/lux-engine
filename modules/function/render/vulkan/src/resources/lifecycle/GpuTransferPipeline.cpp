...
namespace lux::render
{
    ...
    GpuTransferPipeline::GpuTransferPipeline(const Config &cfg)
        : jobs_(cfg.queue_capacity),
          results_(std::max(cfg.result_capacity, cfg.queue_capacity + 1u)),
          notify_work_(cfg.notify_work),
          notify_work_state_(cfg.notify_work_state),
          lifecycle_(cfg.lifecycle),
          lifecycle_state_(cfg.lifecycle_state)
    {
    }

    Expected<std::unique_ptr<GpuTransferPipeline>>
    GpuTransferPipeline::create(const Config& config)
    {
        if (config.device_ctx == nullptr)
            return renderFailure<err::device::VulkanObjectCreationFailed>();

        auto pipeline = std::unique_ptr<GpuTransferPipeline>(
            new GpuTransferPipeline(config)
        );
    ...
    Expected<void> GpuTransferPipeline::initialize(const Config& cfg)
    {
        ...
        mode_ = can_record_transfer_ && transfer_queue_ != graphics_queue_
        ...
        const auto semaphore_result =
            vkCreateSemaphore(device_, &sem_ci, nullptr, &timeline_sem_);
        if (semaphore_result != VK_SUCCESS)
            return renderFailure<err::device::VulkanCallFailed>(
                encodeVkResult(semaphore_result)
            );
        ...
        for (uint32_t i = 0; i < cmd_pool_count_; ++i)
        {
            ...
            const auto pool_result = vkCreateCommandPool(
                device_, &pool_ci, nullptr, &cmd_pools_[i].pool
            );
            if (pool_result != VK_SUCCESS)
                return renderFailure<err::device::VulkanCallFailed>(
                    encodeVkResult(pool_result)
                );
        }
    ...
    GpuTransferPipeline::~GpuTransferPipeline()
    {
        ...
        for (; shutdown_completion_cursor_ < shutdown_completions_.size();
             ++shutdown_completion_cursor_)
            freeUnsubmittedCompletion(
                shutdown_completions_[shutdown_completion_cursor_]);
            ...
            for (uint32_t offset = 0; offset < cmd_pool_count_; ++offset)
            {
                ...
                if (state == EBatchSlotState::SUBMITTED)
                {
                    const uint64_t wait_value =
                        slot.last_timeline_value.load(std::memory_order_acquire);
    ...
    void GpuTransferPipeline::releaseAfterGraphicsAcquire(
        uint32_t batch_slot) noexcept
    {
        ...
        if (slot.state.load(std::memory_order_acquire) !=
            EBatchSlotState::RECORDED)
        {
            renderFatal(
                "GpuTransferPipeline retained batch slot changed state before acquire"
            );
        }
    ...
    {
        const uint64_t wait_value =
            graphics_finalize_timeline_.exchange(0, std::memory_order_acq_rel);
        if (wait_value == 0)
            return false;
    ...
    uint32_t GpuTransferPipeline::drainResults(
        TransferCompletion *out,
        uint32_t max)
    {
        if (max == 0)
            return 0;
        ...
        uint32_t count = 0;
        while (count < max &&
               shutdown_completion_cursor_ < shutdown_completions_.size())
        {
            out[count++] = std::move(
                shutdown_completions_[shutdown_completion_cursor_++]);
        }
        if (shutdown_completion_cursor_ == shutdown_completions_.size() &&
            !shutdown_completions_.empty())
        {
            shutdown_completions_.clear();
            shutdown_completion_cursor_ = 0;
        }
        ...
        GpuTransferResult result{};
        while (count < max &&
               results_.tryPop(result) == lux::cxx::EQueuePopResult::VALUE)
        {
            result_space_epoch_.fetch_add(1, std::memory_order_release);
            result_space_epoch_.notify_one();
            ...
            VkResult sub_res{VK_ERROR_UNKNOWN};
            {
                ...
                sub_res = vkQueueSubmit2(
                    graphics_queue_, 1, &si, VK_NULL_HANDLE);
            }
    ...
    GpuTransferPipeline::submitGraphicsFinalize(VkCommandBuffer command_buffer)
    {
        ...
        {
            const std::scoped_lock queue_lock(*graphics_queue_mutex_);
            submit_result = vkQueueSubmit2(
                graphics_queue_, 1, &submit, VK_NULL_HANDLE);
        }
        if (submit_result != VK_SUCCESS)
            return std::nullopt;

        uint64_t observed =
            graphics_finalize_timeline_.load(std::memory_order_relaxed);
        while (observed < timeline_value &&
               !graphics_finalize_timeline_.compare_exchange_weak(
                   observed,
                   timeline_value,
    ...
    void GpuTransferPipeline::freeUnsubmittedCompletion(TransferCompletion &c)
    {
        ...
        if (c.kind == Kind::Texture2D ||
            c.kind == Kind::TextureCube ||
    ...
    ) noexcept
    {
        if (request_id != UINT32_MAX && lifecycle_ != nullptr)
            lifecycle_(
                lifecycle_state_, request_id, kind,
                resource_handle, resource_gen, state
            );
    }
               ...
               lux::cxx::EQueuePushResult::ACCEPTED)
        {
            if (stop_requested_.load(std::memory_order_acquire))
            {
                ...
                else
                {
                    ...
                    {
                        releaseAfterGraphicsAcquire(
                            completion.retained_batch_slot
                        );
                    }
                }
                return false;
            ...
            const auto epoch =
                result_space_epoch_.load(std::memory_order_acquire);
    ...
    void GpuTransferPipeline::publishRecorded(RecordedBatch batch)
    {
        ...
        VkResult submit_result{VK_ERROR_UNKNOWN};
        {
            ...
            submit_result = vkQueueSubmit2(
                transfer_queue_, 1, &submit, VK_NULL_HANDLE);
        }
        if (submit_result != VK_SUCCESS)
        {
            const auto kind = batch.completion.kind;
    ...
    bool GpuTransferPipeline::submitMeshTransfer(MeshTransferTask task)
    {
        ...
        notifyLifecycle(
            request_id, TransferCompletion::Kind::MeshBuffer,
            resource_handle, resource_gen,
        ...
        if (!accepting_.load(std::memory_order_acquire) ||
            jobs_.tryPush(UploadJob{std::move(task)}) !=
                lux::cxx::EQueuePushResult::ACCEPTED)
        {
            notifyLifecycle(
                request_id, TransferCompletion::Kind::MeshBuffer,
    ...
    bool GpuTransferPipeline::submitTextureTransfer(TextureTransferTask task)
    {
        ...
        const auto kind = task.replacement
            ...
            : TransferCompletion::Kind::Texture2D;
        notifyLifecycle(
            request_id, kind,
            resource_handle, resource_gen,
            EUploadLifecycleState::Accepted
    ...
    bool GpuTransferPipeline::submitCubeTransfer(CubeTransferTask task)
    {
        ...
        const auto resource_gen = task.resource_gen;
        notifyLifecycle(
            request_id, TransferCompletion::Kind::TextureCube,
            resource_handle, resource_gen,
            EUploadLifecycleState::Accepted
        );
        notifyLifecycle(
        ...
        TransferStopToken st)
    {
        if (can_record_transfer_)
        {
            ...
            staging_copied_bytes_.fetch_add(
        ...
        TransferStopToken st)
    {
        ...
        for (uint32_t i = 0; i < provided_mips; ++i)
            if (task.mips[i].data == nullptr) { fail(); return; }
        ...
        TransferStopToken st)
    {
        ...
        const auto fail = [&]{
            pushFailure(TransferCompletion::Kind::TextureCube,
        ...
        for (uint32_t f = 0; f < kFaceCount; ++f)
            if (task.faces[f].data == nullptr) { fail(); return; }
        ...
        for (uint32_t f = 0; f < kFaceCount; ++f)
        {
            std::memcpy(static_cast<std::byte*>(stg.mapped) + offset,
                        task.faces[f].data, static_cast<std::size_t>(face_stride));
            offset += face_stride;
        }
        staging_copied_bytes_.fetch_add(
            static_cast<std::uint64_t>(total_bytes),
