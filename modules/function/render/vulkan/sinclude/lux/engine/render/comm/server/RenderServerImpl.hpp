...
struct GeneralRenderServer::Impl
{
    ...
    std::unique_ptr<OffscreenImagePool> makeTargetPool(const RenderTargetLayout &layout, VkExtent2D extent,
    ...
    {
        RenderSceneId scene{}; // Otherwise this is the existing surface release.
    ...
    void transitionUpload(std::uint32_t request_id, TransferCompletion::Kind kind, std::uint32_t resource_index,
                          std::uint32_t resource_gen, EUploadLifecycleState state) noexcept;
  ...
  private:
    ...
    FinalizeDisposition finalizeMeshCompletion(TransferCompletion &c, bool needs_qfot, uint32_t src_family,
                                               uint32_t dst_family);
    FinalizeDisposition finalizeTexture2DCompletion(TransferCompletion &c, bool needs_qfot, uint32_t src_family,
                                                    uint32_t dst_family);
    FinalizeDisposition finalizeTextureCubeCompletion(TransferCompletion &c, bool needs_qfot, uint32_t src_family,
                                                      uint32_t dst_family);
