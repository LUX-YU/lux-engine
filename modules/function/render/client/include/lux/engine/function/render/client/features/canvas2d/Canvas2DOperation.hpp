...
namespace lux::render
{
    ...
    static_assert(sizeof(Image2DInstanceData) == 56,
    ...
    static_assert(sizeof(PixelField2DInstanceData) == 64,
    ...
    struct LUX_COMM_CONFIG(prefix=Canvas2D, id=lux.render.canvas2d.v2, display=Canvas2D,
                           feature=Canvas2DFeature, multiplicity=single,
    ...
    static_assert(sizeof(Tile2DInstanceData) == 64,
    ...
    struct LUX_OP(lane=frame, kind=stream, name=RemoveImage2D, method=removeImageRaw)
    RemoveImage2DPayload
    {
        RenderSceneId  scene{};
        Image2DHandle handle{};
    };
    ...
    struct LUX_OP(lane=frame, kind=stream, name=UpdateImage2DKey, method=updateKeyRaw)
    UpdateImage2DKeyPayload
    {
        RenderSceneId  scene{};
        Image2DHandle handle{};
        float          priority{0.f};
    ...
    struct LUX_OP(lane=frame, kind=stream, name=AddPixelField2D, method=addPixelFieldRaw, reply=PixelFieldSlotReply)
    ...
    struct LUX_OP(lane=frame, kind=stream, name=UpdatePixelField2DTransform, method=updatePixelFieldTransformRaw)
    ...
    struct LUX_OP(lane=frame, kind=stream, name=RemoveTile2D, method=removeTilemapRaw)
    RemoveTile2DPayload
    {
        RenderSceneId        scene{};
        Tile2DInstanceHandle handle{};
...
namespace lux::render
{
    ...
    [[nodiscard]] LUX_FUNCTION_PUBLIC RenderRequest<Image2DSlotReply> addImage(
        Canvas2DProxy proxy, RenderSceneId scene, const Image2DInstanceData& data,
        float priority, bool visible = true, std::uint32_t group = 0);

    LUX_FUNCTION_PUBLIC void removeImage(
        Canvas2DProxy proxy, RenderSceneId scene, Image2DHandle handle);

    /// 逐条自带 scene 的异构批次(转发生成面)。
    LUX_FUNCTION_PUBLIC void updateTransforms(
        Canvas2DProxy proxy, std::span<const Image2DTransformEntry> entries);

    /// scene 盖章批量:让单场景批次不可能写漏 scene(与上面同名成对)。
    LUX_FUNCTION_PUBLIC void updateTransforms(
        Canvas2DProxy proxy, RenderSceneId scene, std::span<Image2DTransformEntry> entries);

    LUX_FUNCTION_PUBLIC void updateTransform(
        Canvas2DProxy proxy, RenderSceneId scene, Image2DHandle handle,
        const float m[6], const std::int32_t page_delta[2]);
    ...
    LUX_FUNCTION_PUBLIC void updateKey(
        Canvas2DProxy proxy, RenderSceneId scene, Image2DHandle handle,
        float priority, bool visible, std::uint32_t group = 0);

    LUX_FUNCTION_PUBLIC void setEnabled(
        Canvas2DProxy proxy, RenderSceneId scene, bool enabled);

    // ── PixelField ──
    [[nodiscard]] LUX_FUNCTION_PUBLIC RenderRequest<PixelFieldSlotReply> addPixelField(
        Canvas2DProxy proxy, RenderSceneId scene, const PixelField2DInstanceData& data,
        float priority, bool visible = true);

    LUX_FUNCTION_PUBLIC void removePixelField(
        Canvas2DProxy proxy, RenderSceneId scene, PixelFieldInstanceHandle handle);
    ...
    [[nodiscard]] LUX_FUNCTION_PUBLIC RenderRequest<Tile2DSlotReply> addTilemap(
        Canvas2DProxy proxy, RenderSceneId scene, const Tile2DInstanceData& data,
        float priority, bool visible = true);

    LUX_FUNCTION_PUBLIC void removeTilemap(
        Canvas2DProxy proxy, RenderSceneId scene, Tile2DInstanceHandle handle);
