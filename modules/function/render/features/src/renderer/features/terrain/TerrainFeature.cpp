...
Expected<void> TerrainFeature::initAndAttachTo(RenderScene &scene)
{
    ...
    resources_->setRetireScheduler(&contextView().retireScheduler(), static_cast<FrameRetireScheduler::OwnerToken>(
                                                                         reinterpret_cast<std::uintptr_t>(resources_)));
    if (!resources_->initializeGpuCache(context.deviceContext(), context.deferredDestroyQueue(),
    ...
    const auto select_pipeline = context.pipelineManager().registerComputePipelineReflected(
        select_object->module, select_object->info, "TerrainPatchSelect");
    if (!select_pipeline)
    {
...
void TerrainFeature::onFrameBegin(const FeatureFrameContext &context)
{
    ...
    if (cameras_)
    {
        renderScene().forEachActiveView([this](View &view) {
            ...
            wanted_views_.push_back(
                TerrainResources::ViewOrigin{camera->render_origin, camera->coordinate_page_size,
                                             static_cast<float>(std::max(view.current_extent.height, 1u)) *
                                                 std::fabs(camera->camera_view.proj.data()[5]) * 0.5f});
        });
    }
    resources_->reconcileWanted(wanted_views_, config_.wanted_radius, renderScene().maintenanceTime(),
...
void TerrainFeature::addPasses(RGBuilder &builder)
{
    ...
    const auto import_buffer = [&builder](std::string_view name, VkBuffer buffer, std::uint64_t size,
                                          std::uint32_t stride) {
    ...
    const auto full_pages =
        import_buffer("TerrainFullPages", resources_->fullPageBuffer(),
    ...
    const auto fallback_pages = import_buffer("TerrainFallbackPages", resources_->fallbackPageBuffer(),
                                              static_cast<std::uint64_t>(TerrainResources::fallbackPageStride()) *
                                                  resources_->fallbackCapacityPages(),
                                              static_cast<std::uint32_t>(TerrainResources::fallbackPageStride()));

    const auto maximum_patches = std::min(resources_->fallbackCapacityPages() * 64u, config_.maximum_selected_patches);
    ...
    const auto select_descriptors = builder.createTransientDS("TerrainPatchSelectDS", patch_select_layout_,
                                                              {
    ...
    const auto patch_descriptors = builder.createTransientDS("TerrainPatchRasterDS", patch_layout_,
                                                             {
                                                                 {0u, EDescriptorType::STORAGE_BUFFER, metadata},
    ...
    builder.addPass("TerrainGBuffer", ERGPassType::GRAPHICS)
        ...
        .write(builder.referenceTexture(gbuffer.albedo_metallic, ERGReference::Required),
               lux::render::ETextureRole::COLOR_ATTACHMENT)
        .write(builder.referenceTexture(gbuffer.normal_roughness, ERGReference::Required),
               lux::render::ETextureRole::COLOR_ATTACHMENT)
        .write(builder.referenceTexture(gbuffer.emissive_ao, ERGReference::Required),
               lux::render::ETextureRole::COLOR_ATTACHMENT)
        .write(builder.referenceTexture("SceneDepth", ERGReference::Required),
        ...
        .setKernelFn([indirect](const PassRecordContext &context) {
            ...
            vkCmdPushConstants(context.cmd, context.pipeline_layout, VK_SHADER_STAGE_VERTEX_BIT, 0u, sizeof(push),
                               &push);
