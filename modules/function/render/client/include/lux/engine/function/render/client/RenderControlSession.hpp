...
namespace lux::render
{
    ...
    private:
        ...
        template <class Reply, class Record> [[nodiscard]] RenderRequest<Reply> recordReply(Record&& record)
        {
            ...
            if (!builder.valid() || builder.commandCount() != 1u || !publishPacket(std::move(packet)))
            {
        ...
        template <class Record> [[nodiscard]] bool record(Record&& record)
        {
            ...
            builder.begin({});
            record(builder);
            return builder.valid() && builder.commandCount() == 1u && publishPacket(std::move(packet));
        }

        void deferDestroyScene(RenderSceneId scene) noexcept;
        void deferRemoveView(RenderSceneId scene, ViewHandle view, RenderViewReleaseObserver observer) noexcept;
        void deferDestroyTarget(RenderTargetId target, RenderTargetReleaseObserver observer) noexcept;
