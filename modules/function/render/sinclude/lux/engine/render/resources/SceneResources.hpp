...
#include <lux/engine/render/renderer/RenderViewTypes.hpp>  // CameraView, EntityTransform
#include <lux/engine/render/core/RenderTypes.hpp>          // Viewport
...
namespace lux::render
{
    ...
    static_assert(sizeof(ViewGpuData) == 288);
                                ...
                                ViewGpuData&           out)
    {
        ...
        out.viewport[2] = vp.min_depth;
        out.viewport[3] = vp.max_depth;
    }
    ...
    {
    public:
        static constexpr EUploadPhase kSceneUploadPhase = EUploadPhase::PostUpload;
        ...
        void recordBind(VkCommandBuffer cb, VkPipelineBindPoint bind_point,
                        VkPipelineLayout pipeline_layout, uint32_t set_index = UINT32_MAX) const
        {
            VkDescriptorSet ds = getDescriptorSet();
            uint32_t idx = (set_index == UINT32_MAX)
                         ? get_binding_set<ESceneSetBindings>::value : set_index;
            vkCmdBindDescriptorSets(cb, bind_point, pipeline_layout, idx, 1, &ds, 0, nullptr);
        }
        ...
        {
            return "SceneResources: scenes=" + std::to_string(scene_buf_.count())
                 + "/" + std::to_string(scene_buf_.capacity())
                 + " views=" + std::to_string(view_buf_.count())
                 + "/" + std::to_string(view_buf_.capacity());
        }
