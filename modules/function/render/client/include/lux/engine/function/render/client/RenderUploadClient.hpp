...
namespace lux::render
{
    ...
    public:
        ...
        using SubmitFunction = UploadSubmitNoReplyResult (*)(
            void*,
            std::shared_ptr<PreparedUpload>) noexcept;
            ...
            UploadPayloadAccounting accounting = {}) const
        {
            ...
            if (!builder.valid() || builder.commandCount() != 1u ||
                builder.preparedReplyType() == kInvalidTypeId ||
                !packet.sealAccounting())
            {
                return lux::cxx::unexpected(
            ...
            prepared->expected_reply_type = builder.preparedReplyType();
            prepared->callback = std::move(callback);

            auto submitted = control_->submit(
                control_->owner.get(),
                std::move(prepared));
            if (!submitted)
        ...
        tryCreateTexture2DMips(
            ...
            std::int32_t channels,
            EPixelFormat format = EPixelFormat::RGBA8_SRGB,
            bool generate_mips = false) const;
        ...
        tryCreateCubeTexture(
            ...
            std::int32_t channels = 4,
            EPixelFormat format = EPixelFormat::RGBA8_SRGB) const;
        ...
        [[nodiscard]] RenderUploadClientStatistics statistics() const noexcept
        {
            if (!control_ || !control_->metrics)
                return {};
            return {
                .submitted_packets = control_->metrics->submitted_packets.load(
                    std::memory_order_relaxed),
                .payload_shared_bytes =
                    control_->metrics->payload_shared_bytes.load(
                    std::memory_order_relaxed),
                .payload_copied_bytes =
                    control_->metrics->payload_copied_bytes.load(
                    std::memory_order_relaxed),
            };
        }
    ...
    private:
        struct Control final
        {
            ...
            {
                metrics->submitted_packets.fetch_add(
                    1u,
                    std::memory_order_relaxed
                );
                metrics->payload_shared_bytes.fetch_add(
                    accounting.shared_bytes,
                    std::memory_order_relaxed
                );
                metrics->payload_copied_bytes.fetch_add(
