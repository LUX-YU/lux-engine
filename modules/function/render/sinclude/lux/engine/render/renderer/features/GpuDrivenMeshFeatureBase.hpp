...
#include <lux/engine/render/core/VertexLayoutTypes.hpp>   // VertexLayoutId
#include <lux/engine/render/core/MaterialFamily.hpp>      // EShadingModel
...
namespace lux::render
{
    ...
    {
    public:
        static constexpr uint32_t kMaxDrawsPerBucket = ::lux::render::kMaxDrawsPerBucket;
    ...
    protected:
        ...
        // ── Common init / destroy ──
        void initCommon(ShaderHandle view_cull_shader_id, uint32_t extension_flags = 0u);
        void destroyCommon() noexcept;

        [[nodiscard]] uint32_t supportedGeometryMask(EPassDomain pass_domain) const noexcept;
        ...
        void initCompactPipeline(ShaderHandle compact_shader_id, std::string_view debug_name);
        ...
        /// MDC info GPU buffer size (packed offset+section pairs + sentinel).
        [[nodiscard]] VkDeviceSize mdcInfoBufferSize() const noexcept;

        /// Creates or reuses the shared 8-binding cull descriptor set layout.
        ...
        /// per-pass ShaderObjects differ, but the family→frag switch was byte-
        /// identical in both. LegacyLit aliases Unlit until that family lands (S15);
        /// Graph returns @p fs_graph (null when no per-scene override is set, so
        /// that family's pipeline is skipped in registerFamilyPipelines).
        static const ShaderObject* resolveFragmentForFamily(
            EShadingModel       sm,
            const ShaderObject* fs_unlit,
            const ShaderObject* fs_pbr,
            const ShaderObject* fs_stylized,
            const ShaderObject* fs_graph) noexcept
        {
            switch (sm)
            {
        ...
        /// Deferred — they differ only in @p base_template (vertex shader +
        /// blend/depth state), the vertex shader (@p vertex_shader, for the
        /// reflection info), and @p resolve_fragment (family → fragment shader).
        ///
        ...
        // --- R1: inputs captured at registerFamilyPipelines so addPasses can
        //     build a graph material's OWN pipeline from its runtime-submitted
        //     frag shader (same template/_vp/layout the family bootstrap uses).
        //     The template is held by ptr to keep its full definition out of this
        //     header; the cache maps a graph shader handle -> its built PSO. ---
