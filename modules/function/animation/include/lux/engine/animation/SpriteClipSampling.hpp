#pragma once
/**
 * @file SpriteClipSampling.hpp
 * @brief Pure 2D frame-clip sampling (the sampling MACHINERY of A2-01).
 *
 * Capability-module half of the sprite-animation split (ADR:
 * lux-engine-pack-architecture): these are pure functions of
 * (rdesc::SpriteAnimClip × time) — no ECS, no components, no assets.
 * The APPLY side (writing SpriteComponent.uv_rect, the events buffer, the
...
namespace lux::animation
{
    ...
    [[nodiscard]] inline std::uint32_t sampleSpriteClipStep(
        const lux::rdesc::SpriteAnimClip& clip, float t, float total) noexcept
    {
        if (clip.loop)
        {
            t = std::fmod(t, total);
    ...
    inline void appendSpriteClipStepEvents(const lux::rdesc::SpriteAnimClip& clip,
                                           std::uint32_t step,
