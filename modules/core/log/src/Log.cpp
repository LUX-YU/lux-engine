...
namespace lux::log
{
    ...
    {
        state().min_level.store(static_cast<std::uint8_t>(lv), std::memory_order_relaxed);
    }

    ELevel minLevel() noexcept
    {
        return static_cast<ELevel>(state().min_level.load(std::memory_order_relaxed));
    }
    ...
    std::size_t formatRecord(const LogRecord& r, char* out, std::size_t cap) noexcept
    {
        ...
        r.decode(
            ...
            +[](const LogRecord& rec, std::format_args args, void* user) {
                ...
                c.len = std::vformat_to(BufSink{c.out, c.cap}, std::string_view(rec.format, rec.format_len), args)
                            .written();
            },
            &ctx
        );
    ...
    void vlog(ELevel lv, const char* category, std::string_view fmt, std::format_args args) noexcept
    {
