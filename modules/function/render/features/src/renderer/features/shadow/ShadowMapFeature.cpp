...
{
    return {static_cast<float>(light.position_page[0]) * coordinate_page_size + light.position_local.x,
...
template <typename TLightGPU> [[nodiscard]] RenderLargePosition3D lightSpatialPosition(const TLightGPU &light) noexcept
{
...
void setShadowSpatialOrigin(ShadowSliceGPU &slice, const RenderLargePosition3D &origin,
                            float coordinate_page_size) noexcept
{
    for (std::size_t axis = 0; axis != 3u; ++axis)
    {
...
[[nodiscard]] Eigen::Matrix4f makeOrtho(float left, float right, float bottom, float top, float near_z, float far_z)
{
...
[[nodiscard]] float scoreShadowCandidate(const LightType &light, const Eigen::Vector3f &position,
                                         ...
                                         float non_directional_shadow_max_distance)
{
    ...
    if (std::isfinite(non_directional_shadow_max_distance) && non_directional_shadow_max_distance > 0.0f)
    {
        ...
        visibility = std::clamp(1.0f - dist / std::max(non_directional_shadow_max_distance, kEpsilon), 0.0f, 1.0f);
    }
    else
    {
...
void cmdUpdateBufferChunked(VkCommandBuffer cmd, VkBuffer buffer, VkDeviceSize dst_offset, const void *src_data,
                            VkDeviceSize size_bytes)
{
    ...
    const bool is_invalid_upload = is_missing_command_buffer || is_missing_buffer || is_missing_data || is_empty_upload;
...
ShadowMapFeature::ShadowMapFeature(Config cfg)
    : RenderFeature(RenderFeature::Config{.name = std::string(ShadowMapFeature::kFeatureName)}), cfg_(std::move(cfg))
{
...
void ShadowMapFeature::writeEVSMBindings(LightResources &light_res, EVSMShadowResources &evsm_res,
                                         std::span<const VkDescriptorSet> domain_sets, uint32_t domain_binding_offset)
{
...
void ShadowMapFeature::onFrameBegin(const FeatureFrameContext & /*ctx*/)
{
    ...
    renderScene().forEachActiveView([&](View &view) {
        ...
        const bool is_same_light = has_previous_fingerprint && prev_fp->light_config_hash == new_fp.light_config_hash;
        ...
        shadow_res_->setCachedData(scene_key, view.handle.index, state.slices, state.spot_shadow_slice_index,
                                   state.point_shadow_base_slice, state.config);
...
void ShadowMapFeature::addPasses(RGBuilder &builder)
{
    ...
    builder.addPass(kShadowViewUploadPassName, ERGPassType::TRANSFER)
        ...
        .setKernelFn([this, shadow_slices, shadow_config, spot_shadow_map,
                      point_shadow_map](const PassRecordContext &pctx) {
            ...
            const bool is_missing_buffer =
                is_missing_slice_buffer || is_missing_config_buffer || is_missing_spot_map || is_missing_point_map;
            ...
            const VkDeviceSize slices_size = static_cast<VkDeviceSize>(cache->slices.size()) * sizeof(ShadowSliceGPU);
            if (slices_size > 0)
            {
                cmdUpdateBufferChunked(pctx.cmd, slice_buf, 0, cache->slices.data(), slices_size);
            }
            auto frame_config = cache->config;
            frame_config.scene_time = renderScene().maintenanceTime();
            ...
            if (spot_map_size > 0)
            {
                ...
                cmdUpdateBufferChunked(pctx.cmd, spot_map_buf, 0, cache->spot_shadow_slice_index.data(), clamped_size);
            }
            ...
            if (point_map_size > 0)
            {
                ...
                cmdUpdateBufferChunked(pctx.cmd, point_map_buf, 0, cache->point_shadow_base_slice.data(), clamped_size);
            }
...
bool ShadowMapFeature::updateQuality(uint32_t atlas_page_resolution, uint32_t atlas_page_count,
                                     uint32_t max_shadow_slices, float non_directional_shadow_max_distance)
{
...
RenderFeature::EParamApply ShadowMapFeature::applyParams(const void *src, std::size_t size)
{
    ...
    const bool atlas_rebuilt = updateQuality(next.atlas_page_resolution, next.atlas_page_count, next.max_shadow_slices,
...
void ShadowMapFeature::buildSlicesForView(const View &view, LightResources *light_res, PerViewShadowState &out_state)
{
    ...
    light_res->forEachLight<DirectionalLightGPU>([&](uint32_t slot, const DirectionalLightGPU &dl) {
        ...
        const uint32_t directional_tile_request = chooseResolutionForRequiredTiles(
            std::max(dl.shadow_map_size, 1u), cfg_.shadow_config.atlas_page_resolution,
            cfg_.shadow_config.atlas_page_count, cascades + reserved_non_directional_tiles);
        ...
        const bool normalized_splits = directional_csm_enabled ? (dl.cascade_splits[cascades - 1] <= 1.0001f) : false;
        ...
        for (uint32_t c = 0; c < cascades && out_state.slices.size() < max_slices; ++c)
        {
            ...
            const float split_prev_cfg = (c == 0) ? (normalized_splits ? 0.0f : camera_near) : dl.cascade_splits[c - 1];
            ...
            const float split_curr_cfg =
                is_last_cascade ? (normalized_splits ? 1.0f : (directional_csm_enabled ? camera_far : single_slice_far))
                                : dl.cascade_splits[c];
            ...
            Eigen::Vector3f min_ls(std::numeric_limits<float>::max(), std::numeric_limits<float>::max(),
                                   std::numeric_limits<float>::max());
            Eigen::Vector3f max_ls(std::numeric_limits<float>::lowest(), std::numeric_limits<float>::lowest(),
    ...
    // milliseconds. Diagnose with LUX_SHADOW_ALLOC_TRACE=1 (below).
    {
        ...
        for (const CasterChoice &c : caster_choices)
        {
            ...
            requests.push_back(
                {(static_cast<uint64_t>(static_cast<uint8_t>(c.type)) << 32) | slot, c.score, is_spot ? 1u : 6u, res});
        }
        ...
        const auto placed = orderCastersForPlacement(std::move(requests), atlas_packer, max_slices,
                                                     static_cast<uint32_t>(out_state.slices.size()), sticky);
    ...
    for (const CasterChoice &choice : caster_choices)
    {
        ...
        if (choice.type == ECasterType::Spot)
        {
            ...
            ShadowSliceGPU slice = makePerspectiveLightSlice(Eigen::Vector3f::Zero(), dir, up_seed, near_z, far_z,
            ...
            out_state.slices.push_back(slice);
            out_state.spot_shadow_slice_index[spot_candidates[choice.index].slot] = static_cast<int32_t>(slice_index);
            ++shadow_spot_slices;
            continue;
        }
        ...
        for (int face = 0; face < 6; ++face)
        {
            ...
            ShadowSliceGPU slice = makePerspectiveLightSlice(Eigen::Vector3f::Zero(), dir, up_seed, near_z, far_z,
                                                             point_proj_scale, pl.shadow_bias, tiles[face]);
            setShadowSpatialOrigin(slice, lightSpatialPosition(pl), coordinate_page_size);
            out_state.slices.push_back(slice);
        }
        out_state.point_shadow_base_slice[point_candidates[choice.index].slot] = static_cast<int32_t>(base_slice_index);
    ...
    // 换位的槽数。
    {
        ...
        {
            renderContext().reportError(renderError<err::frame::ShadowAtlasAssignmentChurn>(
                                            changed_slots, prev_total, out_state.config.total_slices),
                                        renderScene().sceneId().index, renderScene().frameSerial());
        }
