...
namespace
{
[[nodiscard]] SceneDescriptionFailure failure(ESceneDescriptionError code, system::SystemInstanceId system = {},
                                              std::uint64_t subject_hash = 0U) noexcept
{
    return {code, system, subject_hash};
}

template <class Range> [[nodiscard]] auto findSystem(Range &systems, system::SystemInstanceId id) noexcept
{
    return std::find_if(systems.begin(), systems.end(), [id](const auto &value) noexcept { return value.id == id; });
}
} // namespace
...
lux::cxx::expected<void, SceneDescriptionFailure> SceneDescriptionBuilder::addSystem(
    system::SystemInstanceId id, std::string_view instance_name, const system::SystemTypeId &type,
    std::uint32_t system_version, std::string_view configuration_schema_name,
    std::uint32_t configuration_schema_version, std::span<const std::byte> configuration) noexcept
{
    if (!id.valid())
    {
    ...
    const bool duplicate_name = std::ranges::any_of(
        impl_->systems, [instance_name](const auto &system) noexcept { return system.instance_name == instance_name; });
    if (duplicate_name)
    {
    ...
    try
    {
        impl_->systems.push_back(Impl::PendingSystem{
            id, std::string(instance_name), type, system_version, std::string(configuration_schema_name),
            configuration_schema_version, std::vector<std::byte>(configuration.begin(), configuration.end())});
        return {};
    }
...
lux::cxx::expected<void, SceneDescriptionFailure> SceneDescriptionBuilder::setSystemConfiguration(
    system::SystemInstanceId id, std::span<const std::byte> configuration) noexcept
{
...
lux::cxx::expected<void, SceneDescriptionFailure> SceneDescriptionBuilder::eraseSystem(
    system::SystemInstanceId id) noexcept
{
    auto found = findSystem(impl_->systems, id);
    if (found == impl_->systems.end())
    {
    ...
    std::erase_if(impl_->dependencies,
                  [id](const auto &edge) noexcept { return edge.before == id || edge.after == id; });
    return {};
}

lux::cxx::expected<void, SceneDescriptionFailure> SceneDescriptionBuilder::bindRequirement(
    system::SystemInstanceId system, std::string_view requirement, std::string_view provider) noexcept
{
...
lux::cxx::expected<void, SceneDescriptionFailure> SceneDescriptionBuilder::unbindRequirement(
    system::SystemInstanceId system, std::string_view requirement) noexcept
{
...
lux::cxx::expected<void, SceneDescriptionFailure> SceneDescriptionBuilder::addDependency(
    system::SystemInstanceId before, system::SystemInstanceId after) noexcept
{
    ...
    system::SystemInstanceId before, system::SystemInstanceId after) noexcept
{
    const auto found = std::find_if(
        impl_->dependencies.begin(), impl_->dependencies.end(),
...
lux::cxx::expected<SceneDescription, SceneDescriptionFailure> SceneDescriptionBuilder::buildResolved() && noexcept
{
    return finish(true);
}

lux::cxx::expected<SceneDescription, SceneDescriptionFailure> SceneDescriptionBuilder::finish(bool resolved) noexcept
{
    if (!resolved && impl_->world.isNull())
    {
        return lux::cxx::unexpected(failure(ESceneDescriptionError::INVALID_WORLD));
    }
    if (!resolved && impl_->simulation.isNull())
    {
        return lux::cxx::unexpected(failure(ESceneDescriptionError::INVALID_SIMULATION));
    }
    try
    {
        std::ranges::sort(impl_->systems,
                          [](const auto &left, const auto &right) noexcept { return left.id < right.id; });
        ...
        for (const auto &source : impl_->systems)
        {
            if (source.configuration.size() >
                std::numeric_limits<std::size_t>::max() - result.configuration_payload_.size())
            {
            ...
            result.configuration_payload_.insert(result.configuration_payload_.end(), source.configuration.begin(),
                                                 source.configuration.end());
            result.system_ordinals_.emplace(source.id.value, result.systems_.size());
            result.systems_.push_back(
                {source.id, source.instance_name, source.type, source.version, source.configuration_schema_name,
                 source.configuration_schema_name.empty() ? 0U
                                                          : lux::cxx::Fnv1a64::hash(source.configuration_schema_name),
                 source.configuration_schema_version, offset, source.configuration.size()});
        ...
        for (const auto &binding : impl_->bindings)
        {
            result.requirement_bindings_.push_back(
                {result.system_ordinals_.at(binding.system.value), binding.requirement, binding.provider});
        }
        ...
        std::ranges::sort(result.dependencies_, [](const auto &left, const auto &right) noexcept {
            return std::tie(left.before_system, left.after_system) < std::tie(right.before_system, right.after_system);
        });
