...
namespace lux::render
{
    ...
    struct RGPhysicalResource
    {
        ...
        {
            if (physical_handles.empty())
                return 0;
            // If resource is not per-frame, always return the 0th one
            if (physical_handles.size() == 1)
                return physical_handles[0];
    ...
    resolveRingTarget(const RGPhysicalResourceTable& table, uint32_t res_idx, uint32_t frame_index)
    {
        ...
        {
            if (pr->lifetime == ERGResourceLifetime::PING_PONG && pr->ring_phase > 0 && pr->pingpong_peer >= 0)
            {
    ...
    public:
        ...
        )
        {
            ...
            if (table.contains(logic_index))
            {
                ...
                {
                    res.physical_handles[frame_index % res.physical_handles.size()] =
            ...
            for (uint32_t index : indices)
            {
                ...
                if (res.image_getter)
                {
                    ...
                    const uint32_t count =
                        res.image_getter(new_images.data(), static_cast<uint32_t>(new_images.size()));
                ...
                else if (res.buffer_getter)
                {
                    ...
                    const uint32_t count =
                        res.buffer_getter(new_buffers.data(), static_cast<uint32_t>(new_buffers.size()));
                    if (count > 0)
                    {
                        res.physical_handles.clear();
                        res.physical_handles.reserve(count);
        ...
        virtual void deallocate(const RGPhysicalResourceTable& table) = 0;
        ...
        virtual void deallocateToPool(const RGPhysicalResourceTable& table, const RGGraphDescription& /*graph*/)
        {
            deallocate(table);
        }
    };
