...
namespace lux::gapi::vk
{
    ...
    public:
        ...
        CommandPool(VkDevice device, const VkCommandPoolCreateInfo& info, VkAllocationCallbacks* allocator = nullptr)
        {
            VK_FUNC_INVOKE(
                vkCreateCommandPool,
                "Failed to create CommandPool object",
                device,
                &info,
                allocator,
                &command_pool
        ...
        inline const VkCommandPool* operator&() const noexcept
        {
            return &command_pool;
        }

        inline VkCommandPool handle() const noexcept
        {
            return command_pool;
        }
        inline const VkCommandPool* handlePtr() const noexcept
        {
            return &command_pool;
        }
