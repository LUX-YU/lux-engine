...
namespace lux::object::test::fixture
{
    ...
    public:
        IntSender() = default;
        explicit IntSender(ObjectDispatcherRef dispatcher) noexcept : Object(std::move(dispatcher))
        {
        }
    ...
    public:
        ...
        void publishBase(int value)
        {
            notify<baseChanged>(value);
        }
    };

    class LUX_OBJECT() DerivedSender final : public Object<DerivedSender, BaseSender>
    {
    };
    ...
    public:
        ...
        void publish(MoveOnly& value)
        {
            notify<moveOnly>(value);
        }
    };
    ...
    class LUX_OBJECT() QueuedPayloadSender final : public Object<QueuedPayloadSender>
    {
    public:
        explicit QueuedPayloadSender(ObjectDispatcherRef dispatcher) noexcept : Object(std::move(dispatcher))
        {
        }
    ...
    class LUX_OBJECT() QueuedPayloadReceiver final : public Object<QueuedPayloadReceiver>
    {
    public:
        explicit QueuedPayloadReceiver(ObjectDispatcherRef dispatcher) noexcept : Object(std::move(dispatcher))
        {
        }

        void receive(const QueuedPayload4&) noexcept
        {
            ++observed;
        }
        void receive(const QueuedPayload64&) noexcept
        {
            ++observed;
        }
        void receive(const QueuedPayload256&) noexcept
        {
            ++observed;
        }
        void receive(const QueuedPayload1024&) noexcept
        {
            ++observed;
        }
