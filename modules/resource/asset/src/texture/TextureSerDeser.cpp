...
namespace lux::asset
{
    ...
    TextureSerDeser::~TextureSerDeser() = default;

    lux::cxx::expected<std::unique_ptr<lux::rdesc::Texture>, EAssetError>
    TextureSerDeser::decodeData(
        const void* bytes,
        std::size_t length
    ) noexcept
    {
    ...
    lux::cxx::expected<std::unique_ptr<lux::rdesc::Texture>, EAssetError>
    TextureSerDeser::decodeData(lux::cxx::SharedBytes<> file) noexcept
    {
        if (file.empty())
            return lux::cxx::unexpected(EAssetError::ABNORMAL_FILE_SIZE);
    ...
    TextureSerDeser::fromLuxAssetStream(std::istream& stream)
    {
    ...
    EAssetError TextureSerDeser::exportAsLuxAssetStream(
