...
namespace lux::render
{
    ...
    public:
        ...
        {
            return device_ctx_ && h.isValid() && h.index < capacity_ &&
        ...
        [[nodiscard]] VkDeviceAddress deviceAddress(
            uint32_t slice = 0u) const noexcept
        {
            if (!device_ctx_ || buffer_owned_.get() == VK_NULL_HANDLE ||
                (buffer_usage_ & VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT) == 0u ||
                slice >= this->slices())
            {
                return 0u;
            }
            ...
            const VkDeviceAddress base = vkGetBufferDeviceAddress(
                device_ctx_->logicalDevice(), &info);
        ...
        bool modifyAt(uint32_t index, const T& value) requires HasMirror
        {
            if (!isSlotAlive(index)) return false;
