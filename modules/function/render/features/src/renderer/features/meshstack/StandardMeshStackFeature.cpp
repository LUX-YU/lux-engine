...
lux::render::Expected<void> StandardMeshStackFeature::initAndAttachTo(RenderScene &sc)
{
    ...
    if (fresh)
    {
        ...
        {
            const auto accepted =
                vpr->setDomainWriteTarget(domains->setsFor(rdesc::EBindFrequency::FEATURE),
    ...
    if (fresh_inst)
    {
        ...
        reg.addBeginFrameHook(EUploadPhase::Upload, [inst](const FrameStamp &s) { inst->onFrameBeginMaintenance(s); });
    ...
    {
        const auto accepted =
            inst->setDomainWriteTarget(domains->setsFor(rdesc::EBindFrequency::FEATURE),
