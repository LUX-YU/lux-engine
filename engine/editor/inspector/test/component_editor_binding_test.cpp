...
int main()
{
    ...
    type_collision_binding.component_type =
        lux::cxx::TypeToken{plugin.component_type.hash(), "test.synthetic-type-collision"};
    auto type_collision = inspector::ComponentEditorBindingTable::build(std::vector{plugin, type_collision_binding});
    ...
    auto duplicate_schema =
        inspector::ComponentEditorBindingTable::build(std::vector{plugin, duplicate_schema_binding});
