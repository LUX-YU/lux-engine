...
#include <lux/engine/function/render/client/features/canvas2d/Canvas2DOperation.hpp>  // Image2DInstanceData / Image2DHandle / quantizePriority / status
...
namespace lux::render
{
    ...
    class Canvas2DInstanceArena
    {
        struct Slot
        {
            ...
            std::uint8_t  group{0};   ///< A2-04 offscreen group (image kind only today)
            bool          alive{false};
            bool          visible{false};
        };
        ...
        public:
            ...
            bool remove(std::uint32_t slot, std::uint32_t gen)
            {
                ...
                if (!s)
                {
                    // Contract-violation tripwire: a healthy client never removes a
                    // stale handle, so a hit means an instance upstream was leaked
                    // or double-freed — loud beats a silently ghost-drawn slot.
                    std::fprintf(stderr,
                        "[Canvas2D] remove IGNORED: stale handle slot=%u gen=%u "
                        "(alive=%d cur_gen=%u)\n",
                        slot, gen,
                        slot < slot_count_ ? static_cast<int>(slots_[slot].alive) : -1,
                        slot < slot_count_ ? slots_[slot].gen : 0u);
                    return false;
                }
                s->alive = false;
                ...
                const std::int64_t origin_delta[3]) const noexcept
            {
                     ...
                     ++slot)
                {
                    if (slots_[slot].alive &&
                        !canRebaseRenderPageDelta2D(
                            records_.at(slot).page_delta,
                            origin_delta))
                    {
                        return false;
                    }
    ...
    public:
        ...
        ~Canvas2DInstanceArena() { images_.shutdown(); }
        ...
        void init(const InitInfo& info)
        {
            ...
            initialized_ = (images_.descriptorSet() != VK_NULL_HANDLE &&
        ...
        [[nodiscard]] ECanvas2DCreateStatus add(const Image2DInstanceData& data,
                                                ...
                                                std::uint32_t group = 0)
        {
            ...
            const auto st = images_.add(data, quantizePriority(priority), visible,
                                         clampGroup(group), slot, gen);
            if (st != ECanvas2DCreateStatus::Ok) return st;
            if (visible) order_dirty_ = true;
            out = Image2DHandle{slot, gen};
            return st;
        }

        void remove(Image2DHandle h)
        {
            if (!initialized_ || h.isNull()) return;
            ...
            const std::int32_t page_delta[2])
        {
            ...
            if (auto* r = images_.resolveRecord(h.index, h.gen))
            {
                ...
                std::memcpy(
                    ...
                    page_delta,
                    sizeof(r->page_delta));
                images_.markRecordDirty(h.index);   // order untouched — transforms never re-sort
            }
        }

        void writeVisual(Image2DHandle h, const float uv[4],
        ...
        {
            if (!initialized_ || h.isNull()) return;
            if (images_.writeKey(h.index, h.gen, quantizePriority(priority), visible,
                                  clampGroup(group)))
                order_dirty_ = true;
        }
        ...
        void writeFieldTransform(
            PixelFieldInstanceHandle h,
            const float m[6],
            const std::int32_t page_delta[2])
        {
            if (!initialized_ || h.isNull()) return;
        ...
        void writeTileTransform(
            Tile2DInstanceHandle h,
            const float m[6],
            const std::int32_t page_delta[2])
        {
            if (!initialized_ || h.isNull()) return;
        ...
        [[nodiscard]] VkDescriptorSet descriptorSet(ECanvas2DKind kind) const noexcept
        {
            switch (kind)
            {
            case ECanvas2DKind::Image:     return images_.descriptorSet();
        ...
        [[nodiscard]] bool canRebaseSceneOrigin(
            const std::int64_t origin_delta[3]) const noexcept
        {
            return images_.canRebaseSceneOrigin(origin_delta) &&
                fields_.canRebaseSceneOrigin(origin_delta) &&
                tiles_.canRebaseSceneOrigin(origin_delta);
        }

        void rebaseSceneOrigin(
            const std::int64_t origin_delta[3]) noexcept
        {
            images_.rebaseSceneOrigin(origin_delta);
            fields_.rebaseSceneOrigin(origin_delta);
    ...
    private:
        [[nodiscard]] std::array<IKindStore*, 3> stores() noexcept { return {&images_, &fields_, &tiles_}; }
