...
#include <lux/engine/render/RendererContext.hpp>                    // RenderContext (held subject; also pulls DeferredDestroyQueue)
#include <lux/engine/render/resources/ShaderResources.hpp>          // ShaderResources::add/get
#include <lux/engine/render/core/ShaderObject.hpp>                  // ShaderObject::module/.info
#include <lux/engine/render/resources/BuiltinShaderRegistry.hpp>    // ensureBuiltinShader (module-private)
#include <lux/engine/render/pipeline/GeneralDescriptorSetLayout.hpp>// getLayout(slot)
#include <lux/engine/render/pipeline/PipelineManager.hpp>           // registerGraphicsTemplate/registerComputePipeline
#include <lux/engine/render/pipeline/StandardPipelineLayoutBuilder.hpp> // buildStandardGraphicsPipelineLayout
#include <lux/engine/render/core/Errors.hpp>                        // Expected<T>::value()
...
namespace lux::render
{
    ...
    ResourceRegistryBase& RenderContextView::globalRegistry() noexcept
    {
        return ctx_->globalRegistry();
    }

    const ResourceRegistryBase& RenderContextView::globalRegistry() const noexcept
    {
        return ctx_->globalRegistry();
    }

    // ── Shaders ─────────────────────────────────────────────────────────
    ShaderHandle RenderContextView::loadBuiltinShader(EBuiltinShader builtin, ShaderHandle configured)
    {
        return ensureBuiltinShader(ctx_->globalRegistry().find<ShaderResources>(), configured, builtin);
    }

    ShaderHandle RenderContextView::loadShader(std::span<const std::byte> spirv, const lux::rdesc::ShaderInfo& info)
    {
        return ctx_->globalRegistry().find<ShaderResources>()->add(spirv, info);
    }

    VkShaderModule RenderContextView::shaderModule(ShaderHandle handle) const
    {
        const auto* obj = ctx_->globalRegistry().find<ShaderResources>()->get(handle);
