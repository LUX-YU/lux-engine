...
namespace lux::toolchain::detail
{
    namespace
    {
            ...
            const ProjectBuildUsage& usage)
        {
            ...
            for (const auto& component : usage.required_components)
            {
                ...
                if (contains(name, "camera3dcomponent") ||
                    contains(name, "grid3dcomponent") ||
                    contains(name, "directionallightcomponent") ||
                    contains(name, "pointlightcomponent") ||
                    contains(name, "spotlightcomponent") ||
                    contains(name, "scenesettingscomponent") ||
                    contains(name, "heightfogcomponent") ||
                    contains(name, "skyboxcomponent") ||
                    contains(name, "watersurfacecomponent") ||
                    contains(name, "meshcomponent") ||
                    contains(name, "classicmeshbatchcomponent") ||
                    contains(name, "visuallodnodecomponent") ||
                    contains(name, "terraintilecomponent"))
                {
                ...
                if (contains(name, "camera2dcomponent") ||
                    contains(name, "grid2dcomponent") ||
                    contains(name, "image2dcomponent") ||
                    contains(name, "pixelchunk2dcomponent") ||
        ...
        void appendProductRendererRoots(
            ProjectBuildUsage& usage,
            const std::set<ESystemRecipe>& recipes)
        {
            const auto append = [&usage](std::span<const std::string_view> roots)
            {
                for (const auto root : roots)
                    usage.required_render_features.emplace_back(root);
            };
            if (recipes.contains(ESystemRecipe::PRESENTATION_3D))
            {
                // Build-only mirror of the concrete standard Player recipe:
                // RenderProfile intrinsic roots plus the pure Stages and
                // render-facing behavior installed by that recipe.  These are
                // deliberately not read from SceneDescription.
                static constexpr std::string_view kStandard3DRoots[]{
                    "StandardViewCamera",
                    "SpatialCull",
                    "Grid3DPass",
                    "Light",
                    ...
                    "LinearDepth",
                    "Hzb"};
                append(kStandard3DRoots);
            }
            if (recipes.contains(ESystemRecipe::PRESENTATION_2D))
            {
                static constexpr std::string_view kStandard2DRoots[]{
                    "StandardViewCamera",
                    "Canvas2D",
                    "Grid2D"};
                append(kStandard2DRoots);
            }
        }
            ...
            const std::set<ESystemRecipe>& recipes)
        {
            ...
            {
                source <<
                    ",\n        lux::ecs::ClassicMeshPreparePort classic_mesh"
                    ",\n        lux::ecs::TerrainPreparePort terrain";
            }
        }
            ...
            ESystemRecipe recipe)
        {
            ...
            case ESystemRecipe::TILEMAP_2D:
                ...
                break;
            case ESystemRecipe::SPATIAL_3D:
                source << "lux::runtime::installSpatial3DSystems(builder, components, description)";
                break;
            case ESystemRecipe::PRESENTATION_3D:
                source << "lux::runtime::installPresentation3DSystems(builder, components)";
                break;
            case ESystemRecipe::PRESENTATION_2D:
        ...
        const std::filesystem::path& output_directory) noexcept
    {
        ...
        appendProductRendererRoots(usage, recipes);
        if (auto canonical = canonicalize(usage); !canonical)
            return lux::cxx::unexpected(std::move(canonical.error()));
