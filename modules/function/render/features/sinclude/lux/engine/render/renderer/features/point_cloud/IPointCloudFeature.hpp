...
namespace lux::render
{
    ...
    public:
        ...
        ///
        /// SERVER/RENDER THREAD ONLY — it is reached from a comm operation
        /// handler, which runs there. (This used to say "safe to call from any
        /// thread; implementations must use atomic stores"; the implementations
        /// did, and no other thread ever called it. The atomics were removed —
        /// if a caller on another thread ever appears, that is a protocol
        /// decision to make deliberately, not something to re-add quietly.)
        ///
        /// Per-mode semantics:
