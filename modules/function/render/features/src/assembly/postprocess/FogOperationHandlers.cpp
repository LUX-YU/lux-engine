...
namespace lux::render
{
    ...
    void handleFogSetParams(GeneralRenderServer::Dispatcher::Ctx& context, const FogSetParamsPayload& params)
    {
        if (auto* scene = lookupScene(context.user_state, params.scene_id))
        {
            if (auto* feature = scene->getFeatureAs<FogFeature>(params.feature))
            {
