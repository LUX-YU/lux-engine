...
namespace
{
    lux::render::Expected<lux::render::FeatureHandle> createProbe(
        void*,
        const void*,
        std::size_t) noexcept
    {
...
int main()
{
    ...
    constexpr FeatureDescriptor descriptor{
        .type = stable_type,
        .name = "DynamicLifetimeFeature"};
    const FeatureFactory factory = makeSimpleFactory(
        &createProbe,
        "DynamicLifetimeFeature",
        descriptor);
    ...
    if (!check(second_add.has_value(), "counted registration failed") ||
        !check(second_add->type_id == first_add->type_id,
               "same stable type did not share a registry record"))
        return 1;
    ...
    if (!check(!in_use, "live feature instance did not reject unregister") ||
        !check(!first_weak.expired() && !second_weak.expired(),
               "rejected unregister released a module lease"))
        return 1;

    registry.noteInstanceRemoved(stable_type);
    auto shared_release = registry.release(first_add->type_id);
    if (!check(shared_release.has_value() && !shared_release->has_value(),
               "non-final unregister removed the shared record") ||
        !check(first_weak.expired() != second_weak.expired(),
               "non-final unregister did not release exactly one lease"))
        return 1;

    auto final_release = registry.release(first_add->type_id);
    if (!check(final_release.has_value() && final_release->has_value(),
               "final unregister did not return the removed record") ||
        !check(!first_weak.expired() || !second_weak.expired(),
               "final record did not retain its module through dispatcher cleanup") ||
        !check(!registry.contains(first_add->type_id),
               "final unregister left the record installed"))
        return 1;

    final_release->reset();
    if (!check(first_weak.expired() && second_weak.expired(),
               "module lease outlived the removed factory record"))
        return 1;
