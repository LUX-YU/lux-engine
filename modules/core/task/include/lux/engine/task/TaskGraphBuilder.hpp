...
namespace lux::task
{
    ...
    public:
         ...
         */
        template <class... Args>
        [[nodiscard]] lux::cxx::expected<TaskHandle, TaskGraphFailure> add(Args&&... args) noexcept
        {
            static_assert(sizeof...(Args) != 0U);
            auto arguments = std::forward_as_tuple(std::forward<Args>(args)...);
            return addTuple(std::move(arguments), std::make_index_sequence<sizeof...(Args) - 1U>{});
        }
        ...
        [[nodiscard]] lux::cxx::expected<TaskGraph, TaskGraphFailure> build() && noexcept;
    ...
    private:
        ...
        addTuple(Tuple&& arguments, std::index_sequence<Index...>) noexcept
        {
            constexpr std::size_t kCallableIndex = sizeof...(Index);
            using Callable = std::remove_cvref_t<decltype(std::get<kCallableIndex>(arguments))>;

            static_assert((detail::kTaskProperty<decltype(std::get<Index>(arguments))> && ...));
        ...
        static void collectProperty(PendingTask& pending, TaskResourceAccess property)
        {
            pending.resources.push_back(property);
        }

        static void collectProperty(PendingTask& pending, TaskResources property)
        {
        ...
        static void collectProperty(PendingTask& pending, TaskLifetimePin property)
        {
            if (property.value)
                pending.pins.push_back(std::move(property.value));
        }

        [[nodiscard]] lux::cxx::expected<TaskHandle, TaskGraphFailure> addPending(PendingTask pending) noexcept;
