...
namespace lux::render
{
    ...
    [[nodiscard]] constexpr ERenderImageUsage operator|(ERenderImageUsage lhs, ERenderImageUsage rhs) noexcept
    {
        return static_cast<ERenderImageUsage>(static_cast<std::uint32_t>(lhs) | static_cast<std::uint32_t>(rhs));
    }

    constexpr ERenderImageUsage& operator|=(ERenderImageUsage& lhs, ERenderImageUsage rhs) noexcept
    {
        lhs = lhs | rhs;
        return lhs;
    }

    [[nodiscard]] constexpr bool hasUsage(ERenderImageUsage value, ERenderImageUsage flag) noexcept
    {
    ...
    {
        lux::rdesc::ETextureFormat format{lux::rdesc::ETextureFormat::UNDEFINED};
    ...
    [[nodiscard]] inline bool formatKeyEquals(const RenderTargetLayout& lhs, const RenderTargetLayout& rhs) noexcept
    {
        for (std::size_t index = 0; index < kTargetSlotCount; ++index)
        {
            ...
            if (!lhs_slot)
                continue;
            const bool is_invalid_format = lhs_slot->format != rhs_slot->format;
    ...
    [[nodiscard]] inline std::uint32_t targetSlotMask(const RenderTargetLayout& layout) noexcept
    {
        std::uint32_t mask = 0;
        for (std::size_t index = 0; index < kTargetSlotCount; ++index)
        {
