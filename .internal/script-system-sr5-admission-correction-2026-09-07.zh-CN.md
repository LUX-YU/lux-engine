...
实际CSV记录Windows、24 logical CPU、MSVC-19.44.35228.0、LuaJIT 2.1.1771261233（JIT启用）；
benchmark的workers字段为0，驱动没有覆盖既有场景执行方式。全部运行按既定顺序串行，不与构建或测试并发。
编译缓存为 `/Zi /O2 /Ob1 /DNDEBUG`，没有单侧LTO/PGO；本轮未锁定CPU频率或隔离系统后台负载，
因此特别保留Event等场景的配对波动，不把帧样本当作额外独立进程。完整命令、开始时间与DLL身份在原始包中。
