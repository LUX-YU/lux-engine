...
{
ResourcePane::ResourcePane(scene::SceneEditor &document, ui::UIRenderSystem &window, process::ExecutionRuntime &runtime,
                           std::string id)
...
void ResourcePane::drawImport()
{
    ...
    if (ImGui::Button("Browse..."))
    {
        ...
        if (!selected)
        {
            import_message_ = "File selection failed: " + std::to_string(selected.error().reason);
        }
        else if (*selected)
        {
