...
#include "TextureSerDeser.hpp"
#include "MeshSerDeser.hpp"
#include "SkeletonSerDeser.hpp"
#include "AnimationClipSerDeser.hpp"
...
namespace lux::asset
{
	...
	{
		TextureLoadConfig	texture_load_config;   ///< Configuration for texture loading
	 ...
	 * 
	 * The ModelSerDeser class handles importing 3D models from external file formats
	 ...
	 */
	class LUX_RESOURCE_PUBLIC ModelSerDeser : public TAssetSerDeser<ModelSerDeserConfig>
	{
	public:
		/**
		 * @brief Constructs a ModelSerDeser with the specified asset manager.
		 * @param manager Shared pointer to the asset manager
		 */
		explicit ModelSerDeser(std::shared_ptr<AssetManager> manager);

		/**
		 * @brief Virtual destructor for ModelSerDeser.
		 */
		~ModelSerDeser() override;
	...
	private:
		...
		TextureSerDeser 					 texture_loader_; ///< Texture serialization/deserialization handler
		// (material_loader_ removed in W5b — the importer emits ImportedMaterialDesc PODs;
		//  the editor bakes MaterialAssets, so ModelSerDeser writes no material asset.)
	};
}