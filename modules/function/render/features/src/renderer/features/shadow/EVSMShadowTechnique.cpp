...
namespace lux::render
{
    void EVSMShadowTechnique::ensureBlurPipelines(RenderContext& ctx)
    {
        ...
        const auto blur_h_handle = resolveShaderStage(shaders, ShaderHandle{}, EBuiltinShader::SHADOW_EVSM_BLUR_H_COMP);
        const auto blur_v_handle = resolveShaderStage(shaders, ShaderHandle{}, EBuiltinShader::SHADOW_EVSM_BLUR_V_COMP);
    ...
    void EVSMShadowTechnique::recordPostFrame(const ShadowFrameContext& ctx)
    {
        ...
        auto blur_kernel = [page_res, shadow_res_ptr, scene_ptr](const PassRecordContext& pctx) {
            ...
            for (const auto& s : slices)
            {
                ...
                const int32_t tx1 =
                    static_cast<int32_t>(std::ceil((s.atlas_uv_bias.x() + s.atlas_uv_scale.x()) * fres)) - 1;
                const int32_t ty1 =
                    static_cast<int32_t>(std::ceil((s.atlas_uv_bias.y() + s.atlas_uv_scale.y()) * fres)) - 1;
                if (tx1 < tx0 || ty1 < ty0)
                    continue;
                ...
                vkCmdPushConstants(pctx.cmd, pctx.pipeline_layout, VK_SHADER_STAGE_COMPUTE_BIT, 0, sizeof(BlurPC), &pc);
