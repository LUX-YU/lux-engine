...
namespace lux::serialize
{
            ...
            switch (type_id)
            {
                ...
                case EArchiveType::UInt64:
                    *static_cast<std::uint64_t*>(field_ptr) = ar.readPod<std::uint64_t>();
                    break;
                case EArchiveType::Float:
                    *static_cast<float*>(field_ptr) = ar.readPod<float>();
                    break;
                case EArchiveType::Double:
                    *static_cast<double*>(field_ptr) = ar.readPod<double>();
                    break;
                case EArchiveType::String:
                                            ...
                                            const void* obj)
    {
        for (const auto& field : cls.fields)
        {
            ...
            // Skip fields explicitly marked LUX_NO_MEMBER (their
            // annotation_str contains "luxref::property::skip").
            if (field.annotation_str)
            {
                const std::string_view a(field.annotation_str);
                if (a.find("luxref::property::skip") != std::string_view::npos)
                    continue;
            }
