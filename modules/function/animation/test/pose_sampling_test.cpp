...
static bool
approxEq(float a, float b, float eps = 1e-5f)
{
    return std::fabs(a - b) <= eps;
}
static bool
approxEq(const Eigen::Vector3f& a, const Eigen::Vector3f& b, float eps = 1e-5f)
{
...
static lux::rdesc::AnimationClip
makeWalkClip(bool loop)
{
...
test_exact_keyframe_time()
{
    ...
    REQUIRE(
        approxEq(pose.bone_local[0].translation(), Eigen::Vector3f(1, 0, 0)),
        "exact keyframe at t=1 → translation (1,0,0)"
    );
    REQUIRE(
        approxEq(pose.bone_local[1].translation(), Eigen::Vector3f(0, 0, 0)),
...
test_midpoint_interpolation()
{
    ...
    REQUIRE(
        approxEq(pose.bone_local[0].translation(), Eigen::Vector3f(0.5f, 0, 0)),
        "midpoint t=0.5 → translation (0.5,0,0)"
    );

    lux::animation::samplePose(clip, skel, /*time=*/1.25f, clip.loop, pose);
    REQUIRE(
        approxEq(pose.bone_local[0].translation(), Eigen::Vector3f(1.25f, 0, 0)),
...
test_past_end_loop_wraps()
{
    ...
    lux::animation::samplePose(clip, skel, /*time=*/-0.5f, clip.loop, pose);
    REQUIRE(
        approxEq(pose.bone_local[0].translation(), Eigen::Vector3f(1.5f, 0, 0)),
...
test_past_end_clamp()
{
    ...
    REQUIRE(
        approxEq(pose.bone_local[0].translation(), Eigen::Vector3f(2, 0, 0)),
...
test_untracked_bone_stays_at_bind()
{
    ...
    skel.bones[1].bind_local = Eigen::Affine3f(Eigen::Translation3f(0.0f, 2.0f, 0.0f));
    ...
    REQUIRE(
        approxEq(pose.bone_local[1].translation(), Eigen::Vector3f(0, 2, 0)),
...
test_rotation_slerp_midpoint()
{
    ...
    tr.rotations = {
        ...
        Eigen::Quaternionf(Eigen::AngleAxisf(static_cast<float>(M_PI) * 0.5f, Eigen::Vector3f::UnitZ()))};
    tr.times_s = {0.0f};
    tr.scales = {{1, 1, 1}};
    ...
    Eigen::Quaternionf q_expected(Eigen::AngleAxisf(static_cast<float>(M_PI) * 0.25f, Eigen::Vector3f::UnitZ()));
