...
namespace lux::render
{
    ...
    //(已删 kMaxDrawsPerBucket:全仓仅定义 + 一个无人用的类内别名。它的文档
    // 还声称"由 cull/finalize 着色器与 C++ 记录方共享" —— 实际阴影路径用
    ...
    /// Visible draw payload. Instance slot and shadow slice stay independent;
    /// neither field sacrifices protocol bits to pack the other.
    struct GpuVisibleInstance final
    {
        std::uint32_t instance_slot{~0u};
        std::uint32_t slice_index{0u};
    };
    static_assert(sizeof(GpuVisibleInstance) == 8u);
