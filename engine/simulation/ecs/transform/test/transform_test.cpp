...
namespace
{
    ...
    struct Fixture final
    {
        ...
        Fixture()
            : maintenance(registry, hierarchy, deltas),
              transform2d(registry, hierarchy, deltas),
        ...
        void update()
        {
            ...
            assert(lux::simulation::ecs::applyEcsCommands(
                registry,
                commands
            ));
        }
        ...
        lux::simulation::ecs::Transform2DSystem transform2d;
        lux::simulation::ecs::Transform3DSystem transform3d;
    ...
    void testExistingFoldAndReactiveUpdates()
    {
        ...
        fixture.update();
        auto world = fixture.registry.get<WorldTransform3D>(child)
                         .value.translation();
        ...
        fixture.update();
        assert(detail::TransformSystemTestAccess::visitedNodes(
                   fixture.transform2d
               ) == 0U);
        assert(detail::TransformSystemTestAccess::visitedNodes(
                   fixture.transform3d
        ...
        fixture.registry.patch<Transform3D>(
            root,
            [](Transform3D& value) noexcept
            {
                value.translation.x() = 20.0F;
            }
        );
        fixture.update();
        world = fixture.registry.get<WorldTransform3D>(child)
                    .value.translation();
    ...
    void testDirtyOverflowRequiresExplicitResyncCapacity()
    {
        ...
        constexpr std::array capacities{
            EcsCommandProducerCapacity{8U, 1024U}};
        assert(commands.prepare(capacities));
        ...
        assert(maintenance.update());
        {
            ...
            assert(!updated);
            assert(
                updated.error() == ETransformUpdateError::CAPACITY_EXCEEDED
            );
        }
