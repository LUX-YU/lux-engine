...
namespace lux::toolchain
{
    ...
    using rdesc::AnimationClip;
    using rdesc::Bone_t;
    using rdesc::BoneTrack;
    using rdesc::Mesh;
    using rdesc::Skeleton;
    ...
    struct LoadContext {
        ...
        // Import-Options bake (see ModelImportConfig). import_xform = R*S
     ...
     */
    std::optional<asset_id_t> ModelImporter::importTextureAsset(
        LoadContext& ctx,
     ...
     */
    std::optional<rdesc::ImportedTextureRef> ModelImporter::makeBindingFor(LoadContext& ctx, const aiMaterial* mat, uint8_t type, unsigned index, const aiScene* scene, std::vector<asset_id_t>& texture_ids)
    {
        aiString path;
     ...
     */
    void ModelImporter::fillPBR(rdesc::ImportedMaterialDesc& out, const aiMaterial* m, LoadContext& ctx, const aiScene* scene, std::vector<asset_id_t>& tex_ids)
    {
        // Material factors
        {
            aiColor4D c;
     ...
     */
    void ModelImporter::fillLegacyLit(rdesc::ImportedMaterialDesc& out, const aiMaterial* m, LoadContext& ctx, const aiScene* scene, std::vector<asset_id_t>& tex_ids)
    {
     ...
     */
    std::uint32_t ModelImporter::importMaterial(LoadContext& ctx, unsigned matIdx)
    {
    ...
    //======================== ModelImporter Main Implementation ========================
    ModelImporter::ModelImporter(std::shared_ptr<AssetManager> manager)
    ...
    std::string ModelImporter::seedFor(std::string_view kind,
    ...
    ModelImporter::importFromFile(const std::filesystem::path& external_path)
    {
    ...
    ModelImporter::fromLuxAssetStream(std::istream& ifs)
    {
    ...
    EAssetError ModelImporter::exportAsLuxAssetStream(const LuxAsset& asset, std::ofstream& ofs)
    {
    ...
    EAssetError ModelImporter::loadFrom(const std::filesystem::path& path, ModelNode& model_root,
    ...
    void ModelImporter::processNode(LoadContext& ctx, ModelNode& node, const aiNode* an)
    {
    ...
    void ModelImporter::processMesh(LoadContext& ctx, ModelMeshInfo& mi, aiMesh* mesh)
    {
    ...
    void ModelImporter::buildSkeleton(LoadContext& ctx)
    {
        ...
        if (!config().is_right_handed)
        {
            std::cerr << "[ModelImporter] WARNING: skinned import with left-handed "
            ...
            [&](const aiNode* n, int32_t parent_bone_index, const Eigen::Affine3f& accum)
        {
            ...
            if (it != bone_offsets.end())
            {
                ...
                else if (!accum.matrix().isApprox(Eigen::Matrix4f::Identity()))
                {
                    ...
                    std::cerr << "[ModelImporter] WARNING: non-bone node transform between "
    ...
    void ModelImporter::fillBoneInfluences(LoadContext& ctx,
                                           aiMesh* mesh,
