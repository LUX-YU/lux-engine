...
//  reflected ShaderInfo (CPU only — no RenderFrameSession), then the test does its own
...
//  backend), so it carries no RenderFrameSession/window coupling. ShaderGen #includes
...
namespace lux::mgtest
{
    ...
    compileGraph(const lux::rdesc::MaterialGraph& graph, lux::rdesc::EMaterialPass pass)
    {
        ...
        // 与编辑器烘焙同序:lglsl 发射目录优先(lighting_common 等共享头已迁
        // .lglslh,.glsl 形态由发射流程生成),源树目录兜底。
        return sgg::compileToSpirv(lr->ir, p,
                                   { LUX_LGLSL_EMITTED_DIR, LUX_RENDER_SHADERS_DIR });
    }
    ...
    /// The byte form of the compiled output (the shape the render tests feed into compileShader).
