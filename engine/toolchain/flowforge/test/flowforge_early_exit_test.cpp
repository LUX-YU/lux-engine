...
static void test_early_return()
{
    ...
    const_cast<DataInPin&>(br->dataInPin()).setConstantData(lux::meta::RuntimeObject(bool{true}));
    ...
    check(g_sunk.size() == 1 && g_sunk[0] == 1, "early return: only the taken leg's sink ran (1)");
}
...
static void test_break()
{
    ...
    check(
        const_cast<DataOutPin&>(loop->indexPin()).linkTo(const_cast<DataInPin*>(&eq->lhs()), ll) == ELinkError::SUCCESS,
    ...
    check(g_sunk == std::vector<int>({0, 1, 2, 3}), "break: loop stopped after i==3 (sunk 0,1,2,3)");
}
...
static void test_break_outside_loop()
{
    ...
    if (!built)
    {
        ...
        check(built.error().message.find("loop") != std::string::npos, "message mentions the loop requirement");
    }
}
