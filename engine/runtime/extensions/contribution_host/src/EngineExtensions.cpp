...
namespace lux::extensions
{
        ...
        : public std::enable_shared_from_this<Impl>
    {
        ...
        void publishCatalogs(ModuleLease module) noexcept
        {
            if (!active || !active->runtime_draft)
                return;
            ...
            if (!committed)
            {
                // validateRuntimeCatalogs() ran at the same owner-thread safe
                // point before async installation. With serialized extension
                // commits this publish step has no recoverable failure mode.
                std::terminate();
            }
        ...
        void installOperations(ModuleLease module) noexcept
        {
            if (!active || !active->runtime_draft)
                return;
            if (active->runtime_draft->async_operations.empty())
            {
                publishCatalogs(std::move(module));
                return;
            }

            active->command.publisher.setPhase(
                EExtensionLoadPhase::INSTALLING_ASYNC_OPERATIONS);
            auto sender = services.async.installOperations(
                    active->runtime_draft->takeAsyncOperations())
                | ex::continues_on(
                      lux::exec::mainThreadScheduler(services.async))
                | ex::then(
                      [self = shared_from_this(), module = std::move(module)](
                          lux::cxx::expected<
                              lux::exec::AsyncOperationBundleLease,
                              lux::exec::EAsyncOperationBundleInstallError>
                              result) mutable noexcept
                      {
                          if (!result)
                          {
                              (void)self->services.modules.markFailed(
                                  module->id().view());
                              self->finishFailure(
                                  EExtensionLoadError::
                          ...
                          self->operation_bundles.push_back(
                              std::move(*result));
                          self->publishCatalogs(std::move(module));
                      });
            if (!lux::exec::spawn(scope, std::move(sender)))
            {
                (void)services.modules.markFailed(module->id().view());
                finishFailure(
                    EExtensionLoadError::ASYNC_OPERATION_INSTALL_FAILED);
            }
        }

        void collectContributions(ModuleLease module) noexcept
        {
            if (!active || !module)
                return;
            active->command.publisher.setPhase(
                EExtensionLoadPhase::COLLECTING_CONTRIBUTIONS);
            ...
            if (auto checked = validateRuntimeCatalogs(
                    *active->runtime_draft,
                    services.runtime_catalogs);
                !checked)
            {
                (void)services.modules.markFailed(module->id().view());
            ...
            installOperations(std::move(module));
        }
            ...
            ExtensionModuleLoadFailure> prepared) noexcept
        {
            ...
            collectContributions(active->modules_to_register.front());
        }
        ...
        void finishClose() noexcept
        {
            ...
            close_report.operation_bundles_released =
                operation_bundles.size();
            pending_bundle_closes = operation_bundles.size();
            if (pending_bundle_closes != 0u)
            {
                auto bundles = std::move(operation_bundles);
                operation_bundles.clear();
                for (auto& bundle : bundles)
                {
                    auto sender = services.async.closeOperations(
                            std::move(bundle))
                        | ex::continues_on(
                              lux::exec::mainThreadScheduler(services.async))
                        | ex::then(
                              [self = shared_from_this()](auto result) noexcept
                              {
                                  if (!result)
                                  {
                                      ++self->close_report.
                                          operation_bundle_close_failures;
                                  }
                                  if (--self->pending_bundle_closes == 0u)
                                      self->completeClose();
                              });
        ...
        std::vector<lux::exec::AsyncOperationBundleLease> operation_bundles;
        ...
        std::size_t pending_bundle_closes{0u};
        bool closing{false};
