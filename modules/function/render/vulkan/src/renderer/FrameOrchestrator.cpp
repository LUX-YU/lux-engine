...
Expected<FrameOrchestrator::EFrameStart> FrameOrchestrator::beginRenderFrame(
    RenderTargetRegistry& targets, FrameDriver* driver, FrameTickState& st)
{
    ...
    if (driver)
        for (auto& t : targets.all().values())
        {
            ...
            if (!rebuilt
                && !detail::isRetryableSwapchainFailure(rebuilt.error()))
            {
    ...
    if (driver)
    {
        auto runtime = driver->beginFrame(
            current_stamp_.slotIndex(),
...
namespace
{
    ...
    /// 对既有单层路径是零行为变化。
    RenderTargetLayout layoutForPhase(const RenderTargetLayout& base,
...
void FrameOrchestrator::renderTargets(RenderTargetRegistry& targets,
                                      Renderer&             renderer,
    ...
    auto runChain = [&](const RenderTargetEntry& t, RenderTargetBinding& binding)
    {
        ...
        for (size_t i = 0; i < n; ++i)
        {
            ...
            if (l.kind == Layer::EKind::SceneView)
            {
                ...
                // BeginRendering/屏障。
                if (st.rt.primary_cmd != VK_NULL_HANDLE)
                    renderer.renderSingleView(*item.scene, *item.view, binding, st.rt,
...
Expected<void> FrameOrchestrator::endRenderFrame(
...
)
{
    ...
    // 目标池按**栅栏证实**的完成水位老化回收。
    const uint64_t gpu_completed = driver ? driver->gpuCompletedSerial()
                                          : current_stamp_.serial;
    for (auto& t : targets.all().values())
        if (t.pool)
