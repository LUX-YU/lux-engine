...
namespace
{
    [[nodiscard]] lux::render::CapacityDomainId own(lux::render::CapacityDomainIdView id)
    {
        return lux::render::CapacityDomainId{id.name()};
    }

    [[nodiscard]] lux::render::CapacityCatalog makeCatalog(std::uint64_t instance_device_limit = 100'000u)
    {
...
main()
{
    ...
    exact.set(kActiveInstancesCapacity, CapacityValue::exact(100'000u));
    exact.set(kClassicMeshRecordsCapacity, CapacityValue::exact(100'000u));
    ...
    assert(invalid_domain.error() == CapacityCatalogError::INVALID_DESCRIPTOR);

    auto duplicate_catalog = makeCatalog();
    const auto duplicate_domain = duplicate_catalog.add(CapacityDomainDescriptor{
        .id = own(kActiveInstancesCapacity),
    ...
    assert(duplicate_domain.error() == CapacityCatalogError::DUPLICATE_DOMAIN);
    ...
    assert(exact_plan->find(kActiveInstancesCapacity)->reason == CapacityPlanReason::REQUESTED);

    const auto device_catalog = makeCatalog(150'000u);
    auto device_limited = exact;
    device_limited.set(kActiveInstancesCapacity, CapacityValue::exact(200'000u)
    );
    const auto device_failure = makeCapacityPlan(device_limited, device, device_catalog);
    assert(!device_failure);
    assert(device_failure.error().domain.view() == kActiveInstancesCapacity);
    assert(device_failure.error().reason == CapacityPlanReason::DEVICE_CLAMP);
    ...
    protocol_limited.set(kActiveInstancesCapacity, CapacityValue::exact((1u << 20u) + 1u));
    const auto protocol_failure = makeCapacityPlan(protocol_limited, device, protocol_catalog);
    assert(!protocol_failure);
    assert(protocol_failure.error().reason == CapacityPlanReason::PROTOCOL_CLAMP);
    assert(protocol_failure.error().effective == (1u << 20u));

    auto small_budget = device;
    small_budget.vram_budget_bytes = 10ull << 20u;
    const auto budget_failure = makeCapacityPlan(exact, small_budget, catalog);
    assert(!budget_failure);
    assert(budget_failure.error().reason == CapacityPlanReason::BUDGET_REJECT);
    assert(budget_failure.error().bytes > budget_failure.error().available_bytes);
    ...
    assert(budget_wire.available_bytes == budget_failure.error().available_bytes);
    ...
    const auto auto_plan = makeCapacityPlan(automatic, device, narrow_catalog);
    assert(auto_plan);
    assert(auto_plan->effective(kActiveInstancesCapacity) == 1'000u
    );
    assert(auto_plan->find(kActiveInstancesCapacity)->reason == CapacityPlanReason::DEVICE_CLAMP);

    auto budget_clamped_device = device;
    budget_clamped_device.vram_budget_bytes = 40ull << 20u;
    const auto budget_clamped = makeCapacityPlan(automatic, budget_clamped_device, catalog);
    assert(budget_clamped);
    assert(budget_clamped->find(kActiveInstancesCapacity)->reason == CapacityPlanReason::BUDGET_REJECT);
    assert(budget_clamped->find(kClassicMeshRecordsCapacity)->reason == CapacityPlanReason::BUDGET_REJECT);
    std::uint64_t admitted_bytes = 0u;
    for (const auto& domain : budget_clamped->domains)
        admitted_bytes += domain.estimated_bytes;
    assert(admitted_bytes <= budget_clamped_device.vram_budget_bytes - budget_clamped_device.vram_budget_bytes / 10u);

    auto exhausted_device = device;
    exhausted_device.vram_budget_bytes = 1u;
    exhausted_device.vram_usage_bytes = 1u;
    const auto exhausted = makeCapacityPlan(automatic, exhausted_device, catalog);
    assert(!exhausted);
    ...
    unknown.set(capacityId("lux.test.unknown"), CapacityValue::exact(1u));
    const auto unknown_result = makeCapacityPlan(unknown, device, catalog);
    assert(!unknown_result);
    assert(unknown_result.error().error == CapacityPlanError::UNKNOWN_DOMAIN);
