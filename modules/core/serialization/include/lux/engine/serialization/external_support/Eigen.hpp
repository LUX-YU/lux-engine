...
namespace lux::serialization
{
    ...
    struct Serializer<Eigen::Matrix<Scalar, Rows, Columns, Options, MaxRows, MaxColumns>>
    {
        ...
        write(Writer& writer, const Matrix& value, const SerializationContext& context) noexcept
        {
            ...
            for (Eigen::Index row{}; result && row < value.rows(); ++row)
            {
                ...
                {
                    result = serialization::write(writer, value(row, column), context);
                }
            }
            return result;
        }
        ...
        read(Reader& reader, Matrix& value, const SerializationContext& context) noexcept
        {
            ...
            for (Eigen::Index row{}; result && row < value.rows(); ++row)
            {
                for (Eigen::Index column{}; result && column < value.cols(); ++column)
                {
                    result = serialization::read(reader, value(row, column), context);
                }
            }
            return result;
        }
    ...
    template <class Scalar, int Options> struct Serializer<Eigen::Quaternion<Scalar, Options>>
    {
        ...
        write(Writer& writer, const Quaternion& value, const SerializationContext& context) noexcept
        {
            ...
            if (result)
                result = serialization::write(writer, value.y(), context);
            if (result)
                result = serialization::write(writer, value.z(), context);
            if (result)
                result = serialization::write(writer, value.w(), context);
            return result;
        }
        ...
        read(Reader& reader, Quaternion& value, const SerializationContext& context) noexcept
        {
            ...
            Scalar z{};
            Scalar w{};
            SerializationResult result = serialization::read(reader, x, context);
            if (result)
                result = serialization::read(reader, y, context);
            if (result)
                result = serialization::read(reader, z, context);
            if (result)
                result = serialization::read(reader, w, context);
            if (result)
            {
