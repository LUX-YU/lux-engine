...
namespace lux::gapi::vk
{
	...
	public:
		...
		[[nodiscard]] static SwapchainBuildResult create(
			...
			PFN_vkDestroySwapchainKHR destroy_swapchain =
				&vkDestroySwapchainKHR) noexcept;
		...
		Swapchain(Swapchain&& other) noexcept
			...
			, destroy_swapchain_(
				std::exchange(other.destroy_swapchain_, nullptr))
		...
		Swapchain& operator=(Swapchain&& other) noexcept
		{
			...
			device_ = std::exchange(other.device_, VkDevice{});
			allocator_ = std::exchange(other.allocator_, nullptr);
			destroy_swapchain_ = std::exchange(
		...
		[[nodiscard]] SwapchainImagesResult images(
			PFN_vkGetSwapchainImagesKHR get_swapchain_images =
				&vkGetSwapchainImagesKHR) const noexcept;

		inline VkSwapchainKHR handle() const noexcept { return swapchain_; }
		inline const VkSwapchainKHR* handlePtr() const noexcept { return &swapchain_; }
	...
	private:
		explicit Swapchain(
			...
			, destroy_swapchain_(destroy_swapchain)
			, swapchain_(handle)
		{}
		...
		PFN_vkDestroySwapchainKHR destroy_swapchain) noexcept
	{
		...
		const VkResult result = create_swapchain(
			device,
			&info,
			allocator,
			&handle
	...
	inline SwapchainImagesResult Swapchain::images(
		PFN_vkGetSwapchainImagesKHR get_swapchain_images) const noexcept
	{
		...
		for (std::uint32_t attempt = 0; attempt < 3; ++attempt)
		{
			...
			const VkResult count_result = get_swapchain_images(
				device_,
				swapchain_,
				&image_count,
				nullptr
			...
			const VkResult values_result = get_swapchain_images(
				device_,
				swapchain_,
				&image_count,
				raw_images.data()
	...
	public:
		SwapchainBuilder(const PhysicalDevice& physical_device, const Surface& surface)
			: SwapchainBuilder(
				...
				surface.handle(),
				SwapchainBuildOps::defaults())
		{}

		SwapchainBuilder(
			VkPhysicalDevice physical_device,
			VkSurfaceKHR surface,
			SwapchainBuildOps ops) noexcept
		...
		{
			auto support = detail::querySwapChainSupport(
				physical_device_,
				surface_handle_,
				ops_.surface_queries
			);
			if (!support)
			...
			if (create_info.imageExtent.width == 0
				|| create_info.imageExtent.height == 0
				|| queue_family_index[0] == UINT32_MAX
			...
			create_info.presentMode = choosePresentMode(
				support_details.presentModes,
				vsync_enabled_
			);
			if (!format_explicit_)
			{
				const auto format = chooseSwapSurfaceFormat(
			...
			return Swapchain::create(
				...
				create_info,
				allocator,
				ops_.create_swapchain,
				ops_.destroy_swapchain
			);
		}
		...
		bool presentScalingActive() const noexcept { return scaling_active_; }
