...
std::vector<Sample> sample(std::string_view name, std::size_t listeners,
                           Callable &&callable) {
  ...
  for (std::size_t index = 0; index < kSampleCount; ++index) {
    ...
    const auto inline_before = lux::object::detail::ObjectDiagnosticsAccess::
        inlineMessageStorageCount();
    const auto heap_before =
        lux::object::detail::ObjectDiagnosticsAccess::heapMessageStorageCount();
    ...
    const auto inline_delta = lux::object::detail::ObjectDiagnosticsAccess::
                                  inlineMessageStorageCount() -
                              inline_before;
    const auto heap_delta = lux::object::detail::ObjectDiagnosticsAccess::
                                heapMessageStorageCount() -
                            heap_before;
    samples.push_back({elapsed, allocation_delta, inline_delta, heap_delta});
    std::cout << "raw," << name << ',' << listeners << ',' << index << ','
              << elapsed << ',' << allocation_delta << ',' << inline_delta
              << ',' << heap_delta << '\n';
  }
  return samples;
}

void summarize(std::string_view name, std::size_t listeners,
               std::vector<Sample> samples, std::size_t bytes_per_connection) {
  std::ranges::sort(samples, {},
                    [](const Sample &value) { return value.elapsed_ns; });
  const auto median = (samples[kSampleCount / 2 - 1].elapsed_ns +
                       samples[kSampleCount / 2].elapsed_ns) /
                      2;
  constexpr std::size_t p95_index = (kSampleCount * 95 + 99) / 100 - 1;
  const auto allocation_total =
      std::accumulate(samples.begin(), samples.end(), std::size_t{0},
  ...
  std::cout << "summary," << name << ',' << listeners << ',' << median << ','
            ...
            << bytes_per_connection << ',' << inline_total << ',' << heap_total
            << '\n';
}

template <class Callable>
void benchmark(std::string_view name, std::size_t listeners,
               std::size_t bytes_per_connection, Callable &&callable) {
  summarize(name, listeners,
...
void benchmarkTypedMember(std::size_t listener_count) {
  ...
  for (std::size_t index = 0; index < listener_count; ++index) {
    auto connection = sender.observe<IntSender::changed, &IntReceiver::receive,
  ...
  const auto storage_growth_before =
      lux::object::detail::ObjectDiagnosticsAccess::storageGrowthCount(sender);
  benchmark("object_typed_member", listener_count, kProductionConnectionBytes,
            [&] {
              for (std::size_t index = 0; index < iterations; ++index)
                sender.publish(1);
            });
  LUX_CHECK(lux::object::detail::ObjectDiagnosticsAccess::storageGrowthCount(
                sender) == storage_growth_before);
  LUX_CHECK(receiver.observed ==
...
void benchmarkQueued(std::size_t listener_count) {
  ...
  connections.reserve(listener_count);
  for (std::size_t index = 0; index < listener_count; ++index) {
    auto connection = sender.observe<IntSender::changed, &Receiver::receive,
...
void benchmarkQueuedPayload(std::string_view name, std::size_t listener_count) {
  ...
  for (std::size_t index = 0; index < listener_count; ++index) {
    auto connection =
        sender.observe<Signal, Method, lux::object::EDelivery::QUEUED>(
            receiver);
    LUX_CHECK(connection);
    connections.push_back(std::move(*connection));
  }
...
class PostContentionReceiver final
    : public lux::object::Object<PostContentionReceiver> {
public:
  explicit PostContentionReceiver(
      lux::object::ObjectDispatcherRef dispatcher) noexcept
      : Object(std::move(dispatcher)) {}
...
void benchmarkPostContention(std::size_t producer_count) {
    ...
    producers.emplace_back([&] {
      for (;;) {
        ...
        for (std::size_t index = 0; index < posts_per_producer; ++index) {
          LUX_CHECK(lux::object::postEvent(target, PostContentionEvent{}) ==
  ...
  for (auto &producer : producers)
    producer.join();
  LUX_CHECK(receiver.observed == producer_count * posts_per_producer *
                                     (kWarmupCount + kSampleCount));
}
...
void benchmarkChurn() {
  ...
  benchmark("object_churn", iterations, kProductionConnectionBytes, [&] {
    for (std::size_t index = 0; index < iterations; ++index) {
      auto connection =
          sender.observeScoped<IntSender::changed>([](const int &) noexcept {});
...
void benchmarkDynamicReflection() {
  ...
  const auto signal =
      lux::object::reflection::findSignal(registry, *sender_class, "changed");
  LUX_CHECK(signal);
  ...
  auto connection = lux::object::reflection::observe(
      sender, signal, receiver, *method, lux::object::EDelivery::DIRECT);
  LUX_CHECK(connection);
  constexpr std::size_t iterations = 20'000;
  const auto storage_growth_before =
      lux::object::detail::ObjectDiagnosticsAccess::storageGrowthCount(sender);
...
void benchmarkLayout(std::string_view name, std::size_t listener_count) {
  ...
  constexpr std::size_t iterations = 20'000;
  constexpr auto bytes_per_connection = std::is_same_v<Candidate, CandidateA>
                                            ? sizeof(ListenerControl *)
...
int main() {
  ...
  std::cout << "meta,candidate_b_extra_bytes_per_connection,"
            ...
            << '\n';
  std::cout
      << "record,case,listeners_or_count,sample_or_median,elapsed_or_p95,"
  ...
  for (const auto listeners : std::array<std::size_t, 3>{1, 4, 16}) {
    benchmarkQueuedPayload<QueuedPayloadSender::payload4,
                           static_cast<void (QueuedPayloadReceiver::*)(
                               const QueuedPayload4 &) noexcept>(
                               &QueuedPayloadReceiver::receive),
                           QueuedPayload4>("object_queued_payload_4",
                                           listeners);
    benchmarkQueuedPayload<QueuedPayloadSender::payload64,
                           static_cast<void (QueuedPayloadReceiver::*)(
                               const QueuedPayload64 &) noexcept>(
                               &QueuedPayloadReceiver::receive),
                           QueuedPayload64>("object_queued_payload_64",
                                            listeners);
    benchmarkQueuedPayload<QueuedPayloadSender::payload256,
                           static_cast<void (QueuedPayloadReceiver::*)(
                               const QueuedPayload256 &) noexcept>(
                               &QueuedPayloadReceiver::receive),
                           QueuedPayload256>("object_queued_payload_256",
                                             listeners);
    benchmarkQueuedPayload<QueuedPayloadSender::payload1024,
                           static_cast<void (QueuedPayloadReceiver::*)(
                               const QueuedPayload1024 &) noexcept>(
                               &QueuedPayloadReceiver::receive),
                           QueuedPayload1024>("object_queued_payload_1024",
                                              listeners);
  }
