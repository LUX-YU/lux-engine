...
namespace lux::render
{
	...
	Expected<void> RGVulkanResourceAllocator::createImage(const RGResourceDescription& resource, RGPhysicalResource& phy, VkExtent2D extent, uint32_t frames_in_flight)
	{
		...
		constexpr VkImageUsageFlags kAttachmentOnlyUsage =
			VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT |
			VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT |
			VK_IMAGE_USAGE_INPUT_ATTACHMENT_BIT;
		const bool lazy_attachment =
			phy.lifetime == ERGResourceLifetime::TRANSIENT &&
			image_info.usage != 0 &&
		...
		if (phy.lifetime == ERGResourceLifetime::TRANSIENT)
		{
			if (lazy_attachment && lazy_memory_ == ELazyMemory::Unprobed)
			{
				...
				const VkResult found = vmaFindMemoryTypeIndexForImageInfo(
					context_.vmaAllocator(), &image_info, &probe, &lazy_type);

				lazy_memory_ = (found == VK_SUCCESS) ? ELazyMemory::Available
				                                     : ELazyMemory::Absent;
				if (found == VK_SUCCESS)
				{
