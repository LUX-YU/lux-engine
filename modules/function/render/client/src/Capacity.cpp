...
namespace lux::render
{
    namespace
    {
        ...
        [[nodiscard]] bool isValidCapacityDomainNameImpl(std::string_view name) noexcept
        {
        ...
        [[nodiscard]] bool isCapacityDomainCollision(CapacityDomainIdView left, CapacityDomainIdView right) noexcept
        {
            return left.hash() == right.hash() && left.name() != right.name();
        }

        [[nodiscard]] constexpr std::uint64_t saturatedAdd(std::uint64_t left, std::uint64_t right) noexcept
        {
            return left > std::numeric_limits<std::uint64_t>::max() - right ? std::numeric_limits<std::uint64_t>::max()
                                                                            : left + right;
        }

        [[nodiscard]] constexpr std::uint64_t saturatedMultiply(std::uint64_t left, std::uint64_t right) noexcept
        {
        ...
        [[nodiscard]] constexpr std::uint64_t availableBudget(const CapacityDeviceFacts& device) noexcept
        {
        ...
        estimateBytes(std::uint64_t units, const CapacityDomainDescriptor& descriptor) noexcept
        {
            if (units == 0u || descriptor.bytes_per_granule == 0u)
                return 0u;
            const auto granules = saturatedAdd(units, descriptor.units_per_granule - 1u) / descriptor.units_per_granule;
            return saturatedMultiply(granules, descriptor.bytes_per_granule);
        }
        ...
        {
            return CapacityShortfall{own(domain), error, reason, requested, effective, bytes, available};
        }
    } // namespace
    ...
    void CapacityRequest::set(CapacityDomainIdView domain, CapacityValue value)
    {
    ...
    const CapacityValue* CapacityRequest::find(CapacityDomainIdView domain) const noexcept
    {
    ...
    const CapacityDomainDescriptor* CapacityCatalog::find(CapacityDomainIdView id) const noexcept
    {
    ...
    const CapacityDomainPlan* CapacityPlan::find(CapacityDomainIdView domain) const noexcept
    {
        for (const auto& entry : domains)
            if (entry.domain.view() == domain)
                return &entry;
    ...
    std::uint64_t CapacityPlan::effective(CapacityDomainIdView domain) const noexcept
    {
    ...
    makeCapacityPlan(const CapacityRequest& request, const CapacityDeviceFacts& device, const CapacityCatalog& catalog)
    {
        ...
        for (const auto* descriptor : ordered)
        {
            ...
            {
                plan.effective =
                    std::min({descriptor->automatic_target, descriptor->device_limit, descriptor->protocol_limit});
        ...
        const long double ratio = automatic_bytes == minimum_bytes
                                      ? 0.0L
                                      : static_cast<long double>(automatic_available - minimum_bytes) /
        ...
        while (automatic_bytes > automatic_available)
        {
            ...
            for (std::size_t index = 0u; index < result.domains.size(); ++index)
            {
                ...
                const auto next = std::max(
                    ...
                    ((plan.effective - 1u) / ordered[index]->units_per_granule) * ordered[index]->units_per_granule
                );
