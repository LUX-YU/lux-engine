...
namespace lux::render
{
	...
	bool MaterialResources::init(const InitInfo& info)
	{
		...
		graph_config.buffer_usage = VK_BUFFER_USAGE_STORAGE_BUFFER_BIT |
	...
	void MaterialResources::writeDescriptorsOnSet(uint32_t set_index) const
	{
		...
		for (const auto& t : domain_targets_)
		{
			...
			unlit_ssbo_.writeDescriptor(
				dds, at(static_cast<uint32_t>(ELightingTechnique::Unlit)));
			legacy_lit_ssbo_.writeDescriptor(
				dds, at(static_cast<uint32_t>(ELightingTechnique::LegacyLit)));
			pbr_ssbo_.writeDescriptor(
				dds, at(static_cast<uint32_t>(ELightingTechnique::PbrMetallicRoughness)));
			stylized_ssbo_.writeDescriptor(
				dds, at(static_cast<uint32_t>(ELightingTechnique::Stylized)));
			graph_ssbo_.writeDescriptor(
				dds, at(static_cast<uint32_t>(ELightingTechnique::Graph)));
		}
	}

	Expected<void> MaterialResources::addDomainWriteTarget(const void* owner,
	...
	{
		std::erase_if(domain_targets_,
		              [owner](const DomainTarget& t) { return t.owner == owner; });
	}
	...
	void MaterialResources::packGraphGpu(
	...
	bool MaterialResources::retainForInstance(MaterialHandle slot) noexcept
	{
		if (!slot_records_.find(slot) ||
			slot.index >= destroy_requested_.size() ||
	...
	{
		if (!slot_records_.find(slot) ||
			slot.index >= instance_refcounts_.size() ||
    ...
    Expected<void> ensureGlobalMaterialResources(RenderContext& ctx)
    {
        ...
        greg.addBeginFrameHook(EUploadPhase::Upload,
                               [mat](const FrameStamp& s) { mat->onFrameBeginMaintenance(s); });
