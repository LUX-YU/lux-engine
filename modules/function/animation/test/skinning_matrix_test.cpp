...
static bool approxEq(const Eigen::Matrix4f& a, const Eigen::Matrix4f& b, float eps = 1e-4f) {
    return (a - b).cwiseAbs().maxCoeff() <= eps;
}

static void test_identity_pose_gives_identity_matrices()
{
    ...
    for (int i = 0; i < 3; ++i) {
        REQUIRE(approxEq(mats[i], Eigen::Matrix4f::Identity()),
                "identity-pose + identity-invbind → identity matrix");
    }
}

static void test_two_bone_chain_root_translate_child_rotate()
{
    ...
    pose.bone_local[1] = Eigen::Affine3f(Eigen::AngleAxisf(static_cast<float>(M_PI) * 0.5f,
    ...
    Eigen::Matrix4f expect1 =
        (Eigen::Affine3f(Eigen::Translation3f(1, 0, 0)) *
         Eigen::Affine3f(Eigen::AngleAxisf(static_cast<float>(M_PI) * 0.5f,
...
static void test_inv_bind_world_application()
{
    ...
    REQUIRE(approxEq(mats[0], Eigen::Matrix4f::Identity()),
            "pose==bind → skin matrix is identity (vertex undeformed)");
}

static void test_pose_skeleton_size_mismatch_safe()
{
    ...
    REQUIRE(approxEq(mats[0], Eigen::Matrix4f::Identity()),
            "mismatch → identity fallback (no NaN)");
    REQUIRE(approxEq(mats[1], Eigen::Matrix4f::Identity()),
            "mismatch → identity fallback (no NaN)");
}

static void test_global_transform_applies_to_root_and_propagates()
{
...
static void test_global_transform_identity_is_noop()
{
