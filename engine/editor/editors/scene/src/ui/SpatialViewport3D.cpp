...
{
    return lux::cxx::unexpected(EditorFailure{EEditorError::INVALID_ARGUMENT, "viewport.3d", 0, std::string(reason)});
}
...
class SpatialViewport3D final : public SpatialViewport
{
  public:
                                ...
                                const CameraMotion &motion) override
    {
        ...
        const double yaw = std::remainder(std::atan2(old_forward.x(), -old_forward.z()) + motion.angular_delta.x(),
                                          2.0 * std::numbers::pi);
        const double pitch =
                                       ...
                                       Eigen::Vector2d point, Eigen::Vector2d extent) const override
    {
        ...
        const auto *pose =
            static_cast<const WorldTransform3D *>(editor.component(entity, lux::cxx::typeToken<WorldTransform3D>()));
        const auto *camera = static_cast<const lux::scene::Camera *>(
            editor.component(entity, lux::cxx::typeToken<lux::scene::Camera>()));
    ...
    EditorResult<Eigen::Vector3d> creationPoint(const scene::SceneEditor &editor, lux::scene::SceneInstanceId instance,
                                                const lux::math::Ray3d &ray, double height) const override
    {
...
{
    return {"3D",
            +[](const scene::SceneEditor &editor) { return editor.supportsObjectSpace(scene::EObjectSpace::SPACE_3D); },
