 ...
 * only render/include + the external SDKs. If it compiles, an outside author can
 ...
 *       · loadShader / shaderModule / shaderInfo   (its OWN SPIR-V, not a builtin)
 ...
 * this factory through the public comm API (RenderSession::registerFeatureType +
 * addFeature).
 */
...
#include <lux/engine/render/graph/RGEnums.hpp>                          // ERGPassType / ERenderStage
#include <lux/engine/render/graph/PassRecordContext.hpp>                // PassRecordContext (kernel ctx)
#include <lux/engine/render/pipeline/GraphicsPipelineTemplate.hpp>      // GraphicsPipelineTemplate
#include <lux/engine/render/resources/lifecycle/ResourceRegistry.hpp>   // sceneRegistry().ensure<T>()
...
#include <lux/engine/render/comm/RenderProtocol.hpp>                    // FeatureFactory / makeSimpleFactory
#include <lux/engine/description/ShaderInfo.hpp>                        // lux::rdesc::ShaderInfo (loadShader)
...
namespace sample_ext
{
    ...
    public:
        ...
        void initAndAttachTo(RenderScene & /*scene*/) override
        {
            ...
            const ShaderHandle vsh = cv.loadShader(cfg_.vertex_spirv, vs_info);
            const ShaderHandle fsh = cv.loadShader(cfg_.fragment_spirv, fs_info);
    ...
    inline uint32_t sampleCreateFn(void* scene, const void* param, std::size_t param_size)
    {
        SampleExternalFeature::Config cfg{};
        // A real plugin would deserialize its Config from (param, param_size);
        // this sample keeps the defaults.
        (void)param;
        (void)param_size;
        return lux::render::addFeature<SampleExternalFeature>(scene, std::move(cfg));
    }

    /// The factory a client registers via RenderSession::registerFeatureType().
