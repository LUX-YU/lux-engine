...
extern "C" void lux_test_sink_float(float v)
{
    g_float_value = v;
}
...
static void test_mixed_type_promotion()
{
    ...
    LastLink ll;
    check(
        const_cast<DataOutPin&>(iadd->result()).linkTo(const_cast<DataInPin*>(&fadd->lhs()), ll) == ELinkError::SUCCESS,
...
static void test_unsigned_semantics()
{
    ...
    const_cast<DataInPin&>(udiv->lhs()).setConstantData(lux::meta::RuntimeObject(uint32_t{0xFFFFFFFAu}));
    const_cast<DataInPin&>(udiv->rhs()).setConstantData(lux::meta::RuntimeObject(uint32_t{2u}));
    ...
    check(static_cast<uint32_t>(g_int_value) == 0x7FFFFFFDu, "unsigned: division used udiv, not sdiv");
}
...
static void test_pure_cycle_error()
{
    ...
    if (!built)
    {
        ...
        check(built.error().message.find("cycle") != std::string::npos, "cycle: message mentions the cycle");
    }
}
