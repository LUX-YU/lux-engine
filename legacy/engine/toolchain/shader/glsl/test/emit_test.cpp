// =============================================================================
//  emit_test.cpp  --  shadergen_glsl engine-half self-check (exit 0 = pass)
// -----------------------------------------------------------------------------
//  rdesc::MaterialGraph -> (lowerMaterial) -> ShaderIR -> (emitGlsl/compileToSpirv)
//  -> GLSL/SPIR-V. Verifies what's currently supported: GBuffer (PBR/Unlit) +
//  Forward Unlit; the shell #includes the real single-source-of-truth files
//  (resolved via IncluderInterface, not inlined); anything beyond that scope
//  fails explicitly.
//  Requires libshaderc (pulled in transitively via linking shadergen_glsl).
//  LUX_RENDER_SHADERS_DIR is injected by CMake.
// =============================================================================
...
// Simplest possible PBR material: constant Vec3 -> base_color (everything else defaulted).
...
int main()
{
    ...
    // 1. emitGlsl GBuffer PBR: structural check (#include real SSOT, packGNormalSM, 3 MRT).
    {
    ...
    // 2. compileToSpirv GBuffer PBR: Includer resolves the SSOT -> valid SPIR-V (the core case).
    {
        rdesc::MaterialGraph g = makeConstGraph(rdesc::EMaterialShadingModel::PbrMetallicRoughness);
        auto lr = sgm::lowerMaterial(g);
        sgg::EmitParams p; p.pass = rdesc::EMaterialPass::GBuffer; p.shading_model = lr->shading_model;
    ...
    // 3. GBuffer Unlit.
    {
        ...
        CHECK(contains(glsl, "layout(location = 7) in flat float vTransitionCoverage;") &&
                  contains(glsl, "layout(location = 8) in flat uint vTransitionSeed;") &&
                  contains(glsl, "layout(location = 9) in flat uint vTransitionFadeOut;"),
              "3 GBuffer 图材质消费固定 transition interface");
        CHECK(contains(glsl, "#include \"transition_dither.glsl\"") &&
                  contains(glsl, "luxApplyDirectedTransitionCoverage("),
              "3 GBuffer 图材质执行 directed transition dither");
        auto cs = sgg::compileToSpirv(lr->ir, p, inc);
    ...
    // 4. Forward Unlit.
    {
        ...
        CHECK(contains(glsl, "layout(location = 7) in flat float vTransitionCoverage;") &&
                  contains(glsl, "luxApplyDirectedTransitionCoverage("),
              "4 Forward Unlit 图材质消费 transition interface");
        auto cs = sgg::compileToSpirv(lr->ir, p, inc);
        CHECK(cs.has_value() && !cs->spirv.empty(), "4 Forward Unlit 编译 valid SPIR-V");
    }

    // 5. Forward PBR: the GGX lighting shell. #include lighting_common (the
    //    aggregated SSOT) + shadow_pcf + brdf_ggx; the emitter does not declare
    //    its own input (reuses the SSOT -- this locks down the asymmetry
    //    between the forward and gbuffer shells).
    {
        rdesc::MaterialGraph g = makeConstGraph(rdesc::EMaterialShadingModel::PbrMetallicRoughness);
        auto lr = sgm::lowerMaterial(g);
        ...
        CHECK(contains(glsl, "luxApplyDirectedTransitionCoverage("), "5 forward PBR 执行 directed transition dither");
    ...
    // 5b. Forward PBR + textures: the set-4 Graph SSBO is self-declared, but
    //     uTex/material_types come from lighting_common (not re-declared --
    //     this locks down "reuse the SSOT rather than self-declaring").
    {
        rdesc::MaterialGraph g;
        ...
        CHECK(contains(glsl, "luxSampleTexture(_mat.tex[0]"), "5b representation-aware 采样表达式");
        auto cs = sgg::compileToSpirv(lr->ir, p, inc);
        if (!cs) std::printf("    compile err: %s\n", cs.error().c_str());
        CHECK(cs.has_value() && !cs->spirv.empty(), "5b textured forward PBR 编译 valid SPIR-V");
    }

    // 6. textures: SampleTexture -> set-2 bindless uTex + set-4 GraphMaterialGPU
    //    SSBO, with the family struct resolved via #include material_types.glsl
    //    (another spot where hand-inlining is eliminated).
    {
        rdesc::MaterialGraph g;
        g.texture_slots.push_back({ "albedo" });
        auto out = g.addNode(std::make_unique<rdesc::OutputSurfaceNode>());
        ...
        g.connect(si, 0, out, 0);  // sample(vec4) -> base_color(vec3), implicitly narrowed
        auto lr = sgm::lowerMaterial(g);
        ...
        CHECK(contains(glsl, "#include \"texture_sampling.glsl\""), "6 复用 TextureRefGPU 采样 SSOT");
        ...
        CHECK(contains(glsl, "luxSampleTexture(_mat.tex[0]"), "6 representation-aware 采样表达式");
        auto cs = sgg::compileToSpirv(lr->ir, p, inc);
        if (!cs) std::printf("    err: %s\n", cs.error().c_str());
    ...
    // 7. Proves the Includer is actually doing something: empty include_dirs ->
    //    #include resolution fails -> compilation fails.
    //    (If the shell were hand-inlined instead of using #include, this case
    //    would "pass" -- so it locks down "actually uses the SSOT".)
    {
        rdesc::MaterialGraph g = makeConstGraph(rdesc::EMaterialShadingModel::PbrMetallicRoughness);
        auto lr = sgm::lowerMaterial(g);
        sgg::EmitParams p; p.pass = rdesc::EMaterialPass::GBuffer; p.shading_model = lr->shading_model;
        auto cs = sgg::compileToSpirv(lr->ir, p, {});  // empty search path
        CHECK(!cs, "7 空 include_dirs -> 编译失败（证明真依赖 Includer 解析 SSOT，非内联）");
    }

    // 8. params: Param -> set-4 GraphMaterialGPU SSBO (binding 4), no textures (no set-2).
    {
        rdesc::MaterialGraph g;
    ...
    // 9. Normal-map chain + TbnNormal: SampleTexture -> DecodeNormal ->
    //    TbnTransform -> normal_ts. Transforms the tangent-space normal into
    //    world space using the T/B/N interpolants (tangent@5 vec3 + bitangent@6).
    {
        rdesc::MaterialGraph g;
    ...
    // 11. Forward Stylized (cel/toon): #include brdf_toon + toonStep, does not go through GGX; reuses lighting_common.
    {
