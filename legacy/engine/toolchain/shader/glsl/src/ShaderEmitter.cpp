...
namespace lux::shadergen::glsl
{
    ...
    emitGlsl(const ShaderIR& ir, const EmitParams& p)
    {
        ...
        out << "#version 450\n";
        if (em.tex_mode || forward_lit)   // forward's lighting_common always needs this extension for the unbounded uTex[]
        ...
        else  // GBuffer / Forward-Unlit: declares its own input + out + resource blocks.
        {
            ...
            if (gbuffer)
                out << "#include \"gbuffer_encode.glsl\"\n";   // The real single-source-of-truth, resolved by the Includer (not hand-copied inline)
        }
        ...
        else if (forward_toon)  // Forward Stylized (cel/toon): quantized diffuse + spec band, replicating the old emitter.
        {
