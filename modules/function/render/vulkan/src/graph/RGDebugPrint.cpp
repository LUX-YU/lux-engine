...
namespace lux::render
{
    ...
    static const char* electiveKindToString(EElectiveKind k)
    {
        ...
        case EElectiveKind::CONDITIONAL_CHAIN:   return "CONDITIONAL_CHAIN";
        default:                                  return "UNKNOWN";
        }
    }
    ...
    void printCompiledGraph(const RGCompiledGraph& compiled, std::ostream& os)
    {
        ...
        if (compiled.mesh_bucket_layout)
        {
            ...
            os << "[Mesh Bucket Layout] lanes=" << mesh.lanes.size()
               << "  indirect_bytes=" << mesh.total_indirect_size
               << "  count_bytes=" << mesh.total_count_size << "\n";
            for (const auto& lane : mesh.lanes)
            {
                os << "  pass=" << lane.pass_index
                   << " mdc=" << lane.mdc_index
                   << " bucket=" << lane.bucket_id
                   << " geometry=" << static_cast<unsigned>(lane.geometry_kind)
                   << " pipeline=0x" << std::hex
                   << reinterpret_cast<std::uintptr_t>(lane.pipeline)
                   << std::dec
                   << " indirect_offset=" << lane.indirect_offset
                   << " count_offset=" << lane.count_offset
        ...
        if (compiled.layout_plan)
        {
            ...
            os << "[Descriptor Layout Plan]  sets=" << plan.sets.size()
               << "  hash=0x" << std::hex << plan.plan_hash << std::dec
               << "  relaid=" << plan.templates_relaid << "\n";
            ...
            os << "  最宽管线 set 数:现状 " << plan.max_sets_current
               << " → 域合并后 " << plan.max_sets_projected
               << "(乐观) / " << plan.max_sets_projected_strict
               << "(保守:feature 自有 set 不并入)\n";
            ...
            if (plan.worst_stage_name)
            {
                ...
                os << "  最重 stage(" << plan.worst_stage_name << ",非 UAB):"
                   << "ubo=" << u.uniform_buffer
                   << " ssbo=" << u.storage_buffer
                   << " img=" << u.sampled_image
                   << " simg=" << u.storage_image
                   << " 合计=" << u.total << "\n";
            }
            if (plan.worst_stage_merged_name)
            {
                ...
                os << "  合并后最重 stage(" << plan.worst_stage_merged_name << "):"
                   << "ubo=" << m.uniform_buffer
                   << " ssbo=" << m.storage_buffer
                   << " img=" << m.sampled_image
                   << " simg=" << m.storage_image
                   << " 合计=" << m.total << "  ← 合并的代价\n";
            }
            ...
            for (const auto& s : plan.sets)
            {
                ...
                os << "      set" << s.set << " → set" << dst_set
                   << "  binding +" << s.domain_binding_offset
            ...
            for (const auto& s : plan.sets)
            {
                ...
                switch (s.kind)
                {
                case EPlannedSetKind::EngineShared:    kind = "engine ";  break;
                case EPlannedSetKind::FeatureExplicit: kind = "feature";  break;
                case EPlannedSetKind::PipelinePrivate: kind = "private";  break;
                }
                os << "  [" << kind << "] set=" << s.set;
                if (s.owner_key != 0)
                    os << " owner=0x" << std::hex << s.owner_key << std::dec;
                os << "  bindings=" << s.bindings.size()
                   << (s.update_after_bind ? "  UAB" : "");
                if (s.kind == EPlannedSetKind::EngineShared)
                {
                    static constexpr const char* kDomainName[] = {
                        "GLOBAL", "BINDLESS", "FEATURE", "PASS_LOCAL" };
                    const auto d = static_cast<std::size_t>(s.domain);
                    os << "  域=" << (d < 4 ? kDomainName[d] : "?")
                       << " b@[" << s.domain_binding_offset << ",+"
            ...
            if (!plan.pipelines.empty())
            {
                static constexpr const char* kSetName[] = {
                    "Scene", "Instance", "Texture", "Light",
                    "Material", "Particle", "Compute", "VertexPool" };
                constexpr std::size_t kSetNameCount = std::size(kSetName);
                static constexpr const char* kDomainTag[] = {
                    "GLOBAL", "BINDLESS", "FEATURE", "PASS_LOCAL" };
                    ...
                    for (const auto& s : p.slots)
                    {
                        ...
                        else if (s.kind == 'D')
                        {
                            ...
                            for (std::size_t i = 0; i < s.members.size(); ++i)
                                os << (i ? "," : "")
                                   << (s.members[i] < kSetNameCount ? kSetName[s.members[i]] : "?");
                            os << "}";
                        }
                    }
                    os << "\n";
        ...
        for (std::size_t gi = 0; gi < rp_layout.groups.size(); ++gi)
        {
            ...
            auto load_ch  = [](VkAttachmentLoadOp op) {
                return op == VK_ATTACHMENT_LOAD_OP_CLEAR ? 'C'
                     : op == VK_ATTACHMENT_LOAD_OP_LOAD  ? 'L' : 'D';
            };
            auto store_ch = [](VkAttachmentStoreOp op) {
                return op == VK_ATTACHMENT_STORE_OP_STORE ? 'S' : 'D';
            };
                ...
                if (!group.local_read && !p.pass_color_load_ops.empty())
                {
                    ...
                    for (std::size_t s = 0; s < p.pass_color_load_ops.size(); ++s)
                    {
                        os << load_ch(p.pass_color_load_ops[s])
                           << (s < p.pass_color_store_ops.size()
                                ? store_ch(p.pass_color_store_ops[s]) : '?');
                        if (s + 1 < p.pass_color_load_ops.size()) os << ",";
                    }
                    os << " depth=" << load_ch(p.pass_depth_load_op)
                       << store_ch(p.pass_depth_store_op);
                }
                os << "\n";
            }
            ...
            if (group.local_read)
            {
                ...
                for (std::size_t s = 0; s < group.union_color_res.size(); ++s)
                {
                    os << "#" << group.union_color_res[s] << ":"
                       << (s < group.union_color_load_ops.size()
                            ? load_ch(group.union_color_load_ops[s]) : '?')
                       << (s < group.union_color_store_ops.size()
                            ? store_ch(group.union_color_store_ops[s]) : '?');
                    if (s + 1 < group.union_color_res.size()) os << ", ";
                }
                os << "  depth=#" << group.union_depth_res << ":"
                   << load_ch(group.depth_load_op)
