 ...
 * `PointLightSubsystem`(池化槽位形状,LightSubsystems.hpp)bridges it to
 * `LightProxy::createLight` / `updateLight` / `destroyLight`.
 *
 * Unlike `DirectionalLightComponent` (which stores its direction), a point
 * light's **world position is taken from the entity's ResolvedTransform3DComponent**
 ...
 * to carry a ResolvedTransform3DComponent (the editor ensures one for every
