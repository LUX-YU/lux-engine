...
struct ShadowSliceGPU {
    ...
    float normal_bias;       // Ortho world-texel size (perspective: unused; see normal offset)
    ...
    out vec4 geo_clip, out vec4 biased_clip)
{
    ...
    geo_clip = s.light_vp * vec4(world_pos, 1.0);
    ...
    vec3 biased = world_pos + normalize(world_normal) * offset;
...
float sampleShadowPCF(uint slice_index, vec3 world_pos, vec3 world_normal, float normal_scale)
{
    ...
    vec2 rpdb = computeReceiverPlaneDepthBias(vec3(shadow_uv_local, current_depth));
...
{
    ShadowSliceGPU s = uShadowSlices.slices[slice_index];
    vec4 light_clip = s.light_vp * vec4(world_pos, 1.0);
    vec3 ndc = light_clip.xyz / light_clip.w;
...
float spotShadow(uint light_index, vec3 world_pos, vec3 world_normal)
{
    ...
    vec3 L = normalize(uSpotLights.lights[light_index].position - world_pos);
    float ndotl = max(dot(normalize(world_normal), L), 0.0);
...
float pointShadow(uint light_index, vec3 world_pos, vec3 world_normal)
{
    ...
    vec3 lv = world_pos - uPointLights.lights[light_index].position;
    uint face = selectCubeFace(lv);
    uint si   = base + face;

    vec3 L = normalize(uPointLights.lights[light_index].position - world_pos);
    float ndotl = max(dot(normalize(world_normal), L), 0.0);
...
{
    return -(uViews.views[uPC.view_index].view * vec4(world_pos, 1.0)).z;
}
