...
#include <lux/engine/function/render/client/protocol/RenderCommTypes.hpp>       // TypeId, opcodes, CommandTraits, ExternalDataRef, BlobRef
...
namespace lux::render
{
    ...
    struct LUX_OP(lane=upload, kind=resource, name=UploadGraphMaterial, method=uploadGraphMaterial,
                  reply=MaterialUploadedReply, manual_client=true)
    ...
    struct LUX_OP(lane=upload, kind=blob, name=ModifyGraphMaterial, method=modifyGraphMaterial, opcode=resource)
    ...
    struct LUX_COMM_CONFIG(prefix=Material, id=lux.render.material.v1, display=StandardMaterial,
    ...
    [[nodiscard]] LUX_FUNCTION_PUBLIC lux::cxx::expected<
        RenderRequest<MaterialUploadedReply>, ERenderUploadSubmitError>
    uploadGraphMaterial(MaterialUploadClient client, const GraphMaterialData& data);
    [[nodiscard]] LUX_FUNCTION_PUBLIC lux::cxx::expected<
        RenderRequest<MaterialUploadedReply>, ERenderUploadSubmitError>
    uploadGraphMaterial(
        MaterialUploadClient client, const GraphMaterialData& data,
        ShaderHandle gbuffer_shader, ShaderHandle forward_shader,
        std::uint32_t alpha_mode = 0, bool double_sided = false);
    [[nodiscard]] LUX_FUNCTION_PUBLIC lux::cxx::expected<
        void, ERenderUploadSubmitError> modifyGraphMaterial(
