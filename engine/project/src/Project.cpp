...
namespace lux::project
{
    ...
    std::filesystem::path Project::defaultScenePath() const
    {
        if (manifest_.default_scene.empty())
            return {};
        ...
        auto candidate = root_ / std::filesystem::path(manifest_.default_scene);
        std::error_code ec;
    ...
    std::vector<std::filesystem::path> Project::scanSceneFiles() const
    {
        return scanByExtension(scenesRoot(), kSceneExtension);
    }
