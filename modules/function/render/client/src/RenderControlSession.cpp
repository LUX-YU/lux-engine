...
namespace lux::render
{
                                                                                      ...
                                                                                      std::uint32_t flags)
    {
        return recordReply<TargetReadyReply>(
            [&](Builder &builder, auto callback)
            {
                ...
                payload.extent = extent;
                payload.flags = flags;
                builder.pushWithReply(opcodes::CommandOp, type_ids::CreateOffscreenTarget, payload,
                                      std::move(callback));
            });
    }
    ...
    void RenderControlSession::setLayer(RenderTargetId target, std::uint32_t order, RenderSceneId scene,
                                        ViewHandle view)
    {
        (void)record(
            [&](Builder &builder)
    ...
    RenderRequest<TargetResizedReply> RenderControlSession::requestResizeTarget(RenderTargetId target,
                                                                                lux::math::Extent2u extent)
    {
        return recordReply<TargetResizedReply>(
            [&](Builder &builder, auto callback)
            {
    ...
    RenderRequest<GpuTimingReply> RenderControlSession::queryGpuTiming(RenderSceneId scene, void *dst,
                                                                       std::size_t capacity)
    {
    ...
    RenderRequest<FeatureAddedReply> RenderControlSession::addFeatureRaw(RenderSceneId scene,
    ...
    void RenderControlSession::deferRemoveView(RenderSceneId scene, ViewHandle view,
                                               RenderViewReleaseObserver observer) noexcept
    {
    ...
    Expected<bool> RenderControlSession::flushDeferredReleases() noexcept
    {
        ...
        try
        {
            static_assert(std::is_nothrow_move_assignable_v<DeferredView>);
            static_assert(std::is_nothrow_move_assignable_v<DeferredTarget>);
            while (!deferred_views_.empty())
            {
                auto &item = deferred_views_.front();
                auto request = removeView(item.scene, item.view);
                if (request.isReady() && request.failed())
                    return false;
                if (item.observer)
                    request.then(std::move(item.observer));
                // Commit each accepted obligation before preparing the next one. If that
                // preparation fails, retry must never publish this release a second time.
                deferred_views_.erase(deferred_views_.begin());
            }
            while (!deferred_scenes_.empty())
            {
                if (!destroyScene(deferred_scenes_.front()))
                    return false;
                deferred_scenes_.erase(deferred_scenes_.begin());
            }
            while (!deferred_targets_.empty())
            {
                auto &item = deferred_targets_.front();
                auto request = destroyRenderTarget(item.target);
                if (request.isReady() && request.failed())
                    return false;
                if (item.observer)
                    request.then(std::move(item.observer));
                deferred_targets_.erase(deferred_targets_.begin());
            }
        }
