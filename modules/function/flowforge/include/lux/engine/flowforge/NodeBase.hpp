...
namespace lux::flowforge
{
    ...
    enum class ENodeOperation : uint8_t
    {
        ...
        // object
        CREATE_OBJECT,    // 1 exec in, 1 exec out. n data in (constructor arguments). 1 data out (object reference).
        ...
        GET_FIELD,        // 1 exec in, 1 exec out. 1 data in (object reference). 1 data out (field value).
        SET_FIELD,        // 1 exec in, 1 exec out. 1 data in (object reference). 1 data in (set value). 1 data out (object reference).

        // Operations/Mathematical
        ADD,              // 1 exec in, 1 exec out. 2 data in (operands). 1 data out (result).
        SUBTRACT,         // 1 exec in, 1 exec out. 2 data in (operands). 1 data out (result).
        MULTIPLY,         // 1 exec in, 1 exec out. 2 data in (operands). 1 data out (result).
        DIVIDE,           // 1 exec in, 1 exec out. 2 data in (operands). 1 data out (result).
        MODULO,           // 1 exec in, 1 exec out. 2 data in (operands). 1 data out (result).
        LOGICAL_AND,      // 1 exec in, 1 exec out. 2 data in (operands). 1 data out (result).
        LOGICAL_OR,       // 1 exec in, 1 exec out. 2 data in (operands). 1 data out (result).
        LOGICAL_NOT,      // 1 exec in, 1 exec out. 1 data in (operand).  1 data out (result).
     ...
     */
    class LUX_FUNCTION_PUBLIC ExecInPin : public Pin
    {
     ...
     */
    class LUX_FUNCTION_PUBLIC ExecOutPin : public Pin
    {
     ...
     */
    class LUX_FUNCTION_PUBLIC DataInPin : public Pin
    {
     ...
     */
    class LUX_FUNCTION_PUBLIC DataOutPin : public Pin
    {
     ...
     */
    class LUX_FUNCTION_PUBLIC Node
    {
        friend class Pin;
    ...
    protected:
        ...
        /**
         * @brief Sets the user-defined name for this Node.
         * @param name The new name string.
         */
        void setName(std::string_view name);
	...
	class LUX_FUNCTION_PUBLIC ExecIntermediateNode :
        public Node, 
