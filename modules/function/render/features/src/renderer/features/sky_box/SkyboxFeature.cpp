...
namespace lux::render
{
    ...
    Expected<void> SkyboxFeature::init(const Config& cfg)
    {
        ...
        if (want_equirect)
            wanted.push_back(cfg.equirect_fragment);
        if (want_cubemap)
            wanted.push_back(cfg.cubemap_fragment);
        ...
        auto makeBaseTmpl = [&](VkShaderModule frag_mod) {
            ...
            tmpl.color_write_mask = VK_COLOR_COMPONENT_R_BIT | VK_COLOR_COMPONENT_G_BIT | VK_COLOR_COMPONENT_B_BIT |
    ...
    void SkyboxFeature::addPasses(RGBuilder& builder)
    {
        ...
        const GraphicsPipelineHandle base_pipeline = has_equirect ? equirect_handle_ : cubemap_handle_;
        const uint32_t equirect_variant = has_equirect ? 0u : ~0u;
        const uint32_t cubemap_variant = has_cubemap ? (has_equirect ? 1u : 0u) : ~0u;

        auto pass =
            builder.addPass(kSkyboxPassName, ERGPassType::GRAPHICS)
            ...
            .setKernelFn(
                [this, has_equirect, has_cubemap, equirect_variant, cubemap_variant](const PassRecordContext& ctx) {
                    ...
                    if (active_mode_ == ActiveMode::EQUIRECT)
                    {
                        if (!has_equirect)
                            return;
                        const bool bound = (equirect_variant == 0u) ? ctx.bindPassPipeline()
                    ...
                    else // ActiveMode::CUBEMAP
                    {
                        if (!has_cubemap)
                            return;
                        const bool bound =
                            (cubemap_variant == 0u) ? ctx.bindPassPipeline() : ctx.bindPipelineVariant(cubemap_variant);
                        if (!bound)
                        {
                            ++pipeline_bind_failures_;
                            return;
                    ...
                    vkCmdPushConstants(ctx.cmd, ctx.pipeline_layout, ctx.pc_stage_flags, 8, sizeof(PC), &pc);
    ...
    bool SkyboxFeature::applyEquirectangularHandle(RTextureHandle texture, float rotation_radians, float intensity)
    {
    ...
    bool SkyboxFeature::applyCubemapHandles(RTextureHandle cube, float rotation_radians, float intensity)
    {
