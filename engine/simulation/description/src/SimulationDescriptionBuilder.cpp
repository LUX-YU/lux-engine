...
namespace lux::simulation
{
    ...
    namespace
    {
        ...
        template <class Range> [[nodiscard]] auto findSystem(Range& range, std::string_view name) noexcept
        {
            return std::find_if(range.begin(), range.end(), [name](const auto& system) noexcept {
        ...
        {
            const bool is_invalid_identity = left.type != right.type || left.version != right.version;
            ...
            const bool is_invalid_members = left.capabilities != right.capabilities || left.hooks != right.hooks ||
                left.events.size() != right.events.size();
            const bool is_mismatch = is_invalid_identity || is_invalid_configuration || is_invalid_members;
            if (is_mismatch)
            {
                return false;
            }
            for (std::size_t index{}; index < left.events.size(); ++index)
            {
                ...
                const bool is_invalid_identity = a.name != b.name ||
                    a.dispatch_hook_ordinal != b.dispatch_hook_ordinal || a.target != b.target;
                const bool is_invalid_payload = a.payload_schema_name != b.payload_schema_name ||
                    a.payload_schema_hash != b.payload_schema_hash ||
                    a.payload_schema_version != b.payload_schema_version;
                const bool is_mismatch = is_invalid_identity || is_invalid_payload;
                if (is_mismatch)
                {
                    return false;
                }
    ...
    SimulationDescriptionBuilder::operator=(SimulationDescriptionBuilder&&) noexcept = default;
    ...
    ) noexcept
    {
        ...
        if (version == 0U)
            return lux::cxx::unexpected(
                failure(ESimulationDescriptionError::INVALID_SCHEMA_VERSION, std::move(schema))
            );
    ...
    SimulationDescriptionBuilder::eraseData(const SimulationDataSchemaId& schema) noexcept
    {
        auto data = findData(impl_->data, schema);
        if (data == impl_->data.end())
    ...
    ) noexcept
    {
        ...
        const bool is_invalid_schema_version =
            system.configuration_schema_name.empty() != (system.configuration_schema_version == 0U);
        const bool is_missing_configuration = system.configuration_schema_name.empty() && !configuration.empty();
        const bool is_invalid_configuration = is_invalid_schema_version || is_missing_configuration;
        ...
        for (const auto& hook : system.hooks)
        {
            ...
            {
                const bool is_invalid_identity = parameter.type_id == 0U || parameter.canonical_name.empty();
                const bool is_invalid_type = parameter.canonical_name.empty() ||
                    parameter.type_id != lux::script::scriptSemanticTypeId(parameter.canonical_name);
                const bool is_invalid_parameter = is_invalid_identity || is_invalid_type;
            ...
            {
                const bool is_invalid_identity = result.type_id == 0U || result.canonical_name.empty();
                const bool is_invalid_pass = result.pass != lux::script::EScriptPassMode::VALUE;
                const bool is_invalid_type = result.canonical_name.empty() ||
                    result.type_id != lux::script::scriptSemanticTypeId(result.canonical_name);
                const bool is_invalid_result = is_invalid_identity || is_invalid_pass || is_invalid_type;
        ...
        for (std::size_t index{}; index < system.events.size(); ++index)
        {
            ...
            const bool void_payload = event.payload_cpp_type == lux::cxx::typeToken<void>();
            const bool is_invalid_schema_version =
                event.payload_schema_name.empty() != (event.payload_schema_version == 0U);
            const bool is_invalid_cpp_type = event.payload_cpp_type.isValid() &&
                void_payload != event.payload_schema_name.empty();
            const bool is_invalid_payload = is_invalid_schema_version || is_invalid_cpp_type;
            if (is_invalid_payload)
            {
                return lux::cxx::unexpected(failure(ESimulationDescriptionError::INVALID_EVENT_PAYLOAD_SCHEMA));
            }
        }
        if (failMutationForTest())
        ...
        try
        {
            ...
            candidate.configuration_schema_name = system.configuration_schema_name;
            candidate.configuration_schema_hash = system.configuration_schema_name.empty()
                                                      ? 0U
                                                      : lux::cxx::Fnv1a64::hash(system.configuration_schema_name);
            candidate.configuration_schema_version = system.configuration_schema_version;
            ...
            for (const auto& value : system.hooks)
            {
                ...
                for (const auto& parameter : value.signature.parameters)
                {
                    hook.parameters.push_back(
                        {parameter.type_id, std::string(parameter.canonical_name), parameter.pass}
                    );
                }
                hook.returns.reserve(value.signature.returns.size());
                for (const auto& result : value.signature.returns)
                {
                    hook.returns.push_back({result.type_id, std::string(result.canonical_name), result.pass});
                }
            ...
            for (const auto& event : system.events)
            {
                ...
                candidate.events.push_back(
                    {std::string(event.name),
                     ...
                     event.payload_schema_name.empty() ? 0U : lux::cxx::Fnv1a64::hash(event.payload_schema_name),
    ...
    lux::cxx::expected<void, SimulationDescriptionFailure>
    SimulationDescriptionBuilder::eraseSystem(std::string_view instance_name) noexcept
    {
        ...
        impl_->dependencies.erase(
            std::remove_if(
                ...
                [instance_name](const auto& dependency) noexcept {
                    return dependency.before_system == instance_name || dependency.after_system == instance_name;
                }
            ),
            impl_->dependencies.end()
        );
    ...
    lux::cxx::expected<void, SimulationDescriptionFailure>
    SimulationDescriptionBuilder::addDependency(std::string_view before_system, std::string_view after_system) noexcept
    {
        ...
        try
        {
            Impl::PendingDependency candidate{std::string(before_system), std::string(after_system)};
    ...
    lux::cxx::expected<void, SimulationDescriptionFailure> SimulationDescriptionBuilder::eraseDependency(
    ...
    lux::cxx::expected<void, SimulationDescriptionFailure>
    SimulationDescriptionBuilder::addGlobalScriptMount(const ScriptMountDescription& mount) noexcept
    {
        if (!validScriptMountDescription(mount))
            return lux::cxx::unexpected(failure(ESimulationDescriptionError::INVALID_SCRIPT_MOUNT));
        if (failMutationForTest())
            return lux::cxx::unexpected(failure(ESimulationDescriptionError::ALLOCATION_FAILURE));
        try
        {
            const auto resolve_hook_system =
                [&](const SystemHookBindingTarget& target) noexcept -> const Impl::PendingSystem* {
                if (!target.system_instance.empty())
                {
                    const auto found = findSystem(impl_->systems, target.system_instance);
                    return found != impl_->systems.end() && found->type == target.system_type ? std::addressof(*found)
                                                                                              : nullptr;
                }
            ...
            for (const auto& binding : mount.bindings)
            {
                const auto validation = std::visit(
                    [&](const auto& target) noexcept -> std::optional<ESimulationDescriptionError> {
                        using Target = std::remove_cvref_t<decltype(target)>;
                        if constexpr (std::is_same_v<Target, BehaviorLifecycleBindingTarget>)
                        {
                            return ESimulationDescriptionError::SCRIPT_TARGET_SCOPE_MISMATCH;
                        }
                        else
                        {
                            ...
                            if (!target.system_instance.empty())
                            {
                                const auto system = findSystem(impl_->systems, target.system_instance);
                                if (system == impl_->systems.end())
                                {
                                    return ESimulationDescriptionError::SCRIPT_TARGET_SYSTEM_NOT_FOUND;
                                }
                                if (system->type != target.system_type)
                                {
                                    return ESimulationDescriptionError::SCRIPT_TARGET_TYPE_MISMATCH;
                                }
                            ...
                            else
                            {
                                for (const auto& system : impl_->systems)
                                {
                                    ...
                                    if (resolved)
                                    {
                                        return ESimulationDescriptionError::SCRIPT_TARGET_SYSTEM_AMBIGUOUS;
                                    }
                                    resolved = std::addressof(system);
                                }
                                if (!resolved)
                            ...
                            if constexpr (std::is_same_v<Target, SystemHookBindingTarget>)
                            {
                                const auto hook = std::find_if(
                                    ...
                                    resolved->hooks.end(),
                                    [&](const auto& candidate) noexcept { return candidate.name == target.hook; }
                                );
                                if (hook == resolved->hooks.end())
                                {
                                    return ESimulationDescriptionError::SCRIPT_TARGET_MEMBER_NOT_FOUND;
                                }
                                if (hook->cardinality == ESystemHookCardinality::SINGLE)
                                {
                                    std::size_t handlers{};
                                    const auto count_handlers = [&](const ScriptMountDescription& value) noexcept {
                                        for (const auto& candidate_binding : value.bindings)
                                        {
                                            const auto* candidate = std::get_if<SystemHookBindingTarget>(
                                                std::addressof(candidate_binding.target)
                                            );
                                            if (!candidate || candidate->hook != target.hook)
                                            {
                                                continue;
                                            }
                                            if (resolve_hook_system(*candidate) == resolved)
                                            {
                                                ++handlers;
                                            }
                                        }
                                    ...
                                    for (const auto& existing : impl_->global_script_mounts)
                                    {
                                        count_handlers(existing);
                                    }
                                    if (handlers > 1U)
                                    {
                                        return ESimulationDescriptionError::SINGLE_HOOK_MULTIPLE_HANDLERS;
                                    }
                                }
                                return std::optional<ESimulationDescriptionError>{};
                            }
                            else
                            {
                                const auto event = std::find_if(
                                    ...
                                    resolved->events.end(),
                                    [&](const auto& candidate) noexcept { return candidate.name == target.event; }
                                );
                                if (event == resolved->events.end())
                                {
                                    return ESimulationDescriptionError::SCRIPT_TARGET_MEMBER_NOT_FOUND;
                                }
                                return event->target == ESystemEventTarget::GLOBAL
                                           ? std::optional<ESimulationDescriptionError>{}
                                           : std::optional{ESimulationDescriptionError::SCRIPT_TARGET_SCOPE_MISMATCH};
                            }
                        }
                    },
                    binding.target
                ...
                if (validation)
                {
                    return lux::cxx::unexpected(failure(*validation, {}, binding.function));
                }
            }
            if (std::any_of(
                    impl_->global_script_mounts.begin(),
                    impl_->global_script_mounts.end(),
                    [&](const auto& existing) noexcept { return existing.id == mount.id; }))
            {
                return lux::cxx::unexpected(
                    failure(ESimulationDescriptionError::INVALID_SCRIPT_MOUNT, {}, mount.id.value)
                );
            }
    ...
    lux::cxx::expected<void, SimulationDescriptionFailure>
    SimulationDescriptionBuilder::eraseGlobalScriptMount(ScriptMountId id) noexcept
    {
        const auto mount = std::find_if(
            impl_->global_script_mounts.begin(),
            impl_->global_script_mounts.end(),
            [id](const auto& candidate) noexcept { return candidate.id == id; }
        );
        if (mount == impl_->global_script_mounts.end())
            return lux::cxx::unexpected(failure(ESimulationDescriptionError::SCRIPT_MOUNT_NOT_FOUND));
    ...
    SimulationDescriptionBuilder::build() && noexcept
    {
        ...
        try
        {
            ...
            std::sort(type_sources.begin(), type_sources.end(), [&](std::size_t left, std::size_t right) noexcept {
                return SystemTypeIdLess{}(impl_->systems[left].type, impl_->systems[right].type);
            }
            );
                ...
                for (const auto& hook : source.hooks)
                {
                    ...
                    for (const auto& return_type : hook.returns)
                    {
                        hook_record.returns.push_back(
                            {return_type.type_id, return_type.canonical_name, return_type.pass}
                        );
                    }
            ...
            for (const auto& dependency : impl_->dependencies)
            {
                const auto before_system = std::lower_bound(
                    ...
                    [](const auto& system, std::string_view name) noexcept { return system.instance_name < name; }
                );
            ...
            result.global_script_mounts_ = std::move(impl_->global_script_mounts);
            impl_->data.clear();
            impl_->systems.clear();
            impl_->dependencies.clear();
            impl_->global_script_mounts.clear();
