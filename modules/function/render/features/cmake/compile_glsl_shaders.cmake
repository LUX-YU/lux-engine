...
function(compile_glsl_shaders)
    ...
    # EXTRA_DEPENDS: extra file-level dependencies (e.g. .lglsl emission outputs).
    # The include-dir's *.glsl glob is resolved at configure time, so it can't
    # see headers generated at build time — pass those in through this instead.
    set(_multi_value_arguments SHADERS INCLUDE_DIRS EXTRA_DEPENDS)
	...
	foreach(shader ${ARGS_SHADERS})
		...
		add_custom_command(
			...
			DEPENDS ${SHADER_ABSOLUTE_PATH} ${_INCLUDE_DEPS} ${ARGS_EXTRA_DEPENDS}
...
function(compile_and_pack_shaders)
	...
	set(_multi_value_args SHADERS INCLUDE_DIRS EXTRA_DEPENDS)
	...
	compile_glsl_shaders(
		...
		EXTRA_DEPENDS	${ARGS_EXTRA_DEPENDS}
		SHADERS		${_full_paths}
	)
...
function(compile_and_pack_shader_variants)
	...
	set(_multi_value_args VARIANTS INCLUDE_DIRS EXTRA_DEPENDS)
	cmake_parse_arguments(ARGS "" "${_one_value_args}" "${_multi_value_args}" ${ARGN})
	...
	foreach(_variant_spec ${ARGS_VARIANTS})
		...
		# Defines are COMMA-separated (a raw ';' inside a list element would be
		# re-split by the foreach expansion above — the classic CMake list
		# trap); turn each into -DXXX=YYY for glslc.
		set(_define_args)
		...
		add_custom_command(
			...
			COMMAND ${Vulkan_GLSLC_EXECUTABLE}
				...
				"${_shader_abs}"
				-o "${_spv_out}"
			DEPENDS "${_shader_abs}" ${_inc_deps} ${ARGS_EXTRA_DEPENDS}
...
#[[
	emit_lglsl_shaders(
		EMIT_TARGET   <target>        # custom target driving .lglsl emission
		SOURCE_DIR    <dir>           # base directory; SOURCES entries are bare relative paths
		OUTPUT_DIR    <var>           # [out] emitted root (canonical GLSL mirror tree)
		SOURCES       <relpath> ...   # *.lglsl sources, e.g. postprocess/tonemap.frag.lglsl
	)

	For each "postprocess/tonemap.frag.lglsl":
	  Emits -> ${CMAKE_BINARY_DIR}/lglsl_emitted/postprocess/tonemap.frag
	  (lux_shader_emitter injects canonical set/binding from LayoutContract;
	   feed the emitted tree to compile_and_pack_shaders as its SHADER_DIR).

	.lglsl is the engine's shader source format (design §4.4): authors write
	type + name without a layout prefix; the emitter injects the slot from the
	contract table. The .lglslh header path (--header mode) isn't wired up yet
	— hitting one is a FATAL error until the shared-header migration adds it.
]]
function(emit_lglsl_shaders)
	set(_one_value_args EMIT_TARGET SOURCE_DIR OUTPUT_DIR OUTPUT_FILES)
	set(_multi_value_args SOURCES)
	cmake_parse_arguments(ARGS "" "${_one_value_args}" "${_multi_value_args}" ${ARGN})

	if(NOT ARGS_EMIT_TARGET OR NOT ARGS_SOURCE_DIR OR NOT ARGS_OUTPUT_DIR)
		message(FATAL_ERROR "[emit_lglsl_shaders] EMIT_TARGET, SOURCE_DIR and OUTPUT_DIR are required")
	endif()

	# Same dual-path pattern as embed_builtin_shaders.cmake: in-tree target when
	# present, installed SDK tool (find_program) for external consumers.
	if(TARGET lux_shader_emitter)
		set(_emitter "$<TARGET_FILE:lux_shader_emitter>")
		set(_emitter_dep lux_shader_emitter)
	else()
		if(COMMAND lux_find_host_program)
			lux_find_host_program(
				_emitter
				REQUIRED
				NAMES lux_shader_emitter
			)
		elseif(CMAKE_CROSSCOMPILING)
			message(FATAL_ERROR
				"[emit_lglsl_shaders] Cross builds require lux_find_host_program"
			)
		else()
			find_program(_emitter lux_shader_emitter REQUIRED)
		endif()
		set(_emitter_dep)
	endif()

	set(_emit_root "${CMAKE_BINARY_DIR}/lglsl_emitted")
	set(_outputs)
	foreach(_rel ${ARGS_SOURCES})
		# .lglslh shared header -> same-named .glsl (the name used in include
		# statements) + --header mode; .lglsl full shader -> strip the suffix
		# (foo.frag.lglsl -> foo.frag).
		set(_mode_flags)
		if(_rel MATCHES "\.lglslh$")
			string(REGEX REPLACE "\.lglslh$" ".glsl" _out_rel "${_rel}")
			set(_mode_flags --header)
		elseif(_rel MATCHES "\.lglsl$")
			string(REGEX REPLACE "\.lglsl$" "" _out_rel "${_rel}")
		else()
			message(FATAL_ERROR "[emit_lglsl_shaders] not a .lglsl/.lglslh source: ${_rel}")
		endif()
		set(_src "${ARGS_SOURCE_DIR}/${_rel}")
		set(_out "${_emit_root}/${_out_rel}")
		get_filename_component(_out_dir "${_out}" DIRECTORY)
		file(MAKE_DIRECTORY "${_out_dir}")
		add_custom_command(
			OUTPUT  "${_out}"
			COMMAND ${_emitter} --in "${_src}" --out "${_out}" ${_mode_flags}
			DEPENDS "${_src}" ${_emitter_dep}
			COMMENT "Emitting canonical GLSL from ${_rel}"
			VERBATIM
		)
		list(APPEND _outputs "${_out}")
	endforeach()

	add_custom_target(${ARGS_EMIT_TARGET} DEPENDS ${_outputs})
	set(${ARGS_OUTPUT_DIR} "${_emit_root}" PARENT_SCOPE)
	if(ARGS_OUTPUT_FILES)
		set(${ARGS_OUTPUT_FILES} ${_outputs} PARENT_SCOPE)
	endif()
endfunction()
