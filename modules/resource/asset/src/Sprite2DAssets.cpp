/**
 * @file Sprite2DAssets.cpp
 * @brief SpriteAtlasAsset / SpriteAnimClipAsset + their SerDesers + the
 *        private description codec (A2-00 / A2-01). One TU for the whole
 *        traditional-2D metadata asset family — the two types are structural
 *        twins (pure metadata in the INFO section, data_size == 0) and
 *        splitting them across six files would only scatter the pattern.
 */

#include <lux/engine/asset/Sprite2DSerDeser.hpp>
#include <lux/engine/asset/Sprite2DDescriptionCodec.hpp>
...
namespace lux::asset
{
    ...
    SpriteAtlasAsset::SpriteAtlasAsset(std::unique_ptr<AssetInfo>   info,
                                       std::unique_ptr<SpriteAtlas> atlas)
        : TAsset<SpriteAtlas>(std::move(info), std::move(atlas))
    {
    }

    SpriteAnimClipAsset::SpriteAnimClipAsset(std::unique_ptr<AssetInfo>      info,
                                             std::unique_ptr<SpriteAnimClip> clip)
        : TAsset<SpriteAnimClip>(std::move(info), std::move(clip))
    {
    }

    asset_id_t assetIdFromOpaque(const lux::rdesc::OpaqueAssetId& raw) noexcept
    {
        return asset_id_t(raw.begin(), raw.end());
    }

    lux::rdesc::OpaqueAssetId opaqueFromAssetId(const asset_id_t& id) noexcept
    {
    ...
    namespace detail
    {
        ...
        std::vector<std::byte> encodeSpriteAtlasDescription(const lux::rdesc::SpriteAtlas& atlas)
        {
            std::vector<std::byte> out;
            Writer w{out};
            w.u32(kSaDescMagic);
        ...
        bool decodeSpriteAtlasDescription(std::span<const std::byte> blob,
        ...
        std::vector<std::byte> encodeSpriteAnimClipDescription(const lux::rdesc::SpriteAnimClip& clip)
        {
            std::vector<std::byte> out;
            Writer w{out};
            w.u32(kScDescMagic);
        ...
        bool decodeSpriteAnimClipDescription(std::span<const std::byte>  blob,
    ...
    namespace
    {
        ...
        decodeMetadataImage(const void* bytes, std::size_t len, DecodeFn&& decode_fn) noexcept
        {
            ...
            try { file.resize(len); }
            catch (...) { return unexpected(EAssetError::OUT_OF_MEMORY); }
            if (len > 0) std::memcpy(file.data(), bytes, len);
    ...
    SpriteAtlasSerDeser::SpriteAtlasSerDeser(std::shared_ptr<AssetManager> manager)
        : TAssetSerDeser<SpriteAtlasLoadConfig>(std::move(manager))
    {
    }

    lux::cxx::expected<std::unique_ptr<LuxAsset>, EAssetError>
    SpriteAtlasSerDeser::fromFileStream(std::ifstream& /*ifs*/)
    {
        return unexpected(EAssetError::UNSUPPORTED);   // no standalone external format
    }

    lux::cxx::expected<std::unique_ptr<SpriteAtlas>, EAssetError>
    SpriteAtlasSerDeser::decodeData(const void* bytes, std::size_t len) noexcept
    {
        return decodeMetadataImage<EAssetType::SPRITE_ATLAS, SpriteAtlas>(
            bytes, len, &detail::decodeSpriteAtlasDescription);
    }

    lux::cxx::expected<std::unique_ptr<LuxAsset>, EAssetError>
    SpriteAtlasSerDeser::fromLuxAssetStream(std::istream& ifs)
    {
        std::vector<std::byte> file;
        ...
        AssetFileHeader header{};
        if (auto ec = loadHeaderRaw<EAssetType::SPRITE_ATLAS>(file, header);
        ...
        return std::make_unique<SpriteAtlasAsset>(std::move(ainfo), std::move(*decoded));
    }

    EAssetError
    SpriteAtlasSerDeser::exportAsLuxAssetStream(const LuxAsset& asset, std::ofstream& ofs)
    {
        const auto* aasset = asset.as<SpriteAtlasAsset>();
        if (!aasset)
            return EAssetError::FILE_TYPE_ERROR;
        const SpriteAtlas* atlas = static_cast<const SpriteAtlas*>(aasset->rawData());
        if (!atlas)
            return EAssetError::ASSET_NO_DATA;
        return writeMetadataImage<EAssetType::SPRITE_ATLAS>(
            *aasset->info(), detail::encodeSpriteAtlasDescription(*atlas), ofs);
    }

    SpriteAnimClipSerDeser::SpriteAnimClipSerDeser(std::shared_ptr<AssetManager> manager)
        : TAssetSerDeser<SpriteAnimClipLoadConfig>(std::move(manager))
    {
    }

    lux::cxx::expected<std::unique_ptr<LuxAsset>, EAssetError>
    SpriteAnimClipSerDeser::fromFileStream(std::ifstream& /*ifs*/)
    {
        return unexpected(EAssetError::UNSUPPORTED);
    }

    lux::cxx::expected<std::unique_ptr<SpriteAnimClip>, EAssetError>
    SpriteAnimClipSerDeser::decodeData(const void* bytes, std::size_t len) noexcept
    {
        return decodeMetadataImage<EAssetType::SPRITE_ANIM_CLIP, SpriteAnimClip>(
            bytes, len, &detail::decodeSpriteAnimClipDescription);
    }

    lux::cxx::expected<std::unique_ptr<LuxAsset>, EAssetError>
    SpriteAnimClipSerDeser::fromLuxAssetStream(std::istream& ifs)
    {
        std::vector<std::byte> file;
        if (auto ec = readAll(ifs, file); ec != EAssetError::SUCCESS)
            return unexpected(ec);
        ...
        AssetFileHeader header{};
        if (auto ec = loadHeaderRaw<EAssetType::SPRITE_ANIM_CLIP>(file, header);
            ec != EAssetError::SUCCESS)
            return unexpected(ec);
        ...
        auto ainfo = std::make_unique<AssetInfo>(header.info);
        return std::make_unique<SpriteAnimClipAsset>(std::move(ainfo), std::move(*decoded));
    }

    EAssetError
    SpriteAnimClipSerDeser::exportAsLuxAssetStream(const LuxAsset& asset, std::ofstream& ofs)
    {
        const auto* casset = asset.as<SpriteAnimClipAsset>();
        if (!casset)
            return EAssetError::FILE_TYPE_ERROR;
        const SpriteAnimClip* clip = static_cast<const SpriteAnimClip*>(casset->rawData());
        if (!clip)
            return EAssetError::ASSET_NO_DATA;
        return writeMetadataImage<EAssetType::SPRITE_ANIM_CLIP>(
            *casset->info(), detail::encodeSpriteAnimClipDescription(*clip), ofs);
    }
}
