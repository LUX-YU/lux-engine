 ...
 *
 * Lives in `lux::project` (not `lux::editor`) so the launcher can use
 * it without pulling in editor / render dependencies.
 */
...
namespace lux::project
{
    ...
    [[nodiscard]] LUX_ENGINE_PROJECT_PUBLIC
