...
main()
{
    ...
    assert(reflected->type.alignment == alignof(lux::object::test::ReflectedObject));
    ...
    const auto move_only_signal = lux::object::reflection::findSignal(registry, *reflected, "moveOnly");
    ...
    const auto* free_noexcept = registry.findFunction("lux::object::test::reflectedFreeNoexcept", parameter_ids);
    const auto* free_throwing = registry.findFunction("lux::object::test::reflectedFreeThrowing", parameter_ids);
    ...
    auto dynamic_connection =
        lux::object::reflection::observe(sender, signal, receiver, *on_changed, lux::object::EDelivery::DIRECT);
    assert(dynamic_connection);
    sender.publish(73);
    assert(receiver.last_value == 73);

    auto throwing_connection =
        lux::object::reflection::observe(sender, signal, receiver, *on_throwing, lux::object::EDelivery::DIRECT);
    assert(!throwing_connection);
    assert(throwing_connection.error() == lux::object::reflection::EDynamicObserveError::METHOD_MUST_BE_NOEXCEPT);

    auto saved_connection =
        lux::object::reflection::observe(sender, saved_signal, receiver, *on_saved, lux::object::EDelivery::DIRECT);
    ...
    auto queued_connection =
        lux::object::reflection::observe(sender, signal, *queued_receiver, *on_changed, lux::object::EDelivery::AUTO);
    assert(queued_connection);
    auto direct_cross_affinity =
        lux::object::reflection::observe(sender, signal, *queued_receiver, *on_changed, lux::object::EDelivery::DIRECT);
    assert(!direct_cross_affinity);
    assert(direct_cross_affinity.error() == lux::object::reflection::EDynamicObserveError::DIRECT_CROSS_AFFINITY);
    sender.publish(91);
    {
        std::scoped_lock lock{queued_mutex};
        queued_drain = true;
    ...
    auto incompatible =
        lux::object::reflection::observe(sender, signal, receiver, *wrong_payload, lux::object::EDelivery::DIRECT);
    assert(!incompatible);
    assert(incompatible.error() == lux::object::reflection::EDynamicObserveError::PARAMETER_TYPE_MISMATCH);

    {
        lux::object::test::ReflectedObject sender_without_dispatcher;
        auto missing_sender_dispatcher = lux::object::reflection::observe(
        ...
        assert(
            missing_sender_dispatcher.error() ==
...

    {
        ...
        auto missing_receiver_dispatcher = lux::object::reflection::observe(
            sender,
            signal,
            receiver_without_dispatcher,
            *on_changed,
            lux::object::EDelivery::QUEUED
        );
        assert(!missing_receiver_dispatcher);
        assert(
            missing_receiver_dispatcher.error() ==
            lux::object::reflection::EDynamicObserveError::RECEIVER_HAS_NO_DISPATCHER);

        auto payload_not_queueable = lux::object::reflection::observe(
            sender,
            move_only_signal,
            receiver_without_dispatcher,
            *on_move_only,
        ...
        assert(payload_not_queueable.error() == lux::object::reflection::EDynamicObserveError::PAYLOAD_NOT_QUEUEABLE);
    }
    ...
    const auto base_signal = lux::object::reflection::findSignal(registry, *reflected_base, "baseChanged");
    const auto derived_signal = lux::object::reflection::findSignal(registry, *reflected_derived, "derivedChanged");
    assert(base_signal);
    assert(derived_signal);
    assert(base_signal.name() == "baseChanged");
    assert(derived_signal.name() == "derivedChanged");
    const auto inherited_signal = lux::object::reflection::findSignal(registry, *reflected_derived, "baseChanged");
    assert(inherited_signal);
    assert(inherited_signal.name() == "baseChanged");

    auto wrong_owner =
        lux::object::reflection::observe(sender, base_signal, receiver, *on_changed, lux::object::EDelivery::DIRECT);
    assert(!wrong_owner);
    assert(wrong_owner.error() == lux::object::reflection::EDynamicObserveError::SIGNAL_OWNER_MISMATCH);

    {
        ...
        assert(object_closed.error() == lux::object::reflection::EDynamicObserveError::OBJECT_CLOSED);
    }
