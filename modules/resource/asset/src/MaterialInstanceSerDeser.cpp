...
namespace lux::asset
{
    ...
    MaterialInstanceSerDeser::decodeData(const void* bytes, std::size_t len) noexcept
    {
        ...
        // PURE byte path: copy the image into a vector so the shared loadHeaderRaw /
        // Cursor helpers apply unchanged. Touches ONLY the bytes — never manager_.
        // Wrapped so a vector allocation throw (OOM) becomes an expected error rather
        // than escaping this noexcept boundary.
        std::vector<std::byte> file;
        try
        {
            file.assign(static_cast<const std::byte*>(bytes),
        ...
        std::unique_ptr<MaterialInstanceData> payload;
        try
        {
            payload = std::make_unique<MaterialInstanceData>();
        }
        catch (...)
        {
            return unexpected(EAssetError::ABNORMAL_FILE_SIZE);
