...
#include <lux/engine/render/resources/memory/GPUBuffer.hpp>           // create/destroyGpuBufferVmaBuffer
#include <lux/engine/render/resources/lifecycle/DeferredDestroyQueue.hpp>
#include <lux/engine/render/core/VulkanContext.hpp>                   // DeviceContext::vmaAllocator()
...
namespace lux::render
{
                                ...
                                const InstanceResources&              instances)
    {
        ...
        for (const uint32_t slot : alive)
        {
            ...
            if (cull.bsphere[3] < 0.0f)
                continue;
            extract_scratch_.push_back(InstanceXY{slot, cull.bsphere[0], cull.bsphere[2]});
        }
                                       ...
                                       uint32_t                              slot_count)
    {
        ...
        // cell active 判定走纯函数 cellInRange(同款逻辑被 CPU 自检测试覆盖);此处只加一层
        // per-cell 缓存,避免同一 cell 的多个实例重复算到相机的距离。
