...
namespace sample_ext
{
    ...
    public:
        ...
        explicit SampleExternalFeature(Config cfg)
            : RenderFeature(RenderFeature::Config{cfg.name}), cfg_(std::move(cfg))
        {
        }
        ...
        void initAndAttachTo(RenderScene& /*scene*/) override
        {
            ...
            std::vector<const lux::rdesc::ShaderInfo*> infos = {cv.shaderInfo(vsh), cv.shaderInfo(fsh)};
            tmpl.pipeline_layout =
                cv.buildStandardGraphicsLayout(tmpl.descriptor_set_count, infos, "SampleExternalLayout");
        ...
        void addPasses(RGBuilder& builder) override
        {
            builder.addPass("SampleExternalPass", ERGPassType::GRAPHICS)
                ...
                .bindSceneDS(0)
                .setKernelFn([](const PassRecordContext& ctx) {
                    if (ctx.view == nullptr)
                        return;
        ...
        void onDetachFromScene(RenderScene& /*scene*/) override
        {
            ...
            auto cv = contextView();
            if (buffer_)
                cv.retireBuffer(buffer_, buffer_alloc_);
            if (image_)
                cv.retireImage(image_, image_alloc_);
            if (view_)
                cv.retireImageView(view_);
            if (sampler_)
                cv.retireSampler(sampler_);
