...
Project::Project(object::ObjectDispatcherRef dispatcher, std::uint64_t instance, process::TaskScope &tasks,
                 process::BlockingScheduler blocking)
    : Object(std::move(dispatcher)), tasks_(tasks), blocking_(blocking), instance_(instance)
{
}

EditorResult<std::unique_ptr<Project>> Project::open(ProjectSource &source, process::BlockingScheduler blocking,
                                                     process::TaskScope &tasks, object::ObjectDispatcherRef dispatcher)
{
    ...
    auto result = std::unique_ptr<Project>(new Project(std::move(dispatcher), *instance, tasks, blocking));
...
EditorResult<process::asset_loading::AssetReadPort> Project::captureAssetReads()
{
    auto endpoint =
