...
namespace lux::gapi::vk
{
	...
	public:
		...
		DeviceMemory allocateMemory(VkDevice device, VkDeviceSize allocation_size, uint32_t memory_type_index, VkAllocationCallbacks* allocator = nullptr)
		{
			return DeviceMemory{
				device,
				allocation_size,
		...
		inline operator bool() const noexcept { return buffer != VK_NULL_HANDLE; }
		inline bool operator!() const noexcept { return buffer == VK_NULL_HANDLE; }

		inline operator VkBuffer() const noexcept { return buffer; }
		inline const VkBuffer* operator&() const noexcept { return &buffer; }

		inline VkBuffer handle() const noexcept { return buffer; }
		inline const VkBuffer* handlePtr() const noexcept { return &buffer; }
