...
namespace
{
    ...
    {
    public:
        graph::GraphTopology& topology() noexcept override
        {
            return topology_;
        }
        const graph::GraphTopology& topology() const noexcept override
        {
            return topology_;
        }
        graph::GraphLayout& layout() noexcept override
        {
            return layout_;
        }
        const graph::GraphLayout& layout() const noexcept override
        {
            return layout_;
        }
        ...
        graph::NodeId addNode(graph::NodeTypeId type) override
        {
            ...
            {
                auto pin =
                    topology_.addPin(*id, graph::EPinDirection::OUTPUT, graph::kUnlimitedFan, graph::PinSemanticId{1U});
                if (!pin)
                    return {};
            ...
            else
            {
                ...
                {
                    auto pin = topology_.addPin(*id, graph::EPinDirection::INPUT, 1U, graph::PinSemanticId{semantic});
                    if (!pin)
                        return {};
                    node->pins.push_back(*pin);
                }
        ...
        void failDetachAt(std::size_t call) noexcept
        {
            fail_detach_at_ = call;
        }
        void failAttachAt(std::size_t call) noexcept
        {
            fail_attach_at_ = call;
        }
        void failActionRestoreAt(std::size_t call) noexcept
        {
            fail_action_restore_at_ = call;
        }
        void failNextDetach() noexcept
        {
            failDetachAt(1U);
        }
        void failNextAttach() noexcept
        {
            failAttachAt(1U);
        }
        void failNextActionRestore() noexcept
        {
            failActionRestoreAt(1U);
        }
        [[nodiscard]] std::size_t nodeCount() const noexcept
        {
            return nodes_.size();
        }
        [[nodiscard]] std::string_view title(graph::NodeId node) const noexcept
        {
            return nodes_.at(node)->title;
        }
    ...
    {
    public:
        GraphNodePresentation node(graph::NodeId) const noexcept override
        {
            return {"Node"};
        }
        GraphPinPresentation pin(graph::PinId) const noexcept override
        {
            return {"Pin"};
        }
        std::span<const GraphPaletteEntry> palette() const noexcept override
        {
            return palette_;
        }
    ...
    void testRendererReplacementAndSharedConsumers()
    {
        ...
        selected_renderer.draw(
            "replacement",
            GraphRenderProtocol{document.topology(), document.layout(), presentation, false, session.selectedNode()},
            sink
        );
