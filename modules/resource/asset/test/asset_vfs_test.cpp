...
//     (resolve -> load -> registered, onLoaded fires, idempotent second call),
...
static void test_loose_dir_provider()
{
    ...
    LooseDirProvider provider(tree.root);
    ...
    // open: bytes identical to the on-disk file.
    {
        ...
        check(blob.has_value() && blob.value().size > 0, "open(id_hero) returns bytes");
        if (blob.has_value())
        {
            ...
            check(disk.size() == blob.value().size
                      && std::memcmp(disk.data(), blob.value().data.get(),
                                     disk.size()) == 0,
    ...
    public:
        ...
        open(const asset_id_t& id) const override
        {
            for (const auto& i : items)
            {
                ...
                return AssetBlob{ std::move(data), i.bytes.size() };
            }
            return lux::cxx::unexpected(EAssetError::ASSET_NOT_EXIST);
        }
...
static void test_vfs_mount_semantics()
{
    ...
    // open: id is the identity — both ids stay openable from their mounts.
    {
        ...
        check(b.has_value() && b.value().size == 4
                  && std::memcmp(b.value().data.get(), "BASE", 4) == 0,
...
static void test_manager_roundtrip()
{
    ...
    auto provider = std::make_shared<LooseDirProvider>(tree.root);
    provider->rescan();
    ...
    bool loaded_fired = false;
    mgr.onLoaded(found, [&](asset_id_t, LuxAsset*) { loaded_fired = true; });

    auto ensured = mgr.ensureAsset(found);
    check(ensured.has_value() && ensured.value() != nullptr,
          "ensureAsset loads through the VFS");
    check(loaded_fired, "ensureAsset fired the onLoaded subscriber");
