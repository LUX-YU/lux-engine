...
namespace lux::render
{
    ...
    bool VRAMBudgetGuard::canAllocate(VkDeviceSize bytes, float margin) const noexcept
    {
        ...
        const VkDeviceSize available = snap.total_budget > snap.total_usage ? snap.total_budget - snap.total_usage : 0;
        const auto required = static_cast<VkDeviceSize>(static_cast<double>(bytes) * (1.0 + margin));
