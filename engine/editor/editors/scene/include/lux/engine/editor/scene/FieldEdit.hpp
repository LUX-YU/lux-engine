...
template <class Value> struct FieldValue
{
                           ...
                           })
        {
            ...
            return std::ranges::all_of(first, [&](const auto &item) {
                ...
                return found != second.end() &&
                       FieldValue<typename Value::mapped_type>::equal(item.second, found->second);
            });
        }
        else if constexpr (requires {
    ...
    static bool valid(const Value &value) noexcept
    {
                           ...
                           })
        {
            ...
            return value.coeffs().allFinite() && std::isfinite(norm) && norm > std::numeric_limits<Scalar>::epsilon() &&
        ...
        else if constexpr (lux::meta::HasTypeStaticInfo<Value>)
        {
            return std::apply(
                [&](const auto &...field) {
                    return (
                        FieldValue<std::remove_cvref_t<decltype(value.*field.pointer)>>::valid(value.*field.pointer) &&
                        ...);
                },
                lux::meta::TypeStaticInfo<Value>::fields);
        }
        else if constexpr (requires { std::variant_size<Value>::value; })
        {
            return !value.valueless_by_exception() &&
                   std::visit(
                       [](const auto &item) { return FieldValue<std::remove_cvref_t<decltype(item)>>::valid(item); },
                       value);
        }
        else if constexpr (requires {
                               value.has_value();
        ...
        else if constexpr (std::ranges::range<Value>)
        {
            return std::ranges::all_of(
                value, [](const auto &item) { return FieldValue<std::ranges::range_value_t<Value>>::valid(item); });
        }
        else
        {
    ...
    static std::size_t bytes(const Value &value) noexcept
    {
            ...
            std::apply(
                [&](const auto &...field) {
                    ((result = detail::addFieldBytes(
                          result,
                          FieldValue<std::remove_cvref_t<decltype(value.*field.pointer)>>::bytes(value.*field.pointer) -
                           ...
                           })
        {
            if (value.has_value())
            {
                result = detail::addFieldBytes(result,
                                               FieldValue<typename Value::value_type>::bytes(*value) - sizeof(*value));
            }
        }
        else if constexpr (std::ranges::range<Value> && !requires { Value::SizeAtCompileTime; })
        {
...
template <class Component, class Value, class Access> class FieldEdit final : public SceneFieldEdit
{
  public:
    ...
    {
        return addFieldBytes(sizeof(*this) + field_.capacity() + label_.capacity() + 2 +
        ...
        const editing::ApplyContext &context, editing::EditPreparationBudget &budget) const noexcept override
    {
        ...
        const auto staging = adopt_existing ? sizeof(Prepared) : sizeof(Prepared) + FieldValue<Value>::bytes(next);
  ...
  private:
    ...
    {
        auto component =
            owner_.fieldAccess(owner_.replayTarget(target_, identity_), lux::cxx::typeToken<Component>(), current);
        if (!component)
        {
    ...
    class Prepared final : public editing::PreparedEdit
    {
      public:
        ...
        {
            return FieldValue<Value>::equal(operation_.before_, operation_.after_) ? editing::EEditEffect::NO_CHANGE
                                                                ...
                                                                const Value &value)
{
    ...
    if (auto size =
            checkFieldSize(detail::addFieldBytes(FieldValue<Value>::bytes(*before), FieldValue<Value>::bytes(value)));
        !size)
    {
                                                                ...
                                                                Access access)
{
    ...
    std::unique_ptr<detail::SceneFieldEdit> operation = std::make_unique<detail::FieldEdit<Component, Value, Access>>(
        *this, target, field, label, std::move(access), *before, *before, true);
