...
namespace
{
    ...
    template <int Id> class OrderedTool final
    {
    public:
    ...
    template <std::size_t Id> struct FillerTool final
    {
    ...
    public:
        ...
        [[nodiscard]] lux::cxx::expected<lux::asset::AssetBlob, lux::asset::EAssetStorageError> open(const lux::asset::
                                                                                                         AssetId&)
            const override
        {
    ...
    [[nodiscard]] lux::scene::SceneMetaManager emptySceneMeta()
    {
        ...
        auto scene_meta = lux::scene::SceneMetaManager::build(
            {std::move(*schemas), lux::simulation::SimulationSystemRegistry{}, {}, {}, {}}
        );
        assert(scene_meta);
        return std::move(*scene_meta);
    }

    template <class Type> [[nodiscard]] Type worldId(std::uint8_t tail)
    {
    ...
    [[nodiscard]] std::unique_ptr<lux::scene::Scene> makeEmptyScene(const lux::scene::SceneMetaManager& meta)
    {
        ...
        auto scene = lux::scene::Scene::create(
            {std::move(scene_description), std::move(world_owner), std::move(simulation_owner), meta, {}}
        );
        assert(scene);
        ...
        lux::editor::EditorSelection selection{ui.dispatcherRef()};
        {
            ...
            lux::editor::Toolset toolset;
            {
                ...
                auto capability = context.toolchain().install<CapabilityProbeTool>(
                    capability_state, context.vfs(), context.ui(), context.sceneMeta()
                );
                assert(capability);
