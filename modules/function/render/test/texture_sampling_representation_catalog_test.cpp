...
namespace
{
    ...
    {
        return lux::render::TextureRefGPU{
            1u,
            0u,
            std::max(aux, 1u),
            flags};
...
int main()
{
    ...
    const auto bindless_reference = bindless->resolve_reference(
        123u, 7u, 9u);
    assert(bindless_reference &&
        bindless_reference->representation_index == 0u &&
        bindless_reference->resource_index == 123u &&
        bindless_reference->aux == 7u &&
        bindless_reference->flags == 9u);
    ...
    auto built = TextureSamplingRepresentationCatalog::build(
        std::move(descriptors));
    assert(built);
    const auto* checker = built->find(1u);
    assert(checker && checker->semantic_bindings.empty());
    const auto checker_reference = checker->resolve_reference(
        999u, 0u, 0x00ff8040u);
    assert(checker_reference &&
        checker_reference->representation_index == 1u &&
