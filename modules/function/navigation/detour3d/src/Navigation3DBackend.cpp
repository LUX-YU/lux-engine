...
namespace lux::navigation::detour3d
{
    namespace
    {
        ...
        using RegionMap = std::unordered_map<
            NavigationRegionId,
        ...
        struct QueryPinBatch final
        {
            ~QueryPinBatch() noexcept
            {
                ...
                {
                    const auto previous = region->query_readers.fetch_sub(
        ...
        [[nodiscard]] std::uint32_t remainingStagingGranules(
            const RegionData& data) noexcept
        {
            return static_cast<std::uint32_t>(
                data.layers.size() - data.staging_cursor);
        }

        [[nodiscard]] std::uint32_t remainingRetirementGranules(
            const RegionData& data) noexcept
        {
            return static_cast<std::uint32_t>(
                data.layers.size() - data.retirement_cursor);
        }
            ...
            float maximum_relative_extent) noexcept
        {
            ...
            if (!start || !destination)
            {
                ...
                result.detail =
                    "navigation request exceeds the backend origin extent";
                return result;
            }
            ...
            const float extents[3]{request.nearest_horizontal_extent,
            ...
            if (dtStatusFailed(region.query->findNearestPoly(
                    start->data(),
                    extents,
                    &filter,
                    &start_reference,
                    nearest_start)) ||
                dtStatusFailed(region.query->findNearestPoly(
                    destination->data(),
                    extents,
                    &filter,
                ...
                start_reference == 0u || destination_reference == 0u)
            {
                ...
                result.detail =
                    "navigation endpoint is outside traversable content";
                return result;
            }
            ...
            std::vector<float> point_values(
                static_cast<std::size_t>(request.maximum_path_points) * 3u);
            int point_count = 0;
            ...
            const bool partial =
                dtStatusDetail(route_status, DT_PARTIAL_RESULT) ||
                route[static_cast<std::size_t>(route_count - 1)] !=
                    destination_reference;
            result.status = partial ? ENavigationPathStatus::PARTIAL
        ...
        [[nodiscard]] bool samePortal(
            const NavigationPortal& left,
            const NavigationPortal& right) noexcept
        {
            return left.id == right.id &&
                   left.first_region == right.first_region &&
                   left.second_region == right.second_region &&
                   left.first_position == right.first_position &&
                   left.second_position == right.second_position &&
                   left.traversal_cost_scale == right.traversal_cost_scale &&
                   left.bidirectional == right.bidirectional;
        }

        [[nodiscard]] bool portalLess(
            const NavigationPortal& left,
            const NavigationPortal& right) noexcept
        {
        ...
        [[nodiscard]] std::optional<std::vector<PortalTraversal>>
        findPortalRoute(
            ...
            const RegionMap* required_active) noexcept
        {
            ...
            while (!frontier.empty() && !predecessors.contains(destination))
            {
                ...
                for (const auto& portal : portals)
                {
                    ...
                    {
                        traversal = {current,
                                     portal.second_region,
                                     portal.first_position,
                                     portal.second_position};
                    }
                    else if (portal.second_region == current &&
                    ...
                    if (required_active &&
                        !required_active->contains(traversal.to))
                    {
                        continue;
                    }
                    ...
                    predecessors.emplace(
                        traversal.to,
                        PortalPredecessor{true, traversal});
            ...
            std::uint32_t maximum_points) noexcept
        {
            ...
            {
                if (!destination.points.empty() &&
                    destination.points.back() == point)
                {
                    continue;
                }
        ...
        retireOne(RegionData& data) noexcept
        {
            ...
            if (!query_lock.owns_lock())
                return NavigationRegion3DStepResult{false, 0u, 0u};
            const auto index =
                data.layers.size() - data.retirement_cursor - 1u;
            auto& layer = data.layers[index];
            const auto bytes = static_cast<std::uint64_t>(layer.size);
            if (layer.installed())
            {
                const auto status = data.navigation->removeTile(
                    layer.reference, nullptr, nullptr);
            ...
            const bool complete =
                data.retirement_cursor == data.layers.size();
            if (complete)
            {
                data.query.reset();
    ...
    PreparedNavigationRegion3D::~PreparedNavigationRegion3D() noexcept =
        default;
    PreparedNavigationRegion3D::PreparedNavigationRegion3D(
        PreparedNavigationRegion3D&&) noexcept = default;
    PreparedNavigationRegion3D& PreparedNavigationRegion3D::operator=(
        PreparedNavigationRegion3D&&) noexcept = default;
    ...
    {
        return data_ && data_->region.valid() && data_->navigation &&
               data_->query && !data_->layers.empty() &&
    ...
    lux::cxx::expected<NavigationRegion3DStepResult,
                       NavigationRegion3DFailure>
        ...
        if (!data_)
        {
            return lux::cxx::unexpected(
                fail(ENavigationRegion3DError::INVALID_REQUEST,
                     "navigation prepared retirement has no owner"));
        }
    ...
    Navigation3DBackend::Navigation3DBackend(
    ...
    Navigation3DBackend::~Navigation3DBackend() noexcept
    {
        ...
        if (!control_->staged.empty() || !control_->active.empty() ||
            !control_->retiring.empty() || control_->owned_bytes != 0u)
        {
    ...
    lux::cxx::expected<std::shared_ptr<Navigation3DBackend>,
                       NavigationRegion3DFailure>
    Navigation3DBackend::create(Navigation3DBackendConfig config) noexcept
    {
        if (config.maximum_resident_regions == 0u ||
            !finitePositive(config.maximum_relative_extent))
        {
            return lux::cxx::unexpected(
                fail(ENavigationRegion3DError::INVALID_REQUEST,
                     "navigation backend configuration is invalid"));
        }
        return std::shared_ptr<Navigation3DBackend>{
            new Navigation3DBackend{std::make_shared<Control>(config)}};
    }

    lux::cxx::expected<std::unique_ptr<NavigationRegion3DLease>,
                       NavigationRegion3DFailure>
    Navigation3DBackend::adoptPrepared(
        PreparedNavigationRegion3D&& prepared) noexcept
    {
        if (!control_ || !prepared.valid() || !control_->isOwner())
        {
            return lux::cxx::unexpected(
                fail(ENavigationRegion3DError::INVALID_REQUEST,
                     "prepared navigation region cannot be adopted here"));
        }
        std::unique_lock lock{control_->mutex};
        const auto region = prepared.data_->region;
        const auto count = control_->staged.size() + control_->active.size() +
                           control_->retiring.size();
        if (!control_->alive)
        {
            return lux::cxx::unexpected(
                fail(ENavigationRegion3DError::STALE_GENERATION,
                     "navigation backend is closed"));
        }
        if (control_->staged.contains(region) ||
            control_->active.contains(region) ||
            control_->retiring.contains(region))
        {
            return lux::cxx::unexpected(
                fail(ENavigationRegion3DError::REGION_CONFLICT,
                     "navigation region is already resident"));
        }
        const auto compatibleWith = [&prepared](const RegionMap& regions)
        {
            for (const auto& [_, resident] : regions)
            {
                const auto connects = [](const NavigationPortal& portal,
                                         NavigationRegionId region) noexcept
                {
                    return portal.first_region == region ||
                           portal.second_region == region;
                };
                for (const auto& incoming : prepared.data_->portals)
                {
                    if (connects(incoming, resident->region) &&
                        !detail::sameAgent(prepared.data_->agent,
                                           resident->agent))
                    {
                        return false;
                    }
                    ...
                    if (duplicate != resident->portals.end() &&
                        !samePortal(*duplicate, incoming))
                    {
                        return false;
                    }
                }
        ...
        if (!compatibleWith(control_->staged) ||
            !compatibleWith(control_->active) ||
        ...
        if (count >= control_->config.maximum_resident_regions)
        {
            return lux::cxx::unexpected(
                fail(ENavigationRegion3DError::CAPACITY_EXHAUSTED,
                     "navigation region capacity is exhausted"));
        }
        ...
        if (!inserted.second)
        {
            return lux::cxx::unexpected(
                fail(ENavigationRegion3DError::REGION_CONFLICT,
                     "navigation region became resident during adoption"));
        }
        control_->owned_bytes += data->resident_bytes;
        return std::unique_ptr<NavigationRegion3DLease>{
            new NavigationRegion3DLease{control_, std::move(data)}};
    }

    NavigationPathResult Navigation3DBackend::query(
        const NavigationPathRequest& request) const noexcept
    {
        NavigationPathResult result;
        ...
        if (request.maximum_path_nodes > kMaximumPathNodes ||
            request.maximum_path_points > kMaximumPathPoints)
        {
        ...
        std::uint64_t generation = 0u;
        {
            ...
            {
                region->query_readers.fetch_add(1u,
                                                std::memory_order_relaxed);
                query_pins.regions.push_back(region);
            }
            known = control_->staged;
            known.insert(control_->retiring.begin(),
                         control_->retiring.end());
        }
        result.generation = generation;

        std::vector<NavigationPortal> portals;
        bool conflicting_portals = false;
        const auto collectPortals = [&portals, &conflicting_portals](
                                        const RegionMap& regions)
        {
            for (const auto& [_, region] : regions)
            {
                for (const auto& portal : region->portals)
                {
                    const auto duplicate = std::ranges::find_if(
                        portals,
                        [&portal](const NavigationPortal& candidate) noexcept
        ...
        if (conflicting_portals)
        {
            result.failure = ENavigationPathFailure::BACKEND_FAILURE;
            result.detail =
                "resident navigation regions disagree on a portal contract";
            return result;
        }
            ...
            -> std::shared_ptr<RegionData>
        {
            ...
            {
                if (detail::sameAgent(candidate->agent, request.agent) &&
                    contains(point,
                             candidate->bounds_min,
                             candidate->bounds_max) &&
                    (!found || candidate->region < found->region))
                {
                    found = candidate;
                }
            }
        ...
        const auto findActive = [&active](NavigationRegionId id)
            -> std::shared_ptr<RegionData>
        {
            const auto found = active.find(id);
            return found == active.end() ? std::shared_ptr<RegionData>{}
                                         : found->second;
        };

        std::shared_ptr<RegionData> start_region = request.start_region
            ? findActive(*request.start_region)
            : findContaining(active, request.start);
        std::shared_ptr<RegionData> destination_region =
        ...
        {
            if (std::ranges::find(result.missing_regions, id) ==
        ...
        if (!destination_region)
        {
            if (request.destination_region)
                addMissing(*request.destination_region);
                         ...
                         findContaining(known, request.destination))
                addMissing(pending->region);
        }
        ...
        {
            result = queryRegion(*start_region,
                                 request.start,
                                 request.destination,
                                 request,
        ...
        auto traversals = findPortalRoute(
            start_region->region,
        ...
        if (!traversals)
        {
            auto known_route = findPortalRoute(
                start_region->region,
                destination_region->region,
                portals,
                nullptr);
            if (known_route)
            {
                ...
                result.missing_regions.erase(
                    std::unique(result.missing_regions.begin(),
                                result.missing_regions.end()),
                    result.missing_regions.end());
                if (!result.missing_regions.empty())
                {
                    result.status = ENavigationPathStatus::PENDING;
                    result.detail =
                        "the selected portal route crosses navigation "
            ...
            result.failure = ENavigationPathFailure::LOCATION_NOT_FOUND;
            result.detail =
                "known navigation regions have no connecting portal";
            return result;
        }
        ...
        for (const auto& traversal : *traversals)
        {
            ...
            auto segment = queryRegion(
                *found->second,
                ...
                control_->config.maximum_relative_extent);
            appendPoints(result,
                         std::move(segment.points),
                         request.maximum_path_points);
            ...
            if (segment.status == ENavigationPathStatus::PARTIAL ||
                result.status == ENavigationPathStatus::PARTIAL)
            {
                result.status = ENavigationPathStatus::PARTIAL;
                result.detail =
                    "navigation path reaches only part of a portal route";
                result.generation = generation;
                return result;
            }
    ...
    Navigation3DBackendSnapshot Navigation3DBackend::snapshot() const noexcept
    {
        ...
        result.staged_regions =
            static_cast<std::uint32_t>(control_->staged.size());
        result.active_regions =
            static_cast<std::uint32_t>(control_->active.size());
        result.retiring_regions =
            static_cast<std::uint32_t>(control_->retiring.size());
        ...
        for (const auto& [_, data] : control_->active)
            result.active_granules +=
                static_cast<std::uint32_t>(data->layers.size());
    ...
    Navigation3DBackend::advanceRetirementOne() noexcept
    {
        ...
        for (auto candidate = control_->retiring.begin();
             candidate != control_->retiring.end(); ++candidate)
        {
            if (candidate->first < lowest->first)
                lowest = candidate;
            if (control_->retirement_after &&
                candidate->first > *control_->retirement_after &&
                (selected == control_->retiring.end() ||
    ...
    NavigationRegion3DLease::NavigationRegion3DLease(
        NavigationRegion3DLease&& other) noexcept = default;
    NavigationRegion3DLease&
    NavigationRegion3DLease::operator=(NavigationRegion3DLease&& other) noexcept
    {
        if (this == &other)
            return *this;
        reset();
    ...
    ENavigationRegion3DLeaseState
    NavigationRegion3DLease::state() const noexcept
    {
    ...
    NavigationRegion3DLease::advancePreparationOne() noexcept
    {
        ...
        if (!control || !data_ || !control->isOwner() ||
            state_ != ENavigationRegion3DLeaseState::STAGING)
        {
            return lux::cxx::unexpected(fail(
                ENavigationRegion3DError::INVALID_REQUEST,
        ...
        if (!control->alive || found == control->staged.end() ||
            found->second != data_)
        {
            return lux::cxx::unexpected(
                fail(ENavigationRegion3DError::STALE_GENERATION,
                     "navigation region preparation is stale"));
        }
        ...
        const auto status = data_->navigation->addTile(
            layer.data.get(),
            layer.size,
            DT_TILE_FREE_DATA,
            0u,
        ...
        if (dtStatusFailed(status) || reference == 0u)
        {
            ...
            return lux::cxx::unexpected(
                fail(ENavigationRegion3DError::BUILD_FAILED,
                     "navigation backend rejected one prepared layer"));
        }
        layer.data.release();
        ...
        if (complete)
            state_ = ENavigationRegion3DLeaseState::READY;
        return NavigationRegion3DStepResult{
            complete, 1u, static_cast<std::uint64_t>(layer.size)};
    }

    lux::cxx::expected<void, NavigationRegion3DFailure>
    NavigationRegion3DLease::publish() noexcept
    {
        const auto control = control_.lock();
        if (!control || !data_ || !control->isOwner() ||
            state_ != ENavigationRegion3DLeaseState::READY)
        {
            return lux::cxx::unexpected(
                fail(ENavigationRegion3DError::INVALID_REQUEST,
                     "navigation region cannot be published in this state"));
        }
        std::unique_lock lock{control->mutex};
        const auto found = control->staged.find(data_->region);
        if (!control->alive || found == control->staged.end() ||
            found->second != data_ ||
            data_->staging_cursor != data_->layers.size())
        {
            return lux::cxx::unexpected(
                fail(ENavigationRegion3DError::STALE_GENERATION,
                     "navigation region publication is stale"));
        }
    ...
    lux::cxx::expected<void, NavigationRegion3DFailure>
    NavigationRegion3DLease::hide() noexcept
    {
        if (state_ != ENavigationRegion3DLeaseState::ACTIVE)
        {
            return lux::cxx::unexpected(
                fail(ENavigationRegion3DError::INVALID_REQUEST,
                     "navigation region cannot be hidden in this state"));
        }
        return beginRetirement();
    }

    lux::cxx::expected<void, NavigationRegion3DFailure>
    NavigationRegion3DLease::beginRetirement() noexcept
    {
        ...
        std::unique_lock lock{control->mutex};
        auto& source = state_ == ENavigationRegion3DLeaseState::ACTIVE
                           ? control->active
                           : control->staged;
        const auto found = source.find(data_->region);
        if (!control->alive || found == source.end() ||
            found->second != data_)
        {
            return lux::cxx::unexpected(
                fail(ENavigationRegion3DError::STALE_GENERATION,
                     "navigation region retirement is stale"));
        }
    ...
    NavigationRegion3DLease::advanceRetirementOne() noexcept
    {
        ...
        if (!control->alive || found == control->retiring.end() ||
            found->second != data_)
        {
            return lux::cxx::unexpected(
                fail(ENavigationRegion3DError::STALE_GENERATION,
                     "navigation ownership ledger is inconsistent"));
        }
