...
namespace lux::shadergen::lglsl
{
    namespace
    {
        ...
        [[nodiscard]] std::string_view nextToken(std::string_view& cursor) noexcept
        {
            ...
            std::string_view tok = (e == std::string_view::npos) ? cursor.substr(b) : cursor.substr(b, e - b);
        ...
        [[nodiscard]] bool parseStage(std::string_view v, rdesc::EShaderType& out) noexcept
        {
            if (v == "vertex")
            {
                out = rdesc::EShaderType::VERTEX;
                return true;
            }
            if (v == "fragment")
            {
                out = rdesc::EShaderType::FRAGMENT;
                return true;
            }
            if (v == "compute")
            {
                out = rdesc::EShaderType::COMPUTE;
                return true;
            }
            return false;
        }
        ...
        constexpr std::array kMergeableLayoutQualifiers = {
            ...
            std::string_view{"row_major"},
            std::string_view{"column_major"},
        };
        ...
        private:
            ...
            std::optional<std::string> emitDeclaration(
            ...
            )
            {
                ...
                std::string_view body = full_text.substr(indent_end == std::string_view::npos ? 0 : indent_end);
            ...
            uint32_t next_local_binding_{0
            }; ///< R2-1:契约外局部资源的声明序计数(每文件一个 Emitter 实例,天然按文件归零)
