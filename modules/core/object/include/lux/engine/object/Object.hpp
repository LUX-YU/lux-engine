...
namespace lux::object
{
    ...
    template <typename Derived, typename Base = LuxObject> class Object : public Base
    {
      public:
        ...
        template <auto &StaticSignal, auto Method, EDelivery Delivery = EDelivery::AUTO,
                  class Receiver>
            ...
            requires std::derived_from<Receiver, LuxObject>
        {
            ...
            if constexpr (std::is_void_v<Payload>)
            {
                ...
                static_assert(std::is_nothrow_invocable_r_v<void, decltype(Method), Receiver &>,
                              "Signal callbacks must be noexcept");
            }
            else
            {
                static_assert(Traits::arity == 1);
                static_assert(std::same_as<std::tuple_element_t<0, typename Traits::arguments>,
                                           const Payload &>);
                static_assert(std::is_nothrow_invocable_r_v<void, decltype(Method), Receiver &,
                                                            const Payload &>,
                              "Signal callbacks must be noexcept");
        ...
        template <auto &StaticSignal, auto Function> [[nodiscard]] Connection observe()
        {
            ...
            return this->observeStaticIndexed(
                StaticSignal.descriptor_, [](LuxObject *, const void *payload, void *) noexcept {
                    if constexpr (std::is_void_v<Payload>)
                        Function();
                    else
        ...
        [[nodiscard]] ScopedConnection observeScoped(Callable &&callable)
        {
            ...
            if constexpr (std::is_void_v<Payload>)
                ...
                static_assert(std::is_nothrow_invocable_r_v<void, Stored &, const Payload &>,
                              "Signal callbacks must be noexcept");
