...
namespace lux::classic_mesh
{
    namespace
    {
        ...
        [[nodiscard]] ClassicMeshBatchCodecFailure failure(EClassicMeshBatchCodecError error, std::string detail)
        {
            return {error, std::move(detail)};
        }

        [[nodiscard]] bool finite(const ClassicMeshBatchInstanceV1& instance) noexcept
        {
        ...
        [[nodiscard]] bool unitQuaternion(const std::array<float, 4u>& rotation) noexcept
        {
            double length_squared = 0.0;
            for (const auto value : rotation)
            {
                length_squared += static_cast<double>(value) * static_cast<double>(value);
            }
            return std::isfinite(length_squared) && std::abs(length_squared - 1.0) <= 1.0e-3;
        }

        void writeUuid(ByteWriter& writer, const lux::asset::AssetId& value)
        {
            const auto bytes = value.bytes();
            writer.bytes(bytes.data(), bytes.size());
        }

        [[nodiscard]] bool readUuid(ByteReader& reader, lux::asset::AssetId& value) noexcept
        {
    ...
    validateClassicMeshBatchBlob(const ClassicMeshBatchBlobV1& blob, const ClassicMeshBatchCodecLimits& limits) noexcept
    {
        if (limits.maximum_instances == 0u || limits.maximum_encoded_bytes < kHeaderBytes)
        {
            return lux::cxx::unexpected(
                failure(EClassicMeshBatchCodecError::INVALID_ARGUMENT, "Classic Mesh batch codec limits are invalid")
            );
        }
        ...
        {
            const bool is_non_finite = !finite(instance);
    ...
    decodeClassicMeshBatchBlob(std::span<const std::byte> bytes, const ClassicMeshBatchCodecLimits& limits) noexcept
    {
        ...
        if (!reader.u32(magic) || !reader.u32(version) || !reader.u32(count))
        {
            return lux::cxx::unexpected(
                failure(EClassicMeshBatchCodecError::TRUNCATED, "Classic Mesh batch header is truncated")
            );
        }
        if (magic != kClassicMeshBatchBlobMagic)
        {
            return lux::cxx::unexpected(
                failure(EClassicMeshBatchCodecError::BAD_MAGIC, "Classic Mesh batch magic is invalid")
            );
        }
        ...
        {
            return lux::cxx::unexpected(
                failure(EClassicMeshBatchCodecError::INVALID_INSTANCE, "Classic Mesh batch contains no instances")
            );
        }
        ...
        {
            return lux::cxx::unexpected(
                failure(EClassicMeshBatchCodecError::TRUNCATED, "Classic Mesh batch instance rows are truncated")
            );
        }
        if (bytes.size() > expected_bytes)
        {
            return lux::cxx::unexpected(
                failure(EClassicMeshBatchCodecError::TRAILING_BYTES, "Classic Mesh batch has trailing bytes")
            );
        }
        ...
        for (auto& instance : result.instances)
        {
            ...
            const bool has_valid_instance =
                readUuid(reader, instance.mesh_asset) && readUuid(reader, instance.material_asset) &&
                reader.u64(instance.stable_pick_id) && reader.u32(instance.rgba8) && reader.u32(instance.flags);
            if (!has_valid_instance)
            {
                return lux::cxx::unexpected(
                    failure(EClassicMeshBatchCodecError::TRUNCATED, "Classic Mesh batch instance row is truncated")
                );
            }
        }
        ...
        {
            return lux::cxx::unexpected(
                failure(EClassicMeshBatchCodecError::TRAILING_BYTES, "Classic Mesh batch was not consumed exactly")
            );
        }
