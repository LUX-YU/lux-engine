...
namespace lux::render
{
    ...
    Expected<void> RenderClusterFeature::initAndAttachTo(RenderScene& scene)
    {
        ...
        if (!resources
                 .initializePicking(context.deviceContext(), context.deferredDestroyQueue(), context.framesInFlight()))
        {
        ...
        if (!pick_vertex || !pick_fragment)
        {
            ...
            resources.shutdownGpuCulling();
            resources.shutdownPicking();
            return !pick_vertex ? lux::cxx::unexpected(pick_vertex.error())
        ...
        if (pick_set_layout_ == VK_NULL_HANDLE)
        {
            constexpr VkDescriptorSetLayoutBinding
                pick_result_binding{0u, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 1u, VK_SHADER_STAGE_FRAGMENT_BIT, nullptr};
        ...
        pick_template.vertex_attributes.push_back(VkVertexInputAttributeDescription{
            ...
            static_cast<std::uint32_t>(offsetof(lux::rdesc::Vertex, position))}
        );
    ...
    void RenderClusterFeature::addPasses(RGBuilder& builder)
    {
        ...
        const bool can_render_candidates = has_candidate_source && has_cull_pipelines &&
            has_cull_layouts && has_cull_buffers;
        if (can_render_candidates)
        {
            const auto candidate_capacity = candidate_source->capacity();
            const auto import_cull_input =
                [&builder, resources](std::string name, VkDeviceSize size, std::uint32_t stride, bool clusters) {
                    RGBufferDescription description{};
                    description.size = std::max<VkDeviceSize>(size, stride);
                    description.stride = stride;
                    description.element_count = static_cast<std::uint32_t>(description.size / stride);
                    ...
                    imported.buffer_getter = [resources, clusters](VkBuffer* output, std::uint32_t capacity) {
                        ...
                        {
                            output[index] = clusters ? resources->gpuCullClusterBuffer(index)
            ...
            const auto cluster_input = import_cull_input(
                ...
                static_cast<VkDeviceSize>(candidate_capacity) * sizeof(RenderClusterResources::GpuCullCluster),
            ...
            const auto cluster_instance_input = import_cull_input(
                ...
                static_cast<VkDeviceSize>(candidate_capacity) * sizeof(RenderClusterResources::GpuCullInstance),
            ...
            dispatch_description.usage = static_cast<ERGBufferUsageFlags>(
                static_cast<std::uint32_t>(ERGBufferUsageBits::STORAGE) |
        ...
        pick_pass.setKernelFn([this, resources, instances, meshes, vertex_buffers, index_buffers](
            ...
            resources->forEachVisiblePickObject([&](RenderObjectHandle object, std::uint32_t token) {
                ...
                const bool is_invalid_index_segment = !is_missing_mesh &&
                    entries[mdc].ibo_segment >= index_buffers.size();
                const bool is_invalid_mesh = is_missing_mesh || is_unready_mesh || is_invalid_vertex_segment ||
                    is_invalid_index_segment;
                if (is_invalid_mesh)
                {
                    return;
                }
