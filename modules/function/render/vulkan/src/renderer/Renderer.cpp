...
{
    removeScene(scene_id, current_stamp_.serial);
}

void Renderer::removeScene(RenderSceneId scene_id, uint64_t last_possible_submission)
{
    ...
    retired_scenes_.push_back({std::move(scenes_.at(scene_id)), last_possible_submission});
...
void Renderer::beginFrame(const FrameStamp &stamp)
{
    ...
    for (auto &scene_ptr : scenes_.values())
    {
        if (scene_ptr)
        {
            // Only maintenance follows real time. Each instance publishes
            // its simulation clock in the same FIFO as its content updates.
            scene_ptr->setMaintenanceTime(scene_time);
...
bool Renderer::prepareSceneForRender(RenderScene &scene, const RenderTargetLayout &layout)
{
    ...
    if ((!gs.valid || layout_changed) && !scene.isGraphRecompileSuppressed())
    {
        ...
        {
            ctx_->reportError(renderError<err::frame::GraphRecompileThrash>(), scene.sceneId().index,
        ...
        {
            ctx_->reportError(renderError<err::frame::NoSceneColorTarget>(), scene.sceneId().index,
                              current_stamp_.serial);
        }
    }
...
bool Renderer::renderSingleView(RenderScene &scene, View &view, const RenderTargetBinding &binding, FrameRuntime &rt,
                                uint32_t cross_view_index)
{
    ...
    {
        ctx_->reportError(renderError<err::frame::BindingHasNoLayout>(), scene.sceneId().index, current_stamp_.serial);
        return false;
    }
    ...
    {
        if (!feature->viewReady(view.handle))
        {
            return clearRenderTarget(rt.primary_cmd, binding, rt.stamp.slotIndex(), rt.image_index);
        }
    }
...
bool Renderer::prepareViewForRender(RenderScene &scene, View &view, const RenderTargetBinding &binding,
                                    FrameRuntime &rt)
{
    auto &gs = scene.graphState();
    ...
    const bool resized_or_ready = ResizeViewResources(
        *gs.graph, target_ext, *view.resource_state, scene.graphRecorder(), scene.graphAllocator(),
        ctx_->framesInFlight(), nullptr, [&scene, current_graph_desc](RGResourceState &&retired_state) {
...
void Renderer::renderView(RenderScene &scene, SceneGraphState &gs, View &view, const DrawRequest &req, FrameRuntime &rt,
                          uint32_t cross_view_index)
{
    ...
    scene.graphRecorder().record(*view.resource_state, *gs.graph, frame_ctx, rt.primary_cmd,
    ...
    {
        rt.external_graphics_waits.insert(rt.external_graphics_waits.end(), frame_ctx.external_graphics_waits.begin(),
                                          frame_ctx.external_graphics_waits.end());
    }
    if (!frame_ctx.external_graphics_signals.empty())
    {
        rt.external_graphics_signals.insert(rt.external_graphics_signals.end(),
