#pragma once
// ============================================================================
//  MaterialGraphBake.hpp — the shared "MaterialGraph -> baked MaterialAsset
...
#include <lux/engine/asset/MaterialAsset.hpp> // lux::asset::MaterialData
...
namespace lux::editor
{
    ...
    ///   + param defaults + shading-model + render-state + texture-slot UUIDs + graph.
    /// The shared core of bakeGraphMaterial (which then registers + writes the asset)
    /// and the producers that need a ready-to-upload payload without an asset
    /// (PreviewScene's resident default material, EditorBuiltins' white material).
    ///
    ...
    ///   (params / shading-model / render-state / texture-slot UUIDs / graph)
