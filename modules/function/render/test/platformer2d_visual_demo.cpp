...
namespace
{
    ...
    lux::asset::asset_id_t registerRgba8Texture(
    ...
    lux::asset::asset_id_t registerTileset(lux::asset::AssetManager& mgr)
    {
        ...
        for (std::uint32_t y = 0; y < TH; ++y)
            for (std::uint32_t x = 0; x < TW; ++x)
            {
                ...
                const bool is_left_edge = lx == 0;
                const bool is_top_edge = ly == 0;
                const bool is_right_edge = lx == TS - 1;
                const bool is_bottom_edge = ly == TS - 1;
                const bool is_border = is_left_edge || is_top_edge || is_right_edge || is_bottom_edge;
                if (is_border)
    ...
    lux::asset::asset_id_t registerPlayerSheet(lux::asset::AssetManager& mgr)
    {
        ...
        const auto put = [&](std::uint32_t f, int x, int y, std::uint32_t c) {
            const bool is_before_left = x < 0;
            const bool is_before_top = y < 0;
            const bool is_after_right = x >= static_cast<int>(FS);
            const bool is_after_bottom = y >= static_cast<int>(TH);
            const bool is_out_of_bounds = is_before_left || is_before_top || is_after_right || is_after_bottom;
            if (is_out_of_bounds)
                return;
        ...
        for (std::uint32_t f = 0; f < FRAMES; ++f)
        {
            ...
            for (int y = 0; y < 5; ++y)
            {
                put(f, 6 + (l * y) / 5, y, kLeg);
                put(f, 7 + (l * y) / 5, y, kLeg);
                put(f, 8 + (r * y) / 5, y, kLeg);
                put(f, 9 + (r * y) / 5, y, kLeg);
            }
        }
    ...
    struct AnimAssets
    {
        lux::asset::asset_id_t idle, walk;
    };
    AnimAssets registerAnimAssets(lux::asset::AssetManager& mgr, lux::asset::asset_id_t sheet)
    {
        ...
        for (int i = 0; i < 4; ++i)
            atlas->frames.push_back(
                {"run" + std::to_string(i),
                 ...
                 Eigen::Vector2f(0.5f, 0.f)}
            ); // pivot at the feet
        auto atlas_info = makeInfo("dddd1111-2222-3333-4444-555566667777", lux::asset::EAssetType::TEXTURE_ATLAS);
        const auto atlas_id = atlas_info->id;
        mgr.registerAsset(std::make_unique<lux::asset::TextureAtlasAsset>(std::move(atlas_info), std::move(atlas)));
        ...
        const auto makeClip = [&](const char* uuid, const char* name, std::vector<lux::rdesc::FlipbookFrame> frames) {
            ...
            const auto id = info->id;
            mgr.registerAsset(std::make_unique<lux::asset::FlipbookClipAsset>(std::move(info), std::move(clip)));
            return id;
        };
        AnimAssets out{};
...
main(int argc, char** argv)
{
    ...
    if (!lux::rendertest::VisualSoak::parse(argc, argv, soak))
        return 2;
    std::printf(
        "=== platformer2d_visual_demo ===\n"
        "    A/D or arrows = run, Space = jump. The cyan platform is ONE-WAY.\n"
        "    mode: %s\n",
        soak.enabled() ? "soak — exits by itself" : "interactive — close the window to exit"
    );
    ...
    lux::asset::AssetServices asset_services{assets, async.assetClient()};
    ...
    const bool adopted_fixed = staged.adopt(fixed);
    const bool adopted_gravity = adopted_fixed && staged.adopt(gravity);
    const bool adopted_asset_services = adopted_gravity && staged.adopt(asset_services);
    const bool adopted_tilemap_runtime = adopted_asset_services && staged.adopt(tilemap_runtime);
    const bool is_invalid_services = !adopted_tilemap_runtime;
    ...
    const bool installed_rendering = installed_presentation && lux::ecs::installPresentation2DRendering(
            ...
            *residency_owner);
    const bool installed_physics = installed_rendering && staged.emplace<d2::Physics2DWorld>(gravity);
    const bool is_invalid_assembly = !installed_physics;
    if (is_invalid_assembly)
    {
    ...
    simulation->setPhase(
        ...
        [physics](lux::ecs::Registry& registry, float dt) { physics->step(registry, dt); }
    );
    ...
    if (services.get<d2::Simulation2DSystem>() == nullptr || services.get<d2::Physics2DWorld>() == nullptr)
    {
    ...
    tm.id = d2::TilemapId{uuids::uuid::from_string("73000000-0000-4000-8000-000000000001").value()};
    ...
    level_chunk.tiles.assign(d2::TilemapRuntime::kChunkTileCount, lux::rdesc::kEmptyTile);
    if (!tilemap_runtime.loadChunk(tilemap, std::move(level_chunk)))
    {
        return 1;
    }
    ...
    for (int i = 0; i < 120; ++i)
    {
        ...
        world.emplace<d2::Transform2DComponent>(e).position = {
            ...
            static_cast<double>(0.5f + (static_cast<float>((i * 37) % 40)) * kTile)};
    ...
    while (fx.running())
    {
        ...
        const bool left = snapshot.isKeyHeld(lux::input::EKey::KEY_A) || snapshot.isKeyHeld(lux::input::EKey::KEY_LEFT);
        const bool right =
            snapshot.isKeyHeld(lux::input::EKey::KEY_D) || snapshot.isKeyHeld(lux::input::EKey::KEY_RIGHT);
        ...
        const double target_x = std::min(std::max(pp.x, 2.3), static_cast<double>(MAP_W * kTile - 2.3f));
        world.registry().patch<d2::Transform2DComponent>(cam, [&](auto& tc) {
            auto& cam_p = tc.position;
            cam_p.x += (target_x - cam_p.x) * static_cast<double>(std::min(1.f, dt * 6.f));
            cam_p.y += (pp.y + 0.9 - cam_p.y) * static_cast<double>(std::min(1.f, dt * 3.f));
        }
        );
