...
#include <lux/engine/resource/asset/pak/visibility.h>
...
namespace lux::asset::detail
{
    ...
    LUX_ASSET_PAK_PUBLIC bool writePakFile(
        const std::filesystem::path& out_pak,
        std::vector<PakWriteEntry> entries,
        std::string_view mount_hint,
        std::string* error_out = nullptr);

    LUX_ASSET_PAK_PUBLIC bool readPakHeader(
        std::istream& stream,
        std::uint64_t file_size,
        PakHeader& output,
        std::string* error_out = nullptr);

    LUX_ASSET_PAK_PUBLIC bool readPakPage(
        std::istream& stream,
        std::uint64_t file_size,
        std::uint64_t offset,
        PakPage& output,
    ...
    [[nodiscard]] LUX_ASSET_PAK_PUBLIC bool verifyPakPageDigest(
        const PakPage& page,
        const lux::cxx::algorithm::Sha256Digest& expected) noexcept;

    [[nodiscard]] LUX_ASSET_PAK_PUBLIC PakPageHeader pakPageHeader(
        const PakPage& page) noexcept;

    LUX_ASSET_PAK_PUBLIC bool decodeEntryLeaf(
        const PakPage& page,
        std::vector<PakEntry>& output,
        std::string* error_out = nullptr);

    LUX_ASSET_PAK_PUBLIC bool decodeEntryInternal(
        const PakPage& page,
        std::vector<PakEntryChild>& output,
        std::string* error_out = nullptr);

    LUX_ASSET_PAK_PUBLIC bool decodePathLeaf(
        const PakPage& page,
        std::vector<PakPathRow>& output,
        std::string* error_out = nullptr);

    LUX_ASSET_PAK_PUBLIC bool decodePathInternal(
        const PakPage& page,
    ...
    LUX_ASSET_PAK_PUBLIC bool readAllPakEntries(
        std::istream& stream,
        std::uint64_t file_size,
