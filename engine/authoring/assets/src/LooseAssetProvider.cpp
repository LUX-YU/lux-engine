...
namespace lux::authoring
{
    ...
    LooseAssetProvider::LooseAssetProvider(std::filesystem::path root_dir)
        : root_dir_(std::move(root_dir))
    {
    }

    std::size_t LooseAssetProvider::rescan()
    {
        entries_.clear();
        by_path_.clear();
        by_id_.clear();
    ...
    LooseAssetProvider::filePathOf(const asset_id_t& id) const
    {
    ...
    LooseAssetProvider::resolve(std::string_view rel_vpath) const
    {
    ...
    bool LooseAssetProvider::contains(const asset_id_t& id) const
    {
        return by_id_.contains(id);
    }

    lux::cxx::expected<AssetBlob, EAssetError>
    LooseAssetProvider::open(const asset_id_t& id) const
    {
        const auto it = by_id_.find(id);
        if (it == by_id_.end())
        ...
        return AssetBlob::fromSharedArray(std::move(bytes), size);
    }

    void LooseAssetProvider::enumerate(
    ...
    LooseAssetProvider::pathOf(const asset_id_t& id) const
    {
        const auto it = by_id_.find(id);
        if (it == by_id_.end())
            return std::nullopt;
