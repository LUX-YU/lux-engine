...
bool registeredMetadata(const lux::flowforge::Node &node, const lux::flowforge::FlowSourceEnvironment &environment)
{
    ...
    if (node.operation() == ENodeOperation::NATIVE_FUNC_CALL)
    {
        ...
        if (!call.ownerType())
        {
            return std::ranges::any_of(environment.functions,
                                       [&](const auto *function) { return &function->invokable == &call.info(); });
        }
        return std::ranges::any_of(environment.classes, [&](const auto *type) {
    ...
    if (node.operation() == ENodeOperation::GET_FIELD || node.operation() == ENodeOperation::SET_FIELD)
    {
        ...
        const auto *field =
            reading ? static_cast<const GetFieldNode &>(node).field() : static_cast<const SetFieldNode &>(node).field();
...
{
    return {EEditorError::INVALID_STATE, "flowforge.history", static_cast<std::uint64_t>(failure.code), {}, failure};
}
...
struct FlowForgeEditor::Data final
{
            ...
            if (write != EWrite::ERASE)
            {
                ...
                if (!variable)
                {
                    return lux::cxx::unexpected(editing::makeEditFailure(
                        editing::EEditError::PRECONDITION_FAILED, static_cast<std::uint64_t>(variable.error().code)));
                }
            ...
            return std::visit(
                [&](const auto &mutation) -> editing::EditResult<editing::PreparedEditPtr> {
                    ...
                    if constexpr (std::is_same_v<std::decay_t<decltype(mutation)>, Membership>)
                    {
                        ...
                        return prepareValue(insert ? EWrite::INSERT : EWrite::ERASE, mutation.position, mutation.value,
                                            budget);
                    }
                    else
                    {
      ...
      public:
            ...
            const editing::ApplyContext &context, editing::EditPreparationBudget &budget) const noexcept override
        {
            ...
            auto patch = lux::flowforge::FlowGraphEdit::prepare(
                owner_.source.graph, {insert, delta.erase, delta.connect, delta.disconnect,
                                      input_ ? std::span<const lux::graph::GraphLayoutEntry>{} : std::span{delta.place},
                                      delta.unplace, !input_});
            if (!patch)
            {
                return lux::cxx::unexpected(editing::makeEditFailure(editing::EEditError::PRECONDITION_FAILED,
    ...
    struct Compilation final
    {
        ...
        Compilation(FlowCompileId request, const editing::HistorySnapshot &history,
                    ...
                    process::ExecutionRuntime &execution, std::filesystem::path linker_path, std::string package_path)
...
EditorResult<std::unique_ptr<FlowForgeEditor>> FlowForgeEditor::open(const lux::flowforge::FlowSourceDocument &source,
                                                                     Project &project,
                                                                     process::ExecutionRuntime &runtime,
                                                                     lux::flowforge::FlowSourceEnvironment environment)
{
    ...
    {
        return lux::cxx::unexpected(EditorFailure{EEditorError::SOURCE_FAILURE, "flowforge.materialize",
                                                  static_cast<std::uint64_t>(graph.error().code), graph.error().field,
                                                  graph.error()});
    }
    return adopt(source.id, source.name, *graph, project, runtime, environment);
}
EditorResult<std::unique_ptr<FlowForgeEditor>> FlowForgeEditor::adopt(lux::asset::AssetId id, std::string name,
                                                                      lux::flowforge::FlowGraph &graph,
                                                                      Project &project,
                                                                      process::ExecutionRuntime &runtime,
                                                                      lux::flowforge::FlowSourceEnvironment environment)
{
    ...
    auto data =
        std::make_unique<Data>(Content{id, std::move(name), std::move(graph)}, project, runtime, std::move(*history));
...
EditorResult<lux::flowforge::FlowSourceDocument> FlowForgeEditor::capture() const
{
    ...
    if (!result)
    {
        return lux::cxx::unexpected(EditorFailure{EEditorError::SOURCE_FAILURE, "flowforge.capture",
                                                  static_cast<std::uint64_t>(result.error().code), result.error().field,
                                                  result.error()});
    }
    return std::move(*result);
}
...
editing::EditResult<lux::flowforge::NodeId> FlowForgeEditor::insertNode(std::unique_ptr<lux::flowforge::Node> &input,
    ...
    const lux::flowforge::FlowSourceSignature &signature)
{
    ...
    const bool invalid_name =
        name.empty() || name.size() > FlowSourceLimits{}.max_string_bytes || name.find('\0') != std::string_view::npos;
    ...
    editing::EditOperationPtr edit =
        std::make_unique<Data::GraphEdit>(*data_, std::move(before), std::move(after), "Edit function signature", bytes,
                                          nullptr, std::move(replacements));
    BusyGuard guard(data_->busy);
    ...
    std::vector<lux::flowforge::ExportMethodNode> exports)
{
    ...
    for (const auto &value : exports)
    {
        ...
        const bool invalid_entry =
            found == data_->read_nodes.end() || found->second->operation() != lux::flowforge::ENodeOperation::ON_EVENT;
...
editing::EditResult<editing::ApplyResult> FlowForgeEditor::removeNodes(std::span<const lux::flowforge::NodeId> nodes,
...
editing::EditResult<editing::ApplyResult> FlowForgeEditor::connect(lux::flowforge::PinId from, lux::flowforge::PinId to)
{
    Data::GraphDelta before, after;
    const auto &topology = data_->source.graph.topology();
...
editing::EditResult<editing::ApplyResult> FlowForgeEditor::setVariable(const lux::flowforge::FlowSourceVariable &value)
{
    if (auto allowed = data_->canEdit(); !allowed)
    {
        return lux::cxx::unexpected(allowed.error());
...
EditorResult<SaveRequestId> FlowForgeEditor::requestPublish(FlowCompileId compile, std::string origin)
{
    ...
    data_->save.emplace<FlowSave>(id, *ticket, job.revision, data_->source.id, image, data_->project, data_->runtime,
                                  *data_->history);
    return id;
}
...
struct FlowCodec final
{
    ...
    {
        auto source = lux::flowforge::decodeFlowSource({reinterpret_cast<const char *>(bytes.data()), bytes.size()});
        if (!source)
        {
...
EditorResult<std::unique_ptr<DocumentOpening>> openFlowForgeDocument(Project &project,
