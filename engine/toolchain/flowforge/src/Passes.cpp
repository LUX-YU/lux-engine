...
namespace
{
    ...
    llvm::SmallVector<OpT, 4> collectTerminators(mlir::Region& region)
    {
    ...
    void lowerBranchToCF(lf::BranchOp op)
    {
        ...
        b.create<cf::CondBranchOp>(loc, op.getCond(),
                                   thenEntry, mlir::ValueRange{},
                                   elseEntry, mlir::ValueRange{});
        op.erase();
    }
    ...
    void lowerForLoopToCF(lf::ForLoopOp op)
    {
...

        {
            ...
            mlir::Value cmp = b.create<mlir::arith::CmpIOp>(
                loc, mlir::arith::CmpIPredicate::slt, iv, op.getLast());
            b.create<cf::CondBranchOp>(loc, cmp,
                                       bodyEntry, mlir::ValueRange{iv},
                                       exit, mlir::ValueRange{});
        }
        {
    ...
    void lowerWhileLoopToCF(lf::WhileLoopOp op)
    {
        ...
        for (lf::CondYieldOp cy : collectTerminators<lf::CondYieldOp>(condRegion)) {
            mlir::OpBuilder b(cy);
            b.create<cf::CondBranchOp>(loc, cy.getCond(),
                                       bodyEntry, mlir::ValueRange{},
                                       exit, mlir::ValueRange{});
            cy.erase();
        }
        for (lf::YieldOp y : collectTerminators<lf::YieldOp>(bodyRegion)) {
    ...
    class FlowForgeToCFPass
        : public mlir::PassWrapper<FlowForgeToCFPass,
    ...
    public:
        ...
        {
            reg.insert<cf::ControlFlowDialect, mlir::func::FuncDialect,
        ...
        void runOnOperation() override
        {
            ...
            module.walk([&](mlir::func::FuncOp fn) {
                returns_ok &= lowerReturnsToExit(fn);
            });
            if (!returns_ok)
...
)
{
    ...
    if (!maybeEngine)
        return lux::cxx::unexpected(FlowForgeFailure{
            ...
            .message = "runMainJIT: ExecutionEngine::create failed: "
    ...
    if (!expectedFn)
        return lux::cxx::unexpected(FlowForgeFailure{
            ...
            .message = "runMainJIT: lookup(main) failed: "
    ...
    std::vector<std::byte> state(
        std::max<uint64_t>(ir.impl().state_size, 1), std::byte{0});
    const auto& defaults = ir.impl().state_defaults;
    if (!defaults.empty())
        std::memcpy(state.data(), defaults.data(),
                    std::min(defaults.size(), state.size()));
