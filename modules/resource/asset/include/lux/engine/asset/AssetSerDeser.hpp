...
namespace lux::asset
{
    ...
    public:
        ...
        static EAssetError loadHeader(const std::vector<std::byte>& data, AssetFileHeader& header, InfoType& info)
        {
            uint32_t        magic{};
            asset_version_t version{};
        ...
        template<EAssetType T>
        static EAssetError loadHeaderRaw(const std::vector<std::byte>& data, AssetFileHeader& header)
        {
            uint32_t        magic{};
            asset_version_t version{};
            if (const auto e = peekHeaderPrefix(data, magic, version); e != EAssetError::SUCCESS)
    ...
    private:
        ...
        static EAssetError peekHeaderPrefix(const std::vector<std::byte>& data,
        ...
        template<typename InfoType, EAssetType T> requires std::is_standard_layout_v<InfoType>
        static EAssetError loadHeaderV1(const std::vector<std::byte>& data, AssetFileHeader& header, InfoType& info)
        {
            if (data.size() < sizeof(compat::AssetFileHeaderV1))
                return EAssetError::ABNORMAL_FILE_SIZE;
            compat::AssetFileHeaderV1 v1{};
    ...
    /**
     * @brief Creates the SerDeser owning the given asset type.
     *
     * The single runtime type->SerDeser table. Every magic-dispatched loader
     * (editor content scan, asset tool, pak provider) goes through here —
     * pair with assetTypeOfMagic() to dispatch a file by header magic.
     *
     * @return The matching SerDeser, or nullptr when the type has none
     *         (UNKNOWN / SCENE).
     */
    LUX_RESOURCE_PUBLIC std::unique_ptr<AssetSerDeser>
    createSerDeserFor(EAssetType type, std::shared_ptr<AssetManager> manager);

    // ─── Lazy loading: pure-data decode + shell injection ──────────────────
    //
    // 懒加载(见 .internal/async-asset-redesign-2026-06-17.md §1.5):
    // 启动只造 info 空壳;用到才后台解析纯数据,同步点注入空壳。

    /// 类型擦除的"把已解码的纯数据注入(已注册的)空壳"步骤。由 decodeAssetData
    /// 在后台线程产出(捕获纯数据 + 知道往哪个具体资产类型 setData),主线程同步点
    /// 对空壳调用它即完成注入。move-only(捕获 unique_ptr 数据)。
    using AssetDataInjector = lux::cxx::move_only_function<void(LuxAsset&)>;

    /// 把一段 .luxasset 内存镜像**解析成纯数据**并包成类型擦除注入器。读 magic 分派到
    /// 对应 XxxSerDeser::decodeData(纯计算、不碰 AssetManager → 后台线程安全)。
    /// 清单/非 asset_data_t 型(MODEL / SCRIPT)返回 UNSUPPORTED —— 那些走 eager 全载。
    [[nodiscard]] LUX_RESOURCE_PUBLIC lux::cxx::expected<AssetDataInjector, EAssetError>
    decodeAssetData(const void* bytes, std::size_t len);

    /// 由 info 造一个**无数据的具体资产空壳**(按 type 分派 → make_unique<具体Asset>(info))。
    /// 供启动期懒注册:壳进 AssetManager,数据按需异步注入。可懒加载的 7 类返回壳;
    /// MODEL/SCRIPT/UNKNOWN 返回 nullptr(调用方对它们走 eager 全载)。
    [[nodiscard]] LUX_RESOURCE_PUBLIC std::unique_ptr<LuxAsset>
    makeAssetShell(std::unique_ptr<AssetInfo> info);

    /// 直接由磁盘 .luxasset 文件造一个**无数据空壳**:只读文件头(不载 data 段),
    /// 按 {magic,version} 前缀做版本感知解码(v1 复用 compat::upgradeAssetInfo 升级,
    /// **不裸 memcpy header.info**),再委托 makeAssetShell。供启动期廉价懒注册。
    /// 失败语义:文件打不开 → FILE_OPEN_FAIL;尺寸/头损坏 → ABNORMAL_FILE_SIZE;
    /// 版本不识别 → UNSUPPORTED_VERSION;非懒类型(MODEL/SCRIPT/SHADER/未知,
    /// makeAssetShell 返回 nullptr)→ UNSUPPORTED(调用方据此回退 eager 全载)。
    [[nodiscard]] LUX_RESOURCE_PUBLIC lux::cxx::expected<std::unique_ptr<LuxAsset>, EAssetError>
