...
namespace lux::render
{
    ...
    public:
        ...
        {
            if (!initialized_)
                return "TrajectoryResources: not initialized";
            return "TrajectoryResources: " + std::to_string(global_buf_.usedVertices()) + " vertices";
        }
        ...
        void submitTransfers(TransferScheduler& scheduler)
        {
            ...
            for (auto& op : pending_ops_)
            {
                ...
                case TrajOpKind::Append:
                {
                    ...
                    {
                        const uint32_t needed = existing->vertex_count + count;
