...
Expected<void> RenderClusterFeature::initAndAttachTo(RenderScene &scene)
{
    ...
    if (!resources.initializeGpuCulling(context.deviceContext(), context.deferredDestroyQueue(),
                                        context.framesInFlight(), instances->capacity()))
    {
        return renderFailure<err::feature::ResourceInitFailed>();
    }
    if (!resources.initializePicking(context.deviceContext(), context.deferredDestroyQueue(), context.framesInFlight()))
    {
    ...
    const auto register_pipeline =
        [&]<typename Pipeline, typename Layout>(ShaderHandle configured, EBuiltinShader builtin, const char *name,
                                                Pipeline &output_pipeline, Layout &output_layout) -> Expected<void> {
        const auto shader = resolveShaderStage(shaders, configured, builtin);
        if (!shader)
        {
        ...
        output_layout = context.pipelineManager().computeSetLayout(output_pipeline, 0u);
    ...
    if (auto result = register_pipeline(config_.cluster_cull_compute_shader, EBuiltinShader::RENDER_CLUSTER_CULL_COMP,
                                        "RenderClusterCull", cluster_cull_pipeline_, cluster_cull_set_layout_);
        !result)
    {
    ...
    if (auto result =
            register_pipeline(config_.candidate_expand_compute_shader, EBuiltinShader::RENDER_CLUSTER_EXPAND_COMP,
                              "RenderClusterCandidateExpand", candidate_expand_pipeline_, candidate_expand_set_layout_);
        !result)
    {
        candidate_source.clear();
        resources.shutdownGpuCulling();
    ...
    const auto pick_vertex =
        resolveShaderStage(shaders, config_.pick_vertex_shader, EBuiltinShader::RENDER_CLUSTER_PICK_VERT);
    const auto pick_fragment =
        resolveShaderStage(shaders, config_.pick_fragment_shader, EBuiltinShader::RENDER_CLUSTER_PICK_FRAG);
    if (!pick_vertex || !pick_fragment)
    {
        candidate_source.clear();
        resources.shutdownGpuCulling();
        resources.shutdownPicking();
        return !pick_vertex ? lux::cxx::unexpected(pick_vertex.error()) : lux::cxx::unexpected(pick_fragment.error());
    }
    const std::array<ShaderHandle, 2u> pick_stage_handles{*pick_vertex, *pick_fragment};
    ...
    {
        constexpr VkDescriptorSetLayoutBinding pick_result_binding{0u, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 1u,
                                                                   VK_SHADER_STAGE_FRAGMENT_BIT, nullptr};
        const DescriptorLayoutDesc pick_result_layout{.bindings = {&pick_result_binding, 1u},
    ...
    pick_template.explicit_set_layouts.push_back({2u, pick_set_layout_});
    ...
    pick_template.active_sets_mask = (1u << 0u) | (1u << 2u);
    ...
    pick_template.vertex_bindings.push_back(VkVertexInputBindingDescription{
        0u, static_cast<std::uint32_t>(sizeof(lux::rdesc::Vertex)), VK_VERTEX_INPUT_RATE_VERTEX});
    pick_template.vertex_attributes.push_back(VkVertexInputAttributeDescription{
        0u, 0u, VK_FORMAT_R32G32B32_SFLOAT, static_cast<std::uint32_t>(offsetof(lux::rdesc::Vertex, position))});
    ...
    pick_template.push_constant_ranges.push_back(
        VkPushConstantRange{VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT, 0u, 112u});
    const std::array<const lux::rdesc::ShaderInfo *, 2u> pick_infos{&pick_stages->info(0u), &pick_stages->info(1u)};
    const auto pick_pipeline = context.pipelineManager().registerGraphicsTemplate(pick_template, pick_infos);
    if (!pick_pipeline)
    {
        candidate_source.clear();
        resources.shutdownGpuCulling();
    ...
    pick_set_layout_ = context.pipelineManager().templateSetLayout(pick_pipeline_, 2u);
    if (pick_set_layout_ == VK_NULL_HANDLE)
    {
        candidate_source.clear();
        resources.shutdownGpuCulling();
...
bool RenderClusterFeature::allocateViewState(std::uint32_t view_index, RenderScene &scene)
{
    ...
    scene.forEachActiveView([view_index, resources, instances](auto &view) {
        ...
        resources->forEachVisibleObject([&view, instances](RenderObjectHandle object) {
            const auto slot = instances->resolveSlot(object);
            if (!instances->isAlive(slot))
            {
                return;
            }
            view.registerObject(object, instances->cullMetaAt(slot).bucket_id);
        });
    });
    return true;
}
...
void RenderClusterFeature::onFrameBegin(const FeatureFrameContext &context)
{
    ...
    for (const auto family : resources->hierarchyParents())
    {
        ...
        scene.forEachActiveView([&](const auto &view) {
            const auto *camera = cameras ? cameras->find(view.handle.index) : nullptr;
            if (!camera)
            {
                return;
            }
            ...
            {
                const auto page_delta = static_cast<std::int64_t>(parent->header.bounds_center.page_delta[axis]) -
                                        static_cast<std::int64_t>(camera->render_origin.page_delta[axis]);
                const auto relative = static_cast<double>(page_delta) * camera->coordinate_page_size +
                                      parent->header.bounds_center.local[axis] - camera->render_origin.local[axis];
                distance_squared += relative * relative;
            }
            const auto distance = std::max(0.01, std::sqrt(distance_squared) - parent->header.bounds_radius);
            const auto projection_scale = std::abs(static_cast<double>(camera->camera_view.proj(1, 1)));
            const auto pixel_error = static_cast<double>(parent->header.lod_error) * projection_scale *
                                     camera->extent.height / (2.0 * distance);
            ...
            const auto threshold = resources->prefersChildren(family) ? resources->hlodExitErrorPixels()
                                                                      : resources->hlodEnterErrorPixels();
            prefer_children = prefer_children || pixel_error > threshold;
        });
        if (!has_camera)
        {
        ...
        for (const auto change : resources->reconcileHierarchy(family, prefer_children, scene.maintenanceTime(),
                                                               resources->transitionDurationSeconds()))
        {
            ...
            for (std::size_t index = 0u; index < cluster->objects.size(); ++index)
            {
                ...
                property.transition_start_time = change.transition_start_time;
                property.transition_duration = change.transition_duration;
                property.transition_seed = change.transition_seed;
                property.transition_flags =
                    change.transition == RenderClusterResources::ETransitionAction::NONE
                        ? 0u
                        : 1u | (change.transition == RenderClusterResources::ETransitionAction::FADE_OUT ? 2u : 0u);
                ...
                scene.forEachActiveView([object, bucket, visible = change.visible](auto &view) {
                    if (visible)
                    {
    ...
    if (!resources->prepareGpuCulling(context.frame_index, *instances, capacity_changed))
    {
        ...
        {
            candidate_source->clear();
            scene.invalidateGraph(EGraphInvalidationReason::MDC_STORAGE_GENERATION);
        }
        renderContext().reportError(renderError<err::feature::ResourceInitFailed>(), scene.sceneId().index,
                                    context.frame_index);
        return;
    }
    if (candidate_source)
...
void RenderClusterFeature::addPasses(RGBuilder &builder)
{
    ...
    const auto import_storage = [&builder](std::string name, VkDeviceSize size, std::uint32_t stride, auto getter) {
        RGBufferDescription description{};
        description.size = std::max<VkDeviceSize>(size, stride);
        description.stride = stride;
        description.element_count = static_cast<std::uint32_t>(description.size / stride);
    ...
    const bool has_cull_layouts =
        cluster_cull_set_layout_ != VK_NULL_HANDLE && candidate_expand_set_layout_ != VK_NULL_HANDLE;
    const bool has_cull_buffers = resources->gpuCullBufferCount() != 0u;
    const bool can_render_candidates =
        has_candidate_source && has_cull_pipelines && has_cull_layouts && has_cull_buffers;
    if (can_render_candidates)
    {
        const auto candidate_capacity = candidate_source->capacity();
        const auto import_cull_input = [&builder, resources](std::string name, VkDeviceSize size, std::uint32_t stride,
                                                             bool clusters) {
            RGBufferDescription description{};
            description.size = std::max<VkDeviceSize>(size, stride);
            description.stride = stride;
            ...
            imported.buffer_getter = [resources, clusters](VkBuffer *output, std::uint32_t capacity) {
                const auto count = std::min(capacity, resources->gpuCullBufferCount());
                for (std::uint32_t index = 0u; index < count; ++index)
                {
                    output[index] =
                        clusters ? resources->gpuCullClusterBuffer(index) : resources->gpuCullInstanceBuffer(index);
                }
                return count;
            };
            return builder.importBuffer(std::move(name), description, std::move(imported));
        ...
        const auto cluster_input = import_cull_input("RenderClusterCullInput",
                                                     static_cast<VkDeviceSize>(candidate_capacity) *
                                                         sizeof(RenderClusterResources::GpuCullCluster),
                                                     sizeof(RenderClusterResources::GpuCullCluster), true);
        const auto cluster_instance_input = import_cull_input("RenderClusterInstanceInput",
                                                              static_cast<VkDeviceSize>(candidate_capacity) *
                                                                  sizeof(RenderClusterResources::GpuCullInstance),
                                                              sizeof(RenderClusterResources::GpuCullInstance), false);
        const auto candidate_dynamic_slots =
            import_storage("RenderClusterCandidateDynamicSlots",
                           static_cast<VkDeviceSize>(std::max(instances->capacity(), 1u)) * sizeof(std::uint32_t),
                           sizeof(std::uint32_t), [instances](VkBuffer *output, std::uint32_t capacity) {
        ...
        visibility_description.size = static_cast<VkDeviceSize>(candidate_capacity) * sizeof(std::uint32_t);
        visibility_description.stride = sizeof(std::uint32_t);
        visibility_description.element_count = candidate_capacity;
        visibility_description.usage = static_cast<ERGBufferUsageFlags>(ERGBufferUsageBits::STORAGE);
        visibility_description.memory_usage = ERGMemoryUsage::GPU_ONLY;
        const auto visibility = builder.createBuffer("RenderClusterVisibility", visibility_description);

        RGBufferDescription candidate_description{};
        candidate_description.size = static_cast<VkDeviceSize>(candidate_capacity) * sizeof(std::uint32_t);
        candidate_description.stride = sizeof(std::uint32_t);
        candidate_description.element_count = candidate_capacity;
        candidate_description.usage = ERGBufferUsageBits::STORAGE | ERGBufferUsageBits::TRANSFER_DST;
        candidate_description.memory_usage = ERGMemoryUsage::GPU_ONLY;
        const auto candidates =
            builder.createBuffer(MeshCullCandidateSource::kCandidateSlotsResource, candidate_description);
        ...
        dispatch_description.element_count = sizeof(CandidateDispatchState) / sizeof(std::uint32_t);
        dispatch_description.usage =
            static_cast<ERGBufferUsageFlags>(static_cast<std::uint32_t>(ERGBufferUsageBits::STORAGE) |
                                             static_cast<std::uint32_t>(ERGBufferUsageBits::INDIRECT) |
                                             static_cast<std::uint32_t>(ERGBufferUsageBits::TRANSFER_DST));
        ...
        dispatch_import.buffer_getter = [resources](VkBuffer *output, std::uint32_t capacity) {
            const auto count = std::min(capacity, resources->gpuCullBufferCount());
            for (std::uint32_t index = 0u; index < count; ++index)
            {
                output[index] = resources->gpuCandidateDispatchBuffer(index);
            }
            return count;
        };
        const auto dispatch_args =
            builder.importBuffer(MeshCullCandidateSource::kDispatchArgsResource, dispatch_description, dispatch_import);

        const auto cull_descriptors =
            builder.createTransientDS("RenderClusterCullDS", cluster_cull_set_layout_,
                                      {
        ...
        builder.addPass("RenderClusterCull", ERGPassType::COMPUTE)
            ...
            .setKernelFn([resources, this](const PassRecordContext &context) {
                if (!context.view || context.pipeline_layout == VK_NULL_HANDLE)
                {
                    return;
                }
                const auto cluster_count = resources->gpuCullClusterCount(context.frame.frame_index);
                if (cluster_count == 0u)
                {
                    return;
                }
                ...
                if (camera)
                {
                    ...
                    for (std::size_t axis = 0u; axis < 3u; ++axis)
                    {
                        push.origin_page[axis] = camera->render_origin.page_delta[axis];
                        push.origin_local_page_size[axis] = camera->render_origin.local[axis];
                    }
                    push.origin_local_page_size[3] = camera->coordinate_page_size;
                    const Eigen::Matrix4f relative_view_projection = viewRelativeViewProjection(*camera);
                    std::memcpy(push.view_proj, relative_view_projection.data(), sizeof(push.view_proj));
                }
                vkCmdPushConstants(context.cmd, context.pipeline_layout, VK_SHADER_STAGE_COMPUTE_BIT, 0u, sizeof(push),
                                   &push);
                vkCmdDispatch(context.cmd, (cluster_count + 63u) / 64u, 1u, 1u);
            });

        const auto expand_descriptors =
            builder.createTransientDS("RenderClusterCandidateExpandDS", candidate_expand_set_layout_,
                                      {
                                          {0u, EDescriptorType::STORAGE_BUFFER, cluster_instance_input},
                                          {1u, EDescriptorType::STORAGE_BUFFER, visibility},
                                          {2u, EDescriptorType::STORAGE_BUFFER, candidate_dynamic_slots},
                                          {4u, EDescriptorType::STORAGE_BUFFER, candidates},
                                          {5u, EDescriptorType::STORAGE_BUFFER, dispatch_args},
                                      });

        constexpr std::string_view candidate_clear_pass = "RenderClusterCandidateClear";
        constexpr std::string_view candidate_append_pass = "RenderClusterCandidateAppend";
        builder.addPass(candidate_clear_pass, ERGPassType::TRANSFER)
            ...
            .setKernelFn([candidates, dispatch_args, candidate_capacity](const PassRecordContext &context) {
                const auto candidate_buffer = context.resolveBufferHandle(candidates);
                const auto dispatch_buffer = context.resolveBufferHandle(dispatch_args);
                if (candidate_buffer == VK_NULL_HANDLE || dispatch_buffer == VK_NULL_HANDLE)
                {
                    return;
                }
                synchronizeBeforeBufferTransferWrites(context.cmd, std::array{candidate_buffer, dispatch_buffer});
                vkCmdFillBuffer(context.cmd, candidate_buffer, 0u,
                                static_cast<VkDeviceSize>(candidate_capacity) * sizeof(std::uint32_t),
                                std::numeric_limits<std::uint32_t>::max());
                vkCmdFillBuffer(context.cmd, dispatch_buffer, 0u, sizeof(CandidateDispatchState), 0u);
            });
        ...
        builder.addPass(candidate_append_pass, ERGPassType::COMPUTE)
            ...
            .setKernelFn([resources, instances, candidate_capacity](const PassRecordContext &context) {
                if (!context.view || context.pipeline_layout == VK_NULL_HANDLE)
                {
                    return;
                }
                ...
                push.scene_index = context.frame.scene_index;
                push.view_index = context.frame.view_index;
                push.cluster_instance_count = resources->gpuCullInstanceCount(context.frame.frame_index);
                push.dynamic_count = instances->dynamicCount();
                push.candidate_capacity = candidate_capacity;
                push.cluster_count = resources->gpuCullClusterCount(context.frame.frame_index);
                vkCmdPushConstants(context.cmd, context.pipeline_layout, VK_SHADER_STAGE_COMPUTE_BIT, 0u, sizeof(push),
                                   &push);
                const auto input_count = std::max(push.cluster_instance_count, push.dynamic_count);
                if (input_count != 0u)
                {
                    vkCmdDispatch(context.cmd, (input_count + 63u) / 64u, 1u, 1u);
                }
            });

        builder.addPass(MeshCullCandidateSource::kProducerPass, ERGPassType::COMPUTE)
            .setComputePipeline(candidate_expand_pipeline_)
            .bindTransientDS(0u, expand_descriptors)
            .read(candidates, ERGBufferRole::STORAGE)
            .readWrite(dispatch_args, ERGBufferRole::STORAGE)
            .after(candidate_append_pass)
            .setKernelFn([resources, candidate_capacity](const PassRecordContext &context) {
                if (!context.view || context.pipeline_layout == VK_NULL_HANDLE)
                {
                    return;
                }
                ...
                resources->markGpuCandidateSubmitted(context.frame.frame_index);
            });
    }

    if (!pick_pipeline_.valid() || pick_set_layout_ == VK_NULL_HANDLE || resources->pickBufferCount() == 0u)
    {
        return;
    }
    ...
    result_description.usage = static_cast<ERGBufferUsageFlags>(ERGBufferUsageBits::STORAGE);
    ...
    result_import.buffer_getter = [resources](VkBuffer *output, std::uint32_t capacity) {
        const auto count = std::min(capacity, resources->pickBufferCount());
        for (std::uint32_t index = 0u; index < count; ++index)
        {
    ...
    const auto result = builder.importBuffer("RenderClusterPickResult", result_description, result_import);

    const auto descriptors = builder.createTransientDS("RenderClusterPickDS", pick_set_layout_,
                                                       {
    ...
    const auto import_segmented_buffers = [&builder, meshes](std::string_view prefix, std::uint16_t count,
                                                             VkBuffer (MeshResources::*buffer_at)(std::uint16_t) const,
                                                             ERGBufferUsageBits usage, std::uint32_t stride) {
        ...
        for (std::uint16_t segment = 0u; segment < count; ++segment)
        {
            ...
            import.buffer_getter = [meshes, buffer_at, segment](VkBuffer *output, std::uint32_t capacity) {
                if (!output || capacity == 0u)
                {
                    return 0u;
                }
            ...
            buffers.push_back(
                builder.importBuffer(std::string(prefix) + "." + std::to_string(segment), description, import));
        }
        return buffers;
    };
    const auto index_buffers =
        import_segmented_buffers("ClassicMeshPickIndexBuffer", meshes->iboSegmentCount(), &MeshResources::indexBuffer,
                                 ERGBufferUsageBits::INDEX, sizeof(std::uint32_t));
    const auto vertex_buffers =
        import_segmented_buffers("ClassicMeshPickVertexBuffer", meshes->vboSegmentCount(), &MeshResources::vertexBuffer,
                                 ERGBufferUsageBits::VERTEX, sizeof(lux::rdesc::Vertex));
    if (index_buffers.empty() || vertex_buffers.empty())
    {
        return;
    }

    auto depth_description = RGTextureDescription::Absolute(1u, 1u, lux::rdesc::ETextureFormat::D32_SFLOAT);
    depth_description.usage = static_cast<ERGTextureUsageFlags>(ERGTextureUsageBits::DEPTH_STENCIL);
    depth_description.keep_transient = true;
    const auto depth = builder.createTexture("RenderClusterPickDepth", depth_description);

    auto pick_pass = builder.addPass("RenderClusterRasterPick", ERGPassType::GRAPHICS);
    pick_pass.setPipeline(pick_pipeline_)
        .bindSceneDS()
        .bindTransientDS(2u, descriptors)
        .write(result, ERGBufferRole::STORAGE)
        .write(depth, lux::render::ETextureRole::DEPTH_STENCIL_ATTACHMENT)
    ...
    pick_pass.setKernelFn([this, resources, instances, meshes, vertex_buffers,
                           index_buffers](const PassRecordContext &context) {
        if (!context.view || context.pipeline_layout == VK_NULL_HANDLE)
        {
            return;
        }
        const auto request = resources->pickRequestForView(context.view->handle.index);
        if (!request)
        {
            return;
        }
        ...
        {
            resources->failPick(*request, ERenderPickStatus::STALE);
            return;
        }
        const VkViewport pick_viewport{0.0f, 0.0f, 1.0f, 1.0f, 0.0f, 1.0f};
        const VkRect2D pick_scissor{{0, 0}, {1u, 1u}};
        vkCmdSetViewport(context.cmd, 0u, 1u, &pick_viewport);
        vkCmdSetScissor(context.cmd, 0u, 1u, &pick_scissor);
        ...
        push.scene_time = renderScene().maintenanceTime();
        ...
        resources->forEachVisiblePickObject([&](RenderObjectHandle object, std::uint32_t token) {
            const auto slot = instances->resolveSlot(object);
            if (!instances->isAlive(slot))
            {
                return;
            ...
            const bool is_missing_mesh = mesh == nullptr;
            const bool is_unready_mesh = !is_missing_mesh && !mesh->ready;
            const bool is_invalid_vertex_segment = !is_missing_mesh && mesh->vbo_segment >= vertex_buffers.size();
            const bool is_invalid_index_segment = !is_missing_mesh && entries[mdc].ibo_segment >= index_buffers.size();
            const bool is_invalid_mesh =
                is_missing_mesh || is_unready_mesh || is_invalid_vertex_segment || is_invalid_index_segment;
            if (is_invalid_mesh)
            {
                return;
            }
            ...
            {
                const auto vertex = context.resolveBufferHandle(vertex_buffers[mesh->vbo_segment]);
                if (vertex == VK_NULL_HANDLE)
                {
                    return;
                }
                constexpr VkDeviceSize vertex_offset = 0u;
                vkCmdBindVertexBuffers(context.cmd, 0u, 1u, &vertex, &vertex_offset);
                bound_vbo = mesh->vbo_segment;
            }
            if (entries[mdc].ibo_segment != bound_ibo || entries[mdc].index_type != bound_index_type)
            {
                const auto index = context.resolveBufferHandle(index_buffers[entries[mdc].ibo_segment]);
                if (index == VK_NULL_HANDLE)
                {
                    return;
                }
                vkCmdBindIndexBuffer(context.cmd, index, 0u, entries[mdc].index_type);
                bound_ibo = entries[mdc].ibo_segment;
                bound_index_type = entries[mdc].index_type;
            }
            const auto &section = instances->meshSectionAt(entries[mdc].section_id);
            ...
            push.transition_start_time = property.transition_start_time;
            push.transition_duration = property.transition_duration;
            push.transition_seed = property.transition_seed;
            push.transition_flags = property.transition_flags;
            vkCmdPushConstants(context.cmd, context.pipeline_layout,
                               VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT, 0u, sizeof(PickPush), &push);
            vkCmdDrawIndexed(context.cmd, section.index_count, 1u, section.first_index, section.base_vertex, 0u);
        });
        resources->markPickSubmitted(context.frame.frame_index, *request);
    });
}
} // namespace lux::render
