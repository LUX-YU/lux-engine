...
namespace lux::render
{
    ...
    bool Canvas2DFeature::canRebaseSceneOrigin(const std::int64_t origin_delta[3]) const noexcept
    {
        return arena_ == nullptr || arena_->canRebaseSceneOrigin(origin_delta);
    }

    void Canvas2DFeature::rebaseSceneOrigin(const std::int64_t origin_delta[3]) noexcept
    {
        if (arena_)
    ...
    lux::render::Expected<void> Canvas2DFeature::initAndAttachTo(RenderScene& scene)
    {
        ...
        tmpl.pipeline_layout = cv.buildStandardGraphicsLayout(tmpl.descriptor_set_count, infos, "Canvas2DImageLayout");
            ...
            return [this, group](const PassRecordContext& ctx) {
                ...
                for (const Canvas2DRun& run : arena_->runs())
                {
                    ...
                    if (run.kind != bound_kind)
                    {
                        ...
                        const VkDescriptorSet ds = arena_->descriptorSet(static_cast<ECanvas2DKind>(run.kind));
