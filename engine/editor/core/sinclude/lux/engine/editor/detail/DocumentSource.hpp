...
namespace lux::editor::detail
{
    ...
    template <class Codec> class SourceOpening final : public DocumentOpening
    {
        ...
        static constexpr bool HasPreparation = requires(
            Codec &codec, Source &source, Project &project, process::ExecutionRuntime &runtime, std::stop_token stop,
            PollBudget &budget) { codec.prepare(source, project, runtime, stop, budget); };
        struct Read final
        {
            ...
            ReadResult operator()() const noexcept
            {
                ...
                if (error || size > Codec::max_bytes)
                {
                    return lux::cxx::unexpected(
                        EditorFailure{error ? EEditorError::SOURCE_FAILURE : EEditorError::CAPACITY, "document.read",
                                      static_cast<std::uint64_t>(error.value()), path.string(), error});
                }
        ...
        using ReadSender = decltype(stdexec::then(stdexec::schedule(std::declval<process::BlockingScheduler>()),
                                                  std::declval<Read>()));
        using Sender = decltype(stdexec::then(
            stdexec::continues_on(std::declval<ReadSender>(), std::declval<process::CpuScheduler>()),
            std::declval<Decode>()));
      ...
      public:
        ...
        void poll(PollBudget &budget) override
        {
            if constexpr (HasPreparation)
            {
                ...
                auto ready = codec_.prepare(*source, project_, runtime_, stop_.get_token(), budget);
                if (!ready)
                {
