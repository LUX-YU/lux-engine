...
void writeLocalBoundsFromMesh(InstanceResources *inst, MeshResources *mesh_res, InstanceSlot slot, MeshHandle mesh_h)
{
...
RenderObjectHandle serverAddMeshInstance(
    void *user_state, RenderSceneId scene_id, RMeshHandle mesh, RMaterialHandle material,
    const RenderSpatialTransform3D &transform, std::uint32_t flags, EGeometryKind geometry_kind, PassMask pass_mask,
    std::uint32_t user_meta_index, std::uint32_t transition_milliseconds, std::uint32_t transition_seed,
    EMeshInstanceCreateStatus &out_status) // distinguishes config vs capacity failure
{
    ...
    {
        section_ids[i] = inst->registerMeshSection(sections[i], gpu_rec->ibo_segment,
                                                   gpu_rec->index_type == EIndexType::UInt16 ? VK_INDEX_TYPE_UINT16
    ...
    {
        prop.transition_start_time = scene->maintenanceTime();
    ...
    for (uint32_t i = 0; i < lod_count; ++i)
    {
        meta.lod_mdc[i] = inst->mdcTable().registerInstance(
            static_cast<uint8_t>(geometry_kind), bucket_id, section_ids[i], gpu_rec->ibo_segment,
...
MeshUploadStartResult serverUploadMesh(void *user_state, const lux::rdesc::Mesh &mesh, VertexLayoutId layout,
                                       std::uint64_t request_id, std::shared_ptr<const void> data_owner,
                                       lux::render::CapacityShortfallWire *shortfall_output)
{
    ...
    auto vdata = std::span<const std::byte>(reinterpret_cast<const std::byte *>(mesh.vertices.data()),
    ...
    const bool use_compact_indices = std::ranges::all_of(
        idx_storage->wide, [](std::uint32_t index) { return index <= std::numeric_limits<std::uint16_t>::max(); });
    ...
    task.data_owner = std::make_shared<std::pair<std::shared_ptr<const void>, std::shared_ptr<MeshIndexUploadStorage>>>(
...
RenderObjectHandle detail::createMeshInstance(void *server_state, RenderSceneId scene_id, RMeshHandle mesh,
                                              RMaterialHandle material, const RenderSpatialTransform3D &transform,
                                              std::uint32_t flags, EGeometryKind geometry_kind, PassMask pass_mask,
                                              std::uint32_t user_meta_index, bool visible_in_all_views,
                                              EMeshInstanceCreateStatus &status, std::uint32_t transition_milliseconds,
                                              std::uint32_t transition_seed)
{
    const auto object =
        serverAddMeshInstance(server_state, scene_id, mesh, material, transform, flags, geometry_kind, pass_mask,
                              user_meta_index, transition_milliseconds, transition_seed, status);
...
bool detail::reconfigureMeshInstances(void *server_state, RenderSceneId scene_id,
                                      ...
                                      EMeshInstanceCreateStatus &status)
{
    ...
    const bool is_invalid_configuration =
        is_missing_context || is_missing_scene || is_missing_instances || is_missing_meshes || is_missing_materials;
    ...
    for (const auto &input : revisions)
    {
        ...
        const bool is_invalid_input =
            is_dead_instance || is_missing_previous || is_missing_gpu_record || is_missing_material_slot;
        if (is_invalid_input)
        {
            rollback();
            status = EMeshInstanceCreateStatus::INVALID_CONFIGURATION;
        ...
        for (; registered_sections < lod_count; ++registered_sections)
        {
            next.section_ids[registered_sections] = instances->registerMeshSection(
                sections[registered_sections], gpu_record->ibo_segment,
        ...
        for (std::uint32_t lod = 0u; lod < lod_count; ++lod)
        {
            next.cull_meta.lod_mdc[lod] = instances->mdcTable().registerInstance(
                static_cast<std::uint8_t>(input.geometry_kind), material_slot->variant_bucket, next.section_ids[lod],
...
void detail::makeMeshInstanceVisible(void *server_state, RenderSceneId scene_id, std::uint32_t view_index,
...
void detail::setMeshInstanceVisibility(void *server_state, RenderSceneId scene_id, RenderObjectHandle object,
...
void handleUpsertMeshInstance(GeneralRenderServer::Dispatcher::Ctx &ctx, const UpsertMeshInstancePayload &p)
{
    ...
    if (const auto existing = instances->findSource(p.entity))
    {
        const detail::MeshInstanceRevision revision{
            existing,        p.mesh,      p.material,        p.transform, p.flags & ~kInstanceInternalFlagClusterOwned,
            p.geometry_kind, p.pass_mask, p.user_meta_index, 0xffffffffu};
        ...
        detail::setMeshInstanceVisibility(ctx.user_state, p.scene_id, existing, (p.flags & kInstanceFlagVisible) != 0U);
        return;
    }

    const RenderObjectHandle object = detail::createMeshInstance(
        ctx.user_state, p.scene_id, p.mesh, p.material, p.transform, p.flags & ~kInstanceInternalFlagClusterOwned,
        p.geometry_kind, p.pass_mask, p.user_meta_index, true, status, p.transition_milliseconds, p.transition_seed);
    if (!object)
    {
        ctx.markDispatchError(renderError<err::resource::ModifyFailed>());
        return;
...
void handleRetireMeshInstance(GeneralRenderServer::Dispatcher::Ctx &ctx, const RetireMeshInstancePayload &p)
{
    ...
    if (!scene || !instances ||
        !instances->beginFadeRetirement(object, scene->maintenanceTime(), duration, p.transition_seed))
    {
        detail::destroyMeshInstance(ctx.user_state, p.scene_id, object);
    }
}
...
void handleMakeInstanceVisibleForView(GeneralRenderServer::Dispatcher::Ctx &ctx,
...
void handleUpdateInstanceRenderState(GeneralRenderServer::Dispatcher::Ctx &ctx,
...
void handleUploadMesh(GeneralRenderServer::Dispatcher::Ctx &ctx, const UploadMeshPayload &p)
{
    ...
    const auto started =
        serverUploadMesh(ctx.user_state, *mesh, p.layout_id, ctx.currentRequestId(), std::move(view.owner), &shortfall);
