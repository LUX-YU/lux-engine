...
namespace lux::launcher
{
    ...
    ProjectPickerPanel::ProjectPickerPanel(OnChosen on_chosen)
    ...
    void ProjectPickerPanel::paint()
    {
        ...
        recents_cache_ = lux::project::loadRecentProjects();
                                   ...
                                   ImGuiWindowFlags_AlwaysAutoResize))
        {
            ...
            else if (modal_ == Modal::NewDemoProject)
            {
                ImGui::TextUnformatted(
                    "Target directory (will be created if missing).\n"
                    "Project name is the directory basename.\n"
                    "Result: project skeleton + Scenes/Main.luxscene\n"
            ...
            if (ImGui::Button("OK", ImVec2(bw, 0.f)))
            {
                         ...
                         modal_ == Modal::NewDemoProject)
                {
                    ...
                    else
                    {
                        ...
                        auto created = lux::project::Project::newOnDisk(
                            raw, name);
                        if (!created)
                        {
                        ...
                        else if (modal_ == Modal::NewDemoProject)
                        {
                            ...
                            const auto scene_rel = std::filesystem::path(
                                "Scenes") / "Main.luxscene";
                            const auto scene_abs =
                                created->root() / scene_rel;
                            if (!lux::editor::writeDemoScene(scene_abs))
                            {
                                modal_error_ =
                            ...
                            else
                            {
                                ...
                                created->manifest().default_scene =
                                    "Scenes/Main.luxscene";
                                    ...
                                    !saved)
                                {
                                    modal_error_ =
                                        ...
                                        "update manifest's default_scene: " +
                                        saved.error();
                                }
                                else
                                {
