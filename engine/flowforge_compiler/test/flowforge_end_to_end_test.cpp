...
static std::pair<std::unique_ptr<lux::flowforge::IR>, std::unique_ptr<lux::flowforge::IRContext>> createFlowForgeGraph() {
    ...
    return std::make_pair(std::move(ir), std::move(ir_context));
}
...
static ModuleOp createCompleteModule(MLIRContext& ctx, const lux::flowforge::IR& flowforgeIR) {
    ...
    auto addDecl = func::FuncOp::create(
        builder.getUnknownLoc(), "flowforge_add",
        builder.getFunctionType({i32Type, i32Type}, {i32Type}));
    addDecl.setPrivate();
    module.push_back(addDecl);
    
    auto multiplyDecl = func::FuncOp::create(
        builder.getUnknownLoc(), "flowforge_multiply",
        builder.getFunctionType({i32Type, i32Type}, {i32Type}));
    multiplyDecl.setPrivate();
    module.push_back(multiplyDecl);
    
    auto printDecl = func::FuncOp::create(
        builder.getUnknownLoc(), "flowforge_print",
        builder.getFunctionType({i32Type}, {}));
    ...
    auto fibonacciDecl = func::FuncOp::create(
        builder.getUnknownLoc(), "flowforge_fibonacci",
    ...
    for (int i = 1; i <= 3; ++i) {
        ...
        builder.create<func::CallOp>(
            ...
            printDecl, 
            ValueRange{current_i.getResult()}
        );
        
        // Check if current index is even
        auto isEven = builder.create<func::CallOp>(
            builder.getUnknownLoc(), 
            isEvenDecl, 
            ValueRange{current_i.getResult()}
        );
        ...
        auto fibValue = builder.create<func::CallOp>(
            builder.getUnknownLoc(), 
            fibonacciDecl, 
            ValueRange{current_i.getResult()}
        );
        ...
        // Update max value
        max_value = builder.create<func::CallOp>(
            builder.getUnknownLoc(), 
            maxDecl, 
            ValueRange{max_value, fibValue.getResult(0)}
        ).getResult(0);
        ...
        for (int j = 1; j <= 2; ++j) {
            ...
            auto product = builder.create<func::CallOp>(
                builder.getUnknownLoc(), multiplyDecl, 
            ...
            builder.create<func::CallOp>(
                builder.getUnknownLoc(), 
                printDecl, 
                ValueRange{product.getResult(0)}
            );
        }
    ...
    builder.create<func::CallOp>(
        builder.getUnknownLoc(), 
        printDecl, 
        ValueRange{final_result.getResult()}
    );
...
static bool runAndExecute(ModuleOp module) {
    ...
    engine->registerSymbols([&](llvm::orc::MangleAndInterner mangle) {
        ...
        auto ins = [&](const char* name, void* addr) {
            map[mangle(name)] = ExecutorSymbolDef{
    ...
    if (!expectedFPtr) {
        std::cerr << "Failed to lookup main function: "
                  << llvm::toString(expectedFPtr.takeError()) << std::endl;
        return false;
    }
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        auto [flowforgeIR, ir_context] = createFlowForgeGraph();
