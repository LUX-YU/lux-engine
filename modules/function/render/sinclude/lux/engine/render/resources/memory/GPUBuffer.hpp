...
namespace lux::render
{
    ...
    //  1. Concepts & Enums
    // =========================================================================
    enum class EGpuBufferType { CPU_TO_GPU, GPU_ONLY };
    ...
    template<bool IsSliced>
    struct SlicePolicy;

    template<> struct SlicePolicy<true> {
    protected:
        uint32_t slices_ = 1;
        void initSlices(uint32_t n) { slices_ = std::max(1u, n); }
    public:
        [[nodiscard]] uint32_t slices() const { return slices_; }
    };
    ...
    public:
        [[nodiscard]] constexpr uint32_t slices() const { return 1; }
    };

    // --- Policy: Mapping ---
    // Only CPU_TO_GPU stores the mapped_ pointer
    template<typename T, EGpuBufferType Type>
    struct MapPolicy;

    template<typename T>
    struct MapPolicy<T, EGpuBufferType::CPU_TO_GPU> {
    protected:
        void* mapped_ = nullptr;
        T* getMappedPtr() const { return static_cast<T*>(mapped_); }
    };

    template<typename T>
    struct MapPolicy<T, EGpuBufferType::GPU_ONLY> {
    protected:
        // GPU_ONLY mode has no member variables
        T* getMappedPtr() const { return nullptr; } // Only for internal logic unification; upper layers cannot call this
    };

    // --- Policy: Dirty Tracking ---
    // Determines whether dirty tracking is stored and the data structure used (vector vs single struct)
    struct DirtyRange { VkDeviceSize min; VkDeviceSize max; };

    template<EGpuBufferType Type, bool IsSliced>
    struct DirtyPolicy;

    // Case 1: GPU_ONLY -> no dirty tracking needed
    template<bool IsSliced>
    struct DirtyPolicy<EGpuBufferType::GPU_ONLY, IsSliced> {
    protected:
        void resetDirty(uint32_t) {}
        void markDirty(uint32_t, VkDeviceSize, VkDeviceSize) {}
        DirtyRange getDirty(uint32_t) const { return { 0, 0 }; }
    };

    // Case 2: CPU_TO_GPU + Sliced -> requires vector
    template<>
    struct DirtyPolicy<EGpuBufferType::CPU_TO_GPU, true> {
    protected:
        ...
        DirtyRange getDirty(uint32_t slice) const { return dirty_state_[slice]; }
        DirtyRange& getDirtyRef(uint32_t slice) { return dirty_state_[slice]; }
    };

    // Case 3: CPU_TO_GPU + Single -> only needs a single variable, no vector needed
    template<>
    struct DirtyPolicy<EGpuBufferType::CPU_TO_GPU, false> {
    protected:
        ...
        DirtyRange getDirty(uint32_t) const { return dirty_state_; }
        DirtyRange& getDirtyRef(uint32_t) { return dirty_state_; }
    };
    ...
    template<typename T, bool HasMirror>
    struct HostMirrorPolicy;

    template<typename T>
    struct HostMirrorPolicy<T, false> {
    protected:
    ...
    template<typename T>
    struct HostMirrorPolicy<T, true> {
    protected:
        ...
        std::vector<uint32_t> hm_upload_scratch_; // Reused per upload to coalesce dirty runs (no per-call heap alloc). (#H8)
        ...
        void resizeHostMirror(uint32_t new_cap, uint32_t slices) {
            ...
            if (hm_slice_epoch_.size() != slices) hm_slice_epoch_.assign(slices, 0ull);
        ...
        void hmPushDirty(uint32_t idx) {
            if (idx >= hm_listed_ticket_.size()) return;
        ...
        bool hmAllSlicesSynced(uint64_t epoch) const noexcept {
            for (auto e : hm_slice_epoch_) if (e != epoch) return false;
            return true;
        }
    };
    ...
    public:
        ...
        void setDeferredQueue(DeferredDestroyQueue* q) noexcept { buffer_owned_.setQueue(q); }
        ...
        , public HostMirrorPolicy<T, HostMirror>
    {
        ...
        static_assert(!HasMirror || (IsCpuWritable && IsSliced),
            "HostMirror mode requires CPU_TO_GPU + Sliced (epoch dirty needs host copy and per-slice sync)");

    public:
        GpuBuffer() = default;
        explicit GpuBuffer(const GpuBufferCreateInfo& ci) { init(ci); }
        ~GpuBuffer() { destroy(); }

        // Move only
        GpuBuffer(const GpuBuffer&) = delete;
        GpuBuffer& operator=(const GpuBuffer&) = delete;
        GpuBuffer(GpuBuffer&& o) noexcept { moveFrom(std::move(o)); }
        ...
        void init(const GpuBufferCreateInfo& ci)
        {
            ...
            if constexpr (HasMirror) {
                this->initHostMirror(
                    std::max(1u, ci.initial_capacity),
                    this->slices(),
                    ci.clear_on_remove);
            }
        ...
        {
            if (!ensureCapacity()) return SlotHandle{};  // growth failed -> invalid handle (C-1)
            uint32_t index = 0;
            if (!free_.empty()) {
                index = free_.back();
                free_.pop_back();
            ...
            if (index >= alive_.size()) alive_.resize(index + 1, 0); // Safety
            alive_[index] = 1;
        ...
        {
            return device_ctx_ && h.valid() && h.index < capacity_ &&
                alive_[h.index] && generations_[h.index] == h.gen;
        }

        // ---------- CPU Write (Concept Constraint) ----------
        // Constrained via requires: only available for CPU_TO_GPU buffers
        [[nodiscard]] T* mapped() noexcept requires IsCpuWritable
        {
            return this->getMappedPtr();
        }

        [[nodiscard]] T* mapped(uint32_t slice, const SlotHandle& h) noexcept requires IsCpuWritable
        {
        ...
        [[nodiscard]] T* mapped(const SlotHandle& h) noexcept requires (IsCpuWritable && !IsSliced)
        {
            return mapped(0, h);
        }

        void write(uint32_t slice, const SlotHandle& h, const T& value) requires IsCpuWritable
        {
        ...
        void writeAllSlices(const SlotHandle& h, const T& value) requires (IsCpuWritable&& IsSliced)
        {
            for (uint32_t s = 0; s < this->slices(); ++s) write(s, h, value);
        }

        // Overload for single-slice (non-sliced) buffers
        void write(const SlotHandle& h, const T& value) requires (IsCpuWritable && !IsSliced)
        {
            write(0, h, value);
        }
        ...
        void flush(uint32_t slice = 0) requires IsCpuWritable
        {
            if (!buffer_owned_.valid()) return;
        ...
        std::optional<VkBufferMemoryBarrier2> getBarrier(uint32_t slice = 0) const requires IsCpuWritable
        {
            if (!buffer_owned_.valid()) return std::nullopt;

            auto d = this->getDirty(slice);
            if (d.max <= d.min) return std::nullopt;
            ...
            if (allow_shader_write_) barrier.dstAccessMask |= VK_ACCESS_2_SHADER_STORAGE_WRITE_BIT;

            return barrier;
        }

        // Reset dirty state after barrier has been recorded
        void resetDirtySlice(uint32_t slice = 0) requires IsCpuWritable
        {
            auto& d = this->getDirtyRef(slice);
         ...
         */
        void markDirtyRange(uint32_t slice, uint32_t first_index, uint32_t count) requires IsCpuWritable
        {
            if (count == 0) return;
        ...
        // Original combined method (for backward compatibility and single-use cases)
        void flushAndBarrier(VkCommandBuffer cmd, uint32_t slice = 0) requires IsCpuWritable
        {
            if (!buffer_owned_.valid()) return;

            flush(slice);
            
            if (auto barrier = getBarrier(slice)) {
            ...
            resetDirtySlice(slice);
        }

        // ---------- Staging Upload (GPU_ONLY Only) ----------
        void stageUpload(VkCommandBuffer cmd, VkBuffer stagingBuf, VkDeviceSize stagingOffset,
            uint32_t slice, uint32_t dstIndex, uint32_t count) requires (!IsCpuWritable)
        {
            if (count == 0) return;
            VkBufferCopy region{};
        ...
        // ---------- Capacity ----------
        [[nodiscard]] bool reserve(uint32_t min_capacity, bool preserve_data = true,
                                   VkCommandBuffer cmd = VK_NULL_HANDLE)
        {
            ...
            if constexpr (!IsCpuWritable) usage |= VK_BUFFER_USAGE_VERTEX_BUFFER_BIT | VK_BUFFER_USAGE_INDEX_BUFFER_BIT;

            if (!this->createVmaBuffer(new_size, usage, IsCpuWritable, &new_buf, &new_alloc, IsCpuWritable ? &new_ptr : nullptr))
                return false;
                ...
                else if (preserve_data && old_cap > 0)
                {
                    if constexpr (HasMirror)
                    {
                        ...
                        for (uint32_t s = 0; s < this->slices(); ++s) {
                            ...
                            if (copy_n < new_cap) {
                                std::memset(base + VkDeviceSize(copy_n) * stride_, 0,
            ...
            if constexpr (IsCpuWritable) this->mapped_ = new_ptr;
            ...
            for (uint32_t i = new_cap; i-- > old_cap; ) free_.push_back(i);
        ...
        // ---------- Getters ----------
        [[nodiscard]] VkBuffer buffer() const noexcept { return buffer_owned_.get(); }
        [[nodiscard]] uint32_t capacity() const noexcept { return capacity_; }
        [[nodiscard]] uint32_t count() const noexcept { return count_; }
        [[nodiscard]] uint32_t bufferGeneration() const noexcept { return buffer_gen_; }
        [[nodiscard]] uint32_t stride() const noexcept { return stride_; }

        void writeDescriptor(VkDescriptorSet set, uint32_t binding, VkDescriptorType type = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER) const
        {
            if (!buffer_owned_.valid()) return;
        ...
        void writeDescriptorSlice(VkDescriptorSet set, uint32_t binding, uint32_t slice,
        ...
        SlotHandle add(const T& v) requires HasMirror
        {
            if (!ensureCapacity()) return SlotHandle{};  // growth failed -> invalid handle (C-1)
            uint32_t index = 0;
            ...
            // so that writeDescriptorTight() covers all live slots.
            if (index >= count_) count_ = index + 1;
            if (index >= alive_.size()) alive_.resize(index + 1, 0);
            alive_[index] = 1;
        ...
        bool remove(const SlotHandle& h) requires HasMirror
        {
            if (!isAlive(h)) return false;
            const uint32_t idx = h.index;
        ...
        bool modify(const SlotHandle& h, const T& nv) requires HasMirror
        {
            if (!isAlive(h)) return false;
            const uint32_t idx = h.index;
        ...
        T* get(const SlotHandle& h) requires HasMirror
        { return isAlive(h) ? &this->hm_values_[h.index] : nullptr; }

        const T* get(const SlotHandle& h) const requires HasMirror
        { return isAlive(h) ? &this->hm_values_[h.index] : nullptr; }

        /// Mark slot as modified without changing data (for after raw get() edits).
        bool touch(const SlotHandle& h) requires HasMirror
        {
            if (!isAlive(h)) return false;
        ...
        void uploadDataSlice(VkCommandBuffer cmd, uint32_t slice) requires HasMirror
        {
        ...
        std::optional<VkBufferMemoryBarrier2> uploadDataSliceDeferred(uint32_t slice) requires HasMirror
        {
    ...
    private:
        ...
        std::optional<VkBufferMemoryBarrier2> uploadDataSliceInternal(uint32_t slice) requires HasMirror
        {
            assert(slice < this->slices() && "slice out of range");
            if (slice >= this->slices()) return std::nullopt;
            ...
            if (since == this->hm_global_epoch_) return std::nullopt;
    ...
    public:
        ...
        void writeDescriptorTight(VkDescriptorSet set, uint32_t binding, uint32_t slice = 0) const requires HasMirror
        {
            if (!buffer_owned_.valid()) return;
            const VkDeviceSize offset = getSliceByteOffset(slice);
            const VkDeviceSize size = std::max<VkDeviceSize>(
                VkDeviceSize(count_) * stride_, stride_);
        ...
        void clearAll() requires HasMirror
        {
            for (auto& g : generations_) g++;
            std::fill(alive_.begin(), alive_.end(), 0);
            free_.clear();
            for (uint32_t i = 0; i < capacity_; ++i) free_.push_back(i);
            count_ = 0;
        ...
        [[nodiscard]] uint64_t globalEpoch() const noexcept requires HasMirror
        ...
        [[nodiscard]] const T& hostValue(uint32_t index) const requires HasMirror
        ...
        // =====================================================================
        // Backward compatibility aliases (for SlicedSSBO migration)
        // =====================================================================
        [[nodiscard]] VkBuffer dataBuffer() const noexcept { return buffer_owned_.get(); }
        [[nodiscard]] uint32_t dataStride() const noexcept { return stride_; }
        [[nodiscard]] uint32_t dataBuffergen() const noexcept { return buffer_gen_; }

        void writeDataDescriptor(VkDescriptorSet set, uint32_t binding) const
        { writeDescriptor(set, binding); }

        void writeDataDescriptorSlice(VkDescriptorSet set, uint32_t binding, uint32_t s) const
        { writeDescriptorSlice(set, binding, s); }

        void writeDataDescriptorTight(VkDescriptorSet set, uint32_t binding, uint32_t s = 0) const requires HasMirror
        { writeDescriptorTight(set, binding, s); }
    ...
    private:
        ...
        T* getSlicePtr(uint32_t slice) const requires IsCpuWritable
        {
            return reinterpret_cast<T*>(
                reinterpret_cast<uint8_t*>(this->getMappedPtr()) + getSliceByteOffset(slice));
        }
        ...
        void moveFrom(GpuBuffer&& o) noexcept {
            ...
            stride_ = o.stride_; capacity_ = o.capacity_; count_ = o.count_;
            buffer_gen_ = o.buffer_gen_; allow_shader_write_ = o.allow_shader_write_;
            buffer_usage_ = o.buffer_usage_;
            alive_ = std::move(o.alive_); generations_ = std::move(o.generations_); free_ = std::move(o.free_);

            // Move Policies
            if constexpr (IsSliced) this->slices_ = o.slices_;
            if constexpr (IsCpuWritable) {
                this->mapped_ = o.mapped_;
                if constexpr (IsSliced) this->dirty_state_ = std::move(o.dirty_state_);
                else this->dirty_state_ = o.dirty_state_;
            }
            if constexpr (HasMirror) {
            ...
            o.capacity_ = 0; o.count_ = 0;
            if constexpr (IsCpuWritable) o.mapped_ = nullptr;
        }
    };
