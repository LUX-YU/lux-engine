    ...
    : render_ctx_(std::move(ctx)), config_(cfg), debug_name_(cfg.scene_name)
{
    ...
    // Register per-scene SceneResources in resources_.
    {
        ...
        resources_.addBeginFrameHook(SceneResources::kUploadPhase,
...
Expected<void> RenderScene::removeFeatureInternal(FeatureHandle feature_id, bool check_reverse_deps)
{
    ...
    if (check_reverse_deps)
    {
        ...
        {
            return renderFailure<err::feature::StillRequiredByAnother>(encodeFeatureType(ptr->descriptor_.type),
...
void RenderScene::compileGraphTemplate(const RenderTargetLayout &layout)
{
    ...
    graph_cache_->compile(
        layout, requiredTargetSlotMask(), [this](RGBuilder &b) { feature_set_.contributePasses(b); }, all_views,
        scene_domain_sets_.get(), current_stamp_.serial);
}
...
void RenderScene::dumpGpuTiming(std::ostream &os) const
{
    ...
    for (const auto &sample : graph_cache_->compileHistory())
    {
        ...
        os << "{\"frame\":" << sample.frame_serial << ",\"invalidation_bits\":" << sample.invalidation_bits
           ...
           << ",\"total_ns\":" << sample.total_nanoseconds << ",\"succeeded\":" << (sample.succeeded ? "true" : "false")
           << '}';
    }
    ...
    for (const auto *view : view_set_.active())
    {
        ...
        for (const auto &snapshot : history)
        {
            ...
            first_sample = false;
            os << "{\"frame\":" << snapshot->frame_id << ",\"available\":" << (snapshot->available ? "true" : "false")
...
void RenderScene::beginFrame(const FrameStamp &stamp)
{
    ...
    // for the current slot happens HERE so it never races the in-flight frame.
    {
        ...
        sg.time_sec = simulationTime();
        sg.delta_time = static_cast<float>(static_cast<double>(delta_ns_) * 1e-9);
        sg.frame_number = static_cast<uint32_t>(simulation_step_);
        sg.maintenance_time = maintenance_time_;
        ...
        for (auto *view : view_set_.active())
        {
            ...
            scene_res.writeViewData(view->view_slot, view->view_data_staging.data(), kViewDataStrideBytes, frame_slot);
        }
    }
