#pragma once
// ============================================================================
//  Image2DComponent.hpp — a textured 2D quad (lux::ecs).
//
//  The minimal traditional-2D image (design §4, v2 GPU-driven): a texture region +
//  pivot + world size + tint + a single float draw PRIORITY. Its world POSITION comes
//  from the entity's Transform2D → ResolvedTransform2D (composed by Transform2DSystem);
//  this component only adds the quad's own size/appearance/priority. The retained
//  Image2DSubsystem owns one GPU-resident instance per image entity and pushes DELTAS
//  (create/remove/dirty transform/visual/key) — never per-frame content.
//
//  The bridge resolves `texture` to a bindless index and the image shader samples it at
...
//  TextureAtlasAsset yet — `uv_rect` is authored/hardcoded.
//
...
namespace lux::ecs
{
    struct LUX_COMPONENT() Image2DComponent
    {
        ...
        /// Image size in WORLD units (multiplies the unit quad, on top of Transform2D scale).
        LUX_MEMBER(display_name=Size, tooltip=Image size in world units)
        ...
        LUX_MEMBER(display_name=Visible, tooltip=Whether the image is drawn)
        bool visible = true;
