...
#include <lux/engine/resource/asset/codecs/MeshDescriptionCodec.hpp>
...
namespace lux::asset
{
    ...
    {
        // Raw mesh formats (obj / fbx / glb) are imported by the Toolchain's
        // ModelImporter.
        // (Assimp); a standalone external mesh source format is not supported.
        return unexpected(EAssetError::UNSUPPORTED);
    }

    lux::cxx::expected<std::unique_ptr<lux::rdesc::Mesh>, EAssetError>
    MeshSerDeser::decodeData(const void* bytes, std::size_t len) noexcept
    {
        if (!bytes || len == 0)
            return unexpected(EAssetError::ABNORMAL_FILE_SIZE);

            // Zero-copy view (C8): the whole decode path only READS — copying
            // the entire asset image here doubled every mesh load's traffic
            // (readAll vector → this copy → structured decode = 3 passes).
    ...
    MeshSerDeser::encodeData(
        const asset_id_t& id,
        const lux::rdesc::Mesh& mesh) noexcept
    {
        if (id.is_nil() || mesh.vertices.empty() || mesh.indices.empty() ||
        ...
        const auto mesh_blob = detail::encodeMeshDescription(mesh);
        AssetInfo info{};
        info.id = id;
        info.type = EAssetType::MESH;
        constexpr std::string_view name{"Generated HLOD"};
        std::memcpy(info.display_name, name.data(), name.size());
        auto image = makeHeaderRaw<EAssetType::MESH>(
            info, mesh_blob.size(), 0u);
        image.insert(image.end(), mesh_blob.begin(), mesh_blob.end());
