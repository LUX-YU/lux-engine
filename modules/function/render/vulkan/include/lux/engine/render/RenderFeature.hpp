...
// Cross-target inputs are declarations, not raw handles borrowed from another
// Feature. The frame orchestrator orders producers and resolves a precise
// pool generation/FIF slot before the consumer records its graph.
struct SampledTarget final
{
...
struct SampledTargetImage final
{
    RenderTargetId target;
    std::uint64_t backing_revision{};
    VkImage image{};
...
class LUX_FUNCTION_PUBLIC RenderFeature
{
  public:
    ...
    // receives the ordinary empty-output clear and final transition.
    [[nodiscard]] virtual bool viewReady(ViewHandle) const noexcept
    {
        return true;
    }
    ...
    [[nodiscard]] virtual std::span<const SampledTarget> sampledTargets() const noexcept
    {
        return {};
    }

    [[nodiscard]] virtual Expected<void> bindSampledTargets(std::span<const SampledTargetImage> /*images*/,
