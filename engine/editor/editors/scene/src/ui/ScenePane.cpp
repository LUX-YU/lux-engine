...
ScenePane::ScenePane(scene::SceneEditor &document, lux::render::RenderRuntime &renderer, std::string id,
...
lux::render::RenderView *ScenePane::view() noexcept
{
    return view_owner_.get();
}

void ScenePane::remember(std::string_view operation, const lux::render::RendererFailure &failure)
{
    view_result_ =
...
void ScenePane::poll(PollBudget &budget)
{
    ...
    if (auto *current = view(); current && (closing_ || switching || reopen_))
    {
        ...
        document_.unbindCamera(camera_, current->id());
        camera_ = {};
        ...
        if (*closed != lux::render::ERenderClose::COMPLETE)
        {
            return;
        }
        view_owner_.reset();
    ...
    if (spatial_ && !view() && visible() && (running || author) &&
        renderer_.status().state == lux::render::ERenderRuntimeState::ACTIVE && view_result_)
    {
        if (running)
        {
            auto opened = document_.openRunView(
                run.id, {{640, 480}, lux::render::SampledOutput{}, document_.runCoordinatePageSize()});
            if (!opened)
            {
                const auto *waiting = std::any_cast<lux::render::RendererFailure>(&opened.error().cause);
                if (waiting && (waiting->code == lux::render::ERendererError::NOT_READY ||
                                waiting->code == lux::render::ERendererError::BUSY))
                {
                    return;
                }
        ...
        else
        {
            ...
            auto opened =
                document_.openView({{640, 480}, lux::render::SampledOutput{}, document_.coordinatePageSize()});
            if (!opened)
            {
                const auto *waiting = std::any_cast<lux::render::RendererFailure>(&opened.error().cause);
                if (waiting && (waiting->code == lux::render::ERendererError::NOT_READY ||
    ...
    if (view() && !switching)
    {
        ...
        {
            const Pick location = action_.index() == 1 ? std::get<Pick>(action_) : std::get<Create>(action_).location;
            bool completed = true;
            ...
            else if (const auto *create = std::get_if<Create>(&action_))
            {
                ...
                const auto point = spatial_->creationPoint(document_, location.instance, location.ray, create->plane);
                if (!point)
                {
            ...
            else
            {
                ...
                {
                    const auto selected =
                        document_.select({location.instance, *picked ? hit.entity : lux::simulation::ecs::NullEntity});
                    if (!selected)
                    {
...
void ScenePane::updateCamera(lux::render::PixelExtent extent)
{
    ...
    {
        document_.unbindCamera(camera_, view()->id());
        camera_ = {};
    }
    if (wanted)
    {
        const auto bound = document_.bindCamera(*wanted, view()->id());
        if (bound)
        {
            camera_ = *wanted;
        }
...
void ScenePane::draw(lux::ui::Frame &frame, lux::ui::PaneDrawContext &context)
{
    ...
    else if (next.error().code != lux::render::ERendererError::NOT_READY)
    {
    ...
    if (!displayed_run_.serial && ImGui::BeginDragDropTarget())
    {
        if (const auto *payload = ImGui::AcceptDragDropPayload(kAssetReferencePayload))
        {
            ...
            else if (spatial_)
            {
                ...
                {
                    action_.emplace<Create>(Pick{document_.instance(), *ray, camera_, document_.selection().revision},
                                            *reference, lux::partition::PartitionOrdinal{partition_},
                                            work_plane_height_, document_.historyView()->history.current);
                }
            }
        }
    ...
    const lux::render::PixelExtent extent{pixel(interaction.size.width, input.DisplayFramebufferScale.x),
                                          pixel(interaction.size.height, input.DisplayFramebufferScale.y)};
    if (view_status.state != lux::render::EViewState::FAILED)
    {
    ...
    {
        const auto ray = spatial_->ray(document_, camera_, {interaction.local_pointer.x, interaction.local_pointer.y},
                                       {interaction.size.width, interaction.size.height});
        if (ray)
        {
...
void ScenePane::appendFrameImages(std::vector<lux::render::ViewImage> &images) const
{
