...
namespace
{
    struct Provider final
    {
        ...
        ) noexcept
        {
            ...
            const auto index = std::to_integer<std::uint8_t>(bytes[0]) == 2U ? 1U : 0U;
            auto* lease = new (std::nothrow) Lease{&self, self.modules[index]};
            if (!lease)
                return false;
...
main()
{
    ...
    auto loaded_second = lux::script::loadNativeModule(std::filesystem::path{LUX_SCRIPT_NATIVE_SECOND_FIXTURE});
    assert(loaded_second);
    auto second_module = std::make_shared<lux::script::NativeModule>(std::move(*loaded_second));
    Provider provider{{module, second_module}};
    auto backend =
        std::make_unique<NativeScriptBindingBackend>(NativeModuleResolver{&provider, &Provider::resolve}, 2U, 3U);
    assert(*backend);
    ...
    asset.description.body =
        lux::rdesc::NativeModuleScript{LUX_SCRIPT_ABI_VERSION, module->stateLayoutHash(), 64U, 64U, {}};
    const lux::rdesc::ScriptFunction increment{"Increment", 1U, {}, {}};
    ...
    const lux::rdesc::ScriptFunction pair{
        ...
        3U,
        {{"lux.f32", lux::script::scriptSemanticTypeId("lux.f32"), lux::script::EScriptPassMode::VALUE},
         {"lux.u32", lux::script::scriptSemanticTypeId("lux.u32"), lux::script::EScriptPassMode::VALUE}},
        {}};
    ...
    auto& second_body = std::get<lux::rdesc::NativeModuleScript>(second_asset.description.body);
    ...
    const auto expect_contract_mismatch = [&](const auto& content) {
        ...
        assert(
            descriptor.createInstance(
                descriptor.context,
                ScriptInstanceCreateContext{lux::asset::AssetId{id_bytes}, ScriptMountId{99U}},
                content,
                rejected_instance) == EScriptBackendResult::EXECUTABLE_CONTRACT_MISMATCH);
    ...
    {
        auto tampered = asset;
        auto& body = std::get<lux::rdesc::NativeModuleScript>(tampered.description.body);
        body.abi_version = 1U;
        expect_contract_mismatch(tampered);
    }
    {
    ...
    {
        auto tampered = asset;
        tampered.description.exports[1].args[0].pass = lux::script::EScriptPassMode::CONST_REF;
        expect_contract_mismatch(tampered);
    }
    ...
    assert(
        descriptor.createInstance(
            descriptor.context,
            ScriptInstanceCreateContext{lux::asset::AssetId{id_bytes}, ScriptMountId{1U}},
            asset,
            first_instance) == EScriptBackendResult::SUCCESS);
    assert(
        descriptor.createInstance(
            descriptor.context,
            ScriptInstanceCreateContext{lux::asset::AssetId{id_bytes}, ScriptMountId{2U}},
            asset,
    ...
    assert(
        descriptor.createInstance(
            descriptor.context,
            ScriptInstanceCreateContext{lux::asset::AssetId{id_bytes}, ScriptMountId{3U}},
            second_asset,
            third_instance) == EScriptBackendResult::SUCCESS);
    id_bytes[0] = 3U;
    assert(
        descriptor.createInstance(
            descriptor.context,
            ScriptInstanceCreateContext{lux::asset::AssetId{id_bytes}, ScriptMountId{4U}},
            asset,
            over_capacity) == EScriptBackendResult::CAPACITY_EXCEEDED);
    ...
    assert(
        descriptor.prepareMethod(descriptor.context, third_instance, function, third) == EScriptBackendResult::SUCCESS);
    assert(
        descriptor.prepareMethod(descriptor.context, second_instance, function, second) ==
        EScriptBackendResult::SUCCESS);
    assert(first && second && third && first.context != second.context && second.context != third.context);
    ...
    lux_script_value_slot
        argument{LUX_SCRIPT_VK_FLOAT, {}, sizeof(delta), lux::script::scriptSemanticTypeId("lux.f32"), &delta};
    lux_script_call_frame frame{&argument, 1U, 0U, nullptr, 0U, 0U, nullptr, first.context};
    ...
    assert(
        descriptor.prepareMethod(descriptor.context, first_instance, mismatched, rejected) ==
