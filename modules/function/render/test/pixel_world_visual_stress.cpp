...
main(int argc, char** argv)
{
    ...
    if (!lux::rendertest::VisualSoak::parse(argc, argv, soak))
        return 2;
    std::printf(
        "=== pixel_world_visual_stress ===  (a living pixel world; %s)\n",
    ...
    if (!staged.adopt(runtime) || !staged.adopt(persistent_entities))
    {
    ...
    fcomp.definition = uuids::uuid::from_string("89b15d75-4c61-45f8-8654-0d9a239df52b").value();
    const auto field = runtime.create(
        {d2::PixelFieldId{fcomp.definition},
         d2::EPixelFieldExtent::BOUNDED,
         {{0, 0},
          {static_cast<std::int64_t>((CW - 1u) >> d2::PixelFieldRuntime::kChunkShift),
           static_cast<std::int64_t>((CH - 1u) >> d2::PixelFieldRuntime::kChunkShift)}},
         0u}
    );
    world.emplace<d2::PixelFieldBindingComponent>(field_e, d2::PixelFieldBindingComponent{field, false});
    for (std::int64_t chunk_y = 0;
         chunk_y <= static_cast<std::int64_t>((CH - 1u) >> d2::PixelFieldRuntime::kChunkShift);
         ++chunk_y)
    {
        for (std::int64_t chunk_x = 0;
             chunk_x <= static_cast<std::int64_t>((CW - 1u) >> d2::PixelFieldRuntime::kChunkShift);
             ++chunk_x)
        {
            d2::PixelChunkLoad load;
            load.coordinate = {chunk_x, chunk_y};
            load.materials.assign(d2::PixelFieldRuntime::kChunkCellCount, d2::kEmptyMaterial);
    ...
    const auto stampRect = [&](int x, int y, int w, int h, d2::MaterialId m) {
        ...
        c.field = field;
        c.minimum = {x, y};
        c.extent = {static_cast<std::uint32_t>(w), static_cast<std::uint32_t>(h)};
        c.material = m;
        runtime.enqueue(c);
    };
    ...
    std::printf(
        "world up: %u k cells simulating, %zu images on one depth axis. Close to exit.\n",
        CW * CH / 1000u,
    ...
    while (fx.running())
    {
        ...
        if (soak.enabled())
        {
            if (now - soak_mark >= std::chrono::seconds(10))
            {
                ...
                std::printf(
                    "soak t=%.0fs | tiles=%u scanned=%u | upRev=%llu (+%llu) | evDropped=%llu\n",
                    t,
        ...
        last_time = now;
        if (dt > 0.1f)
            dt = 0.1f; // window drag hiccup → clamp banked time
        ...
        if ((frame_no % 600) == 300)
            stampRect(195 * SC, 68 * SC, 80 * SC, 14 * SC, d2::kEmptyMaterial);
        if ((frame_no % 600) == 599)
            stampRect(200 * SC, 70 * SC, 70 * SC, 6 * SC, stone);
        ...
        if ((frame_no % 1200) == 900)
            stampRect(170 * SC, 8 * SC, 200 * SC, 26 * SC, d2::kEmptyMaterial);
        ...
        if ((frame_no % 150) == 0)
        {
            ...
            for (std::size_t i = 0; i < stars.size(); i += 4)
                world.registry().patch<d2::Image2DComponent>(stars[i], [prio](auto& sp) { sp.priority = prio; });
        }
        ...
        for (std::size_t i = 0; i < swirl.size(); ++i)
        {
            ...
            world.registry().patch<d2::Transform2DComponent>(swirl[i], [&](auto& tc) {
                tc.position = {static_cast<double>(r * 1.4f * std::cos(a)), static_cast<double>(r * std::sin(a))};
            }
            );
        }
        ...
        {
            std::printf("soak final: eventsDropped=%llu\n", static_cast<unsigned long long>(runtime.eventsDropped()));
        ...
        {
            std::printf(
                "fps=%d | tiles=%u scanned=%u moved=%u step=%.2fms | upRev=%llu | images=%zu\n",
                fps_frames,
                runtime.activeTiles(field),
                runtime.cellsScannedLastStep(field),
