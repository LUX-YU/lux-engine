...
namespace {
class ReentrantPane final
    : public lux::object::Object<ReentrantPane, lux::ui::Pane> {
public:
  ReentrantPane(lux::object::ObjectDispatcherRef dispatcher, lux::ui::PaneId id)
      : Object(std::move(dispatcher), std::move(id),
               lux::ui::PaneTypeId{"reentrant.pane"}, "Reentrant Pane") {}
...
public:
  void invoke() noexcept { ++invoke_count; }
  void noop() noexcept {}
  ...
  [[nodiscard]] bool enabledAndReallocate() const noexcept {
    for (int index = 0; index < 32; ++index) {
      auto command = router->defineCommand(
          {lux::ui::UiCommandId{"generated." +
                                std::to_string(next_generated++)},
           "Generated"});
      assert(command);
      auto registration = router->bindGlobal<&CommandReceiver::noop>(
          *command, const_cast<CommandReceiver &>(*this));
      assert(registration);
...
public:
  void invoke() noexcept { invoked = true; }
...
int main() {
  ...
  {
    lux::ui::UISession session;
    ReentrantPane first{session.dispatcherRef(),
                        lux::ui::PaneId{"focus.first"}};
    ReentrantPane second{session.dispatcherRef(),
                         lux::ui::PaneId{"focus.second"}};
    auto first_registration = session.registerPane(first);
    auto second_registration = session.registerPane(second);
    assert(first_registration && second_registration);
    drawFrame(session);
    auto *target = session.focusedPane() == &first ? &second : &first;
    auto *target_registration = target == &first
                                    ? std::addressof(*first_registration)
                                    : std::addressof(*second_registration);
    bool focused_seen = false;
    auto connection = target->observeScoped<lux::ui::Pane::focusChanged>(
...

  {
    ...
    {
      auto pane = std::make_unique<ReentrantPane>(
          session.dispatcherRef(), lux::ui::PaneId{"replaceable"});
      auto registration = session.registerPane(*pane);
      assert(registration);
      stale_registration = std::move(*registration);
    }
    ReentrantPane replacement{session.dispatcherRef(),
                              lux::ui::PaneId{"replaceable"}};
...

  {
    ...
    auto registered = session.registerFactory(lux::ui::PaneFactory{
        lux::ui::PaneTypeId{"replaceable.factory"}, "First",
        [&](lux::object::ObjectDispatcherRef dispatcher,
          ...
          auto replacement = session.registerFactory(lux::ui::PaneFactory{
              lux::ui::PaneTypeId{"replaceable.factory"}, "Replacement",
              [&](lux::object::ObjectDispatcherRef next_dispatcher,
                  lux::ui::PaneId next_id) -> std::unique_ptr<lux::ui::Pane> {
                ++replacement_calls;
                return std::make_unique<ReentrantPane>(
                    std::move(next_dispatcher), std::move(next_id));
              }});
          assert(replacement);
          replacement_factory = std::move(*replacement);
          return std::make_unique<ReentrantPane>(std::move(dispatcher),
                                                 std::move(id));
        }});
    assert(registered);
    first_factory = std::move(*registered);
    assert(session.createPane(lux::ui::PaneTypeIdView{"replaceable.factory"},
                              lux::ui::PaneId{"factory.first"}));
    assert(first_calls == 1 && replacement_calls == 0);
    assert(session.createPane(lux::ui::PaneTypeIdView{"replaceable.factory"},
                              lux::ui::PaneId{"factory.second"}));
...

  {
    ...
    auto state_command = router.defineCommand(
        {lux::ui::UiCommandId{"state.reentrant"}, "State"});
    assert(state_command);
    auto state_registration =
        router.bindGlobal<&CommandReceiver::invoke,
                          &CommandReceiver::enabledAndReset,
                          &CommandReceiver::checked>(*state_command, receiver);
    ...
    auto invoke_command = router.defineCommand(
        {lux::ui::UiCommandId{"invoke.reentrant"}, "Invoke"});
    assert(invoke_command);
    auto invoke_registration =
        router.bindGlobal<&CommandReceiver::invoke,
                          &CommandReceiver::enabledAndReallocate>(
            *invoke_command, receiver);
    assert(invoke_registration);
    receiver.registration_to_reset = std::addressof(*invoke_registration);
    assert(router.invoke(*invoke_command) ==
    ...
    std::array<lux::ui::UiContextIdView, 2> temporary_views;
    {
      ...
      temporary_views = {lux::ui::UiContextIdView{temporary_name},
                         lux::ui::kGlobalContext};
      router.updateRoute(nullptr, temporary_views);
    }
    assert(router.activeContexts()[0].name() == "temporary.context");
  }

  {
    lux::ui::CommandRouter router;
    auto command = router.defineCommand(
        {lux::ui::UiCommandId{"destroy.receiver"}, "Destroy"});
    assert(command);
    auto receiver = std::make_unique<SelfDestroyingReceiver>();
    receiver->owner = &receiver;
    auto registration =
        router.bindGlobal<&SelfDestroyingReceiver::invoke,
                          &SelfDestroyingReceiver::destroyDuringEnabled>(
            *command, *receiver);
    assert(registration);
    assert(router.invoke(*command) ==
