...
namespace lux::async
{
    ...
    public:
        ...
        submit(const OperationPort<T>& port, T operation, Key key, SubmitOptions options = {}) noexcept
        {
            ...
            auto count = control->in_flight.fetch_add(1u, std::memory_order_acq_rel);
            ...
            auto submitted = port.submit(
                ...
                +[](void* value, OperationOutcome<T>&& outcome) noexcept {
                    ...
                    const auto previous =
                        callback->phase.exchange(ECallbackPhase::COMPLETED, std::memory_order_acq_rel);
            ...
            const auto next = submitted ? ECallbackPhase::CALLBACK_OWNED : ECallbackPhase::REJECTED;
            const auto previous = state->phase.exchange(next, std::memory_order_acq_rel);
        ...
        template <class Consumer> std::size_t drain(Consumer&& consume) noexcept
        {
            std::size_t count = 0u;
            while (true)
            {
                std::optional<Completion> completion;
                {
                    ...
                    if (control_->completions.empty())
                        break;
                    completion.emplace(std::move(control_->completions.back()));
        ...
        {
            return control_ ? control_->in_flight.load(std::memory_order_acquire) : 0u;
        }
    ...
    private:
        ...
        struct CallbackState final
        {
            ...
            std::atomic<ECallbackPhase> phase{ECallbackPhase::SUBMITTING};
        };
        ...
        static void publish(CallbackState& callback) noexcept
        {
            ...
            if (owner.completions.size() >= owner.capacity)
                std::terminate();
            owner.completions.push_back(Completion{std::move(callback.key), std::move(*callback.outcome)});
        }

        static void discard(CallbackState& callback) noexcept
        {
            callback.control->in_flight.fetch_sub(1u, std::memory_order_release);
        }
