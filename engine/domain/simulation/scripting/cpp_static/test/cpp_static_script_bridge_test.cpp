...
#include "CppAbilityObjectFixture.hpp"
#include "CppStaticScriptBridgeFixture.Bridge.script.generated.hpp"
#include "CppStaticScriptBridgeFixture.BridgeFree.script.generated.hpp"
#include "CppAbilityObjectFixture.BridgeAbility.script.generated.hpp"
...
{
template <> struct TypeTraits<cpp_static_test_detail::NonTrivialSemantic> final
{
...
lux::cxx::expected<ScriptAwaitableRegistration, EScriptAwaitableCreateError> createAwaitable(
    void *, ScriptInstanceId, std::optional<lux::simulation::script::PreparedResumeType>) noexcept
{
...
struct DelayProvider final
{
    ...
    lux::script::ScriptAbilityStartResult seconds(double, lux::script::ScriptAbilityCompletion<void>) noexcept
    {
        return {};
    }

    lux::script::ScriptAbilityStartResult simulationSeconds(double, lux::script::ScriptAbilityCompletion<void>) noexcept
    {
        return {};
    }

    lux::script::ScriptAbilityStartResult realSeconds(double, lux::script::ScriptAbilityCompletion<void>) noexcept
    {
        return {};
    }
};
...
struct CoroutineAbilityProvider final
{
    ...
    lux::script::ScriptAbilityStartResult run(std::int32_t, lux::script::ScriptAbilityCompletion<std::int32_t>) noexcept
    {
        return reject ? lux::script::ScriptAbilityStartResult{lux::cxx::unexpected<
                            lux::script::ScriptAbilityOperationError>(lux::script::ScriptAbilityOperationError{81})}
                      : lux::script::ScriptAbilityStartResult{};
    }
};

} // namespace
lux::simulation::script::ScriptCoroutine lux::simulation::test::CoroutineAbilityObject::execute(
    script::ScriptCoroutineContext &context) noexcept
{
...
int main()
{
    ...
    static_assert(script::detail::cppStaticPersistentArgumentSupported<std::int32_t>);
    static_assert(script::detail::cppStaticPersistentArgumentSupported<const std::int32_t &>);
    static_assert(!script::detail::cppStaticPersistentArgumentSupported<std::int32_t &&>);
    static_assert(!script::detail::cppStaticPersistentArgumentSupported<std::int32_t &>);
    static_assert(!script::detail::cppStaticPersistentArgumentSupported<std::int32_t *>);
    static_assert(!script::detail::cppStaticPersistentArgumentSupported<const NonTrivialSemantic &>);
    ...
    assert(projected->exports[1].args[0].canonical_name == "lux.test.BridgeRecord");
    assert(projected->exports[1].args[0].pass == lux::semantic::EValuePass::CONST_REF);
    assert(projected->exports[4].args[0].pass == lux::semantic::EValuePass::CONST_REF);
    auto invalid_contract = Bridge;
    invalid_contract.lifecycle.begin_play = 106U;
    assert(!materializeCppStaticScript(invalid_contract));
    auto global = materializeCppStaticScript(BridgeFree);
    assert(global);
    ...
    const std::array capabilities{
        PreparedScriptApiCapability{lux::script::ScriptApiContractId{DelayTraits::Description.id.name()},
                                    DelayTraits::Description.schema_hash, delay_binding.context, delay_binding.dispatch,
                                    DelayTraits::Description.schema_version, delay_binding.erased_methods}};
    ...
    assert(
        descriptor.createInstance(
            ...
            ScriptInstanceCreateContext{
                assetId(), EntityScriptScope{ecs::Entity{1U}}, &behavior, {1U, 1U}, capabilities, event_requirements},
            entity_asset, instance) == EScriptBackendResult::SUCCESS);

    ScriptBackendPreparedMethod call_method;
    assert(descriptor.prepareMethod(descriptor.context, instance, entity_asset.description().exports[0], call_method) ==
    ...
    assert(descriptor.prepareMethod(descriptor.context, instance, entity_asset.description().exports[2],
                                    begin_method_prepared) == EScriptBackendResult::SUCCESS);
    assert(descriptor.prepareMethod(descriptor.context, instance, entity_asset.description().exports[3],
    ...
    assert(descriptor.prepareMethod(descriptor.context, instance, entity_asset.description().exports[4], step_method) ==
           EScriptBackendResult::SUCCESS);
    const auto step_call = step_method.resumable;
    std::int32_t coroutine_input{5};
    lux_script_value_slot coroutine_argument{LUX_SCRIPT_VK_INT32,
                                             {},
    ...
    assert(continuation);
    coroutine_input = 999;
    const ScriptResumePacket packet{suspended.waiting_on, EScriptAwaitableState::READY, nullptr, {}};
    ...
    assert(descriptor.prepareMethod(descriptor.context, instance, entity_asset.description().exports[5],
                                    event_method_prepared) == EScriptBackendResult::SUCCESS);
    const auto event_step_call = event_method_prepared.resumable;
    lux_script_call_frame empty_frame{};
    ScriptStepContext event_context{{1U, 1U}, nullptr, &createAwaitable, &discardAwaitable, &waitEvent};
    ScriptBackendContinuation event_continuation;
    const auto event_suspended =
        event_step_call.invoke(event_step_call.context, empty_frame, event_context, event_continuation);
    assert(event_suspended.state == EScriptStepState::SUSPENDED);
    std::int32_t event_value{73};
    ScriptOwnedResumeValue owned_event;
    owned_event.type = lux::rdesc::ScriptValueType{"lux.i32",
                                                   lux::semantic::typeId("lux.i32"),
    ...
    const ScriptResumePacket event_packet{
        event_suspended.waiting_on, EScriptAwaitableState::READY, std::addressof(owned_event), {}};
    const auto event_completed = event_continuation.resume(event_continuation.state, event_context, event_packet);
    ...
    assert(descriptor.prepareMethod(descriptor.context, instance, entity_asset.description().exports[5],
                                    multi_event_method) == EScriptBackendResult::SUCCESS);
    ...
    const auto first_event_wait =
        multi_event_call.invoke(multi_event_call.context, empty_frame, event_context, first_event_continuation);
    const auto second_event_wait =
        multi_event_call.invoke(multi_event_call.context, empty_frame, event_context, second_event_continuation);
    const auto exhausted_event_wait =
        multi_event_call.invoke(multi_event_call.context, empty_frame, event_context, exhausted_event_continuation);
    ...
    assert(descriptor.prepareMethod(descriptor.context, instance, entity_asset.description().exports[6],
    ...
    assert(descriptor.prepareMethod(descriptor.context, instance, entity_asset.description().exports[7],
                                    twice_method_prepared) == EScriptBackendResult::SUCCESS);
    const auto twice_step_call = twice_method_prepared.resumable;
    ScriptBackendContinuation twice_continuation;
    const auto twice_first =
        twice_step_call.invoke(twice_step_call.context, empty_frame, step_context, twice_continuation);
    assert(twice_first.state == EScriptStepState::SUSPENDED);
    const ScriptResumePacket twice_first_packet{twice_first.waiting_on, EScriptAwaitableState::READY, nullptr, {}};
    const auto twice_second = twice_continuation.resume(twice_continuation.state, step_context, twice_first_packet);
    assert(twice_second.state == EScriptStepState::SUSPENDED);
    assert(test::coroutine_value == 1'115);
    const ScriptResumePacket twice_second_packet{twice_second.waiting_on, EScriptAwaitableState::READY, nullptr, {}};
    const auto twice_completed = twice_continuation.resume(twice_continuation.state, step_context, twice_second_packet);
    ...
    assert(
        descriptor.createInstance(descriptor.context,
                                  ScriptInstanceCreateContext{assetId(), EntityScriptScope{ecs::Entity{2U}}, &behavior},
                                  *tampered, rejected_instance) == EScriptBackendResult::EXECUTABLE_CONTRACT_MISMATCH);
    ...
    assert(descriptor.prepareMethod(descriptor.context, recycled_instance, entity_asset.description().exports[0],
                                    recycled_method) == EScriptBackendResult::SUCCESS);
    const auto recycled_call = recycled_method.synchronous;
    assert(recycled_call.context == call.context);
    descriptor.releaseMethod(descriptor.context, recycled_instance, recycled_method);
    ...
    auto global_asset_result = lux::script::ScriptArtifact::create(std::move(*global), {});
    assert(global_asset_result);
    auto global_asset = std::move(*global_asset_result);
    const std::array global_pools{
        CppStaticScriptPoolDescription{std::addressof(BridgeFree), 1U, 0U, 0U, alignof(std::max_align_t), 1U}};
    ...
    assert(global_descriptor.createInstance(global_descriptor.context,
                                            ScriptInstanceCreateContext{assetId(), SimulationScriptScope{}, nullptr},
                                            global_asset, global_instance) == EScriptBackendResult::SUCCESS);
    ScriptBackendPreparedMethod global_method;
    assert(global_descriptor.prepareMethod(global_descriptor.context, global_instance,
                                           global_asset.description().exports[0],
                                           global_method) == EScriptBackendResult::SUCCESS);
    ...
    auto ability_description = materializeCppStaticScript(BridgeAbility);
    ...
    const std::array coroutine_capabilities{PreparedScriptApiCapability{
        lux::script::ScriptApiContractId{CoroutineAbilityTraits::Description.id.name()},
        CoroutineAbilityTraits::Description.schema_hash, coroutine_binding.context, coroutine_binding.dispatch,
        CoroutineAbilityTraits::Description.schema_version, coroutine_binding.erased_methods}};
    ScriptBackendInstance ability_instance;
    assert(ability_backend_descriptor.createInstance(
               ability_backend_descriptor.context,
               ScriptInstanceCreateContext{
                   assetId(), EntityScriptScope{ecs::Entity{9U}}, &behavior, {3U, 1U}, coroutine_capabilities},
               *ability_artifact, ability_instance) == EScriptBackendResult::SUCCESS);
    ScriptBackendPreparedMethod ability_method_prepared;
    assert(ability_backend_descriptor.prepareMethod(ability_backend_descriptor.context, ability_instance,
                                                    ability_artifact->description().exports.front(),
                                                    ability_method_prepared) == EScriptBackendResult::SUCCESS);
    const auto ability_step = ability_method_prepared.resumable;
    ScriptStepContext ability_step_context{{3U, 1U}, nullptr, &createAwaitable, &discardAwaitable};
    ScriptBackendContinuation ability_continuation;
    const auto ability_suspended =
        ability_step.invoke(ability_step.context, empty_frame, ability_step_context, ability_continuation);
    ...
    owned_async.type = lux::rdesc::makeScriptValueType<std::int32_t>(lux::semantic::EValuePass::VALUE);
    ...
    const ScriptResumePacket ability_packet{
        ability_suspended.waiting_on, EScriptAwaitableState::READY, std::addressof(owned_async), {}};
    const auto ability_completed =
        ability_continuation.resume(ability_continuation.state, ability_step_context, ability_packet);
    ...
    const auto failed_ability_wait =
        ability_step.invoke(ability_step.context, empty_frame, ability_step_context, failed_ability_continuation);
    assert(failed_ability_wait.state == EScriptStepState::SUSPENDED);
    const ScriptResumePacket failed_ability_packet{
        failed_ability_wait.waiting_on, EScriptAwaitableState::FAILED, nullptr, {77}};
    const auto failed_ability = failed_ability_continuation.resume(failed_ability_continuation.state,
                                                                   ability_step_context, failed_ability_packet);
    ...
    const auto rejected_ability =
        ability_step.invoke(ability_step.context, empty_frame, ability_step_context, rejected_ability_continuation);
    assert(rejected_ability.state == EScriptStepState::FAILED);
    assert(!rejected_ability_continuation);
    ability_backend_descriptor.releaseMethod(ability_backend_descriptor.context, ability_instance,
                                             ability_method_prepared);
    ability_backend_descriptor.destroyInstance(ability_backend_descriptor.context, ability_instance);
