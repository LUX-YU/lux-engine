...
namespace lux::render
{
    ...
    // =========================================================================
    // SceneFeature — 可装入场景的单元(基类)
    // =========================================================================

    /**
     * @brief 能被 RenderScene 在运行期装载的一个组成单元。
     *
     * 它提供的是**装载相关**的一切,不含任何"如何渲染"的假设:
     ...
     *
     * 为什么要有这个基类 —— 此前只有 RenderFeature 一个类,而它**唯一的纯虚函数**
     * 是 `addPasses(RGBuilder&) = 0`。于是 5 个只拥有资源、不产任何 pass 的东西
     * (StandardMeshStack / StandardMaterial / Light / StandardViewCamera /
     * SpatialCull)被迫写一个空的 addPasses —— 每一个都在注释里写着 "Resource
     * owner only — no render-graph passes"。代码库早就知道它们是另一类东西,
     * 只是没有类型能表达。让它们继承一个"必须产 pass"的基类,违反 is-a。
     *
     * 现在的划分只有一条判据:**产不产 pass**。
     *     SceneFeature          → 装进场景、参与生命周期,但不产 pass(资源拥有者)
     *     RenderFeature      → SceneFeature + addPasses,描述一种渲染方式
     *
     * 两者的区分**在编译期完成**:RenderScene::addFeature&lt;T&gt;() 拿到的是具体
     * 类型,一个 if constexpr 就够。所以本基类里没有任何形如 asRenderFeature()
     * 的运行期询问接口 —— 基类不必认识自己的派生类,也不需要 dynamic_cast
     * (本代码库避开 RTTI)。
     */
    class LUX_FUNCTION_PUBLIC SceneFeature
    {
    public:
        struct Config
        {
        ...
        SceneFeature(Config cfg)
            : cfg_(std::move(cfg)){}

        virtual ~SceneFeature() = default;
        ...
        // 注意:本基类**不提供**任何"我是不是 RenderFeature"的询问接口。
        // "产不产 pass" 在 RenderScene::addFeature<T>() 处是**编译期事实**
        // (T 是具体类型),由那里用 if constexpr 判定并记进 SceneFeatureSet ——
        // 运行期不必再问,基类也就不必认识自己的派生类。
