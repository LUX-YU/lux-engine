...
namespace lux::render
{
    Expected<void> FogFeature::initAndAttachTo(RenderScene&)
    {
        ...
        auto pipeline = makeFullscreenTemplate("Fog", 8u + static_cast<std::uint32_t>(sizeof(RenderState)), false);
        ...
        input_layout_ = context.pipelineManager().templateSetLayout(pipeline_, 1u);
    ...
    void FogFeature::setParams(const FogSetParamsPayload& params) noexcept
    {
        ...
        params_.maximum_opacity = std::clamp(params.maximum_opacity, 0.0f, 1.0f);
    ...
    void FogFeature::addPasses(RGBuilder& builder)
    {
        ...
        const auto output = builder.createTexture("FogColor", description);
