...
struct CommandRouter::Impl final {
  ...
  [[nodiscard]] bool
  sameContexts(std::span<const UiContextIdView> contexts) const noexcept {
  ...
  contextRank(UiContextIdView context) const noexcept {
    const auto found =
        std::ranges::find_if(context_ranks, [context](const auto &ranked) {
          return ranked.first == context;
        });
  ...
  void rebuild() {
...
#endif
    std::erase_if(bindings, [](const Binding &binding) {
      return !binding.endpointsAlive();
    });
    ...
    selected_ranks.assign(commands.size(),
...
lux::cxx::expected<CommandHandle, ECommandDefinitionError>
CommandRouter::defineCommand(Command command_value) {
  ...
  const auto handle =
      CommandHandle{static_cast<std::uint32_t>(impl_->commands.size()),
...
std::optional<CommandHandle>
CommandRouter::findCommand(UiCommandIdView id) const noexcept {
  const auto found =
...
const Command *CommandRouter::command(CommandHandle command) const noexcept {
  return impl_->valid(command)
             ? std::addressof(impl_->commands[command.dense_index_])
             : nullptr;
}

lux::cxx::expected<CommandRegistration, ECommandBindingError>
CommandRouter::bindErased(CommandHandle command, UiContextId context,
                          lux::object::LuxObject *activation_scope,
                          lux::object::LuxObject &receiver, InvokeThunk invoke,
                          StateThunk enabled, StateThunk checked) {
  ...
  std::erase_if(impl_->bindings, [](const Binding &binding) {
    return !binding.endpointsAlive();
  });
  const auto duplicate =
      std::ranges::any_of(impl_->bindings, [&](const Binding &binding) {
...
std::span<const UiContextIdView>
CommandRouter::activeContexts() const noexcept {
  return impl_->active_contexts;
}

void CommandRouter::unbind(std::uint64_t token) noexcept {
  std::erase_if(impl_->bindings, [token](const Binding &binding) {
