...
int main()
{
    ...
    s.beginFrame({});
    const auto view = await(s, w, s.addView(scene.scene_id, {W, H}, "GraphParamsView"));
    ...
    s.beginFrame({});
    const auto mat_red = await(s, w, MaterialProxy(s, material_ops).uploadGraphMaterial(red));
    s.beginFrame({});
    const auto mat_green = await(s, w, MaterialProxy(s, material_ops).uploadGraphMaterial(green));
    ...
    s.beginFrame({});
    const auto inst_l = await(s, w, MeshStackProxy(s, mesh_stack_ops).addMeshInstance(scene.scene_id, mesh.handle, mat_red.handle,   xf_left.data()));
    s.beginFrame({});
    const auto inst_r = await(s, w, MeshStackProxy(s, mesh_stack_ops).addMeshInstance(scene.scene_id, mesh.handle, mat_green.handle, xf_right.data()));
    s.beginFrame({});
