        ...
        class MaterialPane final : public DocumentPane<MaterialPane, material::MaterialEditor>
        {
          ...
          public:
            MaterialPane(material::MaterialEditor &document, std::string id)
                : DocumentPane(document, std::move(id), "Material"), canvas_(createCanvas()),
                  name_(document.source().name),
                ...
                for (const auto &record : graph.topology().nodes())
                {
                    ...
                    if (const auto *constant = node->as<lux::material::ConstantNode>())
                    {
                        ...
                        if (auto *gesture = std::get_if<ConstantGesture>(&constant_);
                            gesture && gesture->node == record.id)
                        {
                            value = gesture->value;
                        }
                        ...
                        if (ImGui::IsItemActivated())
                        {
                            constant_.emplace<ConstantGesture>(record.id, document_.historyView()->history.current,
                                                               value);
                        }
                        if (auto *gesture = std::get_if<ConstantGesture>(&constant_);
                            gesture && gesture->node == record.id)
            ...
            template <class Slot> void drawSlots(SlotDraft<Slot> &draft, editing::StateId current)
            {
                ...
                for (std::size_t index{}; index < draft.values.size(); ++index)
                {
                    ...
                    if (ImGui::TreeNodeEx("slot", ImGuiTreeNodeFlags_DefaultOpen, "Slot %zu", index))
                    {
                        ...
                        if constexpr (textures)
                        {
                            const auto picked = drawAssetReference(document_.project(), slot.texture,
    ...
    GuiDocumentProvider materialDocumentProvider()
    {
        return {
               ...
               process::ExecutionRuntime &) -> EditorResult<void>
            {
                ...
                const auto id = "material-" + std::to_string(document->historyId().value);
                for (const auto &view : document->views())
                {
                    if (view->id() == id)
                    {
                        ...
                        {
                            return lux::cxx::unexpected(EditorFailure{EEditorError::BUSY, "material.views"});
                        }
                        return {};
                    }
                }
                auto pane = std::make_unique<MaterialPane>(*document, id);
