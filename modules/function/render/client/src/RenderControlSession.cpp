...
namespace lux::render
{
    ...
    bool RenderControlSession::destroyScene(RenderSceneId scene)
    {
        return record([&](Builder& builder) {
            builder.push(opcodes::CommandOp, type_ids::DestroyScene, DestroyScenePayload{scene});
        }
        );
    }
    ...
    RenderRequest<ViewCreatedReply>
    RenderControlSession::addView(RenderSceneId scene, lux::math::Extent2u extent, const char* name)
    {
    ...
    RenderViewLease
    RenderControlSession::adoptView(RenderSceneId scene, ViewHandle view, RenderViewReleaseObserver observer) noexcept
    {
    ...
    RenderControlSession::createOffscreenRenderTarget(lux::math::Extent2u extent, std::uint32_t flags)
    {
    ...
    RenderRequest<TargetReadyReply>
    RenderControlSession::createSurfaceRenderTarget(std::uint64_t native_window_handle, lux::math::Extent2u extent)
    {
        return recordReply<TargetReadyReply>([&](Builder& builder, auto callback) {
    ...
    RenderTargetLease
    RenderControlSession::adoptTarget(RenderTargetId target, RenderTargetReleaseObserver observer) noexcept
    {
    ...
    void
    RenderControlSession::setLayer(RenderTargetId target, std::uint32_t order, RenderSceneId scene, ViewHandle view)
    {
    ...
    RenderControlSession::readbackTarget(RenderTargetId target, void* dst, std::size_t capacity, TargetSlot slot)
    {
    ...
    RenderRequest<ReadbackTargetReply> RenderControlSession::readbackTargetAsync(
    ...
    RenderRequest<RenderGraphDumpReply>
    RenderControlSession::dumpRenderGraph(RenderSceneId scene, void* dst, std::size_t capacity)
    {
    ...
    RenderRequest<GpuTimingReply>
    RenderControlSession::queryGpuTiming(RenderSceneId scene, void* dst, std::size_t capacity)
    {
    ...
    RenderControlSession::queryFeatureParams(RenderSceneId scene, void* dst, std::size_t capacity)
    {
    ...
    RenderRequest<ShaderCompiledReply>
    RenderControlSession::compileShader(std::span<const std::byte> spirv, std::span<const std::byte> shader_info)
    {
    ...
    RenderControlSession::registerFeatureType(const FeatureFactory& factory, std::shared_ptr<const void> module_lease)
    {
    ...
    {
        if (scene.isValid())
            deferred_scenes_.push_back(scene);
    }
    ...
    ) noexcept
    {
        if (scene.isValid() && view.isValid())
            deferred_views_.push_back({scene, view, std::move(observer)});
    }

    void RenderControlSession::deferDestroyTarget(RenderTargetId target, RenderTargetReleaseObserver observer) noexcept
    {
        if (target.isValid())
    ...
    {
        if (deferred_views_.empty() && deferred_scenes_.empty() && deferred_targets_.empty())
            return true;
        ...
        deferred_views_.erase(
            ...
            deferred_views_.begin() + static_cast<std::ptrdiff_t>(published)
        );
        if (!deferred_views_.empty())
            return false;

        published = 0;
        while (published < deferred_scenes_.size() && destroyScene(deferred_scenes_[published]))
            ++published;
        deferred_scenes_.erase(
            deferred_scenes_.begin(),
            deferred_scenes_.begin() + static_cast<std::ptrdiff_t>(published)
        );
        ...
        deferred_targets_.erase(
            ...
            deferred_targets_.begin() + static_cast<std::ptrdiff_t>(published)
        );
