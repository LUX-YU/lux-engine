      ...
      selection_connection_(document.observeScoped<scene::SceneEditor::selectionChanged>(
          [this](const scene::SelectionNotice &value) noexcept {
              ...
              LUX_UI_MEASURE(std::printf("D3_DIAGNOSTIC Inspector selection pane=%p revision=%llu\n",
                                         static_cast<void *>(this), static_cast<unsigned long long>(value.revision)));
          })),
...
void InspectorPane::draw(lux::ui::Frame &frame, lux::ui::PaneDrawContext &context)
{
    ...
    if (directory_dirty_)
    {
        ...
        for (auto &component : document_.components(selection_.object))
        {
            ...
            const auto *found = binding != bindings_->end() && binding->type == component.type ? &*binding : nullptr;
...
{
    // UIRenderSystem revokes focus after an actual layout/collapse decision. No new
