...
QueryResult<std::shared_ptr<const MeshQueryGeometry>> MeshQueryGeometry::build(
    std::span<const Eigen::Vector3f> positions, std::span<const std::uint32_t> indices)
{
...
struct MeshQuerySystem::Impl
{
    ...
    void query(std::uint32_t node_index, const math::Ray3d &ray, RayHit3D &best, bool &found, MeshQueryWork &work) const
    {
        ++work.tree_nodes;
...
{
    return {.type = system::systemTypeId(MeshQuerySystem::Description.canonical_name),
            .cpp_type = lux::cxx::typeToken<MeshQuerySystem>(),
            .description = &MeshQuerySystem::Description,
            .install = +[](SceneBuilder &builder,
                           SceneSystemDescription description) noexcept -> lux::cxx::expected<void, SceneSystemBuildFailure> {
                ...
                const auto maintenance = builder.addMaintenanceTask<MeshQuerySystem>(
                    ...
                    [](MeshQuerySystem &query, SceneStageContext &context) noexcept -> SceneStageResult {
                        context.invalidated |= query.hasPendingChanges();
                ...
                return builder.addStablePointTask<MeshQuerySystem>(
                    description.instanceId(), [](MeshQuerySystem &query) noexcept -> SceneStageResult {
