...
namespace lux::render
{
    ...
    lux::render::Expected<void> PCFeatureIndirectBase::initAndAttachTo(RenderScene& /*scene*/)
    {
        ...
        {
            renderScene().transferScheduler().contributors().add(makeTransferContributor(pc_res, /*priority=*/1));
    ...
    {
        const VkDeviceSize indirect_size = static_cast<VkDeviceSize>(16u + max_nodes_ * sizeof(IndirectDrawCommand));
        const VkDeviceSize instance_size = static_cast<VkDeviceSize>(16u + max_nodes_ * sizeof(PointCloudInstance));
        ...
        RGResourceHandle node_rg{};
        {
            RGBufferDescription desc{};
            desc.size =
                node_buf_ ? (GpuOctreeNodeBuffer::kHeaderSize + node_buf_->maxNodes() * sizeof(GpuOctreeNode)) : 1;
            desc.stride = 1;
            desc.element_count = 1;
        ...
        auto cull_tds = builder.createTransientDS(
            "PC" + pass_label_ + "CullDS",
            cull_layout_,
            {
