...
namespace lux::render
{
    ...
    )
    {
        ...
        for (size_t i = 0; i < src.size(); ++i)
        {
            ...
            // 运行时布局参数化:final_layout 不再参与模板编译键,末屏障
            // 的 newLayout(与配套 dst 同步域)按当前录制 target 的槽位描述
            // 补丁——同一模板渲往 SAMPLED / 可回读 / swapchain 目标各得其所。
            if (ctx.target_layout && ctx.slot_resource_idx)
            {
