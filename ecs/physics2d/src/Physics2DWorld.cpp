...
namespace lux::ecs
{
    ...
    void Physics2DWorld::step(lux::meta::EntityRegistry& registry, float fixed_dt)
    {
        ...
        statics_.clear();
        if (const auto controllers = registry.view<
                CharacterController2DComponent,
                ResolvedTransform2DComponent>();
            controllers.begin() != controllers.end())
        {
            physics_origin_ = controllers.get<ResolvedTransform2DComponent>(
                *controllers.begin()).position;
        }
        else if (const auto colliders = registry.view<
                     Collider2DComponent,
                     ResolvedTransform2DComponent>();
                 colliders.begin() != colliders.end())
        {
            physics_origin_ = colliders.get<ResolvedTransform2DComponent>(
        ...
        registry.view<Collider2DComponent, ResolvedTransform2DComponent>().each(
                ...
                const ResolvedTransform2DComponent& wt)
            {
                ...
                if (!position)
                    return;
                const Eigen::Vector2f center = *position + c.offset;
        ...
        registry.view<CharacterController2DComponent, Collider2DComponent,
                ...
                const Collider2DComponent& col, Transform2DComponent& t)
            {
                ...
                if (!relative)
                    return;
                Eigen::Vector2f center = *relative + col.offset;
                ...
                const auto local = center - col.offset;
                registry.patch<Transform2DComponent>(
                    e,
                    [&local, this](Transform2DComponent& transform)
                    {
                        transform.position = {
                            physics_origin_.x + static_cast<double>(local.x()),
                            physics_origin_.y + static_cast<double>(local.y())};
                    });
            });
    }
