 ...
 *   - neutral types (ETextureFormat, EBlendFactor, ...) where cheap;
 *   - forward-declared Vulkan HANDLES here, so SDK authoring headers that only
 *     traffic handles (record context, IRenderContextView returns, GraphicsPipelineDesc
 *     shader/layout handles) do NOT force <vulkan/vulkan.h> onto a consumer that
