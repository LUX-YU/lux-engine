...
namespace lux::render
{
    ...
    {
        return !buffer_copies_.empty() || !image_copies_.empty() || !qfot_acquires_.empty() ||
               !extra_post_barriers_.empty();
    }
    ...
    void TransferScheduler::buildPreBarriers()
    {
        ...
        for (const auto& c : image_copies_)
        {
            ...
            b.srcStageMask = (c.old_layout == VK_IMAGE_LAYOUT_UNDEFINED) ? VK_PIPELINE_STAGE_2_NONE : sa.stages;
            b.srcAccessMask = (c.old_layout == VK_IMAGE_LAYOUT_UNDEFINED) ? VK_ACCESS_2_NONE : sa.access;
            b.dstStageMask = VK_PIPELINE_STAGE_2_TRANSFER_BIT;
            b.dstAccessMask = VK_ACCESS_2_TRANSFER_WRITE_BIT;
            ...
            b.subresourceRange = {
                ...
                1,
                c.subresource.baseArrayLayer,
                c.subresource.layerCount,
            };
    ...
    void TransferScheduler::recordCopies(VkCommandBuffer cmd)
    {
        ...
        std::stable_sort(
            buffer_copies_.begin(),
            buffer_copies_.end(),
            [](const BufferCopyRequest& a, const BufferCopyRequest& b) { return a.priority < b.priority; }
        );
