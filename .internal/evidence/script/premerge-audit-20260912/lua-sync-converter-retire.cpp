...
static std::size_t audit_body_calls;
...
static int resumeAuthorityCase(bool stop)
{
    ...
    operation.push = [](lua_State* state, const void* value) noexcept {
        ...
        else if (!audit_no_retire) resume_registry->destroy(resume_entity);
    ...
    description.exports = {{"tick", kTick, {lux::rdesc::makeScriptValueType<ValuePose>(lux::semantic::EValuePass::CONST_REF)}, {}}};
    ...
    constexpr std::string_view code = "return {tick=function(self,p) p.audit() end}";
    ...
    const std::array mounts{ScriptRuntimeMount{ScriptMountId{1}, asset, EntityScriptScope{entity},
        {{kTick, EventScriptTarget{kOwner, event_id}}}}};
    const auto plan = planScriptRuntimeCapacity(mounts);
    assert(plan);
    const auto descriptor = backend->descriptor();
