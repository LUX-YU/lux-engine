...
namespace lux::render
{
    ...
    {
        if (config_.page_capacity == 0u ||
            config_.maximum_selected_patches == 0u)
            return renderFailure<err::feature::ResourceInitFailed>();
        resources_ = &scene.sceneRegistry().ensure<TerrainResources>(
            config_.page_capacity);
        auto& context = renderContext();
        resources_->setRetireScheduler(
            &contextView().retireScheduler(),
            static_cast<FrameRetireScheduler::OwnerToken>(
                reinterpret_cast<std::uintptr_t>(resources_)));
        ...
        const auto select_shader = resolveShaderStage(
            shaders,
            config_.patch_select_shader,
            EBuiltinShader::TERRAIN_PATCH_SELECT_COMP);
        if (!select_shader)
        ...
        const auto select_pipeline = context.pipelineManager().
        ...
        patch_select_layout_ = context.pipelineManager().computeSetLayout(
            patch_select_pipeline_, 0u);
        if (patch_select_layout_ == VK_NULL_HANDLE)
            return renderFailure<err::pipeline::ReflectedSetLayoutMissing>(0u);

        const auto vertex_shader = resolveShaderStage(
            shaders,
            config_.patch_vertex_shader,
            EBuiltinShader::TERRAIN_PATCH_VERT);
        const auto fragment_shader = resolveShaderStage(
            shaders,
            config_.patch_fragment_shader,
            EBuiltinShader::TERRAIN_PATCH_FRAG);
        if (!vertex_shader || !fragment_shader)
        {
            return !vertex_shader
                ? lux::cxx::unexpected(vertex_shader.error())
                : lux::cxx::unexpected(fragment_shader.error());
        }
        const std::array<ShaderHandle, 2u> stage_handles{
        ...
        terrain_template.active_sets_mask =
            (1u << 0u) | (1u << 1u) | (1u << 2u);
        ...
        terrain_template.push_constant_ranges.push_back(
            VkPushConstantRange{
                VK_SHADER_STAGE_VERTEX_BIT,
                0u,
                16u});
        const std::array<const lux::rdesc::ShaderInfo*, 2u> stage_infos{
            &stages->info(0u), &stages->info(1u)};
        const auto terrain_pipeline = context.pipelineManager().
            registerGraphicsTemplate(terrain_template, stage_infos);
        if (!terrain_pipeline)
            return lux::cxx::unexpected(terrain_pipeline.error());
        patch_pipeline_ = *terrain_pipeline;
        const auto private_layout = context.pipelineManager().
            templatePrivateSetLayout(patch_pipeline_, 2u);
        if (!private_layout)
    ...
    bool TerrainFeature::canRebaseSceneOrigin(
        const std::int64_t origin_delta[3]) const noexcept
    {
        if (resources_ &&
            !resources_->canRebaseSceneOrigin(origin_delta))
        {
            return false;
        }
    ...
    void TerrainFeature::rebaseSceneOrigin(
    ...
    void TerrainFeature::onFrameBegin(const FeatureFrameContext& context)
    {
        ...
        if (!cameras_)
            cameras_ = renderScene().sceneRegistry().find<
                ViewCameraResource>();
        wanted_views_.clear();
        if (cameras_)
        {
            renderScene().forEachActiveView([this](View& view)
            {
                ...
                wanted_views_.push_back(TerrainResources::ViewOrigin{
                    ...
                    static_cast<float>(
                        std::max(view.current_extent.height, 1u)) *
                        std::fabs(camera->camera_view.proj.data()[5]) *
                        0.5f});
            });
        }
    ...
    {
        if (!resources_ || !patch_select_pipeline_.valid() ||
            !patch_pipeline_.valid())
        {
            return;
        }

        const auto import_buffer = [&builder](
            ...
            std::uint32_t stride)
        {
            ...
            description.element_count = stride == 0u
                ? 0u
                : static_cast<std::uint32_t>(size / stride);
        ...
        metadata_description.size = static_cast<std::uint64_t>(sizeof(
            TerrainResources::GpuPageMeta)) *
            resources_->fallbackCapacityPages();
        metadata_description.stride =
            sizeof(TerrainResources::GpuPageMeta);
        metadata_description.element_count =
            resources_->fallbackCapacityPages();
        metadata_description.usage =
        ...
        metadata_import.buffer_getter = [resources = resources_](
            VkBuffer* output, std::uint32_t capacity)
        {
            const auto count = std::min(
                capacity, resources->pageMetadataBufferCount());
            for (std::uint32_t index = 0u; index < count; ++index)
                output[index] = resources->pageMetadataBuffer(index);
            return count;
        };
        const auto metadata = builder.importBuffer(
            "TerrainPageMetadata",
        ...
        const auto full_pages = import_buffer(
            ...
            static_cast<std::uint64_t>(
                TerrainResources::fullPageStride()) *
                resources_->capacityPages(),
        ...
        const auto fallback_pages = import_buffer(
            ...
            static_cast<std::uint64_t>(
                TerrainResources::fallbackPageStride()) *
                resources_->fallbackCapacityPages(),
            static_cast<std::uint32_t>(
                TerrainResources::fallbackPageStride()));

        const auto maximum_patches = std::min(
            resources_->fallbackCapacityPages() * 64u,
            config_.maximum_selected_patches);
        ...
        count_import.buffer_getter = [resources = resources_](
            VkBuffer* output, std::uint32_t capacity)
        {
            const auto count = std::min(
                capacity, resources->selectionCountBufferCount());
            for (std::uint32_t index = 0u; index < count; ++index)
                output[index] = resources->selectionCountBuffer(index);
            return count;
        };
        const auto selected_count = builder.importBuffer(
            "TerrainSelectedPatchCount",
            count_description,
            count_import);
        RGBufferDescription selection_description{};
        selection_description.size =
            static_cast<std::uint64_t>(maximum_patches) *
            sizeof(TerrainResources::GpuTerrainPatch);
        selection_description.stride =
            sizeof(TerrainResources::GpuTerrainPatch);
        selection_description.element_count = maximum_patches;
        selection_description.usage = static_cast<ERGBufferUsageFlags>(
            ERGBufferUsageBits::STORAGE);
        selection_description.memory_usage = ERGMemoryUsage::GPU_ONLY;
        const auto selection = builder.createBuffer(
            "TerrainPatchSelection", selection_description);
        ...
        const auto indirect = builder.createBuffer(
            "TerrainPatchIndirect", indirect_description);
        ...
        builder.addPass("TerrainPatchSelect", ERGPassType::COMPUTE)
            ...
            .setKernelFn([this, indirect, selected_count, maximum_patches](
                const PassRecordContext& context)
            {
                const auto indirect_buffer =
                    context.resolveBufferHandle(indirect);
                const auto count_buffer =
                    context.resolveBufferHandle(selected_count);
                if (indirect_buffer == VK_NULL_HANDLE ||
                    count_buffer == VK_NULL_HANDLE ||
                ...
                synchronizeBeforeBufferTransferWrites(
                    context.cmd,
                    std::array{indirect_buffer, count_buffer}
                );
                vkCmdFillBuffer(
                    context.cmd,
                    indirect_buffer,
                ...
                auto* cameras = renderScene().sceneRegistry().find<
                    ViewCameraResource>();
                const auto* camera = context.view && cameras
                ...
                if (camera)
                {
                    ...
                    {
                        push.camera_page[axis] =
                            camera->render_origin.page_delta[axis];
                        push.camera_local_page_size[axis] =
                            camera->render_origin.local[axis];
                    }
                    push.camera_local_page_size[3] =
                        camera->coordinate_page_size;
                    push.viewport_projection[0] = static_cast<float>(
                        std::max(context.view->current_extent.height, 1u));
                    push.viewport_projection[1] =
                        std::fabs(camera->camera_view.proj.data()[5]);
                    push.viewport_projection[2] = 64.0f;
                    push.viewport_projection[3] =
                        std::fabs(camera->camera_view.proj.data()[0]);
                    for (std::size_t column = 0u; column < 3u; ++column)
                    {
                        push.view_rotation_row_0[column] =
                            camera->camera_view.view(0, column);
                        push.view_rotation_row_1[column] =
                            camera->camera_view.view(1, column);
                        push.view_rotation_row_2[column] =
                            camera->camera_view.view(2, column);
                    }
                }
                else
                {
                ...
                vkCmdDispatch(
                    ...
                    1u,
                    1u);
                resources_->markSelectionSubmitted(
        ...
        builder.addPass("TerrainGBuffer", ERGPassType::GRAPHICS)
            ...
            .setKernelFn([indirect](const PassRecordContext& context)
            {
                ...
                push.view_index = context.frame.view_index;
                push.full_stride = static_cast<std::uint32_t>(
                    TerrainResources::fullPageStride());
                push.fallback_stride = static_cast<std::uint32_t>(
                    TerrainResources::fallbackPageStride());
                vkCmdPushConstants(
                    context.cmd,
                    context.pipeline_layout,
