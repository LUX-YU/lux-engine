...
namespace
{
    ...
    [[nodiscard]] Eigen::Vector3f lightScenePosition(
        const TLightGPU& light,
        float coordinate_page_size) noexcept
    {
        return {
            static_cast<float>(light.position_page[0]) * coordinate_page_size
                + light.position_local.x,
            static_cast<float>(light.position_page[1]) * coordinate_page_size
                + light.position_local.y,
            static_cast<float>(light.position_page[2]) * coordinate_page_size
                + light.position_local.z};
    }

    template <typename TLightGPU>
    [[nodiscard]] RenderLargePosition3D lightSpatialPosition(
        const TLightGPU& light) noexcept
    {
        ...
        {
            {
                ...
                if (auto* domains = scene.domainDescriptorSets())
                {
                    ...
                    evsm_domain_offset = engineSetDomainOffset(
                        static_cast<uint32_t>(EDescriptorSetSlot::Light));
                }
                writeEVSMBindings(*light_res_, evsm->resources(),
                                  evsm_domain_sets, evsm_domain_offset);
            }
                                          ...
                                          uint32_t domain_binding_offset)
{
    ...
    for (uint32_t fi = 0; fi < static_cast<uint32_t>(domain_sets.size()); ++fi)
    {
        ...
        w[0].dstSet          = ds;
        w[0].dstBinding      = domain_binding_offset
                             + static_cast<uint32_t>(ELightSetBindings::SHADOW_ATLAS_EVSM);
        ...
        w[1].dstSet          = ds;
        w[1].dstBinding      = domain_binding_offset
                             + static_cast<uint32_t>(ELightSetBindings::SHADOW_EVSM_CONFIG);
...
uint64_t ShadowMapFeature::computeLightConfigHash(LightResources* light_res) const
{
    ...
    light_res->forEachLight<PointLightGPU>([&](uint32_t, const PointLightGPU& pl) {
        ...
        mix_val(pl.position_local.x); mix_val(pl.position_local.y); mix_val(pl.position_local.z);
    ...
    light_res->forEachLight<SpotLightGPU>([&](uint32_t, const SpotLightGPU& sl) {
        ...
        mix_val(sl.position_local.x); mix_val(sl.position_local.y); mix_val(sl.position_local.z);
    ...
    const View& view, LightResources* light_res, PerViewShadowState& out_state)
{
    ...
    light_res->forEachLight<DirectionalLightGPU>([&](uint32_t slot, const DirectionalLightGPU& dl) {
        ...
        auto full_frustum_corners = buildFrustumCorners(
            (camera.proj * rotation_view).inverse()
        );
        ...
        for (uint32_t c = 0; c < cascades && out_state.slices.size() < max_slices; ++c)
        {
            ...
            setShadowSpatialOrigin(
                slice,
                vfd.render_origin,
                vfd.coordinate_page_size
            );
    ...
    light_res->forEachLight<SpotLightGPU>([&](uint32_t slot, const SpotLightGPU& sl) {
        ...
        if ((sl.flags & LF_CAST_SHADOW) == 0u)
            return;
        const Eigen::Vector3f pos = lightScenePosition(
            sl,
    ...
    // milliseconds. Diagnose with LUX_SHADOW_ALLOC_TRACE=1 (below).
    {
        ...
        for (const CasterChoice& c : caster_choices)
        {
            ...
            const uint32_t slot = is_spot ? spot_candidates[c.index].slot
                                          : point_candidates[c.index].slot;
            const uint32_t res  = is_spot
                ? std::max(spot_candidates[c.index].light.shadow_map_size, 1u)
        ...
        for (uint32_t s = 0; s < static_cast<uint32_t>(prev_spot.size()); ++s)
            if (prev_spot[s] >= 0)
                sticky.push_back(
                    (static_cast<uint64_t>(static_cast<uint8_t>(ECasterType::Spot)) << 32) | s);
        for (uint32_t s = 0; s < static_cast<uint32_t>(prev_point.size()); ++s)
            if (prev_point[s] >= 0)
                sticky.push_back(
                    (static_cast<uint64_t>(static_cast<uint8_t>(ECasterType::Point)) << 32) | s);
        std::sort(sticky.begin(), sticky.end());

        const auto placed = orderCastersForPlacement(
            std::move(requests), atlas_packer, max_slices,
        ...
        for (const auto& r : placed)
        {
            ...
            for (const CasterChoice& c : caster_choices)
            {
                ...
                const uint32_t c_slot = (type == ECasterType::Spot)
                    ? spot_candidates[c.index].slot
                    : point_candidates[c.index].slot;
                if (c_slot == slot) { reordered.push_back(c); break; }
            }
        }
    ...
    for (const CasterChoice& choice : caster_choices)
    {
        ...
        if (choice.type == ECasterType::Spot)
        {
            ...
            ShadowSliceGPU slice = makePerspectiveLightSlice(
                Eigen::Vector3f::Zero(), dir, up_seed, near_z, far_z,
                /*proj_xy_scale=*/ 1.f / tf, sl.shadow_bias, tile);
            setShadowSpatialOrigin(
                slice,
                lightSpatialPosition(sl),
                coordinate_page_size
            );
    ...
    // 换位的槽数。
    {
        ...
        const auto countDiff = [&changed_slots](const std::vector<int32_t>& a,
        ...
        if (prev_built
            && (changed_slots != 0 || prev_total != out_state.config.total_slices))
        {
