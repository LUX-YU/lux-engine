...
namespace
{
    ...
    std::vector<RGLogicalPassView> makeViews(const std::vector<OwnedPass>& passes)
    {
...
main()
{
    ...
    static_assert(static_cast<std::int32_t>(ETextureRole::COLOR_ATTACHMENT) == 0);
    static_assert(static_cast<std::int32_t>(ETextureRole::INPUT_ATTACHMENT) == 4);

    std::vector<OwnedPass> passes{OwnedPass{.name = "A"}, OwnedPass{.name = "Z"}, OwnedPass{.name = "B"}};
    ...
    const auto first = DependencyAnalyzer::analyze(RGLogicalGraphView{.resource_count = 1, .passes = views});
    const auto second = DependencyAnalyzer::analyze(RGLogicalGraphView{.resource_count = 1, .passes = views});
    ...
    ok &= check(first.pass_topological_order == second.pass_topological_order, "topological order is not stable");
    ok &= check(
        first.lifetime.size() == 1 && first.lifetime[0].resource.index == 0 && first.lifetime[0].first_pass == 0 &&
    ...
    const auto cycle = DependencyAnalyzer::analyze(RGLogicalGraphView{.resource_count = 0, .passes = cyclic_views});
    ...
    std::vector<OwnedPass> forward_reference{
        OwnedPass{.name = "Tonemap", .stage = ERenderStage::PostProcess, .after = {"WaterComposite"}},
    ...
    const auto ordered_forward_reference =
        DependencyAnalyzer::analyze(RGLogicalGraphView{.resource_count = 1, .passes = forward_reference_views});
    ok &= check(
        !ordered_forward_reference.has_cycle &&
            ordered_forward_reference.pass_topological_order == std::vector<std::uint32_t>{1, 0},
