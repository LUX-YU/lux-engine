#pragma once
/**
 * @file TextureAtlasDescriptionCodec.hpp (private)
 * @brief Compact binary encoder/decoder for `lux::rdesc::TextureAtlas` and
 *        `lux::rdesc::FlipbookClip`. Used internally by TextureAtlasSerDeser.
 *
...
namespace lux::asset::detail
{
    ...
    encodeTextureAtlasDescription(const lux::rdesc::TextureAtlas& atlas);

    LUX_ASSET_CODECS_PUBLIC bool
    decodeTextureAtlasDescription(std::span<const std::byte> blob,
                                 lux::rdesc::TextureAtlas&   out,
                                 std::string*               error_out = nullptr) noexcept;

    LUX_ASSET_CODECS_PUBLIC std::vector<std::byte>
    encodeFlipbookClipDescription(const lux::rdesc::FlipbookClip& clip);

    LUX_ASSET_CODECS_PUBLIC bool
    decodeFlipbookClipDescription(std::span<const std::byte>  blob,
