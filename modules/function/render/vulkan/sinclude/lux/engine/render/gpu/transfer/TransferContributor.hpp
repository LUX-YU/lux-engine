 ...
 */
template <typename T>
TransferContributorEntry makeTransferContributor(T *resource, int priority = 0) noexcept
{
    return TransferContributorEntry{
        .resource = static_cast<void *>(resource),
        .submit_fn = [](void *r, TransferScheduler &s)
        {
            static_cast<T *>(r)->submitTransfers(s);
        },
        .priority = priority,
    };
}
...
TransferContributorEntry makeTransferContributorWithPost(T *resource, int priority = 0) noexcept
{
    return TransferContributorEntry{
        ...
        .post_transfer_fn = [](void *r, VkCommandBuffer cmd)
        {
            static_cast<T *>(r)->postTransfer(cmd);
        },
        .priority = priority,
    };
}
    ...
    void remove(void *resource) noexcept
    {
        entries_.erase(
            std::remove_if(entries_.begin(), entries_.end(),
                           [resource](const TransferContributorEntry &e)
    ...
    void dispatch(TransferScheduler &scheduler)
    {
        if (dirty_)
        {
            std::stable_sort(entries_.begin(), entries_.end(),
                             [](const TransferContributorEntry &a,
                                const TransferContributorEntry &b)
