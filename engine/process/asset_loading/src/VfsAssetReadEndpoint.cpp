...
{
    Impl(asset::AssetVfsView view, BlockingScheduler scheduler, TaskScope &scope,
...
VfsAssetReadEndpoint::CreateResult VfsAssetReadEndpoint::create(asset::AssetVfsView vfs, BlockingScheduler blocking,
                                                                TaskScope &tasks,
                                                                VfsAssetReadEndpointConfig config) noexcept
{
...
lux::async::SubmitResult VfsAssetReadEndpoint::submit(ReadAssetImage operation, void *completion_state,
                                                      void (*complete)(void *, Outcome &&) noexcept,
                                                      lux::async::SubmitOptions) noexcept
{
    ...
    try
    {
        request = std::make_shared<Impl::Request>(Impl::Request{shared_from_this(), completion_state, complete});
        ...
        auto terminal =
            exec::materialize(std::move(read)) |
            stdexec::then([request]<class Tag, class... Values>(Tag, Values &&...values) noexcept {
                ...
                else if constexpr (std::same_as<Tag, stdexec::set_error_t>)
                {
                    ...
                    request->finish(lux::cxx::unexpected(
                        lux::async::OperationFailure<asset::EAssetStorageError>::runtime(mapExecutionError(error))));
                }
                else
                {
                    request->finish(
                        lux::cxx::unexpected(lux::async::OperationFailure<asset::EAssetStorageError>::runtime(
        ...
        std::lock_guard lock{impl_->mutex};
        --impl_->admitted;
        return lux::cxx::unexpected(started.error() == ETaskStartError::STOPPING
                                        ? lux::async::ESubmitError::STOPPING
