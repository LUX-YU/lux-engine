...
namespace lux::simulation
{
    ...
    namespace detail
    {
        ...
        inline constexpr bool kValidHookParameter = []
        {
            ...
            if constexpr (!lux::script::ScriptSemanticTypeDeclared<Base> ||
        ...
        template <class Type>
        [[nodiscard]] consteval lux::script::ScriptSemanticType
        hookParameterType() noexcept
        {
            static_assert(kValidHookParameter<Type>);
            using Value = std::remove_cv_t<std::remove_reference_t<Type>>;
            constexpr auto pass = std::is_lvalue_reference_v<Type>
        ...
        inline constexpr bool kValidHookReturn =
            std::is_void_v<Type> ||
            (lux::script::ScriptSemanticTypeDeclared<std::remove_cv_t<Type>> &&
             !std::is_reference_v<Type> && !std::is_pointer_v<Type>);

        template <class Signature>
        struct SystemHookSignatureStorage;

        template <class Return, class... Parameters>
        struct SystemHookSignatureStorage<Return(Parameters...)> final
        {
            ...
            inline static constexpr auto return_types = []
            {
                if constexpr (std::is_void_v<Return>)
                    ...
                    return std::array{
                        lux::script::makeScriptSemanticType<
                            std::remove_cv_t<Return>>()};
            }();

            [[nodiscard]] static constexpr
            lux::script::ScriptFunctionSignatureView view() noexcept
            {
                return {
                    std::span<const lux::script::ScriptSemanticType>{
                        parameter_types},
                    std::span<const lux::script::ScriptSemanticType>{
                        return_types}
                };
    ...
    {
        return SystemHookPoint{
            name,
            cardinality,
            detail::SystemHookSignatureStorage<Signature>::view()};
    }
}
