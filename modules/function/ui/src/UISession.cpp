...
struct PaneStateAccess final {
  static void setFocused(Pane &pane, bool focused) { pane.setFocused(focused); }
...
struct PaneHandle final {
  ...
  [[nodiscard]] explicit operator bool() const noexcept { return token != 0; }
...
struct UISession::Impl final {
  ...
  [[nodiscard]] PaneRecord *findPane(std::uint64_t token) noexcept {
    ...
    const auto pending =
        std::ranges::find(pending_panes, token, &PaneRecord::token);
  ...
  [[nodiscard]] Pane *resolveRegistered(const PaneHandle &pane) const noexcept {
    ...
    return record && !record->tombstone ? pane.lifetime.getAsOnCurrent<Pane>()
                                        : nullptr;
  }
  ...
  sameContexts(const std::vector<UiContextId> &owned,
  ...
  [[nodiscard]] static std::vector<UiContextId>
  ownContexts(std::span<const UiContextIdView> contexts) {
  ...
  void rebuildFocusedContexts() {
    ...
    const auto append_unique = [&](UiContextIdView context) {
      ...
      const auto found = std::ranges::find_if(
          focused_context_ids, [context](const UiContextId &value) {
    ...
    command_router.updateRoute(focused, focused_contexts);
  }
  ...
  void commitFocus(PaneHandle pane,
                   std::span<const UiContextIdView> local_contexts) {
    if (pane && !resolveRegistered(pane))
      pane.reset();
    const bool pane_changed = focused_pane.token != pane.token;
    const bool contexts_changed =
        !sameContexts(focused_local_context_ids, local_contexts);
...
PaneRegistration::PaneRegistration(
    std::weak_ptr<detail::SessionControl> control, std::uint64_t token) noexcept
...
std::unique_ptr<Pane> UISession::createPane(PaneTypeIdView type, PaneId id) {
  const auto found =
      std::ranges::find_if(impl_->factories, [&](const FactoryRecord &record) {
...
bool UISession::requestFocus(PaneIdView pane) {
  if (!impl_->frame_open)
    impl_->compactPaneRecords();
  const auto found =
      std::ranges::find_if(impl_->panes, [&](const PaneRecord &record) {
        return !record.tombstone && record.id.view() == pane &&
               record.lifetime.alive();
      });
...
UISession::restoreLayout(const LayoutSnapshot &snapshot) {
