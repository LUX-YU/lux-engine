...
[[nodiscard]] LUX_CORE_PUBLIC std::size_t
inlineMessageStorageCountForTest() noexcept;
[[nodiscard]] LUX_CORE_PUBLIC std::size_t
heapMessageStorageCountForTest() noexcept;
#endif
...
private:
  ...
  friend MessageEnvelope makeMessage(Callable &&callable);
  ...
  [[nodiscard]] static const Ops &ops() noexcept {
    static const Ops value{
        ...
        [](void *source, void *destination) noexcept {
          if constexpr (Inline) {
            std::construct_at(static_cast<Callable *>(destination),
