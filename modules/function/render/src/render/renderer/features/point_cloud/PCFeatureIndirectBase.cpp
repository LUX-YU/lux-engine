...
#include <lux/engine/render/scene/ViewFrameData.hpp>
...
lux::render::Expected<void> PCFeatureIndirectBase::initAndAttachTo(RenderScene& /*scene*/){
    ...
    compute_shader_id_ = ensureBuiltinShader(shaders, compute_shader_id_, EBuiltinShader::PC_CULLING_COMP);
    auto& cs = *shaders->get(compute_shader_id_);
    ...
    const VkPipelineLayout compute_layout = ctx.pipelineLayoutService().getOrCreate({
    ...
    auto* pc_res = renderScene().sceneRegistry().find<PointCloudResources>();
    if (!pc_res) {
        renderScene().sceneRegistry().emplace<PointCloudResources>();
        pc_res = renderScene().sceneRegistry().find<PointCloudResources>();
        pc_res->init(ctx.vmaAllocator(), 4'000'000, max_nodes_);
    }
...
void PCFeatureIndirectBase::extractRenderObjects(
