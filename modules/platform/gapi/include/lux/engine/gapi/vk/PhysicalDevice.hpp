...
namespace lux::gapi::vk
{
    ...
    template <typename T> using SwapchainBuildExp = lux::cxx::expected<T, SwapchainBuildError>;
    ...
    namespace detail
    {
        ...
        [[nodiscard]] SwapchainBuildExp<std::vector<T>> enumerateSurfaceValues(
        ...
        {
            if (physical_device == VK_NULL_HANDLE || surface == VK_NULL_HANDLE || query == nullptr)
            {
            ...
            for (std::uint32_t attempt = 0; attempt < 3; ++attempt)
            {
                ...
                const VkResult count_result = query(physical_device, surface, &count, nullptr);
                ...
                const VkResult values_result = query(physical_device, surface, &count, values.data());
        ...
        {
            if (physical_device == VK_NULL_HANDLE || surface == VK_NULL_HANDLE || ops.capabilities == nullptr)
            {
                return lux::cxx::unexpected(SwapchainBuildError{
            ...
            VkSurfaceCapabilitiesKHR capabilities{};
            const VkResult result = ops.capabilities(physical_device, surface, &capabilities);
        ...
        {
            auto capabilities = querySurfaceCapabilities(physical_device, surface, ops);
            if (!capabilities)
            ...
            auto present_modes = querySurfacePresentModes(physical_device, surface, ops);
            if (!present_modes)
    ...
    public:
        ...
        const std::vector<VkQueueFamilyProperties2>& queueFamilyProperties() const noexcept
        {
            return _queue_family_properties;
        }
        const VkPhysicalDeviceMemoryProperties2& memoryProperties() const noexcept
        {
            return _memory_properties;
        }
        const VkPhysicalDeviceProperties2& properties() const noexcept
        {
            return _properties;
        }
        const VkPhysicalDeviceFeatures2& features() const noexcept
        {
            return _features;
        }

        const VkPhysicalDeviceDescriptorIndexingProperties& descriptorIndexingProperties() const noexcept
        {
            return _descriptor_index_properties;
        }
        ...
        const VkPhysicalDevice* operator&() const noexcept
        {
            return &_physical_device;
        }
        ...
        inline const VkPhysicalDevice* handlePtr() const noexcept
        {
            return &_physical_device;
        }
