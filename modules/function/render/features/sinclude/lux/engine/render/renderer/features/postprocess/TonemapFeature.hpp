...
public:
    ...
    void setExposure(float exposure)             { pass_params_.scalars.exposure = exposure; }
    void setGamma(float gamma)                   { pass_params_.scalars.gamma    = gamma; }
    void setToneMapOperator(EToneMapOperator op) { pass_params_.scalars.tone_map_op = static_cast<std::uint32_t>(op); }
    ...
    [[nodiscard]] void* paramData() noexcept override { return &pass_params_.scalars; }
