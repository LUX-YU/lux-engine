...
> 关联：[../.internal/shadow-lighting-audit-2026-06-05.md](../.internal/shadow-lighting-audit-2026-06-05.md)（完整审计 + RenderDoc 数据）、[../.internal/plan/evsm-shadow-implementation-guide.md](../.internal/plan/evsm-shadow-implementation-guide.md)（实现计划）
...
`IShadowTechnique` 的 `recordShadowPasses / recordFrameSetup / recordPostFrame` 虚函数 hook **目前未被调用**——真实的 pass 调度仍是 `MeshShadowFeature::addPasses` 里散落的 `if (use_evsm)` 分支。在实现方案 A 时，建议顺带把 EVSM 的 caster + blur 调度真正搬进 `EVSMShadowTechnique`,让"新增阴影技术零修改既有代码"的目标落地。
