    ...
    if (ImGui::BeginDragDropTarget())
    {
        if (const auto *payload = ImGui::AcceptDragDropPayload(kAssetReferencePayload))
        {
            ...
            else if (projection)
            {
                ...
                const auto distance = std::abs(direction.y()) > 1e-8 ? -camera_.position().y() / direction.y() : -1.0;
                const Eigen::Vector3d position = camera_.position() + direction * (distance > 0 ? distance : 5.0);
                auto accepted =
                    document_.requestModelPlacement(*reference, position, lux::partition::PartitionOrdinal{partition_});
                if (accepted)
                {
                    placement_ = *accepted;
                    status_.clear();
