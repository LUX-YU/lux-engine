...
namespace lux::render
{
    ...
    public:
        ...
        void setView(uint32_t view_id, const ViewFrameData& data) { views_.insert(view_id, data); }
        ...
        [[nodiscard]] const ViewFrameData* find(uint32_t view_id) const noexcept { return views_.tryGet(view_id); }
    ...
    [[nodiscard]] inline Eigen::Matrix4f viewRelativeViewProjection(
        const ViewFrameData& view) noexcept
    {
    ...
    [[nodiscard]] inline ViewCameraResource* resolveViewCameraOnce(
        ViewCameraResource*& cache, ResourceRegistry& reg) noexcept
    {
        if (cache == nullptr)
