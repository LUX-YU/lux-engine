                        ...
                        asset::AssetId id, std::stop_token stop)
{
    ...
    auto read = stdexec::then(process::portSender(std::move(port), process::asset_loading::ReadAssetImage{id}),
                              [](asset::AssetBlob value) noexcept -> ReadResult
                              {
    ...
    auto stopped =
        stdexec::upon_stopped(std::move(errors),
                              []() noexcept -> ReadResult
                              {
...
struct SceneEditor::Data final
{
    ...
    class ObjectEdit final : public editing::EditOperation
    {
            ...
            editing::EditResult<void> prepare()
            {
                ...
                if (insert_)
                {
                    ...
                    for (const auto &object : edit_.objects_)
                    {
                        ...
                        for (const auto &component : object.components)
                        {
                            ...
                            auto decoded =
                                schema.decode_emplace(prepared_, identities_, entity, schema.version, component.bytes);
                            if (!decoded)
                            {
                ...
                else
                {
                    ...
                    std::erase_if(rows_,
                                  [&](const auto &row)
                                  {
                                      return members.contains(row.object);
                                  });
                    std::erase_if(versions_,
                                  [&](const auto &value)
                                  {
                                      return members.contains(value.object);
                                  });
                }
                if (insert_)
                {
                    selection_.object = edit_.creation_ ? edit_.objects_.front().row.object : edit_.selection_before_;
                }
      ...
      public:
        ...
        editing::EditResult<editing::PreparedEditPtr>
        prepare(const editing::ApplyContext &context, editing::EditPreparationBudget &budget) const noexcept override
        {
            std::size_t labels{};
    ...
    class ParentEdit final : public editing::EditOperation
    {
        class Plan final : public editing::PreparedEdit
        {
          ...
          private:
            void apply() noexcept override
            {
                ...
                {
                    owner.scene->registry().emplace_or_replace<ecs::Parent>(entity, owner.identities.entity(parent));
                }
                else
                {
                ...
                std::erase_if(owner.component_versions,
                              [&](const auto &value)
                              {
      ...
      public:
        ...
        {
            const auto *value =
                owner.scene->registry().try_get<lux::simulation::ecs::Parent>(owner.identities.entity(target.object));
        ...
        prepare(const editing::ApplyContext &context, editing::EditPreparationBudget &budget) const noexcept override
        {
            ...
            if (entity == ecs::NullEntity || (parent.valid() && owner_.identities.entity(parent) == ecs::NullEntity))
            {
            ...
            while (parent.valid())
            {
                ...
                const auto *ancestor = owner_.scene->registry().try_get<ecs::Parent>(owner_.identities.entity(parent));
    ...
    Data(Project &owner, process::ExecutionRuntime &process, NativeScene content,
        ...
        : project(owner), runtime(process), source(std::make_shared<const NativeScene>(std::move(content))),
          metadata(std::move(meta)), scene(std::move(value)), identities(std::move(mapping)), history(std::move(edits)),
...
EditorResult<std::unique_ptr<SceneEditor>>
SceneEditor::open(NativeScene &source, Project &project, process::ExecutionRuntime &runtime,
                  rendering::EditorRenderer &renderer, std::shared_ptr<const lux::scene::SceneMetaManager> metadata)
{
    ...
    if (!materialized)
    {
        return lux::cxx::unexpected(EditorFailure{
            EEditorError::SOURCE_FAILURE, "world.materialize", static_cast<std::uint64_t>(materialized.error().code),
...
EditorResult<SceneCapture> SceneEditor::captureSource() const
{
    ...
    for (const auto &changed : data_->component_versions)
    {
        ...
        auto value =
            schema->capture(data_->scene->registry(), data_->identities.entity(changed.object), schema->code_lifetime);
        if (!value)
        {
...
editing::EditResult<lux::world::WorldObjectId>
SceneEditor::createObject(editing::StateId base, lux::partition::PartitionOrdinal partition, EObjectSpace space)
{
    namespace ecs = lux::simulation::ecs;
...
editing::EditResult<editing::ApplyResult> SceneEditor::eraseObjects(editing::StateId base,
                                                                    std::span<const lux::world::WorldObjectId> input)
{
    auto allowed = checkStructure(base);
    if (!allowed)
    {
...
editing::EditResult<std::vector<lux::world::WorldObjectId>>
SceneEditor::placeModel(editing::StateId base, const lux::asset::ModelAsset &asset, const Eigen::Vector3d &position,
                        lux::partition::PartitionOrdinal partition)
{
    namespace ecs = lux::simulation::ecs;
...
std::uint64_t SceneEditor::componentVersion(lux::world::WorldObjectId object, lux::cxx::TypeToken type) const noexcept
{
                                                          ...
                                                          const void *next) const
{
    ...
    if (type == lux::cxx::typeToken<lux::rdesc::LightDescription>())
    {
        ...
        const auto finite = [](float number)
        {
...
EditorResult<void> SceneEditor::addViews(std::vector<std::unique_ptr<DocumentView>> &batch)
{
    ...
    for (std::size_t index{}; index < batch.size(); ++index)
    {
        ...
        const auto matches = [id](const auto &view)
        {
            return view->id() == id;
        };
...
void SceneEditor::poll(PollBudget &budget)
{
    if (auto *placement = std::get_if<Data::Placement>(&data_->placement); placement && placement->pending())
    {
        ...
        if (placement->work.index() != 1)
        {
            ...
            EditingGuard guard(data_->editing_busy);
            notify<modelPlacementFinished>(placement->id);
        }
    }
    ...
    std::erase_if(data_->views,
                  [](const auto &view)
                  {
                      return view->closeStatus().state == ECloseState::CLOSED;
                  });
