...
namespace lux::window
{
    ...
    void LuxWindow::subscribeKeyEvent()
    {
        glfwSetKeyCallback(
            ...
            [](GLFWwindow* window, int key, int scancode, int action, int mods) {
                ...
                self->on_key.emit(KeyEvent{data});
            }
        );
    }
    ...
    void LuxWindow::subscribeCursorPositionCallback()
    {
        glfwSetCursorPosCallback(
            ...
            {
                auto self = static_cast<LuxWindow*>(glfwGetWindowUserPointer(window));
                self->on_cursor_move.emit(CursorMoveEvent{xpos, ypos});
            }
        );
    }
    ...
    void LuxWindow::subscribeScrollCallback()
    {
        glfwSetScrollCallback(
            ...
            [](GLFWwindow* window, double xoffset, double yoffset)
            {
                ...
                self->on_mouse_scroll.emit(MouseScrollEvent{data});
            }
        );
    }
    ...
    void LuxWindow::subscribeDropCallback()
    {
        glfwSetDropCallback(
            ...
            [](GLFWwindow* window, int count, const char** paths)
            {
                ...
                self->on_file_drop.emit(std::move(ev));
            }
        );
    }
    ...
    void LuxWindow::subscribeMouseButtonCallback()
    {
        glfwSetMouseButtonCallback(
            ...
            [](GLFWwindow* window, int button, int action, int mods)
            {
                ...
                self->_pending_events.emplace_back(data);
                self->on_mouse_button.emit(MouseButtonEvent{data});
            }
        );
    }
    ...
    void LuxWindow::subscribeWindowSizeChangeCallback()
    {
        glfwSetWindowSizeCallback(
            ...
            {
                auto self = static_cast<LuxWindow*>(glfwGetWindowUserPointer(window));
                self->on_resize.emit(WindowResizeEvent{common::Size2D{(uint32_t)width, (uint32_t)height}});
            }
        );
    }
    ...
    void LuxWindow::subscribeFramebufferSizeChangeCallback()
    {
        glfwSetFramebufferSizeCallback(
            ...
            {
                auto self = static_cast<LuxWindow*>(glfwGetWindowUserPointer(window));
                self->on_framebuffer_resize.emit(FramebufferResizeEvent{common::Size2D{(uint32_t)width, (uint32_t)height}});
            }
        );
    }
    ...
    int LuxWindow::exec()
    {
        while (!glfwWindowShouldClose(_glfw_window))
        {
            ...
            on_draw_ready.emit(DrawReadyEvent{});

            newFrame();

            on_draw_finished.emit(DrawFinishedEvent{});
        }
