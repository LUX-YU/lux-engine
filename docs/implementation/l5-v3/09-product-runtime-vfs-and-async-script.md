...
> Product/VFS/AssetRead ownership in this document remains normative. Detailed Script Ability/coroutine/Event/Delay semantics are owned by `11`; Script object lifecycle by `13`; implementation order by `07`. Older S0/S1/S2 sequencing previously written here is historical context only. Shipping Pak/index/update semantics are owned by `15-luxpak-indexed-content-container-and-incremental-update.md`.
...
Shipping composition may mount immutable LuxPak-backed providers plus higher-priority patch providers. The VFS sees provider visibility/priority semantics; it does not parse LuxPak TOC structures itself.
...
## 4. PakProvider / LuxPak relation

Shipping cooked content uses the ordinary provider boundary:
...
PakProvider
        ↓
LuxPak immutable TOC + chunk payload
```

`LuxPak` is not a new VFS and is not a runtime Asset manager.

The specialized storage contract in `15` freezes:
...
Pak indexes are generated by the Cooker and consumed directly. Runtime MUST NOT scan Pak payload or rebuild per-Pak `unordered_map`/heap radix indexes merely to mount content.

---

## 5. AssetRead / blocking IO isolation
...
stable Asset identity/address
    ↓
AssetVfsView resolution / PakProvider TOC lookup
    ↓
...
Pak chunk decompression belongs to the authorized storage/read path. It does not justify a feature-private worker pool or package-specific async framework.

---

## 6. General cross-time ownership
...
LuxPak storage identity does not solve this semantic blocker. `AssetId`, Chunk hash and Pak offsets are storage/identity facts; they are not an implicit residency handle.

---

## 9. GPU / Physics cross-frame operations
...
## 11. Source Plugin Packages and resource/product composition
...
The package does not gain global resource authority. Runtime content is cooked into/selected for the same product-wide LuxPak/VFS/AssetRead model as project/engine content.

A Plugin Package MUST NOT create a private PluginPak updater/VFS/asset database merely because its source is distributed separately.
...
See `14-plugin-package-and-extension-composition.md` and `15-luxpak-indexed-content-container-and-incremental-update.md`.

---

## 12. Release/update ownership

Runtime installed storage and network/update distribution are separate concerns.

```text
LuxPak / PatchLuxPak
    = installed immutable runtime content

Release Manifest / Updater
    = determines which changed/new content must be acquired and atomically published
```

The release/update layer may compare strong cooked `ContentHash`/Chunk identities and produce/download only changed content.

Normal runtime Asset loading MUST NOT silently become:

```text
loadAsset()
    -> hidden HTTP/CDN request
```

If a future product needs on-demand install/cloud streaming, it requires an explicit provider/product contract.

Platform/store binary-delta patching may be used as an additional optimization but is not the semantic basis for Lux incremental content updates.

Encryption/DRM/key management is intentionally outside the current storage closure; `15` only reserves fail-closed future security flags.

---

## 13. Product Track P gate
...
Base/Patch package selection policy
Release Manifest/update composition inputs
host-tool requirements
CMake/generator output contract
```

`PAK-0`/`PAK-1` define the cooked container/provider/update-storage contract and should be closed before final shipping Product P claims incremental asset packaging.

Product P still owns the project-level question of **which** selected project/plugin content is cooked into which physical containers and release variants.
...
parallel package manifest universe unrelated to Product P
```

---

## 14. Normative prohibitions
...
No LuxPak payload scan/runtime index rebuild on mount.
No in-place Base Pak mutation as the normal update model.
No hidden CDN/network request in normal loadAsset().
No encryption/DRM framework without a separately approved security requirement.
```
