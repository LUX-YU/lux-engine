...
namespace lux::object::detail
{
    struct ObjectDiagnosticsAccess final
    {
        ...
        template <class SignalType> [[nodiscard]] static std::size_t denseIndex(const SignalType& signal) noexcept
        {
            return signal.descriptor_.dense_index_;
        }

        template <class SignalType> [[nodiscard]] static std::size_t lineageSize(const SignalType& signal) noexcept
        {
