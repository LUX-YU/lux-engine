...
namespace lux::navigation::detour3d
{
    namespace
    {
        ...
        template <class Value> void writeBinary(lux::cxx::BinaryVectorWriter& writer, Value value)
        {
            if constexpr (std::is_floating_point_v<Value>)
            {
                static_cast<void>(writer.writeFloat(value, lux::cxx::EFloatingPointPolicy::PRESERVE_BITS));
            }
        ...
        template <class Value> [[nodiscard]] bool readBinary(lux::cxx::BinaryReader& reader, Value& value) noexcept
        {
            if constexpr (std::is_floating_point_v<Value>)
            {
                return reader.readFloat(value, lux::cxx::EFloatingPointPolicy::PRESERVE_BITS);
            }
            else if constexpr (std::is_signed_v<Value>)
            {
        ...
        {
            [[nodiscard]] std::size_t operator()(QuantizedPoint value) const noexcept
            {
                const auto packed = static_cast<std::uint64_t>(value.x) | (static_cast<std::uint64_t>(value.y) << 16u) |
                                    (static_cast<std::uint64_t>(value.z) << 32u);
                return static_cast<std::size_t>(packed ^ (packed >> 29u) ^ (packed << 17u));
            }
        };
        ...
        struct UndirectedEdge final
        {
            ...
            friend bool operator==(const UndirectedEdge&, const UndirectedEdge&) = default;
        };

        struct UndirectedEdgeHash final
        {
            [[nodiscard]] std::size_t operator()(UndirectedEdge value) const noexcept
            {
                return static_cast<std::size_t>(value.first) | (static_cast<std::size_t>(value.second) << 16u);
            }
        };
        ...
        [[nodiscard]] NavigationRegion3DFailure fail(ENavigationRegion3DError code, std::string detail)
        {
        ...
        [[nodiscard]] bool validDescription(const NavigationRegion3DDescription& value) noexcept
        {
            ...
            for (const auto& area : value.areas)
            {
                ...
                for (std::size_t index = 1u; index + 1u < area.boundary.size(); ++index)
                {
                    const auto& first = area.boundary[index];
                    const auto& second = area.boundary[index + 1u];
                    signed_area +=
                        (first.x - origin.x) * (second.z - origin.z) - (second.x - origin.x) * (first.z - origin.z);
                }
                if (!std::isfinite(signed_area) || std::abs(signed_area) <= 1.0e-12)
                {
                    return false;
                }
            }
        ...
        decodeRegion(const NavigationRegion3DBlob& blob) noexcept
        {
            if (!blob.valid())
            {
                return lux::cxx::unexpected(
                    fail(ENavigationRegion3DError::INVALID_REQUEST, "navigation region blob is incomplete")
                );
            }
            ...
            const bool has_valid_header =
                readBinary(reader, magic) && readBinary(reader, version) && readBinary(reader, result.region.high) &&
                readBinary(reader, result.region.low) && readBinary(reader, result.agent.radius) &&
                readBinary(reader, result.agent.height) && readBinary(reader, result.agent.maximum_climb) &&
            ...
            {
                return lux::cxx::unexpected(
                    fail(ENavigationRegion3DError::INVALID_CONTENT, "navigation region header is invalid")
                );
            }
            result.areas.reserve(area_count);
            for (std::uint32_t area_index = 0u; area_index < area_count; ++area_index)
            {
                ...
                {
                    return lux::cxx::unexpected(
                        fail(ENavigationRegion3DError::INVALID_CONTENT, "navigation area header is invalid")
                    );
                }
                area.boundary.resize(point_count);
                for (auto& point : area.boundary)
                {
                    if (!readBinary(reader, point.x) || !readBinary(reader, point.y) || !readBinary(reader, point.z))
                    {
                        return lux::cxx::unexpected(
                            fail(ENavigationRegion3DError::INVALID_CONTENT, "navigation area boundary is truncated")
                        );
                    }
                }
            ...
            for (std::uint32_t portal_index = 0u; portal_index < portal_count; ++portal_index)
            {
                NavigationPortal portal;
                std::uint8_t bidirectional = 0u;
                const bool has_valid_portal =
                    readBinary(reader, portal.id.high) && readBinary(reader, portal.id.low) &&
                    readBinary(reader, portal.first_region.high) && readBinary(reader, portal.first_region.low) &&
                    readBinary(reader, portal.second_region.high) && readBinary(reader, portal.second_region.low) &&
                    readBinary(reader, portal.first_position.x) && readBinary(reader, portal.first_position.y) &&
                    readBinary(reader, portal.first_position.z) && readBinary(reader, portal.second_position.x) &&
                    readBinary(reader, portal.second_position.y) && readBinary(reader, portal.second_position.z) &&
                    readBinary(reader, portal.traversal_cost_scale) && readBinary(reader, bidirectional);
                const bool is_invalid_portal = !has_valid_portal || bidirectional > 1u;
                if (is_invalid_portal)
                {
                    return lux::cxx::unexpected(
                        fail(ENavigationRegion3DError::INVALID_CONTENT, "navigation portal is truncated")
                    );
                }
            ...
            {
                validation.areas.push_back({area.boundary, area.area_class, area.traversal_flags});
            }
            validation.portals = result.portals;
            if (!reader.requireFullyConsumed() || !validDescription(validation))
            {
                return lux::cxx::unexpected(
                    fail(ENavigationRegion3DError::INVALID_CONTENT, "navigation region payload failed validation")
                );
            }
            return result;
        }
    ...
    encodeNavigationRegion3D(const NavigationRegion3DDescription& description) noexcept
    {
        ...
        {
            return lux::cxx::unexpected(
                fail(ENavigationRegion3DError::INVALID_REQUEST, "navigation region description is invalid")
            );
        }
        ...
        const auto portal_count = static_cast<std::uint32_t>(description.portals.size());
        writeBinary(writer, portal_count);
        for (const auto& area : description.areas)
        {
            const auto point_count = static_cast<std::uint8_t>(area.boundary.size());
    ...
    navigationRegion3DBlobFromBytes(lux::cxx::SharedBytes<> payload) noexcept
    {
        ...
        {
            return lux::cxx::unexpected(
                fail(ENavigationRegion3DError::INVALID_CONTENT, "navigation region blob header is invalid")
            );
        }
    ...
    prepareNavigationRegion3D(NavigationRegion3DBlob blob, std::uint64_t request_generation) noexcept
    {
        ...
        lux::math::Position3d minimum = decoded->areas.front().boundary.front();
        ...
        std::unordered_map<QuantizedPoint, std::uint16_t, QuantizedPointHash> point_indices;
        ...
        for (const auto& area : decoded->areas)
        {
            ...
            for (const auto& point : area.boundary)
            {
                ...
                const auto x = quantize(point.x, minimum.x, decoded->horizontal_resolution);
                const auto y = quantize(point.y, minimum.y, decoded->vertical_resolution);
                const auto z = quantize(point.z, minimum.z, decoded->horizontal_resolution);
                if (!x || !y || !z)
                {
                    return lux::cxx::unexpected(
                ...
                if (found == point_indices.end())
                {
                    ...
                    {
                        return lux::cxx::unexpected(
                            fail(ENavigationRegion3DError::INVALID_CONTENT, "navigation region has too many points")
                        );
                    }
        ...
        constexpr std::size_t kNoNeighbour = std::numeric_limits<std::size_t>::max();
        std::vector<std::array<std::size_t, kPointsPerArea>> neighbours(area_points.size());
        ...
        std::unordered_map<UndirectedEdge, EdgeUse, UndirectedEdgeHash> edge_uses;
        edge_uses.reserve(area_points.size() * kPointsPerArea);
        for (std::size_t area_index = 0u; area_index < area_points.size(); ++area_index)
        {
            ...
            for (std::size_t edge = 0u; edge < indices.size(); ++edge)
            {
                ...
                const UndirectedEdge key{std::min(from, to), std::max(from, to)};
                const auto [found, inserted] = edge_uses.emplace(key, EdgeUse{area_index, edge, from, to, false});
                if (inserted)
                    continue;
        ...
        const auto toLocal = [&minimum](const lux::math::Position3d& point) noexcept {
        ...
        for (std::size_t layer_index = 0u; layer_index < layer_count; ++layer_index)
        {
            ...
            const auto last_area = std::min(decoded->areas.size(), first_area + kAreasPerLayer);
            ...
            std::vector<std::uint16_t> polygons(area_count * kPointsPerArea * 2u, 0xffffu);
            ...
            for (std::size_t area_index = first_area; area_index < last_area; ++area_index)
            {
                ...
                for (std::size_t edge = 0u; edge < indices.size(); ++edge)
                {
                    ...
                    const auto [found, inserted] =
                        local_indices.emplace(global_point, static_cast<std::uint16_t>(local_indices.size()));
                    if (inserted)
                    {
                    ...
                    {
                        polygons[base + kPointsPerArea + edge] = static_cast<std::uint16_t>(neighbour - first_area);
                        continue;
                    }
                    if (area_index > neighbour)
                        continue;

                    const auto& boundary = decoded->areas[area_index].boundary;
                    const auto& first = boundary[edge];
                    const auto& second = boundary[(edge + 1u) % boundary.size()];
                    ...
                    connection_flags.push_back(decoded->areas[area_index].traversal_flags);
                    connection_ids.push_back(connection_id++);
                }
                area_classes.push_back(decoded->areas[area_index].area_class);
                flags.push_back(decoded->areas[area_index].traversal_flags);
            }
            ...
            parameters.offMeshConVerts = connection_vertices.empty() ? nullptr : connection_vertices.data();
            parameters.offMeshConRad = connection_radii.empty() ? nullptr : connection_radii.data();
            parameters.offMeshConDir = connection_directions.empty() ? nullptr : connection_directions.data();
            parameters.offMeshConAreas = connection_areas.empty() ? nullptr : connection_areas.data();
            parameters.offMeshConFlags = connection_flags.empty() ? nullptr : connection_flags.data();
            parameters.offMeshConUserID = connection_ids.empty() ? nullptr : connection_ids.data();
            parameters.offMeshConCount = static_cast<int>(connection_ids.size());
            ...
            parameters.bmax[0] = std::max(decoded->horizontal_resolution, static_cast<float>(maximum.x - minimum.x));
            parameters.bmax[1] = std::max(decoded->vertical_resolution, static_cast<float>(maximum.y - minimum.y));
            parameters.bmax[2] = std::max(decoded->horizontal_resolution, static_cast<float>(maximum.z - minimum.z));
            parameters.buildBvTree = true;

            unsigned char* navigation_data = nullptr;
            int navigation_data_size = 0;
            if (!dtCreateNavMeshData(&parameters, &navigation_data, &navigation_data_size) || !navigation_data ||
                navigation_data_size <= 0)
            {
        ...
        if (!navigation_raw)
            std::abort();
        std::unique_ptr<dtNavMesh, detail::NavigationOwner> navigation{navigation_raw};
