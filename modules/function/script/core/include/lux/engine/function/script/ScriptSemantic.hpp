...
namespace lux::script
{
    ...
    [[nodiscard]] constexpr std::uint64_t scriptSemanticTypeId(std::string_view canonical_name) noexcept
    {
    ...
    [[nodiscard]] constexpr ScriptSymbolId scriptSymbolId(std::string_view canonical_symbol_identity) noexcept
    {
    ...
    struct ScriptSemanticType final
    {
        ...
        operator==(const ScriptSemanticType&, const ScriptSemanticType&) noexcept = default;
    };
    ...
    sameScriptSignature(ScriptFunctionSignatureView left, ScriptFunctionSignatureView right) noexcept
    {
    ...
    template <class Type> struct ScriptSemanticTypeTraits;

#define LUX_SCRIPT_BUILTIN(tag, cpp_type, canonical, abi_kind_value)                                                   \
    template <> struct ScriptSemanticTypeTraits<cpp_type> final                                                        \
    {                                                                                                                  \
...
#undef LUX_SCRIPT_BUILTIN
    ...
    concept ScriptSemanticTypeDeclared = requires {
        { ScriptSemanticTypeTraits<std::remove_cv_t<Type>>::CanonicalName } -> std::convertible_to<std::string_view>;
    };

    template <class Type>
        requires ScriptSemanticTypeDeclared<Type>
    [[nodiscard]] consteval ScriptSemanticType
    makeScriptSemanticType(EScriptPassMode pass = EScriptPassMode::VALUE) noexcept
    {
        constexpr auto name = ScriptSemanticTypeTraits<std::remove_cv_t<Type>>::CanonicalName;
...
#define LUX_SCRIPT_BUILTIN(tag, cpp_type, canonical, abi_kind_value)                                                   \
    ScriptSemanticLayout{                                                                                              \
        scriptSemanticTypeId(canonical),                                                                               \
        canonical,                                                                                                     \
        abi_kind_value,                                                                                                \
        sizeof(cpp_type),                                                                                              \
        alignof(cpp_type)},
...
#undef LUX_SCRIPT_BUILTIN
    ...
    [[nodiscard]] constexpr const ScriptSemanticLayout* scriptBuiltinLayout(std::uint64_t type_id) noexcept
    {
