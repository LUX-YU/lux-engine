...
namespace lux::gapi::vk
{
	...
	public:
		...
		{
			VK_FUNC_INVOKE(vkCreateFramebuffer, "Failed to create FrameBuffer object", device, &info, allocator, &frame_buffer);
		}
		...
		inline operator VkFramebuffer() const noexcept { return frame_buffer; }
		inline const VkFramebuffer* operator&() const noexcept { return &frame_buffer; }

		inline VkFramebuffer handle() const noexcept { return frame_buffer; }
		inline const VkFramebuffer* handlePtr() const noexcept { return &frame_buffer; }
