...
namespace lux::ui
{
    ...
    struct UISession::Impl final
    {
        ...
        [[nodiscard]] PaneRecord* findPane(std::uint64_t token) noexcept
        {
            ...
            return pending == pending_panes.end() ? nullptr
                                                  : std::addressof(*pending);
        }
        ...
        [[nodiscard]] Pane* resolveRegistered(const PaneHandle& pane) const noexcept
        {
            ...
            return record && !record->tombstone
                       ? pane.lifetime.getAsOnCurrent<Pane>()
                       : nullptr;
        }
        ...
        [[nodiscard]] static bool sameContexts(
            const std::vector<UiContextId>& owned,
            std::span<const UiContextIdView> views
        ) noexcept
        {
        ...
        void rebuildFocusedContexts()
        {
            ...
            const auto append_unique = [&](UiContextIdView context)
            {
                ...
                const auto found = std::ranges::find_if(
                    focused_context_ids,
                    [context](const UiContextId& value)
            ...
            for (auto iterator = focused_local_context_ids.rbegin();
                 iterator != focused_local_context_ids.rend();
                 ++iterator)
            {
    ...
    void UISession::beginFrame(ImVec2 display_size, float delta_seconds)
    {
        ...
        impl_->compactPaneRecords();
        if (impl_->focused_pane &&
            !impl_->resolveRegistered(impl_->focused_pane))
        {
            impl_->clearFocus();
        }
        if (impl_->hovered_pane &&
            !impl_->resolveRegistered(impl_->hovered_pane))
        {
            impl_->commitHover({});
        }
        auto& io = ImGui::GetIO();
...
#if defined(LUX_UI_NEXT_TEST_DIAGNOSTICS)
                    const auto focused_capacity =
                        impl_->frame_focused_contexts.capacity();
#endif
    ...
    void UISession::unregisterFactory(std::uint64_t token) noexcept
    {
        ...
        {
            const auto found =
                std::ranges::find(records, token, &FactoryRecord::token);
