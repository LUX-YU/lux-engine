...
/**
 * @file Sprite2D.hpp
 * @brief Pure-data descriptions for traditional 2D: sprite ATLAS (named
 *        sub-rects of one texture) and sprite ANIMATION CLIP (a frame
...
namespace lux::rdesc
{
    ...
    /// pivot — exactly the fields SpriteComponent consumes (uv_rect / pivot),
    ...
    /// load-bearing (SpriteAnimClip indexes frames by ordinal).
    struct SpriteAtlas
    {
        std::string                   name;
        ...
        [[nodiscard]] const SpriteAtlasFrame* findFrame(std::string_view frame_name) const noexcept
        {
    ...
    // ── A2-01: sprite animation clip ────────────────────────────────────────

    /// One animation step: which atlas frame (ordinal into SpriteAtlas::frames)
    ...
    {
        std::uint32_t frame_index{0};   ///< ordinal into SpriteAnimClip::frames
    ...
    {
        std::string                  name;
        OpaqueAssetId                atlas_uuid{};   ///< the SPRITE_ATLAS asset (opaque)
        std::vector<SpriteAnimFrame> frames;
        std::vector<SpriteAnimEvent> events;
        bool                         loop{true};
