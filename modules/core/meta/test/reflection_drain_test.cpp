...
namespace
{
    ...
    static_assert(
        lux::meta::deduce_base<EByteBacked>() ==
        lux::meta::EBaseType::Uint8);
    static_assert(
        lux::meta::deduce_base<ESignedBacked>() ==
        lux::meta::EBaseType::Int32);
    static_assert(
        static_cast<lux::meta::EBaseType>(
            lux::meta::__builtin_qual_type<EByteBacked>.base) !=
    ...
    void pluginModuleRegister(lux::meta::ReflectionRegistry&        registry,
                              lux::meta::qual_type_index_fix_list&  /*fix_list*/)
    {
...
int main()
{
...

    {
        ...
        check(static_cast<bool>(draft),
              "plugin metadata collected into a valid draft");
        check(lux::meta::ReflectionRegistry::instance().findClass("lux::test::PluginType") == nullptr,
        ...
        check(lux::meta::ReflectionRegistry::instance().findClass("std::string") != nullptr,
              "baseline type SURVIVED the drain (a second init would have lost it)");
    }

    {
        lux::meta::MetaModuleRegistrar duplicate_registrar(
            &pluginModuleRegister);
        auto duplicate = lux::meta::meta_module_drain_draft();

        check(!static_cast<bool>(duplicate),
              "duplicate reflected type rejects the whole draft");
        if (!duplicate)
        {
            check(
                duplicate.error().error ==
                    lux::meta::EReflectionRegistrationError::DUPLICATE_CLASS,
                "duplicate draft reports a structured error");
        }
        check(lux::meta::ReflectionRegistry::instance().findClass(
                  "lux::test::PluginType") != nullptr,
