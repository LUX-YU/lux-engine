...
namespace
{
    ...
    makeTerrainPage()
    {
        ...
        for (std::uint32_t y = 0u;
             y < kTerrainWireSampleEdge;
             ++y)
        {
            for (std::uint32_t x = 0u;
                 x < kTerrainWireSampleEdge;
                 ++x)
            {
                writeU16(
                    *bytes,
                    height_offset +
                        static_cast<std::size_t>(
                            y * kTerrainWireSampleEdge + x) *
                            sizeof(std::uint16_t),
                    static_cast<std::uint16_t>(
                        30000u + (x + y) % 4096u));
            }
        }
        offset += static_cast<std::size_t>(header.height_count) *
            sizeof(std::uint16_t);
        for (std::uint32_t sample = 0u;
             sample < header.height_count;
             ++sample)
        {
            (*bytes)[offset + sample * 4u] = std::byte{0xffu};
        }
        offset += static_cast<std::size_t>(
            header.weight_plane_bytes) * 2u;
        offset += header.hole_bytes;
        const auto min_max_offset = offset;
        const auto root_offset = min_max_offset +
            static_cast<std::size_t>(header.min_max_node_count - 1u) *
                sizeof(std::uint16_t) * 2u;
        writeU16(*bytes, root_offset, 30000u);
        writeU16(
            *bytes,
            root_offset + sizeof(std::uint16_t),
            30512u);
        offset += static_cast<std::size_t>(
            header.min_max_node_count) * sizeof(std::uint16_t) * 2u;
        for (std::uint32_t y = 0u; y < fallback_edge; ++y)
        {
            for (std::uint32_t x = 0u; x < fallback_edge; ++x)
            {
                const auto source_index =
                    (y * 2u) * kTerrainWireSampleEdge + x * 2u;
                writeU16(
                    *bytes,
                    offset + static_cast<std::size_t>(
                        y * fallback_edge + x) * sizeof(std::uint16_t),
                    readU16(
                        *bytes,
                        height_offset + static_cast<std::size_t>(
                            source_index) * sizeof(std::uint16_t)));
            }
        }
        return bytes;
    }
        ...
        std::shared_ptr<const std::vector<std::byte>> bytes)
    {
        ...
        auto submitted = client.trySubmit<
            lux::render::TerrainPageUploadedReply>(
            [bytes, payload, operation = operations.id<
...
int main()
{
    ...
    std::atomic<int> validation_errors{0};
    {
        ...
        const auto scene = fixture.makeSceneWithView(
            "TerrainProbe", "TerrainProbeView");

        const auto material_type = fixture.awaitControl(
            fixture.control().registerFeatureType(kMaterialFeatureFactory));
        if (!fixture.awaitControl(fixture.control().addFeature(
                scene.scene_id,
                material_type.feature_type_id,
                MaterialCommTag{})).feature.isValid())
        {
            return 1;
        }
        const auto mesh_type = fixture.awaitControl(
            fixture.control().registerFeatureType(kMeshStackFeatureFactory));
        if (!fixture.awaitControl(fixture.control().addFeature(
                scene.scene_id,
                mesh_type.feature_type_id,
                MeshStackCommTag{})).feature.isValid())
        {
            return 2;
        }
        const auto camera_type = fixture.awaitControl(
            fixture.control().registerFeatureType(kViewCameraFeatureFactory));
        if (!fixture.awaitControl(fixture.control().addFeature(
                scene.scene_id,
                camera_type.feature_type_id,
                ViewCameraCommTag{})).feature.isValid())
        {
            return 3;
        }
        const auto camera_ops = ViewCameraOperationIds::fromOps(
            camera_type.ops, camera_type.op_count);
        if (!camera_ops.valid())
            return 4;

        const auto gbuffer_type = fixture.awaitControl(
            fixture.control().registerFeatureType(
                kDeferredGBufferFeatureFactory));
        DeferredGBufferCommConfig gbuffer_config{};
        if (!fixture.awaitControl(fixture.control().addFeature(
                scene.scene_id,
                gbuffer_type.feature_type_id,
                gbuffer_config)).feature.isValid())
        {
            return 5;
        }
        const auto terrain_type = fixture.awaitControl(
            fixture.control().registerFeatureType(kTerrainFeatureFactory));
        const auto terrain_attached = fixture.awaitControl(
            fixture.control().addFeature(
                scene.scene_id,
                terrain_type.feature_type_id,
                TerrainCommConfig{}));
        if (!terrain_attached.feature.isValid())
        {
            std::fprintf(
                ...
                formatRenderError(
                    renderErrorRegistry(), terrain_attached.error).c_str());
            return 6;
        }
        const auto terrain_ops = TerrainOperationIds::fromOps(
            terrain_type.ops, terrain_type.op_count);
        ...
        fixture.flush(10u);
        TerrainControlClient terrain_control{
            fixture.control(), terrain_ops};
        const auto stats = fixture.awaitControl(terrain_control.stats(
            {scene.scene_id}));
        if (stats.resident_pages != 1u ||
            stats.full_resolution_pages != 1u ||
            stats.selected_patch_count_valid == 0u ||
            stats.selected_patch_count != 64u ||
            stats.gpu_resident_bytes == 0u)
        {
            std::fprintf(
                stderr,
        ...
        viewCameraUpdateTransient(
            ...
            behind_position);
        fixture.flush(10u);
        const auto culled = fixture.awaitControl(terrain_control.stats(
            {scene.scene_id}));
        if (culled.selected_patch_count_valid == 0u ||
            culled.selected_patch_count != 0u)
        {
            std::fprintf(
                stderr,
        ...
        const auto removed = fixture.awaitControl(terrain_control.remove(
            {scene.scene_id, upload.id, 2u}));
        if (removed.status != 0u)
            return 12;
        fixture.flush(4u);
        const auto empty = fixture.awaitControl(terrain_control.stats(
            {scene.scene_id}));
