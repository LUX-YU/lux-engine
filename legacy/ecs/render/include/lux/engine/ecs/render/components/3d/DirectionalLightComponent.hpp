 ...
 * scene. `DirectionalLightSubsystem`(池化槽位形状,LightSubsystems.hpp)
...
namespace lux::ecs
{
    struct LUX_COMPONENT() DirectionalLightComponent
    {
        ...
        /// subsystem folds it into `DirectionalLightDesc::flags` bit 0
        /// (`LIGHT_FLAG_CAST_SHADOW`) in LightSubsystems.hpp.
