...
{
  public:
    explicit CapturedVolumes(std::span<const lux::cxx::SharedBytes<>> volumes)
        ...
        lux::async::SubmitOptions) noexcept override
    {
        ...
        {
            complete(state, lux::cxx::unexpected(Failure::domain(
                                {loading::EWorldStorageRuntimeError::INVALID_VOLUME, request.volume, request.offset})));
            return {};
        }
        ...
        {
            complete(state, lux::cxx::unexpected(Failure::domain(
                                {loading::EWorldStorageRuntimeError::RANGE_OVERFLOW, request.volume, request.offset})));
            return {};
        }
        complete(state,
...
using PartitionResult = std::variant<lux::world::WorldPartitionData, loading::WorldStorageRuntimeFailure, Cancelled>;
    ...
    const lux::asset::AssetDecodeLimits &limits)
{
    const auto found = std::lower_bound(entries.begin(), entries.end(), id,
                                        [](const auto &entry, auto identity) { return entry.metadata.id < identity; });
...
lux::cxx::expected<loading::WorldStorageSource, loading::WorldStorageRuntimeFailure> NativeScene::storageSource() const
{
    return loading::WorldStorageSource::create(
        std::shared_ptr<const lux::world::WorldDescription>(world, &world->data()),
        lux::async::OperationPort<loading::ReadWorldStorageRange>{std::make_shared<CapturedVolumes>(volumes)});
}
                                                                      ...
                                                                      bool retain_partition_sources)
{
    ...
    auto source = loading::WorldStorageSource::create(
        ...
        lux::async::OperationPort<loading::ReadWorldStorageRange>{std::make_shared<CapturedVolumes>(result.volumes)});
    if (!source)
    {
