...
namespace lux::scene
{
    lux::cxx::expected<Eigen::Matrix4d, ECameraError> cameraProjection(const Camera &camera) noexcept
    {
        if (!std::isfinite(camera.aspect_ratio) || camera.aspect_ratio <= 0.0)
        {
        ...
        return std::visit(
            [&](const auto &projection) -> lux::cxx::expected<Eigen::Matrix4d, ECameraError>
            {
                ...
                if constexpr (std::is_same_v<std::decay_t<decltype(projection)>, PerspectiveProjection>)
                {
                    ...
                    result(0, 0) = scale / camera.aspect_ratio;
                    result(1, 1) = -scale;
                ...
                else
                {
                    ...
                    result(0, 0) = 2.0 / (extent * camera.aspect_ratio);
