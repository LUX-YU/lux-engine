...
namespace lux::render
{
    ...
    public:
         ...
         */
        InstanceContext(const std::vector<const char*>& required_extensions, DebugCallback debug_callback = nullptr, VkAllocationCallbacks* allocator = nullptr);

        ~InstanceContext();

        /// @brief Get mutable reference to Vulkan instance
        lux::gapi::vk::Instance& instance() { return instance_; }
        
        /// @brief Get immutable reference to Vulkan instance
        const lux::gapi::vk::Instance& instance() const { return instance_; }

        /// @brief Get mutable reference to debug report
        lux::gapi::vk::DebugReport& debugReport() { return debug_report_; }
        
        /// @brief Get immutable reference to debug report
        const lux::gapi::vk::DebugReport& debugReport() const { return debug_report_; }
    ...
    public:
        ...
        lux::gapi::vk::PhysicalDevice& physicalDevice() { return physical_device_; }
        
        /// @brief Get immutable reference to physical device
        const lux::gapi::vk::PhysicalDevice& physicalDevice() const { return physical_device_; }
        ...
        std::array<uint8_t, VK_UUID_SIZE> deviceUUID() const { return physical_device_.deviceUUID(); }
        ...
        bool supportsExternalMemory() const { return supports_external_interop_; }
        ...
        bool supportsSwapchainMaintenance1() const { return supports_swapchain_maintenance1_; }
        ...
        EFeatureLevel featureLevel() const { return feature_level_; }

        /// @brief Get mutable reference to logical device
        lux::gapi::vk::LogicalDevice& logicalDevice() { return logical_device_; }

        /// @brief Get immutable reference to logical device
        const lux::gapi::vk::LogicalDevice& logicalDevice() const { return logical_device_; }

        /// @brief Get mutable reference to graphics queue
        lux::gapi::vk::Queue& graphicsQueue() { return graphics_queue_; }

        /// @brief Get immutable reference to graphics queue
        const lux::gapi::vk::Queue& graphicsQueue() const { return graphics_queue_; }
        ...
        uint32_t graphicsQueueFamilyIndex() const { return graphics_queue_family_index_; }

        /// @brief Check if a dedicated async compute queue is available
        bool hasAsyncComputeQueue() const { return has_async_compute_; }

        /// @brief Get mutable reference to async compute queue (falls back to graphics queue if unavailable)
        lux::gapi::vk::Queue& asyncComputeQueue() { return has_async_compute_ ? async_compute_queue_ : graphics_queue_; }

        /// @brief Get immutable reference to async compute queue
        const lux::gapi::vk::Queue& asyncComputeQueue() const { return has_async_compute_ ? async_compute_queue_ : graphics_queue_; }

        std::mutex& asyncComputeQueueMutex() noexcept
        {
            return has_async_compute_
                ? async_compute_queue_mutex_
                : graphics_queue_mutex_;
        }

        /// @brief Get async compute queue family index (falls back to graphics family if unavailable)
        uint32_t asyncComputeQueueFamilyIndex() const { return has_async_compute_ ? async_compute_queue_family_index_ : graphics_queue_family_index_; }

        /// @brief Check if a dedicated transfer queue is available
        bool hasTransferQueue() const { return has_transfer_; }

        /// @brief Get mutable reference to transfer queue (falls back to graphics queue if unavailable)
        lux::gapi::vk::Queue& transferQueue() { return has_transfer_ ? transfer_queue_ : graphics_queue_; }

        /// @brief Get immutable reference to transfer queue
        const lux::gapi::vk::Queue& transferQueue() const { return has_transfer_ ? transfer_queue_ : graphics_queue_; }

        std::mutex& transferQueueMutex() noexcept
        {
            return has_transfer_
                ? transfer_queue_mutex_
                : graphics_queue_mutex_;
        }
        ...
        uint32_t transferQueueFamilyIndex() const { return has_transfer_ ? transfer_queue_family_index_ : graphics_queue_family_index_; }

        /// @brief Get mutable reference to instance context
        InstanceContext& instanceContext() { return instance_context_; }

        /// @brief Get immutable reference to instance context
        const InstanceContext& instanceContext() const { return instance_context_; }

        /// @brief Get VMA allocator
        VmaAllocator vmaAllocator() { return vma_allocator_; }
        
        /// @brief Get VMA allocator (const)
        const VmaAllocator& vmaAllocator() const { return vma_allocator_; }
    ...
    public:
        ...
        lux::gapi::vk::DescriptorPool& descriptorPool() { return descriptor_pool_; }

        /// @brief Get immutable reference to descriptor pool
        const lux::gapi::vk::DescriptorPool& descriptorPool() const { return descriptor_pool_; }

        /// @brief Get mutable reference to command pool
        lux::gapi::vk::CommandPool& commandPool() { return command_pool_; }

        /// @brief Get immutable reference to command pool
        const lux::gapi::vk::CommandPool& commandPool() const { return command_pool_; }

        /// @brief Get mutable reference to compute command pool (for async compute queue)
        lux::gapi::vk::CommandPool& computeCommandPool() { return compute_command_pool_; }

        /// @brief Get immutable reference to compute command pool
        const lux::gapi::vk::CommandPool& computeCommandPool() const { return compute_command_pool_; }

        /// @brief Get mutable reference to transfer command pool (for dedicated transfer queue)
        lux::gapi::vk::CommandPool& transferCommandPool() { return transfer_command_pool_; }

        /// @brief Get immutable reference to transfer command pool
        const lux::gapi::vk::CommandPool& transferCommandPool() const { return transfer_command_pool_; }

        /// @brief Get mutable reference to instance context
        InstanceContext& instanceContext() { return device_context_.instanceContext(); }

        /// @brief Get immutable reference to instance context
        const InstanceContext& instanceContext() const { return device_context_.instanceContext(); }
        ...
        const DeviceContext& deviceContext() const { return device_context_; }

        // Convenience methods to reduce chain calls
        /// @brief Get mutable reference to logical device
        lux::gapi::vk::LogicalDevice& logicalDevice() { return device_context_.logicalDevice(); }

        /// @brief Get immutable reference to logical device
        const lux::gapi::vk::LogicalDevice& logicalDevice() const { return device_context_.logicalDevice(); }

        /// @brief Get mutable reference to graphics queue
        lux::gapi::vk::Queue& graphicsQueue() { return device_context_.graphicsQueue(); }

        /// @brief Get immutable reference to graphics queue
        const lux::gapi::vk::Queue& graphicsQueue() const { return device_context_.graphicsQueue(); }

        /// @brief Get VMA allocator
        VmaAllocator vmaAllocator() const { return device_context_.vmaAllocator(); }

        /// @brief Get mutable reference to physical device
        lux::gapi::vk::PhysicalDevice& physicalDevice() { return device_context_.physicalDevice(); }

        /// @brief Get immutable reference to physical device
        const lux::gapi::vk::PhysicalDevice& physicalDevice() const { return device_context_.physicalDevice(); }

        /// @brief Get graphics queue family index
        uint32_t graphicsQueueFamilyIndex() const { return device_context_.graphicsQueueFamilyIndex(); }
