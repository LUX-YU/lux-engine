...
int main() {
  ...
  const auto *reflected =
      registry.findClass("lux::object::test::ReflectedObject");
  assert(reflected);
  ...
  const auto signal =
      lux::object::reflection::findSignal(registry, *reflected, "changed");
  assert(signal);
  assert(signal.ownerType() ==
         lux::cxx::typeToken<lux::object::test::ReflectedObject>());
  assert(signal.name() == "changed");
  assert(signal.hasPayload());
  const auto saved_signal =
      lux::object::reflection::findSignal(registry, *reflected, "saved");
  assert(saved_signal);
  ...
  auto saved_connection = lux::object::reflection::observe(
      sender, saved_signal, receiver, *on_saved,
  ...
  auto queued_connection = lux::object::reflection::observe(
      sender, signal, *queued_receiver, *on_changed,
  ...
  auto incompatible = lux::object::reflection::observe(
      sender, signal, receiver, *wrong_payload, lux::object::EDelivery::DIRECT);
  assert(!incompatible);
  assert(
      incompatible.error() ==
  ...
  const auto *non_default =
      registry.findClass("lux::object::test::NonDefaultObject");
  assert(non_default);
  assert(non_default->construct == nullptr);
  assert(non_default->destruct != nullptr);

  const auto *reflected_base =
      registry.findClass("lux::object::test::ReflectedBase");
  ...
  const auto base_signal = lux::object::reflection::findSignal(
      registry, *reflected_base, "baseChanged");
  const auto derived_signal = lux::object::reflection::findSignal(
      registry, *reflected_derived, "derivedChanged");
  assert(base_signal);
  ...
  const auto inherited_signal = lux::object::reflection::findSignal(
      registry, *reflected_derived, "baseChanged");
  assert(inherited_signal);
  assert(inherited_signal.name() == "baseChanged");

  auto wrong_owner = lux::object::reflection::observe(
      sender, base_signal, receiver, *on_changed,
      lux::object::EDelivery::DIRECT);
  assert(!wrong_owner);
