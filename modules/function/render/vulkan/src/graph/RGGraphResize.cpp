...
namespace lux::render
{
    ...
    )
    {
        ...
        if (had_old_state)
        {
            ...
            {
                // Immediate release — only correct because the caller declared, by
                // passing no retirement callback, that it has waited the device
                // idle. See the parameter's note in RGGraphResize.hpp.
