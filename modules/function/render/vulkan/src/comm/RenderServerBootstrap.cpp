...
[[nodiscard]] lux::cxx::expected<lux::render::CapacityCatalog, lux::render::CapacityCatalogError> makeCapacityCatalog(
...
Expected<void> GeneralRenderServer::Impl::init(ServerConfig cfg)
{
    ...
    if (cfg.enable_validation)
    {
        ...
        debug_cb = [err_counter = cfg.validation_error_counter, text_sink = cfg.validation_message_sink,
    ...
    if (auto r = dev_ctx_->init(cfg.prefer_discrete_gpu ? EPhysicalDeviceSelectionPolicy::DISCRETE_GPU_PREFERRED
    ...
    // TextureResources
    {
        ...
        global_reg->addBeginFrameHook(EUploadPhase::Upload,
    ...
    render_ctx_->globalTransferScheduler().contributors().add(
        makeTransferContributorWithPost(&render_ctx_->globalRegistry().must<TextureResources>(),
                                        /*priority=*/10));
    ...
    // 8. Single-owner GPU transfer pipeline
    {
        ...
        ucfg.lifecycle =
            +[](void *state, std::uint32_t request_id, TransferCompletion::Kind kind, std::uint32_t resource_index,
                std::uint32_t resource_gen, EUploadLifecycleState lifecycle_state) noexcept {
                static_cast<Impl *>(state)->transitionUpload(request_id, kind, resource_index, resource_gen,
                                                             lifecycle_state);
            };
...
GeneralRenderServer::Impl::~Impl()
{
    ...
    if (transfer_pipeline_)
    {
        ...
        for (auto &pending : pending_graphics_finalizes_)
        {
            ...
            {
                vkFreeCommandBuffers(dev_ctx_->logicalDevice().handle(), res_ctx_->commandPool(), 1,
...
RenderTargetLayout makeOffscreenTargetLayout(bool sampled)
{
    ...
    if (sampled)
    {
        ...
        // 服务对预览视图的异步回读依赖此位)。
        color.usage =
            ERenderImageUsage::COLOR_ATTACHMENT | ERenderImageUsage::SAMPLED | ERenderImageUsage::TRANSFER_SOURCE;
...
void handleDestroyScene(Ctx &ctx, const DestroyScenePayload &p)
{
    ...
    {
        replyToCurrent<DestroyScenePayload>(ctx, GenericOkReply{});
        return;
    }
    const auto retire_serial = im.frame_driver_ ? im.frame_driver_->lastSubmittedSerial() : 0;
    ...
    im.renderer_->removeScene(p.scene_id, retire_serial);
...
void handlePublishSceneTime(Ctx &ctx, const PublishSceneTimePayload &p)
{
    auto &im = impl(ctx);
    auto *scene = im.renderer_->getScene(p.scene_id);
    if (!scene)
    {
        im.error_sink_.emit(renderError<err::scene::NotFound>(p.scene_id.index), p.scene_id.index,
                            im.current_stamp_.serial);
        return;
    }
    if (p.delta_ns < 0 || p.elapsed_ns < p.delta_ns)
    {
        im.error_sink_.emit(renderError<err::scene::InvalidTime>(p.scene_id.index), p.scene_id.index,
                            im.current_stamp_.serial);
        return;
    }
    scene->publishTime(p.elapsed_ns, p.delta_ns, p.step);
}
...
void handleAddView(Ctx &ctx, const AddViewPayload &p)
{
    ...
    if (sc == nullptr)
    {
        replyToCurrent<AddViewPayload>(ctx, ViewCreatedReply{{}, renderError<err::scene::NotFound>(p.scene_id.index)});
        return;
    }
    ...
    if (!handle.isValid())
    {
        ...
        replyToCurrent<AddViewPayload>(ctx, ViewCreatedReply{{}, renderError<err::memory::CapacityExhausted>()});
        return;
    }
...
void handleCreateSurfaceTarget(Ctx &ctx, const CreateSurfaceTargetPayload &p)
{
    ...
    if (p.native_window_handle == 0 ||
        !surface.initFromNative(p.native_window_handle, VkExtent2D{p.extent.width, p.extent.height},
...
void handleSetLayer(Ctx &ctx, const SetLayerPayload &p)
{
    ...
    const auto layer = GeneralRenderServer::Impl::RenderTargetEntry::CompositeLayer::sceneView(p.scene_id, p.view);
...
uint32_t submitReadbackCopy(GeneralRenderServer::Impl &im, GeneralRenderServer::Impl::PendingReadback &j)
{
    ...
    barrier(from_layout, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT,
    ...
    barrier(VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, from_layout, VK_ACCESS_TRANSFER_READ_BIT,
...
void finishReadbackCopy(GeneralRenderServer::Impl &im, GeneralRenderServer::Impl::PendingReadback &j,
                        ReadbackTargetReply &reply)
{
    VmaAllocator vma = im.dev_ctx_->vmaAllocator();
    vmaInvalidateAllocation(vma, j.alloc, 0, j.needed);
    std::memcpy(reinterpret_cast<void *>(static_cast<std::uintptr_t>(j.dst_ptr)), j.mapped,
...
void handleReadbackTarget(Ctx &ctx, const ReadbackTargetPayload &p)
{
    ...
    const VkResult wres = vkWaitForFences(im.dev_ctx_->logicalDevice(), 1, &j.fence, VK_TRUE, kReadbackFenceTimeoutNs);
...
void GeneralRenderServer::pollPendingReadbacks()
{
    ...
    const bool any_done =
        std::ranges::any_of(im.pending_readbacks_, [](const Impl::PendingReadback &pending) { return pending.done; });
...
void registerServerHandlers(GeneralRenderServer::Dispatcher &d)
{
    ...
    d.registerUnary<DestroyScenePayload, &handleDestroyScene>(opcodes::CommandOp, type_ids::DestroyScene,
                                                              "DestroyScene");
    d.registerUnary<RebaseSceneOriginPayload, &handleRebaseSceneOrigin>(opcodes::CommandOp, type_ids::RebaseSceneOrigin,
                                                                        "RebaseSceneOrigin");
    d.registerUnary<AddViewPayload, &handleAddView>(opcodes::CommandOp, type_ids::AddView, "AddView");
    d.registerUnary<PublishSceneTimePayload, &handlePublishSceneTime>(opcodes::CommandOp, type_ids::PublishSceneTime,
                                                                      "PublishSceneTime");
    d.registerUnary<RemoveViewPayload, &handleRemoveView>(opcodes::CommandOp, type_ids::RemoveView, "RemoveView");
    d.registerUnary<ReadbackTargetPayload, &handleReadbackTarget>(opcodes::CommandOp, type_ids::ReadbackTarget,
                                                                  "ReadbackTarget");
    d.registerUnary<ReadbackTargetAsyncPayload, &handleReadbackTargetAsync>(
        opcodes::CommandOp, type_ids::ReadbackTargetAsync, "ReadbackTargetAsync");
    ...
    d.registerUnary<SetActiveScenePayload, &handleSetActiveScene>(opcodes::CommandOp, type_ids::SetActiveScene,
                                                                  "SetActiveScene");
    // ── RenderTarget 一等化命令面──
    d.registerUnary<CreateOffscreenTargetPayload, &handleCreateOffscreenTarget>(
        opcodes::CommandOp, type_ids::CreateOffscreenTarget, "CreateOffscreenTarget");
    d.registerUnary<CreateSurfaceTargetPayload, &handleCreateSurfaceTarget>(
        opcodes::CommandOp, type_ids::CreateSurfaceTarget, "CreateSurfaceTarget");
    d.registerUnary<DestroyTargetPayload, &handleDestroyTarget>(opcodes::CommandOp, type_ids::DestroyTarget,
                                                                "DestroyTarget");
    ...
    d.registerUnary<ResizeTargetPayload, &handleResizeTarget>(opcodes::CommandOp, type_ids::ResizeTarget,
                                                              "ResizeTarget");
    d.registerUnary<BindSwapchainPayload, &handleBindSwapchain>(opcodes::CommandOp, type_ids::BindSwapchain,
                                                                "BindSwapchain");
