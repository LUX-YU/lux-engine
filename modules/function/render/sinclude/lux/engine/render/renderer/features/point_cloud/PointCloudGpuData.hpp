...
{
    // =====================================================================
    // GpuPointVertex — 16 bytes, matches pointcloud.vert input layout
    // =====================================================================
    ...
    static_assert(
        ...
        "GpuPointVertex must be 16 bytes to match pointcloud.vert layout"
    );
    ...
    static_assert(
        sizeof(GpuOctreeNode) == 96,
        "GpuOctreeNode must be 96 bytes to match shader layout"
    );
    ...
    static_assert(
        sizeof(PointCloudInstance) == 16,
        "PointCloudInstance must be 16 bytes to match shader layout"
    );
    ...
    /// Frame constants for point cloud rendering (pointcloud.vert, set 0, binding 0).
    ...
    inline void applyViewState(PointCloudFrameConstants& dst,
                               const PointCloudCameraState& src) noexcept
    {
        if (src.view_proj) std::memcpy(dst.view_proj_matrix, src.view_proj, 64);
    ...
    static_assert(
        sizeof(IndirectDrawCommand) == 16,
        "IndirectDrawCommand must match VkDrawIndirectCommand layout"
    );
    ...
    static_assert(
        sizeof(IndirectDrawHeader) == 16,
        "IndirectDrawHeader must be 16 bytes for alignment"
    );
