...
namespace lux::render
{
            ...
            for (const auto& tex_ref : cpass.pass->textures)
            {
                ...
                for (uint32_t frame = 0; frame < frames_in_flight; ++frame)
                {
                    ...
                    if (full == VK_NULL_HANDLE)
                        return make_error_code(ERenderError::VulkanObjectCreationFailed);
    ...
    // ================================
    // Logical resource -> VkImageView mapping
    // ================================
    VkImageView RGVulkanRecorder::resolveImageView(
        const RGCompiledGraph& graph,
        const RGPhysicalResourceTable& physical_resources,
        RGRecordContext& record_context,
        RGResourceHandle handle,
        uint32_t frame_index,
        lux::common::ETextureRole /*role*/) const
    {
        const auto& res_desc = graph.original_graph.resources[handle.index];
        if (res_desc.type != ERGResourceType::TEXTURE)
            return VK_NULL_HANDLE;

        if (!physical_resources.contains(handle.index))
            return VK_NULL_HANDLE;

        if (handle.index < record_context.per_frame_views.size())
        {
            const auto& fv = record_context.per_frame_views[handle.index];
            if (frame_index < fv.size() && fv[frame_index] != VK_NULL_HANDLE)
                return fv[frame_index];
        }
