...
#define LUX_CHECK(condition)                                                                                           \
    ...
    void benchmark(std::string_view name, std::size_t count, std::size_t contexts, Callable&& callable)
    {
        ...
        for (std::size_t index = 0; index < kSampleCount; ++index)
        {
            ...
            std::cout << "raw," << name << ',' << count << ',' << contexts << ',' << index << ',' << elapsed << ','
                      << allocation_delta << '\n';
        }
    ...
    private:
        std::array<lux::ui::UiContextIdView, 1> contexts_{lux::ui::UiContextIdView{"benchmark.context"}};
    };
    ...
    void benchmarkPanes(std::size_t pane_count)
    {
        ...
        const auto steady_growth = lux::ui::detail::UISessionDiagnosticsAccess::wrapperGrowthCount(session);
        benchmark("ui_steady_frame", pane_count, 0, [&] { drawFrame(session); });
        LUX_CHECK(
            lux::ui::detail::CommandRouterDiagnosticsAccess::rebuildCount(session.commandRouter()) == steady_rebuilds
        );
        LUX_CHECK(lux::ui::detail::UISessionDiagnosticsAccess::wrapperGrowthCount(session) == steady_growth);
    ...
    struct CommandFixture final
    {
        ...
        CommandFixture(std::size_t binding_count, std::size_t context_count)
        {
            ...
            {
                auto command =
                    router.defineCommand({lux::ui::UiCommandId{"command." + std::to_string(index)}, "Command"});
                LUX_CHECK(command);
                commands.push_back(*command);
                auto global =
                    router.bindGlobal<&CommandReceiver::invoke, &CommandReceiver::enabled, &CommandReceiver::checked>(
                        *command,
                        receiver
                    );
                LUX_CHECK(global);
                ...
                if (context_count > 1)
                {
                    ...
                    auto contextual =
                        router.bind<&CommandReceiver::invoke, &CommandReceiver::enabled, &CommandReceiver::checked>(
                            *command,
            ...
            lux::ui::detail::CommandRouterDiagnosticsAccess::updateRoute(router, &scope, active_contexts);
        }
    };

    void benchmarkCommands(std::size_t binding_count, std::size_t context_count, std::size_t query_count)
    {
        CommandFixture fixture{binding_count, context_count};
        const auto rebuilds_before = lux::ui::detail::CommandRouterDiagnosticsAccess::rebuildCount(fixture.router);
        lux::ui::detail::CommandRouterDiagnosticsAccess::updateRoute(
            fixture.router,
            &fixture.scope,
            fixture.active_contexts
        );
        LUX_CHECK(lux::ui::detail::CommandRouterDiagnosticsAccess::rebuildCount(fixture.router) == rebuilds_before);

        const auto query_growth = lux::ui::detail::CommandRouterDiagnosticsAccess::storageGrowthCount(fixture.router);
        ...
        LUX_CHECK(lux::ui::detail::CommandRouterDiagnosticsAccess::storageGrowthCount(fixture.router) == query_growth);
        benchmark("ui_command_invoke", binding_count, context_count, [&] {
            for (std::size_t index = 0; index < query_count; ++index)
            {
                const auto command = fixture.commands[index % fixture.commands.size()];
                LUX_CHECK(fixture.router.invoke(command) == lux::ui::ECommandDispatchResult::EXECUTED);
            }
        }
        );
        LUX_CHECK(lux::ui::detail::CommandRouterDiagnosticsAccess::storageGrowthCount(fixture.router) == query_growth);
    ...
    void benchmarkBindingChurn(std::size_t command_count)
    {
        ...
        for (std::size_t index = 0; index < command_count; ++index)
        {
            auto command = router.defineCommand({lux::ui::UiCommandId{"churn." + std::to_string(index)}, "Churn"});
            LUX_CHECK(command);
            commands.push_back(*command);
        }
