...
namespace lux::serialization
{
    ...
    public:
        ...
        {
            capture(writer_.writeUnsigned(value));
        }
        ...
        {
            capture(writer_.writeSigned(value));
        }
        void f32(float value) noexcept
        {
            capture(writer_.writeFloat(value));
        }
        ...
        [[nodiscard]] lux::cxx::expected<std::vector<std::byte>, SerializationFailure> take() && noexcept
        {
            if (failure_)
            {
    ...
    private:
        template <std::unsigned_integral T> bool readUnsigned(T& value) noexcept
        {
            if (failed_)
                return false;
