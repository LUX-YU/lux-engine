...
main()
{
    ...
    static_assert(!std::is_move_constructible_v<SystemEventBuffer<OwnedPayload>>);
    static_assert(!std::is_move_assignable_v<SystemEventBuffer<OwnedPayload>>);
    static_assert(std::is_move_constructible_v<SystemEventBuffer<OwnedPayload>::Writer>);
    static_assert(!std::is_copy_constructible_v<SystemEventBuffer<OwnedPayload>::Writer>);
    ...
    std::optional<SystemEventBuffer<OwnedPayload>::Writer> first_writer_state{std::move(*first_writer)};
    std::optional<SystemEventBuffer<OwnedPayload>::Writer> second_writer_state{std::move(*second_writer)};
    ...
    const auto active_drain = buffer.drain([](lux::simulation::ecs::Entity, const OwnedPayload&) noexcept {});
    assert(!active_drain);
    assert(active_drain.error() == ESystemEventBufferError::ACTIVE_WRITER);

    std::atomic_size_t callbacks{};
    TaskGraphBuilder builder;
    auto first = builder.add([&first_writer_state]() noexcept {
        OwnedPayload first_payload{10U, {10U}};
        OwnedPayload second_payload{11U, {11U}};
        assert(first_writer_state->emit(lux::simulation::ecs::NullEntity, first_payload));
        assert(first_writer_state->emit(lux::simulation::ecs::NullEntity, second_payload));
        first_writer_state.reset();
    }
    );
    auto second = builder.add([&second_writer_state]() noexcept {
        OwnedPayload payload{20U, {20U}};
        assert(second_writer_state->emit(lux::simulation::ecs::NullEntity, payload));
        second_writer_state.reset();
    }
    );
    assert(first && second);
    auto safe = builder.add(on(ETaskAffinity::CALLER_THREAD), dependsOn(*first), dependsOn(*second), [&]() noexcept {
        std::vector<std::uint32_t> observed;
        observed.reserve(3U);
        assert(buffer.drain([&](lux::simulation::ecs::Entity, const OwnedPayload& payload) noexcept {
    ...
    assert(overflow_writer->emit(lux::simulation::ecs::NullEntity, payload));
    const auto overflow = overflow_writer->emit(lux::simulation::ecs::NullEntity, payload);
    assert(!overflow);
    assert(overflow.error() == ESystemEventBufferError::CAPACITY_EXCEEDED);
    overflow_writer = {};
    const auto failed_drain = buffer.drain([](lux::simulation::ecs::Entity, const OwnedPayload&) noexcept {});
    assert(!failed_drain);
