...
namespace lux::render
{
    bool LightResources::canRebaseSceneOrigin(const std::int64_t origin_delta[3]) const noexcept
    {
        const auto can_rebase = [origin_delta](const auto& ssbo) {
            ...
            if constexpr (std::is_same_v<Value, PointLightGPU> || std::is_same_v<Value, SpotLightGPU>)
            {
                for (std::uint32_t index = 0u; index < ssbo.count(); ++index)
                {
                    if (ssbo.isSlotAlive(index) &&
                        !canRebaseRenderPageDelta(ssbo.hostValue(index).position_page, origin_delta))
                    {
                        return false;
                    }
                }
        ...
        const auto rebase = [origin_delta](auto& ssbo) {
            ...
            if constexpr (std::is_same_v<Value, PointLightGPU> || std::is_same_v<Value, SpotLightGPU>)
            {
                for (std::uint32_t index = 0u; index < ssbo.count(); ++index)
                {
                    ...
                    rebaseRenderPageDelta(value.position_page, origin_delta);
    ...
    std::optional<float> LightResources::intensity(LightHandle handle) const noexcept
    {
    ...
    bool LightResources::setIntensity(LightHandle handle, float value) noexcept
    {
        const auto* record = binding_map_.find(handle);
        if (!record)
            return false;
    ...
    bool LightResources::beginFadeOut(LightHandle handle, float scene_time, float duration_seconds)
    {
        const auto* record = binding_map_.find(handle);
        if (!record)
            return false;
    ...
    void LightResources::advanceIntensityTransitions(float scene_time)
    {
        ...
        for (auto transition : intensity_transitions_)
        {
            ...
            const float factor = std::clamp(elapsed / transition.duration, 0.0f, 1.0f);
            const float value =
                transition.start_intensity + (transition.target_intensity - transition.start_intensity) * factor;
    ...
    static void fillSpatialPosition(TLightGPU& gpu, const RenderLargePosition3D& position) noexcept
    {
    ...
    template <typename TLightGPU>
    static void fillCommonFromDesc(
        TLightGPU& gpu,
