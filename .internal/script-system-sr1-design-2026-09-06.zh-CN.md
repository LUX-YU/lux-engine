...
## 16. SR-3 持续 Event 有限调查（2026-09-07）

本轮只处理持续 FlowForge Event，保留 `99c1d095` 的全部生产实现。验证快照为
`8a6e6ef468f7eb60265678ab25116719b42a01a6`，仅新增 single-flight/代次复用负例、计时区间外完整性观察及测量工具。
完整结果见 [持续 Event 专项报告](script-system-sr3-event-closeout-2026-09-07.zh-CN.md) 和
[新证据增量](evidence/script/sr3-event/README.md)；原有证据与历史身份保持不变。

实际 300 帧计数为 3,000,000 Hook 候选、2,400,000 次 single-flight 跳过、600,000 次新 step 及 resume，
稳态 backlog=8,000。优化产物保留跳过前票据计数增减；resumeOne 的两层计数作用域也确实重叠。
提前跳过试验减少票据但引入额外调用边界，长批五对中位 +0.97%；单独合并恢复保护中位 -0.51%，均未获稳定净收益，已撤销。
Instances/Bindings/Preparer 所有权及所有原保护窗口不变，未提前开始 SR-4。

最终 clean clone 全量验证为 Toolchain 107/107、Developer 113/119，六项 Scene Lua 失败与 B0/B2 的同一断言一致。
14/14 安装消费者、严格 6+8 轨迹、wire、七点重入/同步错误、身份/pin/frontier/预算及 8/8,192 规模回归通过。
Flow 使用匹配源码和计时后保留错误观察，旧 CSV errors/failures 仍为 null。
3,000 帧正式同量配对 B2→验证快照中位 +0.30%，B0→验证快照 +13.72%，后者差值中位 +360.4816 ms，
即完整帧摊销 +60.080 ns/resume、+0.120161 ms/本例帧；不是孤立 resume 函数增量。
